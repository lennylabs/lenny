...
class Handler(Protocol):
    ...
    task, :meth:`on_message` for every inbound §28.5.3 message frame,
...
class _Session:
    ...
    def run(self) -> None:
        ...
        # the order the loop reads them: this is the §15.4
    ...
    def _loop(
        ...
        types are ignored for forward compatibility (§28.5.3).
        """
    ...
    def _dispatch_worker(
        ...
        """Process queued §28.5.3 message frames one at a time, in
        arrival order."""
        while True:
            env = messages.get()
            if env is None:
    ...
    def _handle_message(self, env: MessageEnvelope) -> None:
        """Invoke :meth:`Handler.on_message` for one decoded §28.5.3
        ...
        (§28.5.3 error reporting via response).
        """
        assert self._writer is not None
        self._sequence += 1
        msg = Message(
    ...
    def _handle_shutdown(self, frame: dict[str, Any]) -> None:
        """Decode the §28.5.3 shutdown frame and record the termination
    ...
    def _stop_frame_loop(self) -> None:
        """Signal the §28.5.3 frame loop to exit. The lifecycle channel
