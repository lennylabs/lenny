...
function echoParts(input, seq) {
    return input.map((p) => p.type === "text" && p.inline
...
class DelegateHandler {
    ...
    async onMessage(msg, tools) {
        ...
        try {
            ...
            const handle = await platform.delegateTask("delegate-child", echoParts(input, this.seq));
...
run(new DelegateHandler(), { level: "standard" }).then(() => process.exit(EXIT_OK), (err) => {
