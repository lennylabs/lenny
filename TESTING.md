...
- `tests/tier7b_load_kind/scenarios/startup_latency/main.go` is the executable benchmark harness. It records a baseline at `tests/tier7b_load_kind/baselines/startup_latency.json` (heartbeat round-trip P50=2 ms, P90=2 ms, P95=3 ms over 200 iterations on the development machine). The default iteration count is 200, which is roughly the conventional minimum for a stable p95 estimate at one-sample resolution; the harness omits the p99 column because that quantile would need approximately 1000+ samples to be defensible. spec-reviews: F-6.3.14.
