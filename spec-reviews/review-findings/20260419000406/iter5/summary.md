...
|---|-------------|---------|---------|
| OBS-031 | Observability | `RedisUnavailable` runbook trigger references an undefined alert | §17.7 line 740; §16.5 line 482 |
| OBS-032 | Observability | `MinIOUnavailable` runbook trigger references an undefined alert | §17.7 line 760; §16.5 |
| CMP-054 | Compliance | Legal-hold escrow bucket is platform-singular — no per-region escrow pipeline | §12.* escrow pipeline + T4 topology |
| CMP-057 | Compliance | `complianceProfile` downgrade has no ratchet — silent HIPAA→none regression | `PUT /v1/admin/tenants/{id}`; §12.9 |
| CRD-015 | Credential | Credential deny-list keying contract broken for user-scoped revocation | Credential revoke flow |
