...
### K8S-041. Admission-webhook NetworkPolicy row over-grants gateway internal-port egress to all eight webhook Deployments [Medium]
...
### K8S-042. `lenny-sandboxclaim-guard` PATCH/PUT rule examines `SandboxClaim.status.phase`, but the documented state machine lives on `Sandbox` [Medium]
...
### K8S-044. Fallback claim path creates a `SandboxClaim` with stale `agent_pod_state` mirror data [Medium]
...
### K8S-045. `lenny-pool-config-validator` rule set 1 can wedge PSC reconciliation under stale-Postgres conditions [Medium]
...
Section: spec/13_security-model.md §13.4; spec/04_system-components.md §4.7 (artifact staging)
...
### SEC-012. Admin-time RBAC live-probe caller identity and impersonation path unspecified [Medium]
...
### SEC-013. Interceptor weakening cooldown timestamp is admin-writable, negating the control against a compromised admin [Medium]
...
### NET-061. `lenny-ops-egress` storage/monitoring rules lack pod selectors [High]
...
### NET-064. `global.spiffeTrustDomain` / `saTokenAudience` defaults only warn, allowing cross-deployment impersonation [Medium]

**Section:** 10.3, 23.x (install preflight)
...
### NET-067. DNS egress rules use broad `namespaceSelector` without CoreDNS `podSelector` [Medium]
...
### PRF-006. PDB `maxUnavailable: 1` Blocks HPA Scale-Down Throughput at Tier 3 [High]
...
### PRF-007. Tier 3 MinIO Burst Throughput Target Unachievable with Recommended Topology [High]
...
### PRF-008. Tier 3 `minWarm` 1,260 Uses 1.2× Safety Factor — Insufficient Headroom for a 35s Failover [Medium]
...
### PRF-009. Default `ResourceQuota` `pods: 200` Does Not Scale with Tier — Blocks Tier 3 Warm Pool [Medium]
...
### OBS-023. §25.13 tier-aware defaults still reference alerts that don't exist in §16.5 [High]
...
### OBS-024. LLM-proxy active-connections metric is inconsistently named across sections [Medium]
...
### OBS-025. `PodStateMirrorStale` alert referenced in §10 but undefined in §16.5; backing metric unregistered [Medium]
...
### OBS-026. Gateway-internal metrics referenced in §10 are absent from the §16.1 registry [Medium]
...
### OBS-027. `coordinator_resume_deduplicated_total` metric and `coordinator_resume_meta_source` label both lack `lenny_` prefix [Medium]
...
### OBS-028. Extraction-threshold metrics from §4.1 are not registered in §16.1; §16.1 footnote violates the single-source rule [Medium]
...
### CMP-049. DeleteByUser sequence has no legal-hold preflight [High]
...
### CMP-050. chainIntegrity enum lacks post-erasure-redaction state; legitimate erasures trigger tamper alerts [Medium]
...
### CMP-051. Dead-letter receipt reconciliation with downstream SIEM not specified [Medium]
...
### CMP-052. Tenant force-delete bypasses per-resource legal-hold filtering [Medium]
...
### CMP-053. ArtifactStore MinIO replication has no runtime residency fail-closed guard [Medium]
...
### API-010. — `CREDENTIAL_SECRET_RBAC_MISSING`, `GIT_CLONE_AUTH_UNSUPPORTED_HOST`, `GIT_CLONE_AUTH_HOST_AMBIGUOUS` use HTTP 400 for server-state rejections [High]

**Section:** (unspecified)



### API-011. — `ISOLATION_MONOTONICITY_VIOLATED` consolidated to 422 but cognate delegation-admission POLICY codes remain 403 [Medium]

**Section:** (unspecified)



### API-012. — `DELEGATION_PARENT_REVOKED` HTTP 409 PERMANENT inconsistent with existing `CREDENTIAL_REVOKED` 403 POLICY [Medium]

**Section:** (unspecified)



### API-013. — REST/MCP contract-test matrix not updated to cover new session-creation error codes (`VARIANT_ISOLATION_UNAVAILABLE`, etc.) [Medium]
...
### CPS-006. Orphaned partial-chunk objects when gateway crashes between chunk commit and manifest write [High]
...
### CPS-007. CheckpointBarrier timeout loses recoverable partial state (asymmetric with Stage 2 partial manifest) [Medium]
...
### CPS-008. `partial-{n}.tar.gz` naming convention inconsistent with `chunk_encoding: tar` [Medium]
...
### CPS-011. iter3 CPS-003 / CPS-006 / CPS-007 remain unresolved in current spec [Medium]
...
### WPL-001. Schedulability precondition missing on scrub_warning cleanup transition [High]
...
### WPL-002. Gateway lacks Node RBAC + informer to evaluate schedulability precondition [High]

**Section:** spec/06_warm-pod-model.md §6.3 ("Host-node schedulability precondition"), spec/04_system-components.md §4.6.3 (Gateway ServiceAccount), iter3 EXM-008
...
### WPL-003. preStop cap-selection alert is not computable from the emitted metric labels [Medium]
...
### WPL-004. Unschedulable-node branch missing from `task_cleanup` state diagram [Medium]
...
### CRD-012. Admin API endpoint table missing `PUT` and `DELETE` credential endpoints referenced by RBAC live-probe and CLI [Medium]
...
### DOC-014. Cross-file anchor `25_agent-operability.md#253-endpoint-split-between-gateway-and-lenny-ops` does not exist [Medium]
...
### DOC-015. Cross-file anchor `25_agent-operability.md#251-overview` does not exist [Medium]
...
### DOC-016. Intra-file anchor `#165-alerts` does not resolve in `16_observability.md` [Medium]
...
### MSG-012. MSG-007 iter3 fix omitted LTRIM from §12.4 Redis command allowlist [Medium]
...
### MSG-014. `message_expired` event schema not canonicalized; only the `reason` enum is defined [Medium]
...
### POL-018. Audit payload field names in §11.6 inconsistent with §16.7 and §4.8 after iter3 POL-015/017 fixes [Medium]
...
### POL-019. `AdmissionController` admission-path behavior during Redis outage is unspecified (fail-open/fail-closed posture undefined) [Medium]
...
### EXM-009. `Host-node schedulability precondition` not applied to scrub_warning re-warm transition [Medium]
...
### WPP-012. `apiKey`-mode bearer mint point is undocumented; `POST /v1/playground/token` rejects `Authorization: Bearer` [Medium]
