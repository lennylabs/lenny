...
### STR-003 Checkpoint Storage Fails Silently on Eviction — No Fallback Storage Path [High] — PARTIALLY FIXED
**Section:** 4.4
...
**Resolution:** Recommendation (1) was already addressed prior to this review: Section 4.4 already defines a Postgres-based "minimal session state record" fallback for eviction checkpoints when MinIO is unavailable. This fallback preserves conversation context (session_id, generation, conversation_cursor, last_message_context) and notifies the client via `resumeMode: "conversation_only"` with `workspaceLost: true`. This is a better solution than the ConfigMap/Secret approach recommended by the finding — Postgres is more durable, supports transactional writes, and avoids etcd size constraints. The finding's claim of "no fallback storage" was inaccurate against the current spec text. Recommendation (2) was implemented: the eviction retry budget was increased from ~5s to 30s (initial 500ms, factor 2x, capped at 5s per attempt), with explicit rationale documenting why the longer budget is appropriate for eviction (pod is terminating, 120s grace period provides headroom, transient MinIO issues typically resolve within 10-20s). Recommendation (3) was partially implemented: a new metric `lenny_checkpoint_eviction_fallback_total` (counter, labeled by `pool` and `had_prior_checkpoint`) was added, which distinguishes evictions with a prior full checkpoint from those without one. The metric name differs from the recommendation but provides equivalent observability. Explicit irrecoverability documentation was not added as a separate callout because the existing `workspaceLost: true` semantics and the Postgres fallback record already document this path clearly.

---

### STR-004 "No Shared RWX Storage" Non-Goal Not Validated Against Real Agent Workflows [High] — Partially Fixed
...
**Resolution:** Recommendation (1) was skipped — adding workflow validation appendices is scope creep for a technical design spec. The "no shared RWX storage" non-goal is a deliberate architectural decision, not an untested assumption; the spec documents the alternative mechanism (file export model, §8.8) in sufficient detail for implementers to evaluate its fitness. Recommendation (2) was implemented: Section 6.4 now specifies that `/workspace/shared/` is populated by the gateway during pod initialization from the Runtime's `sharedAssets` configuration, before any slot is assigned. It is mounted read-only at the container level via a separate `emptyDir` volume with `readOnly: true` on the volumeMount, so write attempts return `EROFS`. If no shared assets are configured, the directory is still mounted empty and read-only. Recommendation (3) was partially addressed: the default `maxTotalSize` was kept at 100MB (a deliberately conservative default appropriate for most delegation workflows), but deployer guidance was added to §8.3 documenting that the limit is configurable per delegation preset, can be raised up to the 500MB workspace SLO ceiling for large-artifact workflows, and that higher limits increase gateway I/O and MinIO bandwidth proportionally.

---

### STR-005 Checkpoint Scaling Under Long-Running Sessions — Unbounded Storage Growth [High] — Fixed
**Section:** 4.4, 12.5
...
**Resolution:** All three recommendations were implemented. (1) Section 12.5 checkpoint retention policy now explicitly states that in concurrent-workspace mode, the "latest 2" limit applies per `(session_id, slot_id)` pair, with a concrete example (8 slots × 2 = 16 checkpoint objects). (2) Concern about session derivation chains (item 2 in the finding) is already addressed by STR-002's copy-on-derive fix — `POST /v1/sessions/{id}/derive` always performs a full byte copy; `parent_workspace_ref` is a metadata lineage pointer only, not a GC dependency. No additional change needed. (3) Section 12.8 now documents that legal holds prevent TTL-based deletion but do not prevent checkpoint rotation — the "latest 2" retention policy still applies under legal hold, bounding storage growth. Full checkpoint history preservation under legal hold requires deployer-configured archival (e.g., MinIO bucket replication). A new metric `lenny_checkpoint_storage_bytes_total` (gauge, labeled by `tenant_id` and `pool`) was added to §16.1, and a `CheckpointStorageHigh` warning alert was added to §16.5.

---

### STR-006 Data-at-Rest Encryption Completeness — Disk-Backed emptyDir Unaddressed for Workspace [High] — Partially Fixed
...
**Resolution:** Recommendation (1) was partially implemented as a non-blocking preflight warning rather than a blocking check. The finding's suggestion to use cloud provider APIs (AWS `encrypted: true`, GCP CMEK) is impractical as a preflight gate: it requires cloud-specific IAM permissions the preflight Job may not have, varies across providers and bare-metal deployments, and cannot reliably determine which node pools will schedule agent pods (a scheduling-time concern). Instead, a `NodeDiskEncryptionUnverified` preflight warning was added to §17.6 that alerts deployers to verify node disk encryption manually, with provider-specific guidance (AWS EBS, GCP CMEK, Azure SSE). Recommendation (2) was fully implemented: Section 12.5 now mandates SSE-KMS with tenant-specific KMS keys for T4 tenants' workspace snapshots and session transcripts in object storage, enabling per-tenant key rotation and cryptographic erasure. The `ArtifactStore` selects the encryption key based on `workspaceTier` (T3 = deployment-wide key, T4 = `tenant:{tenant_id}` scoped KMS key), with runtime validation returning `CLASSIFICATION_CONTROL_VIOLATION` if the tenant-scoped key is unavailable. Recommendation (3) was replaced by the preflight warning approach — a continuous runtime alert is not feasible because Lenny has no mechanism to detect node disk encryption status from inside the cluster without cloud-provider API access. Section 6.4 was also updated to document the inherent T4 limitation for in-flight emptyDir data: node-level encryption + pod isolation (gVisor/Kata) + one-session-only lifecycle provide defense-in-depth, and deployers handling T4 workloads should use Kata or gVisor isolation profiles.
