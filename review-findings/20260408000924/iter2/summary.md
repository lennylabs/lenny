...
|---|------|-------------|---------|---------|
| 1 | NET-043 | Network Security | `dnsPolicy: cluster-default` DNS egress rule lacks pod-level scoping — degrades DNS security for all pools in namespace | 13.2 | **Fixed** |
| 2 | SCL-047 | Scalability | Tier 2 `burst_arrival_rate` (20/s) below sustained rate (30/s), undersizing `minReplicas` on Prometheus Adapter path | 17.8.2, 16.5 | **Fixed** |
| 3 | API-064 | API Design | No admin API endpoints for `runtime_tenant_access` / `pool_tenant_access` grants | 15.1, 4.2 | **Fixed** |
...
**Status:** ✅ Fixed — Added `CREATE` to the webhook's intercepted operations. Clarified that `PATCH`/`PUT` checks the existing resource's phase in etcd, and `CREATE` checks for any existing non-terminal `SandboxClaim` targeting the same `Sandbox`. Explicitly noted this protects the Postgres fallback claim path.
...
**Status:** ✅ Fixed — Added "Node drain timeout interaction" note with concrete example (840s for 8 slots), webhook warning when computed floor exceeds 600s, and preflight check.
...
**Status:** ✅ Fixed — Replaced MUST CRD write with SHOULD `AdapterTerminating(session_id, reason)` gRPC message on the existing pod-to-gateway control channel. Gateway transitions session immediately if received; otherwise falls back to orphan reconciler (60s). Acknowledged 60s worst-case delay as cost of zero-RBAC model.
...
**Status:** ✅ Fixed — Added `/run/lenny/credentials.json` to step 6 verification with explicit scrub failure if file persists.
...
**Status:** ✅ Fixed — Added "Tenant isolation (concurrent-stateless)" paragraph specifying same two-layer tenant pinning as task-mode. Pool controller rejects `allowCrossTenantReuse: true` for stateless pools.
...
**Status:** ✅ Fixed — Added "Cross-tenant validation" paragraph requiring `tenant_id` match before routing, rejecting with `CROSS_TENANT_MESSAGE_DENIED`. Validation occurs before scope evaluation and rate limiting.
...
**Status:** ✅ Fixed — Specified `lenny.dev/dns-policy: cluster-default` label scoping for the supplemental rule. WPC sets label only on opted-out pool pods. Label added to immutability enforcement in `lenny-label-immutability` webhook.
...
**Status:** ✅ Fixed — Changed Tier 2 `burst_arrival_rate` from 20/s to 30/s in both Path A and Path B tables. Path B `minReplicas` updated from 6 to 9.
...
**Status:** ✅ Fixed — Changed "approximately 23%" to "approximately 19%" in Section 16.5 `PostgresWriteBurstIops` alert description.
...
**Status:** ✅ Fixed — Documented array response as intentional Lenny extension. Added per-runtime standard-compliant endpoint `GET /a2a/runtimes/{name}/.well-known/agent.json` returning single AgentCard object per A2A spec.
...
**Status:** ✅ Fixed — Added supplementary session-level state mapping table after the task-level table. Maps pre-running states to `submitted`, `suspended`/`resume_pending` to `working` with metadata, `awaiting_client_action` to `input_required`/`input-required`.
...
**Status:** ✅ Fixed — Added `adapterLocalTools` array to manifest JSON example and field reference table with type, required status, and tier relevance.
...
**Status:** ✅ Fixed — Added `flush(stdout)` after every `write_line(stdout, ...)` in Standard-tier and Full-tier samples.
...
**Status:** ✅ Fixed — Updated Full-tier sample's `supported` array to include `"deadline_signal"`.
...
**Status:** ✅ Fixed — Replaced with `lenny-ctl admin pools get echo-pool`.
...
**Status:** ✅ Fixed — Added exception note to Section 24 preamble. Updated Section 24.2 with dual-mode description (standalone pre-install and API-backed post-install).
...
**Status:** ✅ Fixed — Extended Phase 4 to delete access tables, role mappings, custom roles, and tenant config record with `erasure_salt` KMS destruction. Tenant row retained as tombstone with `state = 'deleted'`.
...
**Status:** ✅ Fixed — Added "Resource tenant-scoping classification" table in Section 4.2 classifying all resource types.
...
**Status:** ✅ Fixed — Specified `__all__` sentinel mechanism with RLS policy clause. Restricted to `platform-admin` code path after RBAC verification. Integration test requirements added.
...
**Status:** ✅ Fixed — Replaced non-atomic check with `storage_quota_reserve.lua` atomic Lua script. Post-upload reconciliation adjusts reservation to confirmed bytes.
...
**Status:** ✅ Fixed — Added `^[a-zA-Z0-9_-]{1,128}$` validation at tenant creation and OIDC claim extraction with `INVALID_TENANT_ID` / `TENANT_CLAIM_INVALID_FORMAT` error codes.
...
**Status:** ✅ Fixed — Extended `budget_reserve.lua` to atomically read and validate `childrenTotal` and `parallelChildren` counters. Operation count updated to 5 READs + 4 WRITEs = 9. Added `CHILDREN_TOTAL_EXCEEDED` and `PARALLEL_CHILDREN_EXCEEDED` return codes. `budget_return.lua` decrements `parallelChildren`.
...
**Status:** ✅ Fixed — Added full field description for `perChildRetryBudget` (max automatic retry attempts per child, default 1, independent of `retryPolicy.maxRetries`, reuses token slice, retries count against `maxTreeSize`). Added to non-extendable field list.
...
**Status:** ✅ Fixed — Added `created` to the precondition states with description of `created`-state behavior.

### SLC-053. Adapter CRD write contradicts zero-RBAC agent pod security model [High → Duplicate of K8S-048]
**Status:** ✅ Fixed — See K8S-048.
...
**Status:** ✅ Fixed — Expanded query to check `running`, `attached`, `starting`, `suspended`, `finalizing`, `input_required`.
...
**Status:** ✅ Fixed — Added both canonical histogram metrics to 16.1 table. Updated burn-rate alert entries to reference them with appropriate label selectors.
...
**Status:** ✅ Fixed — Replaced "P99 latency" with "500ms, the P99 SLO target".
...
**Status:** ✅ Fixed — Added 14 missing metrics to 16.1 under three new subsection headers: Coordination & Reconciliation, Checkpoint Failure Tracking, Erasure & Tenant Lifecycle.
...
**Status:** ✅ Fixed — Changed enforcement condition from `gdpr` or `hipaa` to any regulated value (`soc2`, `fedramp`, `hipaa`) in both Section 12.8 and Section 18 defaults table.
...
**Status:** ✅ Fixed — Added scoped INSERT grant with `BEFORE INSERT` trigger restricting to `event_type LIKE 'gdpr.%'`.
...
**Status:** ✅ Fixed — Added 6 tenant-access endpoints (POST/GET/DELETE for both runtimes and pools). Added `tenantAccess` to bootstrap seed schema. Added CLI commands in new Section 24.3 Runtime Management and Section 24.4 Pool Management.
...
**Status:** ✅ Fixed — Added POST, GET, DELETE endpoints for billing-correction-reasons to Section 15.1.
...
**Status:** ✅ Fixed — Changed `{name}` to `{user_id}` in Section 15.1 and Section 24.9 CLI reference.
...
**Status:** ✅ Fixed — Added conditional transition `task_cleanup → sdk_connecting` for preConnect-enabled task-mode pods in Section 6.2.
...
**Status:** ✅ Fixed — Added `+ adjusted_burst_claims × pod_warmup_seconds` term with `delegation_burst_claims` variable. Updated worked example and recommendation numbers.
...
**Status:** ✅ Fixed — Added `source` field and conditional `user_id`/`credential_ref` or `pool_id`/`credential_id` to the `credential.leased` event.
...
**Status:** ✅ Fixed — Added fallback specification to proactive lease renewal step 2.
...
**Status:** ✅ Fixed — Added local timer specification to `anthropic_direct` provider table entry. Adapter deletes credential file and reports `AUTH_EXPIRED` on expiry without replacement.
...
**Status:** ✅ Fixed — Updated example to `lenny-blob://tenant_acme/session_xyz/part_ws001?ttl=2592000&enc=aes256gcm`.
...
**Status:** ✅ Fixed — Changed `TaskResult.status` to `TaskResult.state` in both completed and failed examples.
...
**Status:** ✅ Fixed — Added `slotId` to MessageEnvelope JSON schema, field descriptions, Protocol Reference examples, and ignorable envelope fields list.
...
**Status:** ✅ Fixed — Removed `RotateCredentials RPC` from Phase 12a. Added clarification that it's delivered in Phase 11.
...
**Status:** ✅ Fixed — Updated parallelism note to include Phase 5 (`ExternalAdapterRegistry`).
...
**Status:** ✅ Fixed — Added disambiguation paragraph to Section 11.6. Updated cross-references in Sections 4.1, 4.3, and 10.2 to clarify they refer to automatic, in-memory breakers distinct from operator-managed ones.
...
**Status:** ✅ Fixed — Changed `experiment_id` type from `uuid` to `TEXT` in the `EvalResult` schema.
...
**Status:** ✅ Fixed — Changed both "Section 5.3" references to "Section 5.2".
...
**Status:** ✅ Fixed — Added tree-memory counter to `budget_reserve.lua` (6 READs + 5 WRITEs = 11 operations). Added `TREE_MEMORY_EXCEEDED` error code. `budget_return.lua` decrements tree memory.
...
**Status:** ✅ Fixed — Updated to specify snapshotting covers all `DelegationPolicy` resources including `maxDelegationPolicy` and derived-runtime policy.
...
**Status:** ✅ Fixed — Replaced incorrect persistence claim with accurate termination description.
...
**Status:** ✅ Fixed — Added `attached → cancelled` and `cancelled → task_cleanup` transitions to task-mode state machine.
