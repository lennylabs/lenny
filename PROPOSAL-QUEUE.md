...
- **status:** landed:0049 (implemented by proposal-A 2026-07-20; SPEC-4 §25.5 deliveries-pagination amendment applied and verified as its own commit; the reached tiers are green — build/vet, tier-0 static, tier-1/2 touched packages, tier-3 contract (deliveries-cursor + SSE/CloudEvents framing), tier-4 integration (gateway-buffer fallback), tier-7a load (all §25.5 event-stream tests), tier-8 chaos (recovery flush + source switch under fault injection), and tier-9 security (read-path cross-tenant isolation) all run and pass locally via Docker; only tier-5 e2e-Kind (two-replica install with real DNS-SRV) is un-run, needing a Kind cluster — it is un-skipped and compiles. Closes T-25.5.1/.5/.7/.15/.20 + T-25.3.5 (RESOLVED with shas), records T-25.5.14. T-25.5.6 (single-implementor aggregator) and T-25.12.6 (MCP-native subscription) deferred to follow-on clusters — see Integrator inbox. Ready for --no-ff integration.)
- **assigned:** proposal-A
...
- **status:** open
- **assigned:** (unassigned — held for proposal-A's lane per the pod-conflict rule; assign after C-14 and proposal-B's C-22 eviction work quiesce to avoid a three-way pod-claim collision)
- **findings:** T-8.2.17
...
- [ ] from proposal-A (0049 / `proposal-a/c-02-ops-event-stream-read`): FILE A NEW CLUSTER for the §25.12 MCP-native operational-event subscription (`notifications/subscribe` + `notifications/message` streaming over `/mcp/management`), finding **T-25.12.6**. Deferred out of C-02/0049 on human sign-off: it needs a §25.12 **spec amendment first** (the three sentences at spec/25:4661-4663 leave the transport, the `notifications/subscribe` param structure, and the `notifications/message` payload envelope underspecified) and it **restructures the management server's one-request-one-response contract** (`pkg/ops/mcp/server.go:119-144` decodes exactly one JSON-RPC request and writes exactly one response). It layers on the §25.5 read source 0049 builds, so sequence it after 0049 lands. Recommend a change-proposal in *new* mode staging the §25.12 transport/param/payload amendment before the server code. T-25.12.6 stays OPEN meanwhile. Suggested cluster name: "§25.12 MCP-native event subscription".
- [ ] from proposal-A (0049 / `proposal-a/c-02-ops-event-stream-read`): tier-7a load-scenario flake, NOT from this branch (no podclaim/slot code touched) — `TestScenarios/slot_leaked_counted_race` (§4.6/§8 pod-slot-counter release path) breaches its latency floor once (`floor_breaches=1`, `leaked_release=64`, `context deadline exceeded`, ~0.03% error) under full-suite host/Docker contention, but PASSES 2/2 in isolation. Observed while verifying 0049's tier-7a run. For a §4.6/§8 owner to assess (tune the floor or investigate the release tail latency); low severity. Not filed as a TEST-GAPS finding since it is environmental load-sensitivity in another cluster's domain, not a coverage gap.
- [ ] from proposal-A (0049 / `proposal-a/c-02-ops-event-stream-read`): FILE A NEW CLUSTER (or reclassify) for the unified `EventStreamService` production aggregator, finding **T-25.5.6**. Deferred out of C-02/0049: the eight-method interface (`pkg/ops/events/interface.go:26-35`) has zero runtime consumers — no production code accepts, returns, injects, or stores it, and no `var _ EventStreamService = …` assertion exists; the HTTP routes are the contract and 0049's read-side build lands against the concrete types. Building an aggregator solely to satisfy an assertion against a documentation-only interface adds a synthetic parallel type with no consumer, so 0049 did not build it. Note: 0049 DOES build the `ListDeliveries` keyset cursor and amends §25.5 to document deliveries pagination (the cursor sub-question of T-25.5.6/T-25.5.7 closes with 0049); the residual for this cluster is only the single-implementor-of-the-8-method-interface question. The integrator may prefer to reclassify T-25.5.6 as won't-build (interface is illustrative) rather than open a cluster. T-25.5.6 stays OPEN meanwhile.
