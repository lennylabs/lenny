...
- **status:** open — **trigger proposal 0062 SUPERSEDED 2026-07-27, DO NOT IMPLEMENT.** Its spec edits and partial implementation were reverted on branch `proposal-b/c-22-eviction-trigger` (commit `54292da1`), so nothing it staged is applied and the branch carries no eviction code. The trigger work is re-scoped into steps R12, R13, R16, and R18 of `gateway-runtime-comms-remediation.md`, and step R1 of that plan renames the control stream 0062 was written against, so resuming 0062 would collide head-on with the rename. Any automated drain (`build-gaps-spec-unblock`) must skip it. Prereqs 1 and 2 stay landed and are unaffected. Prereq history: prereq 1 landed:0050 (2026-07-19); prereq 2 **landed:0052** (integrated `--no-ff` 2026-07-22 on branch-recorded sign-off, per the settled trust-branch-sign-off policy). 0052 is a single §4.6.1 spec-prose clarification: an evicted agent pod's preStop cannot open the gateway-driven `Checkpoint` stream, so it signals its coordinating gateway replica over the existing per-pod control channel (`AdapterTerminating`), and that replica — the session-coordination-lease holder — drives `CheckpointWithTrigger(TriggerEviction)` under its held lease; an unreachable coordinator is recovered via the §10.1 TTL-driven handoff, fail-closed. Plus one tier-11 route-consistency test; no product code. Verified on integration: `go build ./...` green, validate-maps 15/15, the tier-11 route-consistency test passes, reconcile clean. 0052 closes no finding; unblocks-not-closes T-4.4.14/T-4.6.4. The within-window takeover of an UNREACHABLE coordinator, the forward-to-holder transport, and any takeover primitive are deferred to the trigger-wiring proposal (coordinator-direct seam resolved). prereq 1 = **0050 slot-aware checkpointing** (integrated 2026-07-19). **Prereq 3 (agent-pod grace budget) turned out a no-op** per proposal-B's analysis (2026-07-22): the grace-budget arithmetic needed no new spec/code work, but the analysis surfaced a standalone admission-correctness defect in the pool-config grace-floor webhook, fixed and landed separately as **C-48/0056** (nil-bypass + gateway/agent BarrierAck conflation). Prereq 3 is therefore considered discharged. **Prerequisite 4 (runtime-container preStop) VALIDATED as REAL (proposal-B 4-angle check 2026-07-22) but its literal scope-note framing was a code misreading; its real content is a runtime-container quiescence-ordering race (nothing holds/quiesces the third-party sidecar runtime container before kubelet SIGTERM, so the eviction checkpoint's quiesce handshake loses the race → workspace lost). Per reviewer decision, prereq 4 is FOLDED INTO the trigger proposal rather than a separate branch. **C-22 trigger 0062 SUPERSEDED (2026-07-27) and reverted; the design record below is retained for the re-scoped steps. Decision 2's forward was reframed (2026-07-24, human directive) to a GENERIC coordinator-forward transport (extensible request-type payload, eviction-drive as first consumer; §7.2 message semantics out of scope, named as future consumer) — error-checked clean via review mode.** 0062 is the CULMINATING C-22 change (closes T-4.4.14/T-4.6.4, folds prereq 4, closes the cluster). Design: a new §4.7 `AdapterEvicting` adapter→gateway control event that the adapter's SIGTERM handler emits per live bound session on kubelet termination; a NEW gateway-side control-stream consumer drives the existing `CheckpointWithTrigger(TriggerEviction)` under the held lease. Prereq 4's runtime-quiescence race is resolved NOT by a runtime-hold but by a BEST-EFFORT eviction snapshot: the /workspace emptyDir survives the runtime's SIGTERM and the still-alive adapter archives it directly (gated on an in-process `evicting` flag, not the shared TriggerEviction value which barrier.go:228 also drives on running pods), resuming as resumeMode:full/workspaceLost:false. Embedded model unchanged; barrier driver reused; no consistency-marker field (no carrying surface); T-4.4.20 already resolved in code. Tiers 1/3/4/5/7a/8. 3 open decisions: (1) ratify best-effort snapshot vs the heavyweight runtime-hold; (2) THE KEY ONE — confirm the coordinator-holds-the-control-stream invariant OR ratify forwarding to the holder (else AdapterEvicting on a non-holder → ErrNoBinding → silent no-checkpoint → workspace loss; this is the within-window transport 0052 deferred); (3) accept the unreachable-coordinator TTL-handoff residual (no forced-takeover primitive). (the eviction trigger + prereq-4 runtime quiescence-ordering + the deferred 0052 unreachable-coordinator/forward-to-holder decisions + the §4.4 gateway-pod grace doc-nit). C-22 stays open and assigned to proposal-B. **RENUMBERED 0057→0059→0062 (2026-07-26): integration landed a different 0059 (C-50). REVISED against the co-located coordination model of proposal 0060, which the branch now merges and which must land first. Co-location retires the fresh-dial driver leg (0060 DELETES barrier/wiring.go Conn-or-Dial and the coordinator now always holds the binding, so the drive reuses the binding-gated CheckpointWithTrigger unchanged and adds no binding-free entry point); the FORWARD leg survives on the concurrent-session case alone (one pod carries one control stream but its slots bind, and so coordinate, per session, so a maxConcurrentSessions>1 pod can have slots on different replicas), so coordinator_address, the inter-replica port, SPEC-4 and CHART-1 all stay; the forwarded payload drops the pod dial address; the mirror seed folds into 0060's acquireCoordinationLease; and new SPEC-6 corrects three clauses of the ALREADY-APPLIED spec text that 0060 falsifies. Section 12 of the proposal records the revision.**
- **assigned:** proposal-B
...
- **status:** claimed:proposal-A on `proposal-a/c-49-path6-remaining` (re-approached 2026-07-26 on the post-0060 co-location baseline). Filed 2026-07-22 by the integrator from proposal-A's C-14/0058 inbox item; the deferred cross-section follow-ons of the §7.2 path-6 build 0058 landed. **Superseded branch:** the first attempt, `proposal-a/path6-remaining-pieces` (proposal 0056, converged but never signed off; 3 commits on a pre-0060 base), was abandoned in favor of the re-approach — C-52/0060 changed the ground under it by establishing the lease/binding co-location invariant, which is what makes the T-JRN.15 forward-to-coordinator wiring correct. The integrator has NOT deleted the superseded branch (it is unmerged and carries unique commits); retire it once the re-approach lands and its content is confirmed carried over. Note the scope drifted during the first attempt (Piece 3 reframed to async shared-inbox with ForwardMessage retired, a Piece 5 durable-inbox-as-default added), so reconcile this cluster's `findings:` line against the final scope at integration.
- **assigned:** proposal-A
...
### C-50 — Open Responses server-side `previous_response_id` continuation — §15/§14.1
- **status:** landed:0059 (filed and integrated together 2026-07-25 by the integrator on branch-recorded human sign-off, at proposal-C's inbox request. Minted 0056 on the branch, which COLLIDED with the landed C-48 0056 — renamed to **0059** on integration; 0060 was skipped because proposal-B has claimed it for an in-flight proposal (`coordinator-binding-colocation`). The branch's stale refs to "proposal 0055" (C-13's pre-renumber number) were corrected to 0057. Fixes a silent wrong-answer bug: `POST /v1/responses` accepted and echoed `previous_response_id` but never honored it server-side, so a multi-turn Responses SDK client got the model answering with zero prior context. Build: `Session.ContinuationParentID` (deliberately distinct from `ParentSessionID` to stay clear of the §8.2/§8.6 delegation and orphan-cleanup sweep) persisted through the §14.1 request-envelope JSONB bundle (no column, no migration); a translator-local `continuity.go` helper resolving `previous_response_id` fail-closed (unknown/cross-tenant → native 404), chain-walking the `ContinuationParentID` chain over per-response single-turn transcript buckets and rehydrating the full prior conversation ahead of the new turn in one `exec.Send`; GET echoes `ContinuationParentID`. Sign-off decisions: chain-walk rather than copy-forward (O(N) storage, single-source-of-truth erasure); `truncation` ignored (context-window management is the runtime's job); no replay cap; translator-local helper. One §15 single-shot continuation clause applied as its own commit; the `:581` proper-superset clause was NOT narrowed. Verified on integration: `go build ./...` + vet green, translator/sessionstore/pgstore tests pass, validate-maps 16/16, reconcile clean. Worker verified tiers 0/1/2 (tier-2 against a real Postgres container) at 98.2% changed-line coverage. Closes no TEST-GAPS finding — a follow-on discharging C-13/0057's Open decision 1 option (b).)
- **assigned:** proposal-C
- **findings:** (none — follow-on work discharging a C-13 sign-off decision; closed no pre-existing finding)
- **root spec gap:** §15 defined `previous_response_id` acceptance for the Open Responses adapter but no server-side continuation semantics, so the single-shot compute model C-13/0057 built had no way to carry prior conversation across turns.
- **severity:** High (silent wrong-answer for multi-turn Responses SDK clients)
...
- **proposal scope:** Run the captured draft through the change-proposal pipeline: define the idle-TTL pod-hold for single-shot adapter sessions (hold vs release decision point, TTL default and override, §6.2 recycle/disposition interaction, warm-pool capacity accounting, and the eviction/expiry path), then build it. Sequence after C-50 (it builds on both the C-13/0057 single-shot model and C-50/0059's continuation). Shares the warm-pool claim/release primitives, so coordinate with proposal-B's C-22 eviction lane via the inbox if it touches them.
- **severity:** Medium
- **relates to:** C-13 (landed 0057), C-50 (landed 0059) — the same §15 single-shot adapter stack, kept single-worker on proposal-C.

### C-52 — Co-locate the session-coordination lease with the pod binding — §10.1/§4.6.1/§7.2/§7.3
- **status:** landed:0060 (filed and integrated together 2026-07-26 by the integrator at proposal-B's request, on explicit human sign-off — "Approved 2026-07-25 by jaf sign-off with a direction to implement". No renumber: 0060 was free because the integrator deliberately skipped it when renumbering C-50, reserving it for this in-flight branch. A jaf-directed architectural change wiring the invariant the spec already assumes: the replica holding the session-coordination lease is always the replica that can serve the session over its live adapter connection. Mechanism: acquire the lease at BIND time on the binding replica (`registerBinding`, hoisted ahead of the running-commit on single-call `/start`, the delegated-child path, and checkpoint-restore); reconcile the Sweeper into a renewer plus crash-takeover adopter (adopts only a lapsed lease of a still-running-pod session, probes held-channel liveness, and evicts binding + executor stream + releases the lease on a dead channel); re-establish the binding EAGERLY on the crash-takeover edge via a fence-first re-adopt (dial and `CoordinatorFence` before the version handshake because the pod is in §10.1 hold state, publish on fence-ack, hold the connection); relinquish and back off on terminal fence failure; retire the production-dead barrier fresh-dial. **No spec edit** — the invariant and re-adopt-on-handoff are already normative in §10.1/§4.6.1/§7.2/§7.3, so the gap was code wiring. No forced-acquisition primitive, no checkpoint restore, no new wire field. Converged hard: 12 adversarial rounds, 19 findings fixed (lapsed-vs-fresh lease conflation, lease-steal windows, fence-first ordering, dead-channel recovery, executor-stream-cache shadowing). Builds the two-replica gateway harness (`tests/testinfra/coordfixture`), which **closes T-4.2.4** (marked RESOLVED by the integrator on integration — the branch left TEST-GAPS untouched). Sign-off decisions ratified: D1 acquire-at-bind; D2 re-adopt eagerly; D3 fresh-dial retired rather than gated; D4 forwarding out of scope; resume-path `ErrHeld` fail-closed as an interim. Verified on integration: `go build ./...` + vet green, coordination/podsession/executor/adapterclient tests pass, validate-maps 16/16, reconcile clean. Worker verified tiers 0/1/2/4/7a/8 at 87.4% changed-line coverage.)
- **assigned:** proposal-B
- **findings:** T-4.2.4 (RESOLVED on integration — the two-replica split-brain fence harness this cluster built)
- **root spec gap:** none — the co-location invariant and the re-adopt-on-handoff behavior are already normative in §10.1/§4.6.1/§7.2/§7.3. The defect was that the code never wired them: the coordination lease and the pod binding were acquired independently, so the lease-holding replica was not necessarily the replica able to serve the session over its live adapter connection.
- **severity:** High (distributed-coordination correctness under multi-replica HA)
- **relates to:** **C-49/T-JRN.15** — the off-holder REST forwarding (messages/interrupt/upload-to-session over `ForwardMessage`) was deliberately scoped OUT of 0060 and belongs to proposal-A's C-49: co-location establishes the invariant that makes forward-to-coordinator correct, and C-49 wires the forwarding. **C-22** — 0060 unblocks simplifying the eviction trigger's `coordinator_address`/fresh-dial workaround; the `proposal-b/c-22-eviction-trigger` branch was paused for this and **must rebase onto 0060 before it integrates** (same coordination-lease seam, same worker).

### C-53 — Slot-aware checkpoint RESTORE companion for concurrent-session pods — §4.4/§6.4/§10.1
- **status:** landed:0073 (superseded by proposal 0073, 2026-08-26 — C-53's scope lands in proposal 0073 rather than as its own cluster, and no branch is opened for it. 0073 binds every session to a slot on every pod and makes the per-slot tree the only layout, so the restore companion this cluster was filed for is built there: restore, `ExportPaths`, and the gateway's workspace-root derivation resolve the slot root from the session identifier, `Binder.Resume` reserves the resumed session's slot on the pod its connect already claimed, and the persisted checkpoint tables re-key on `session_id` alone. 0073 carries the sibling defects a point fix would have left standing, which are the export root, the §7.3 step (d) expected-workspace-root guard, and the coordinator hold-state arming. T-4.4.21 stays OPEN for its remaining test leg: 0073 lands the restore path and the tier-4 concurrent-pool capture-independence case, and the concurrent-pool capture, pod-loss, and resume round trip the finding asks for is not yet in the tree. Filed 2026-07-27 by the integrator from proposal-B's C-22 follow-up audit. Verified against impl before filing: T-4.4.21 was High and OPEN, and no cluster's `findings:` line owned it — it appeared only in two `[x]` inbox notes from the 0050 integration, so it had been carried as a note rather than as tracked work.)
- **assigned:** proposal-B
- **findings:** T-4.4.21 (stays OPEN in `TEST-GAPS.md`, tracked on that finding rather than on this closed cluster — proposal 0073 lands the slot-tree restore path, the tier-4 concurrent-pool capture-independence case, the adapter unit cases that pin extraction into the resuming session's own slot tree, and the single-session Kind round trip; the concurrent-pool capture-loss-resume round trip the finding asks for is still unwritten)
- **root spec gap:** Proposal 0050 (C-22 prerequisite 1) implemented only the **capture** side of slot-aware checkpointing: `slot_id` on `CheckpointStart`, slot-scoped `checkpointRoots()`, and the per-slot op-lock. The **restore companion** that makes a concurrent-session pod (`maxConcurrentSessions > 1`) actually resumable is unbuilt, so such a pod can be checkpointed but never restored. That leaves 0050's capture side, and any eviction trigger built on it, without an end-to-end path on exactly the pools the slot work exists to serve.
- **proposal scope:** Build the restore side of slot-aware checkpointing so a concurrent-session pod round-trips: slot-scoped restore and materialization, the resume path's slot binding, and the end-to-end capture and restore coverage on a `maxConcurrentSessions > 1` pool that 0050 could not write. Superseded in part: proposal 0073 builds the first two, addressing the slot by the session identifier rather than by a separate `slot_id`. The third is not built (no test captures a checkpoint on a `maxConcurrentSessions > 1` pool, loses that pod, and resumes the session onto a replacement), and it stays tracked on T-4.4.21 rather than on this closed cluster.
- **severity:** High
- **relates to:** C-22 (0050 landed the capture half; this is the companion that completes it). Independent of the `gateway-runtime-comms` remediation re-scope — the restore gap stands regardless of how the trigger is re-clustered.
...
- [x] from proposal-C (0059 / `proposal-c/openai-responses-continuity`): **FILE A NEW CLUSTER and integrate — 0059 signed off and IMPLEMENTED green.** Follow-on to C-13/0057, discharging proposal 0055's Open decision 1 option (b): server-side `previous_response_id` continuation for `OpenResponsesAdapter` (`POST /v1/responses`), which 0057 accepted+echoed but did not honor server-side (a silent wrong-answer bug for multi-turn Responses SDK clients that get the model answering with zero prior context). Build: a new `Session.ContinuationParentID` (distinct from `ParentSessionID` to avoid the §8.2/§8.6 delegation + orphan-cleanup sweep) persisted through the §14.1 request-envelope JSONB bundle (no column, no migration); a translator-local `pkg/gateway/environment/translator/continuity.go` helper that resolves `previous_response_id` fail-closed (unknown/cross-tenant → native 404), CHAIN-WALKs the `ContinuationParentID` chain reading each response's own single-turn transcript bucket, and rehydrates the full prior conversation (no cap) prepended ahead of the new turn in the one `exec.Send`; each turn recorded on its own bucket; GET now echoes `ContinuationParentID`. Design settled by human sign-off: chain-walk (not copy-forward — O(N) storage, single-source-of-truth erasure); `truncation` IGNORED (context-window management is the runtime's job; a runtime overflow flows the existing 0057 runtime-error path); no replay cap (full history); translator-local helper (no standalone package). One §15 spec amendment (single-shot continuation clause), applied as its own commit `86ddc9d9`; the `:581` proper-superset clause is NOT narrowed. Independently verified: green + review-clean, tiers 0/1/2 (tier-2 ran against a real Postgres container), 98.2% changed-line coverage (107/109; only a defensive transcriptstore infra-error branch uncovered). Closes NO TEST-GAPS finding (follow-on); TEST-GAPS untouched. **Branch is based on pre-C-13-merge impl**, so it needs the same on-integration reconciliation done for C-42 (0050→0054) and C-13 (0055→0057): renumber 0056 (collides with C-48's 0056 on impl) and reconcile the `proposals/` dir + PROPOSAL-QUEUE. Also captured on the branch: `proposals/0057_draft_single-shot-adapter-pod-hold-idle-ttl.md`, an unconverged design draft for a future follow-up (hold the single-shot adapter pod for an idle TTL to avoid the pod-per-message claim + rehydration cost) — please open a cluster for it too and sequence it after this. → Done 2026-07-25 (integrator): filed cluster **C-50** (landed:0059) for the continuation build and **C-51** (open) for the captured idle-TTL pod-hold draft, both assigned proposal-C and sequenced C-50 → C-51. Renumbered 0056→**0059** (collided with landed C-48) and the draft 0057→**0061** (collided with landed C-13; 0060 skipped — claimed in-flight by proposal-B's coordinator-binding-colocation), and corrected the branch's stale "proposal 0055" refs to 0057. Integrated `--no-ff`; verified build/vet green, translator + sessionstore/pgstore tests pass, validate-maps 16/16, reconcile clean.
- [x] from proposal-B (0060 / `proposal-b/coordinator-binding-colocation`): **jaf-directed ARCHITECTURAL proposal — please create/assign a cluster.** Co-locate the session-coordination lease with the pod binding so the coordinating replica always serves the session (the spec's assumed-unified model the code never wired). Mechanism: acquire the lease at BIND time on the binding replica (registerBinding + hoisted ahead of the running-commit on single-call /start, delegated-child, checkpoint-restore); reconcile the Sweeper into a renewer + crash-takeover adopter (adopts only a lapsed lease of a still-running-pod session; probes held-channel liveness and evicts binding+executor-stream+releases-lease on a dead channel); re-establish the binding EAGERLY on the crash-takeover edge via a fence-first re-adopt (dial + CoordinatorFence before the version handshake since the pod is in §10.1 hold state, publish on fence-ack, hold the connection); relinquish+backoff on terminal fence failure; retire the (production-dead) barrier fresh-dial. NO spec edit (the invariant + re-adopt-on-handoff are already normative in §10.1/§4.6.1/§7.2/§7.3 — the gap is code wiring), NO forced-acquisition, NO checkpoint restore, NO new wire field. Converged HARD: 12 rounds, 2 clean, 19 findings fixed (deep distributed-coordination bugs: lapsed-vs-fresh lease conflation, lease-steal windows, fence-first ordering, dead-channel recovery, executor-stream-cache shadowing). Reaches tiers 0/1/2/4/7a/8; builds the two-replica gateway harness (closes T-4.2.4). **Deliberately scoped OUT: the off-holder REST forwarding (messages/interrupt/upload-to-session over ForwardMessage) = C-49/T-JRN.15 — co-location establishes the INVARIANT that makes forward-to-coordinator correct; the forward wiring is that separate cluster.** Downstream (not built here): unblocks simplifying the C-22 eviction trigger's coordinator_address/fresh-dial workaround (proposal-b/c-22-eviction-trigger, paused). Status: **IMPLEMENTED GREEN — jaf signed off 2026-07-25 and directed implementation; independently verified, ready for `--no-ff` integration.** All open decisions ratified on sign-off (D1 acquire-at-bind, D2 re-adopt-eagerly staged; D3 fresh-dial RETIRED not gated; D4 forwarding out-of-scope to C-49/T-JRN.15; resume-path `ErrHeld` fail-closed interim). NO spec edit (validate-maps unchanged). Independently verified green on reached tiers 0/1/2/4/7a/8: tier-0 build/vet clean (the 13 lint findings are all pre-existing in files this branch does not touch); tier-1 executor/barrier/adapterclient/cmd/coordination `-race`; tier-2 envtest podsession + sessionserver + tier2-component/coordination (run SOLO); tier-4 `TestCoordinationSplitBrainFenceAcrossTwoReplicas_spec_10_1` on the compose stack; tier-7a `TestColocationInvariantUnderConcurrentHandoff_spec_10_1` `-race`; tier-8 `TestCoordinatorFailoverCrashTakeover_spec_10_1` (3 subtests: lapsed-lease adopt+re-fence+fence-out-stale, dead-held-channel evict+re-adopt+re-fence, terminal-fence relinquish+backoff-without-generation-climb). Changed-line coverage 87.4%. **On integration: 0060 is free on the current tip (integrator already reserved it by skipping it for this branch); no renumber expected. Two-replica gateway harness added closes T-4.2.4.** → Done 2026-07-26 (integrator): filed cluster **C-52** (landed:0060), assigned proposal-B. No renumber needed — 0060 was reserved by skipping it during C-50's renumber. Marked **T-4.2.4 RESOLVED** (the two-replica split-brain fence harness closes it; the branch left TEST-GAPS untouched). Recorded the C-49/T-JRN.15 boundary (off-holder forwarding stays proposal-A's) and the note that `proposal-b/c-22-eviction-trigger` must rebase onto 0060 before it integrates. Verified build/vet green, coordination/podsession/executor/adapterclient tests pass, validate-maps 16/16, reconcile clean.
- [x] integrator infra observation 2026-07-26 (during C-52/0060 integration): **`pkg/gateway/podlifecycle/podsession` now exceeds Go's default 600s test timeout on its own** — it FAILED at 600.7s co-running with two other suites (stack in `controller-runtime .../process.(*State).Start`, the envtest control plane failing to start), then PASSED **solo at 623.9s**. So a plain `go test` with no `-timeout` fails this package regardless of contention, and a green change reads as red. This is the second instance of the class proposal-B reported for `pkg/controller/warmpool` (~587-600s). Run the envtest packages with an explicit `-timeout` (and solo, as 0060's own ask advised) in CI and on integration. Test-infrastructure timing, not a spec-coverage gap, so no TEST-GAPS finding; recorded here as the durable trail.
- [ ] from proposal-B (C-22 follow-up audit, 2026-07-26): **FILE A CLUSTER for T-4.4.21's restore half — it is the one C-22 deferral with no owning cluster.** Auditing everything the eviction-trigger proposal (0062, in review on `proposal-b/c-22-eviction-trigger`) leaves for follow-up against this file, every item resolves to a cluster except one. Tracked and needing nothing: the §7.2 message subsystem and the three off-holder-broken coordinator consumers (`POST /v1/sessions/{id}/messages`, `interrupt`, `upload-to-session`) are **C-49/T-JRN.15**; **T-4.2.4** was resolved on the C-52/0060 integration; the accepted unreachable-coordinator residual is **C-47** (open, proposal-B). **Untracked: T-4.4.21** (High, OPEN in TEST-GAPS) — proposal 0050 (C-22 prereq 1) implemented only the CAPTURE side of slot-aware checkpointing (`slot_id` on `CheckpointStart`, slot-scoped `checkpointRoots()`, per-slot op-lock); the **restore companion** that makes concurrent-session pods (`maxConcurrentSessions > 1`) actually resumable is unbuilt, and no cluster lists T-4.4.21 in its findings — it appears only in two `[x]` inbox notes from the 0050 integration, so it has been carried as a note rather than as work. Until it lands, a concurrent-session pod can be checkpointed but not restored, which leaves the capture side of 0050 and the eviction trigger of 0062 without an end-to-end path on exactly the pod class both were extended to cover. Suggested cluster: "§4.4 slot-aware checkpoint restore (T-4.4.21 restore companion)", §4.4/§5.2/§6.4, sequenced after C-22 and kept with proposal-B since it shares `pkg/checkpoint` and the slot model with 0050/0062. Secondary heads-up, no cluster needed: 0062's TEST-3 wires the deferred checkpoint half of `tests/tier5_e2e_kind/scaffolds_test.go::TestNodeDrainDuringActiveSession` and `checkpoint_resume_test.go`, which need a Kind cluster to run — expect them implemented-and-unrun locally under the standing infra escape hatch when 0062 lands.
- [ ] from proposal-B (C-22 / 0062, 2026-07-27): **0062 is SUPERSEDED and its spec plus code are reverted; C-22's trigger needs re-clustering, and no automated drain should pick 0062 up.** Two documents now sit at the repo root: `gateway-runtime-comms.md` maps every communication channel between the gateway, the agent pod, the adapter, and the runtime, and records thirty verified gaps; `gateway-runtime-comms-remediation.md` sequences twenty-five steps that close them. Both were built from independent derivations and adversarial verification, and seventeen of twenty load-bearing claims required correction, so the labels in them are load-bearing: WIRED means reachable from production, UNWIRED means implemented with no production caller. **Why 0062 was superseded rather than resumed:** its applied spec text asserted that the seeded binding replica holds no coordination lease, which proposal 0060's co-location (landed as C-52) made false; its adversarial error-check ran twelve rounds without converging, oscillating on the read-side routing predicate; and the root cause is structural, in that the adapter admits one control stream per pod (`pkg/adapter/controlchannel.go:110`) while co-located coordination is per session, so a concurrent-session pod cannot route its events by that design. Its gateway half was never built, so the adapter-side event it staged had no production consumer. Its content is re-scoped into remediation steps R12, R13, R16, and R18. **What the integrator needs to decide:** whether to re-cluster C-22 against the remediation plan's steps, and how the plan's twenty-five steps map onto clusters generally. The plan's step R1 renames the control stream 0062 was written against, and it must land before any re-scoped step begins, which is why 0062 is frozen now rather than later. Prereqs 0050 and 0052 stay landed and are unaffected. No spec, code, or chart change is requested by this note.
- [x] from proposal-B (0067 apply, `proposal-b/gateway-runtime-comms-remediation`, 2026-08-03): **the `spec/28` ownership transfer that proposal 0067 stages into proposal 0064 landed whole; nothing on this axis is open.** 0067's amendment enumerates thirteen sentence-level edits into `proposals/0064_fix_name-the-communication-channels-and-move-them-into-the-spec.md`, and all thirteen are applied and verified: the §28 targets and change text of the naming-pass and identifier-pass sub-steps, the §3.5 build-order bullet, the §4.8 lead-in, the §11 files-touched bullet, §3.1's opening paragraph, and the two landing arguments of the sub-step that appends §28.5 through §28.8. Proposal 0064 now credits proposal 0067 wherever it names the file, its §28.1 through §28.4 headings, or the register they carry; §3.1 reads one new specification file; and the §28.8 bijection-timing argument stands unchanged, with the `tests/spec-map.json` key and the `tests/spec-map-exceptions.yaml` entry for those headings left with the naming-pass sub-step, which is the obligation half the amendment keeps there. An earlier revision of this note recorded the transfer as ten edits with two sub-step clauses and the §3.1 newness claim left standing and asked for a follow-up amendment; those three are edits 11 through 13 of the same amendment and are applied, so the ask is discharged. A documentation-tier check now sweeps this record alongside proposal 0064, so a note here and that document cannot state opposite things about who creates the section. The status line, the §1 scope paragraph, and the convergence record of proposal 0064 are untouched, as the amendment requires. No spec, code, or chart change is requested by this note.
