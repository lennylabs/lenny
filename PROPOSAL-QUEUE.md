...
- **status:** in-review:0050 on `proposal-c/c-42-delegate-pod-allocation`, **AWAITING human sign-off (not integrated).** ⚠ Proposal number 0050 COLLIDES with the landed C-22 prereq-1 (0050); on integration the integrator renames it to the next free number (**0054** after 0052 C-22-prereq-2 and 0053 C-02) and updates every reference. Converged on option (b): reconcile the §8.2 wording to the deferred create-then-materialize model (spec-only, no behavior change; touches no shared pod-claim primitive → no C-22/C-01 serialization needed). Investigation found delegated children do NOT materialize today (a StateCreated child traverses no start path), so the residual steps-5-9 materialization build is re-homed to session-start and filed as a new BUILD-GAPS finding rather than built here. One gating open decision at sign-off: option (b) deferred [rec] vs (a) inline delegation-time build. — Prior assignment rationale: proposal-C owns the whole §8.2/§8.3 delegation-credential stack (C-20/C-40/C-41), so it is the most qualified and most isolated fit; C-42 extends its own C-40/0044 delegation-time availability pre-check.
- **assigned:** proposal-C
- **findings:** T-8.2.17
