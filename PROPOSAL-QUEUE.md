...
- **status:** in-review:0044 (add the §8.3:470 delegation-time availability gate via a CredentialAvailabilityChecker interface on *sessionserver.Server reusing resolveCredentialPools — one engine for both inherit/independent via a synthetic prospective-child row; reject CREDENTIAL_POOL_EXHAUSTED/USER_CREDENTIAL_NOT_FOUND before the child row. NOTE the review found the delegate path claims no pod today, so the racy tier-4 test can't pass as-written — re-scoped to a deterministic pre-check test; the one-winner/N-1/pod-release coverage re-files against the future §8.2 delegate-path pod-claim build. Two open decisions (sequencing; racy-test disposition). See inbox re: T-8.3.1 breadth + the un-skip-race-test scope note)
- **assigned:** proposal-C
- **findings:** T-8.3.1
