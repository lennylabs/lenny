...
- **proposal scope:** Build the restore side of slot-aware checkpointing so a concurrent-session pod round-trips: slot-scoped restore and materialization, the resume path's slot binding, and the end-to-end capture and restore coverage on a `maxConcurrentSessions > 1` pool that 0050 could not write. Superseded: proposal 0073 builds all three, addressing the slot by the session identifier rather than by a separate `slot_id`. Nothing is left for this cluster to scope beyond T-4.4.21's remaining round-trip case, which stays tracked on that finding.
- **severity:** High
