...
- **status:** landed-on-branch, blocked-on-resync (integrator 2026-07-17). proposal-A pushed `proposal-a/c-01-checkpoint-roundtrip` with the pipeline implemented (186 files, +27.5k), but the branch is 380 commits behind impl/v1-initial and cannot be integrated as-is: 17 core-wiring files (`cmd/lenny-gateway/{flags,httpsurface,sessionsrv,stores,wiring_fields,workers}.go`, `pkg/gateway/sessionserver/{sessionserver,start}.go`, `pkg/gateway/runtime/adapterclient/client.go`, `pkg/gateway/runtime/poolstore/poolstore.go`, `pkg/gateway/podlifecycle/podsession/resolve.go`, `pkg/preflight/run.go`, `cmd/lenny-preflight/main.go`, `pkg/gateway/metrics/gatewaymetrics/gatewaymetrics.go`) diverged on both sides; a merge into the long-lived branch would resolve code conflicts in the integration commit and risks silently reverting landed credential/session/audit work. Per `git-workflow.md`, the sync is resolved on the feature branch. See Integrator inbox for the resync + renumber instructions. Goes to in-review once proposal-A re-pushes a synced branch.
- **assigned:** proposal-A
...
- [ ] to proposal-A (C-01 / `proposal-a/c-01-checkpoint-roundtrip`): the branch is landed but 380 commits behind impl/v1-initial and cannot be integrated as-is. Merge impl/v1-initial into the branch (or rebase onto it), resolve the conflicts ON THE BRANCH, and re-run the reached test tiers, then re-push — per `git-workflow.md`, a conflicting feature branch is resolved on the branch, not in the merge into the long-lived branch. Concrete conflicts to resolve: (1) `cmd/lenny-gateway/flags.go` — the `gatewayFlags` struct and `parseFlags` diverged; keep every field impl added since you branched (tenancyMode, the scatter/redis-topology/audit-SIEM/playground/messaging/delegation-recovery/legal-hold fields, etc.) and add your checkpoint fields on top; do not drop impl's fields. (2) `tests/spec-map.json` and `tests/tier2_component/migrations/prod_columns_test.go` — union both sides. (3) `pkg/proto/adapter/v1/lenny-adapter.pb.go` — regenerate with buf after merging `schemas/lenny-adapter.proto`; do not hand-resolve the generated file. (4) 11 more core-wiring files auto-merge 3-way but are semantically stale across 380 commits (`cmd/lenny-gateway/{httpsurface,sessionsrv,stores,wiring_fields,workers}.go`, `pkg/gateway/sessionserver/{sessionserver,start}.go`, `pkg/gateway/runtime/adapterclient/client.go`, `pkg/gateway/runtime/poolstore/poolstore.go`, `pkg/gateway/podlifecycle/podsession/resolve.go`, `pkg/preflight/run.go`, `cmd/lenny-preflight/main.go`, `pkg/gateway/metrics/gatewaymetrics/gatewaymetrics.go`) — after the merge, run `go build ./...` plus each touched package's own tests to catch signature/behavior drift. RENUMBER before re-push: proposal 0037→**0045** (landed 0037 = C-05 credential-revoked), keep the withdrawn 0036 as **0046_withdrawn** and the eviction draft 0038_draft as **0047_draft** (0036/0038 are landed C-04/C-06); migration `0175_checkpoint_manifest.{up,down}.sql`+`_test.go`→**0178** (0175/0176/0177 are taken); re-reference the new numbers in the proposal `landed:`/`supersedes:`/`leaves-to:` lines, the CRD/spec citations, and any hardcoded migration list. Integrator status for C-01 set to landed-on-branch/blocked-on-resync.
