...
- **status:** open — prereq 1 landed:0050 (integrated 2026-07-19); **prereq 2 in-review:0052 on branch `proposal-b/c-22-coordinator-compare-and-steal`, SIGNED OFF + IMPLEMENTED green 2026-07-22, ready for `--no-ff` integration. Trigger-seam decision resolved: coordinator-direct. The change is a single §4.6.1 spec-prose clarification (the evicted pod's preStop signals its live coordinator over the existing per-pod control channel; that coordinator drives the checkpoint under its held lease; an unreachable coordinator is recovered via the §10.1 TTL handoff, fail-closed) plus one tier-11 route-consistency test. No product code. Tiers 0+11 independently verified, validate-maps green, TEST-GAPS untouched. Branch merged onto current impl/v1-initial (0 behind). The within-window takeover of an UNREACHABLE coordinator, the forward-to-holder transport, and any takeover primitive are deferred to the trigger-wiring proposal. 0052 closes no finding; unblocks-not-closes T-4.4.14/T-4.6.4.** C-22 is being resolved prerequisite-first as a sequence of small proposals: **0050 = slot-aware checkpointing** (`slot_id` on `CheckpointStart`, slot-scoped `checkpointRoots()`, per-slot op-lock, §4.7 op-lock queue-depth spec edit) is now integrated (build green, adapter/checkpointer/barrier tests pass, static PASS). It closes no C-22 finding by itself and unblocks-but-does-not-close T-4.4.21 (capture side done; the restore-side companion closes it); T-4.4.14/T-4.6.4 stay for the later trigger proposal. **C-22 stays open and assigned to proposal-B** for prerequisites 2-4 (coordinator compare-and-steal, agent-pod grace budget, runtime-container preStop) + the trigger, each a separate proposal/branch.
- **assigned:** proposal-B
