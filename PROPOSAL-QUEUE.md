...
- **status:** open — prereq 1 landed:0050 (integrated 2026-07-19); **prereq 2 in-review:0052 on branch `proposal-b/c-22-coordinator-compare-and-steal`, AWAITING jaf sign-off. NOTE: adversarial review reduced 0052 to a single §4.6.1 spec-prose clarification — the compare-and-steal premise was refuted on the common path (the evicted pod's preStop reaches its live coordinator over the existing per-pod control channel, and that coordinator drives the checkpoint under its held lease; a forced steal of a still-live lease would break the leasestore holder invariant and is unnecessary). The within-window takeover of an UNREACHABLE coordinator is deferred to the trigger-wiring proposal, gated on the trigger-seam decision. 0052 closes no finding; unblocks-not-closes T-4.4.14/T-4.6.4.** C-22 is being resolved prerequisite-first as a sequence of small proposals: **0050 = slot-aware checkpointing** (`slot_id` on `CheckpointStart`, slot-scoped `checkpointRoots()`, per-slot op-lock, §4.7 op-lock queue-depth spec edit) is now integrated (build green, adapter/checkpointer/barrier tests pass, static PASS). It closes no C-22 finding by itself and unblocks-but-does-not-close T-4.4.21 (capture side done; the restore-side companion closes it); T-4.4.14/T-4.6.4 stay for the later trigger proposal. **C-22 stays open and assigned to proposal-B** for prerequisites 2-4 (coordinator compare-and-steal, agent-pod grace budget, runtime-container preStop) + the trigger, each a separate proposal/branch.
- **assigned:** proposal-B
