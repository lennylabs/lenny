...
- **status:** in-review:0050 (converged on option (b): reconcile the §8.2 wording to the deferred create-then-materialize model — SPEC-2 ties the step-2a cycle rationale to the child-row INSERT not step-5; SPEC-4 locates the §8.3:470 post-claim pod-claim/lease/race at the child's session-start; SPEC-6 qualifies the §8.8:879 + §15.1:630 `created`-state "warm pod claimed" notes for a delegated child; CODE-1 fixes the taskHandle.State comment. Investigation found delegated children do NOT materialize today — a StateCreated child traverses no start path — so the residual steps-5-9 materialization build is re-homed to session-start and FILED as a new BUILD-GAPS finding (not built here). Spec-only, no behavior change, touches no shared pod-claim primitive → no C-22/C-01 serialization needed for this proposal. Converged 3 rounds/2 clean, 1 finding fixed. One gating open decision: option (b) deferred [rec] vs (a) inline delegation-time build. Awaiting sign-off.)
- **assigned:** proposal-C
- **findings:** T-8.2.17
