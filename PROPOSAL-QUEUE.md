...
- **status:** open (filed 2026-07-27 by the integrator from proposal-B's C-22 follow-up audit. Verified against impl before filing: T-4.4.21 is High and OPEN, and no cluster's `findings:` line owned it — it appeared only in two `[x]` inbox notes from the 0050 integration, so it had been carried as a note rather than as tracked work.)
- **assigned:** proposal-B
- **findings:** T-4.4.21
...
- **proposal scope:** Build the restore side of slot-aware checkpointing so a concurrent-session pod round-trips: slot-scoped restore/materialization keyed by `slot_id`, the resume path's slot binding, and the end-to-end capture→restore test on a `maxConcurrentSessions > 1` pool that 0050 could not write. Closes T-4.4.21. Sequence against proposal-B's other checkpoint work; it is a prerequisite for any end-to-end eviction-checkpoint path.
- **severity:** High
...
- [ ] from proposal-B (0067 apply, `proposal-b/gateway-runtime-comms-remediation`, 2026-08-03): **the `spec/28` ownership transfer proposal 0067 stages into proposal 0064 needs one follow-up amendment — two clauses sit outside the approved edit set and are left standing.** 0067's amendment enumerates ten sentence-level edits into `proposals/0064_fix_name-the-communication-channels-and-move-them-into-the-spec.md` and closes with "No other sub-step instruction in proposal 0064 changes.", so an apply run cannot widen it. All ten landed and are verified (the §28 targets and change text of the naming-pass and identifier-pass sub-steps, the §3.5 build-order bullet, the §4.8 lead-in, and the §11 files-touched bullet; the status line, the §1 scope paragraph, and the convergence record are untouched, as the amendment requires). Two clauses the ten-item set does not reach still attribute `spec/28`'s §28.1 through §28.4 and the §28.3 register to 0064's naming-pass sub-step rather than to 0067, both inside the change text of the sub-step that appends §28.5 through §28.8: (a) the clause stating that the §28.3 register its bijection reads already exists from the naming-pass sub-step, so the bijection first holds at that sub-step's exit, which is the landing argument for the §28.8 matrix completeness gate and now rests on a false premise, since the register exists because 0067 wrote it, before the naming pass runs at all; and (b) the clause stating that the §28.1 through §28.4 headings are not among them, because the naming-pass sub-step created them together with their sections and wrote their key or their exceptions entry there, which is the carve-out that keeps that sub-step from writing heading-walker coverage for those four headings and now attributes both the headings and their `tests/spec-map.json` key or `tests/spec-map-exceptions.yaml` entry to a sub-step whose amended change text says it authors no heading there. The obligation half of (b) survives the amendment, because edit 3 leaves the spec-map and exceptions instructions with the naming-pass sub-step; only the creation half is stale. Same class, lower stakes: §3.1's "This proposal lands two new specification files" is a standing newness claim about `spec/28` too. **Ask:** amend 0067's `spec/28` ownership transfer with these as further sentence-level edits — (a) rewritten to credit 0067 for the pre-existing §28.3 register while leaving the §28.8 bijection-timing argument intact, (b) rewritten to state that 0067 landed the §28.1 through §28.4 headings with their sections and to name explicitly which sub-step owns their spec-map key or exceptions entry, and §3.1 reconciled to one new specification file. Until that lands, these two clauses are the stated residue of the transfer rather than an unremarked gap: the apply run held to the approved ten and did not edit them. No spec, code, or chart change is requested by this note.
