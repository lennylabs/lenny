...
- [ ] from proposal-C (C-40/proposal-c-40-delegation-credential-availability): C-40's finding T-8.3.1 is broader than C-40's stated proposal-scope. T-8.3.1 is the umbrella "credentialPropagation (inherit/independent/deny) has no implementation" finding; proposal 0043 (C-20) already implemented the field/enum/validator, the cross-env inherit compatibility gate, and the inherit shared-pool assignment. C-40 will build the remaining AVAILABILITY pre-check (CREDENTIAL_POOL_EXHAUSTED for inherit/independent) + un-skip the race test, per C-40's scope. But two pieces of T-8.3.1 still remain AFTER C-40: (1) deny-mode credential suppression (needs a persisted credentialPropagation mode column, which 0043 deliberately omitted), and (2) the full inherit→independent→deny assignment-matrix tier-4 test (T-8.3.1's suggested `delegation_credential_propagation_test.go`). So when C-40 lands I will leave T-8.3.1 OPEN with a note rather than RESOLVED. Please either split T-8.3.1 (a new finding for deny-mode + the assignment-matrix test) or confirm C-40 should expand to cover them too.
- [ ] from proposal-C (C-40/proposal-c-40-delegation-credential-availability): FILE A NEW CLUSTER for the §8.2 delegate-path pod-claim and credential-lease-assignment flow (spec §8.2 steps 5-9). **Spec-vs-code divergence:** `spec/08_recursive-delegation.md:64` states pod allocation happens at step 5 (at delegation time, before the child session id exists), but the code commits the child in `session.StateCreated` with no `PodAssignment` (`pkg/gateway/mcpfabric/delegation/service.go:1425`) and defers the actual warm-pod claim + credential-lease assignment to the shared session-start path (`handleStart`/`handleCreateAndStart` → `resolveCredentialPools` → `credrouter.PreClaim`, `pkg/gateway/sessionserver/start.go:1295`). Proposal 0043's CODE-5 confirms a delegated `inherit` child does reach `resolveCredentialPools` at start (`start.go:1333`), and T-8.1.3 confirms delegated child pods exist on live clusters — so the flow exists via the shared start path, just not inline at step 5 as the spec requires. **Pod-claim-touching:** per this queue's pod-conflict hold-back rule it belongs on proposal-A's lane or must be coordinated with C-01's pod/resume work. **Depends on:** C-40/0044 (landed — the delegation-time availability pre-check). **Relates to:** T-8.1.3 (isolation monotonicity on the delegated child pod). **How it improves the just-landed 0044 work — please carry this context so the cluster upgrades rather than duplicates it:** (1) it makes 0044's point-in-time pre-check a true pre-allocation fail-fast — today the gate rejects before the child row is created, but the pod is only claimed later at start, so the spec's "no warm pod wasted" benefit is deferred; building step 5 makes the pre-check guard an actual inline pod claim. (2) It enables the `spec/08:470` clauses 2-3 that 0044 deliberately left out of scope: the post-pod-claim assignment race, the losing delegation's pod released back to the warm pool, and the `CREDENTIAL_POOL_EXHAUSTED` / one-winner-of-N outcome (consistent with §7.1). (3) It unblocks **T-ADV.14** (the re-filed racy one-winner/N-1 + pod-release tier-4/tier-8 coverage that 0044 removed as architecturally unbuildable on the current delegate path). **Findings to attach:** T-ADV.14 (the re-filed race test); the production step-5 pod-allocation divergence itself is currently untracked as a production finding — create one or fold it into the cluster.
