...
- **status:** in-review:0062 (converged the captured `0061_draft`: per-runtime opt-in `SessionPolicy.PodHoldIdleTtlSeconds` (zero=no hold, admin-API-only); warm path Open-Responses-only via the `ContinuationParentID` affinity key; held pod-session is its own `StateRunning` row decoupled from the `StateCompleted` response rows; release via the watchdog idle-sweep with a non-holder by-name reclaim fallback; 0059 rehydration stays the cold-path floor; C-22 eviction cooperation an accepted v1 gap. Four spec edits (§15/§5.2/§6.2/§16) as prerequisites. Converged 11 rounds/2 clean, 20 findings fixed. Three open decisions: reap-under-pressure policy, warm-continuation admission gating, deploy-wide TTL ceiling. Awaiting sign-off.) Filed 2026-07-25 by the integrator from proposal-C's inbox request; an unconverged design draft is captured on impl as `proposals/0061_draft_single-shot-adapter-pod-hold-idle-ttl.md`, renamed from the branch's 0057 which collided with the landed C-13 0057.
- **assigned:** proposal-C
