...
- **status:** converging:0043 (revising in place per reviewer sign-off decisions — folding in the concrete inherit shared-pool credential ASSIGNMENT (spec/08:470 (b)): an admitted inherit delegation assigns the child a credential from the origin pool constrained to the origin∩child provider intersection, in addition to the compatibility gate; decisions 1/2 resolved as live re-resolution + stricter tenant-policy∩supportedProviders. §8.3:468 availability pre-check still out of scope — see inbox item)
- **assigned:** proposal-C
- **findings:** T-BED.8
...
- [ ] from proposal-C (C-20/proposal-c-20-crossenv-credential-compat): the §8.3 general credential-**availability** pre-check at delegation time (the `CREDENTIAL_POOL_EXHAUSTED` pre-claim wastage guard for all `inherit`/`independent` delegations, cross-env or not; §8.3 "Credential availability pre-check at delegation time"; tested by the skipped `tests/tier4_integration/delegation_credential_pool_race_test.go`) is a distinct root gap from C-20's cross-environment provider-**compatibility** check but shares the same delegate-handler insertion point. C-20 builds only the compatibility check (`CREDENTIAL_PROVIDER_MISMATCH`); please route the availability pre-check as its own finding/cluster — it can reuse the pre-claim step C-20 adds before the pod-claiming Delegate call.
