...
- **status:** claimed:proposal-C@2026-07-19T16:06:56Z (§8.2 delegate-path step-5 allocation timing; extends C-40/0044. Design fork to converge: (a) move the warm-pod claim + credential-lease assignment inline to delegation step 5, or (b) amend the spec to define delegation-time allocation as deferred to session-start. **Coordination:** if it modifies the shared warm-pool claim/release primitives (`poolstore`, `credrouter.PreClaim`) that proposal-B's C-22 also touches, flag the Integrator inbox before pushing so the integrator can serialize the two.)
- **assigned:** proposal-C
- **findings:** T-8.2.17
