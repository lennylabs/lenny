...
type SessionScrubReporter interface {
	// ReportSessionScrub reports the per-session cleanup outcome for the
	// released session, which is also the identifier of the slot it held. A
	// transport or gateway failure is returned as a wrapped error.
	ReportSessionScrub(ctx context.Context, podID, sessionID string, outcome gatewaycontrol.SessionScrubOutcome) error
}
...
// after a session release, keyed on the adapter's cached pod identity and
// the released session. It is the sole production writer of sessions_served
...
// spec: §4.7 (ReportSessionScrub); §5.2 (maxSessionsPerPod). F-5.2.31.
func (s *Server) reportSessionScrub(ctx context.Context, sessionID string, closeErr error) {
	...
	if err := s.SessionScrubReporter.ReportSessionScrub(ctx, s.podID, sessionID, outcome); err != nil {
		...
		slog.Error("adapter: ReportSessionScrub failed",
			"pod", s.podID, "session", sessionID, "outcome", outcome, "err", err)
	}
}
