...
// spec: 5.2 (a Shutdown with no recycle disposition ends the pod's use of
// the runtime), 4.7
//
// diagnosis: a failure means the adapter stopped tearing the runtime down
...
func TestShutdownWithoutRecycleClosesTheRuntime_spec_5_2(t *testing.T) {
	...
	if got := rt.closedSnapshot(); len(got) != 1 || got[0] != "sess-1" {
		t.Errorf("runtime closed = %v, want [sess-1]", got)
	}
	if got := rt.releasedSnapshot(); len(got) != 0 {
		t.Errorf("runtime released = %v off the recycle boundary; the session's end is the pod's end here", got)
	}
}
