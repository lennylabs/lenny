...
// spec: §6.4 (the per-slot tree is the only workspace layout), §5.2 (a
// concurrent pod multiplexes several sessions), §7.3 (the gateway derives
// the resumed session's workspace root from the workspace base and the
// session identifier).
//
// A pod already running one session restores a second session's checkpoint
// into that second session's own slot tree. The archive carries a file whose
// name collides with one the co-tenant already holds, so an extraction that
// resolved a pod-global root, or the root of the session already on the pod,
// would overwrite the co-tenant's copy and leave the resuming session's tree
// without one.
func TestResumeOnAPodHoldingASessionRestoresIntoTheResumingSessionsTree_spec_6_4(t *testing.T) {
	s, _, root := sessionServer(t)
	if _, err := s.StartSession(context.Background(), startReq("sess-1")); err != nil {
		t.Fatalf("StartSession: %v", err)
	}

	// The co-tenant's own copy of the colliding file, written into its slot
	// tree before the second session resumes.
	coTenantFile := filepath.Join(slotWorkspaceRoot(root, "sess-1"), "notes.txt")
	if err := os.MkdirAll(filepath.Dir(coTenantFile), 0o755); err != nil {
		t.Fatalf("create the co-tenant slot tree: %v", err)
	}
	if err := os.WriteFile(coTenantFile, []byte("sess-1 own content"), 0o644); err != nil {
		t.Fatalf("seed the co-tenant workspace: %v", err)
	}

	chunks := serveArchive(s, archiveOf(t, map[string]string{"notes.txt": "sess-2 restored content"}))
	req := resumeReqChunks("sess-2", "ckpt-2", chunks)
	req.ExpectedWorkspaceRoot = slotWorkspaceRoot(root, "sess-2")
	if _, err := s.Resume(context.Background(), req); err != nil {
		t.Fatalf("Resume for a second session on the pod: %v", err)
	}

	got, err := os.ReadFile(filepath.Join(slotWorkspaceRoot(root, "sess-2"), "notes.txt"))
	if err != nil {
		t.Fatalf("the checkpoint was not restored into the resuming session's slot tree: %v", err)
	}
	if string(got) != "sess-2 restored content" {
		t.Errorf("restored file = %q, want the resuming session's checkpoint content", got)
	}

	co, err := os.ReadFile(coTenantFile)
	if err != nil {
		t.Fatalf("the co-tenant workspace file is gone after the second session resumed: %v", err)
	}
	if string(co) != "sess-1 own content" {
		t.Errorf("co-tenant file = %q, want it untouched by the other session's restore", co)
	}
}
