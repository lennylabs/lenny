...
type Inputs struct {
	...
	// `autoscaling.provider` value (e.g., "hpa" or "keda"). §17.8.3 makes KEDA a NO-GO criterion at Tier 3: a promotion
	// whose chart renders the Prometheus Adapter path fails the gate.
	// spec: §17.8.3, §17.8.2 Path A header.
	...
	// peak load). §17.8.3 marks an unattested ratio above
	...
	// peak load). §17.8.3 marks an unattested GC pause as a
	...
	// opposed to remaining at its §4.1 provisional baseline. §17.8.3 makes calibration a NO-GO for Tier 3 promotion.
	// spec: §17.8.3.
