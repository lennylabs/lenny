...
// expiringSoonPayload is the §11.3 session_expiring_soon SSE event
...
// the §11.3 contract: it emits the `session_expiring_soon` SSE event
...
// CH-RUNTIMEOPS (Basic/Standard integration level, §15) still
// gets the SSE event.
//
// spec: §11.3. F-11.3.5.
...
// signalDeadlineApproaching dispatches the §11.3 DEADLINE_APPROACHING
...
// spec's expected posture (§15) rather than an error. The call is
...
func (s *Server) signalDeadlineApproaching(ctx context.Context, sess sessionstore.Session, remainingSeconds int) {
	...
	// trigger is "session_age": the §11.3 warning is keyed off the
