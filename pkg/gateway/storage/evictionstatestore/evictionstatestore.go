...
// Record is one eviction-state row. Fields mirror the §4.4 columns the §4.4 fallback writer populates when MinIO
...
type Record struct {
	...
	// DeletedAt is the §4.4 soft-delete
...
// drives them in the documented dependency order. Per §4.4
...
// per §4.4 — the §7.2 resume path never observes a
// tombstoned record.
