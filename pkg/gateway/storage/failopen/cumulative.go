...
//
// spec: §12.4; §11.2 ("Maximum Overshoot
// Formula").
package failopen
...
// DefaultCumulativeStatePath is the §12.4 local file each replica
...
// CumulativeTimer is the §12.4 cumulative fail-open timer: a true
...
type CumulativeConfig struct {
	...
	// lenny_quota_failopen_cumulative_seconds gauge. spec: §12.4.
	OnChange func(cumulativeSeconds float64)
}
...
// treated as a cold start (the timer resets to zero) per §12.4.
...
func (t *CumulativeTimer) restore() {
	...
	now := t.now()
	// spec: §12.4 — resume only when the file's timestamp is
...
// window ending now, including any in-flight episode. spec: §12.4.
