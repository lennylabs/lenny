...
// tenantIDPattern is the §12.3 / migration 0047
...
// §12.3 format check. The helper rejects the value before
...
// against the §12.3 format check (tenantIDPattern) before
...
func InTx(ctx context.Context, pool *pgxpool.Pool, tenantID string, fn func(pgx.Tx) error) error {
	...
	// spec: §4.2 — SET LOCAL form. The value is interpolated
...
// platform-admin role, and per §12.3 every such call MUST
...
// via SET LOCAL, satisfying the §4.2 pooler-mode guard:
...
func InAllTenants(ctx context.Context, pool *pgxpool.Pool, fn func(pgx.Tx) error) error {
	...
	// spec: §4.2 — SET LOCAL form. The constant AllTenantsSentinel
...
// lenny.admin_mode = 'true'. The §4.2 trigger on
...
//
// spec: §4.2 ("BEFORE INSERT/UPDATE/DELETE trigger ...
...
func InAdminMode(ctx context.Context, pool *pgxpool.Pool, fn func(pgx.Tx) error) error {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	// spec: §4.2 — SET LOCAL form. The value is a
