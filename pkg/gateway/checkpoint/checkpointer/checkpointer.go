	...
	start := &adapterv1.CheckpointRequest{
		Msg: &adapterv1.CheckpointRequest_Start{
			Start: &adapterv1.CheckpointStart{
				...
				// spec: §5.2; §10.1.7 — the opening frame is where the
				// stream is addressed, because neither the CheckpointRequest
				// envelope nor the RPC signature carries a session. The
				// adapter resolves the session's checkpoint roots and its
				// op-lock key from it.
