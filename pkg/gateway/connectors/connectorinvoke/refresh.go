...
// RefreshCapabilities runs the §9.3 / §5.1 capability inference
...
// outbound call (§15.1), so capability inference is this
...
// the WARN list for unannotated-admin tools. It emits the §5.1
...
func inferCapabilities(connectorID string, tools []ToolDescriptor, mode capabilityinference.Mode) CapabilityRefreshResult {
	...
	for _, t := range tools {
		...
		if res.InferredAdminUnannotated {
			...
			// spec: §5.1 — emit the verbatim registration-time WARN.
			...
			// outbound call (§15.1). Routing through WarnMessage
