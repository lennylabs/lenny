...
type recordingCheckpointer struct {
	...
	// was opened concurrently with the barrier RPC (§10.1.8): the
...
// spec: §10.1.8 — the barrier drives one gateway-side Checkpoint
...
// spec: §10.1.8 — the ack is never blocked on a stream nobody
...
// spec: §10.1.8 — a barrier-window Checkpoint that errors does not
...
// spec: §10.1.8 — a nil Checkpointer leaves the barrier firing
