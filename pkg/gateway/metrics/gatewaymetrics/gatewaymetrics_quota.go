...
type quotaMetrics struct {
	// rateLimitRejected counts §11.1 admission rejections by
	...
	// RateLimitDegraded; §11.1 fail-open semantics.
	...
	// spec: §11.1 fail-open observability.
	rateLimitCounterFailure prometheus.Counter
	// quotaFailopenCumulativeSeconds is the §12.4 cumulative
	...
	// configured maximum. spec: §12.4; §16.1 / §16.5.
	...
	// billingFlushPressure counts §12.3 billing_flush_pressure
	...
	// auditBatchingNoSIEM is the §12.3 AuditBatchingNoSIEM
	...
	// postgresWriteIops is the §12.3 sustained Postgres
	...
	// postgresWriteCeilingIops is the §12.3
	...
	// siemMaxDeliveryLag is the §12.3
...
func newQuotaMetrics(reg *prometheus.Registry) (quotaMetrics, error) {
	var m quotaMetrics
	// §11.1 — `lenny_rate_limit_rejected_total` counts ratelimit
	...
	// §11.1 — `lenny_rate_limit_counter_failure_total` counts
	...
	// §12.4 — `lenny_quota_failopen_cumulative_seconds` is the
	...
	// §12.4 — `lenny_quota_user_failopen_fraction` exports the
	...
	// §12.3 — billing_flush_pressure: emitted when the failover
	...
	// §12.3 AuditBatchingNoSIEM — incremented once at startup
	...
	auditBatchingNoSIEM, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "§12.3 AuditBatchingNoSIEM: production audit.batchingEnabled is set with no SIEM endpoint, so buffered T2 events are lost on crash with no external copy.",
	}, nil)
	if err != nil {
		return m, err
	}
	// §12.3 — sustained Postgres write IOPS, sampled from
	...
	// §12.3 — configured postgres.writeCeilingIops, emitted at
	...
	postgresWriteCeilingIops, err := metrics.NewGauge(prometheus.GaugeOpts{
		...
		Help: "§12.3 configured Postgres sustained write-IOPS ceiling (postgres.writeCeilingIops).",
	}, nil)
	if err != nil {
		return m, err
	}
	// §16.1 lenny_audit_siem_delivery_lag_seconds — the §12.3
	...
	// §12.3 audit.siem.maxDeliveryLagSeconds — emitted at
	...
	siemMaxDeliveryLag, err := metrics.NewGauge(prometheus.GaugeOpts{
		...
		Help: "§12.3 configured SIEM delivery-lag threshold (audit.siem.maxDeliveryLagSeconds).",
	}, nil)
	if err != nil {
		return m, err
	}
