...
type sessionLifecycleMetrics struct {
	// sessionStartupDuration is the §16.1 / §6.3
	...
	// sessionStartupPhaseDuration is the §6.3 per-phase
	...
	// sessionTimeToFirstToken is the §16.1 / §6.3
	...
	// numerator of the §6.3 demotion-rate ratio over
	// warmpoolClaims. spec: §6.1, §16.1.
	warmpoolSDKDemotions *prometheus.CounterVec
	// warmpoolSDKDemotionDuration is the §6.3
	...
	// warmpoolWarmupFailure is the §16.1
	...
	// workspaceSealDuration is the §7.1 seal-and-export
	...
	// checkpointStorageFailure counts the §4.4 non-eviction
	...
	// checkpointEvictionFallback counts the §4.4 eviction-
...
func newSessionLifecycleMetrics(reg *prometheus.Registry) (sessionLifecycleMetrics, error) {
	...
	// §16.1 / §6.3 — `lenny_session_startup_duration_seconds`
	...
	sessionStartupDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "End-to-end pod-warm session startup: pod claim through agent session ready, excluding upload and workspace materialization (§6.3).",
		Buckets: []float64{0.05, 0.1, 0.25, 0.5, 1, 1.5, 2, 3, 5, 8, 13},
	}, []string{"pool", "runtime_class", "isolation_profile"})
	if err != nil {
		return m, err
	}
	// §6.3 — `lenny_session_startup_phase_duration_seconds`
	...
	sessionStartupPhaseDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "Per-phase pod-warm session startup latency (§6.3).",
	...
	// le="10" bucket boundary directly. spec: §16.1; §6.3.
	sessionTimeToFirstToken, err := metrics.NewHistogram(prometheus.HistogramOpts{
		Name:    "lenny_session_time_to_first_token_seconds",
		Help:    "End-to-end time to first token: session start request to first agent-streamed response event (§6.3, §16.1).",
		Buckets: []float64{0.1, 0.5, 1, 2, 3, 5, 8, 10, 15, 30, 60},
	}, []string{"pool", "runtime_class", "isolation_profile"})
	if err != nil {
		return m, err
	}
	// §16.1 / §6.3 — `lenny_warmpool_claims_total`
	...
	warmpoolClaims, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Idle→claimed warm-pool transitions per pool (§6.3, §16.1).",
	}, []string{"pool", "runtime_class"})
	if err != nil {
		return m, err
	}
	// §6.1 / §16.1 — `lenny_warmpool_sdk_demotions_total`
	...
	warmpoolSDKDemotions, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "SDK-warm pods demoted to pod-warm before session assignment per pool (§6.1, §16.1).",
	}, []string{"pool"})
	if err != nil {
		return m, err
	}
	// §6.3 — the SDK teardown penalty per demotion ("typically
	...
	warmpoolSDKDemotionDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "SDK teardown penalty (DemoteSDK wall-clock) per demotion, per pool (§6.3).",
	...
	// §16.1 — `lenny_warmpool_warmup_failure_total{error_type}`
	...
	warmpoolWarmupFailure, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Warm-pool warm-up failures by error_type (§16.1, §7.3).",
	}, []string{"error_type"})
	if err != nil {
		return m, err
	}
	...
	// §7.1 — `lenny_workspace_seal_duration_seconds` tracks
	...
	workspaceSealDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "Workspace seal-and-export completion time by outcome (§7.1).",
	...
	checkpointStorageFailure, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Checkpoint or artifact writes failed (§4.4 retry-exhausted, §12.5 ll. 303 kms_unavailable).",
	}, []string{"pool", "level", "trigger", "reason"})
	if err != nil {
		return m, err
	}
	// §4.4 — `lenny_checkpoint_eviction_fallback_total`
	...
	checkpointEvictionFallback, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Checkpoint storage fallbacks to Postgres minimal state (§4.4).",
