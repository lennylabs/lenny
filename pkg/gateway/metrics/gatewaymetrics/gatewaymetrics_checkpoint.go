...
type checkpointMetrics struct {
	// checkpointStaleSessions reports the §4.4 freshness
	...
	// partialManifestCleanup counts the §4.4 partial-manifest
	...
	// checkpointOrphanedObjects counts the §4.4 abort-cleanup
	...
	// checkpointSizeExceeded counts the §4.4 pre-checkpoint
	// workspace-size probe rejections. Labeled by pool and level.
	checkpointSizeExceeded *prometheus.CounterVec
	// sessionEvictionTotalLoss counts the §4.4 total-loss
	...
	// checkpointEvictionPartialKeysLogged counts the §4.4
	...
	// checkpointDuration is the §4.4 end-to-end checkpoint
...
func newCheckpointMetrics(reg *prometheus.Registry) (checkpointMetrics, error) {
	...
	// §4.4 — `lenny_checkpoint_stale_sessions` per-pool/level
	...
	// §4.4 — `lenny_partial_manifest_cleanup_total` records
	...
	partialManifestCleanup, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Partial checkpoint manifest cleanup outcomes (§4.4).",
	}, []string{"outcome"})
	if err != nil {
		return m, err
	}
	...
	// §4.4 — `lenny_checkpoint_orphaned_objects_total` is
	...
	checkpointOrphanedObjects, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Checkpoint abort cleanup failed to delete partial objects (§4.4).",
	}, []string{"pool", "trigger"})
	if err != nil {
		return m, err
	}
	// §4.4 — `lenny_checkpoint_size_exceeded_total` is
	...
	checkpointSizeExceeded, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Pre-checkpoint workspace-size probe exceeded the limit (§4.4).",
	}, []string{"pool", "level"})
	if err != nil {
		return m, err
	}
	// §4.4 — `lenny_session_eviction_total_loss_total` is
	...
	sessionEvictionTotalLoss, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Session eviction total-loss events (§4.4).",
	}, []string{"pool", "had_prior_checkpoint"})
	if err != nil {
		return m, err
	}
	// §4.4 — `lenny_checkpoint_eviction_partial_keys_logged_total`
	...
	checkpointEvictionPartialKeysLogged, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Partial MinIO key sets logged on the eviction loss path (§4.4).",
	}, []string{"pool", "keys_committed"})
	if err != nil {
		return m, err
	}
	// §4.4 — `lenny_checkpoint_duration_seconds` is observed
	...
	checkpointDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "End-to-end checkpoint wall time in seconds (§4.4).",
