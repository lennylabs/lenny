...
type orphanMetrics struct {
	// maxOrphanTasksPerTenant is the §8.10 deployer-configured
	...
	// orphanCleanupRuns counts the §8.10 background sweep
...
func newOrphanMetrics(reg *prometheus.Registry) (orphanMetrics, error) {
	var m orphanMetrics
	// spec: §8.10, §16.5 OrphanTasksPerTenantHigh alert reads
	...
	maxOrphanTasksPerTenant, err := metrics.NewGauge(prometheus.GaugeOpts{
		...
		Help: "Configured maxOrphanTasksPerTenant ceiling — drives the OrphanTasksPerTenantHigh alert threshold (§8.10).",
	}, nil)
	if err != nil {
		return m, err
	}
	...
	orphanCleanupRuns := prometheus.NewCounter(prometheus.CounterOpts{
		...
		Help: "Background orphan cleanup job executions (§8.10 / §16.1).",
	})
	...
	orphanTasksActivePerTenant, err := metrics.NewGauge(prometheus.GaugeOpts{
		...
		Help: "Per-tenant active orphan task count; drives the OrphanTasksPerTenantHigh alert (§8.10 / §16.1).",
	}, []string{"tenant_id"})
	if err != nil {
		return m, err
	}
	...
	treeRecoveryDuration, err := metrics.NewHistogram(prometheus.HistogramOpts{
		...
		Help:    "Delegation tree-recovery wall-clock duration by outcome (§8.10 / §16.1).",
	...
	treeRecoveryTimeout, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Delegation tree-recovery timeouts by timeout type (`level` | `tree`) (§8.10 / §16.1).",
