...
type credentialMetrics struct {
	// credentialPreclaimMismatch counts the §4.9 races where
	...
	// pool, provider, and error type. spec: §4.9.
	...
	// §16.1.1 (no high-cardinality attribute). spec: §4.9.
	...
	// slotFailure counts the §5.2 concurrent-workspace slot bind
	...
	// slotRehydration counts the §5.2 post-recovery slot-counter
...
func newCredentialMetrics(reg *prometheus.Registry) (credentialMetrics, error) {
	...
	// §4.9 — `lenny_credential_preclaim_mismatch_total`
	...
	credentialPreclaimMismatch, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Pre-claim credential availability check passed but assignment failed (§4.9).",
	}, []string{"pool", "provider"})
	if err != nil {
		return m, err
	}
	// §16.1 — `lenny_credential_lease_assignments_total` counts
	...
	// §16.1 — `lenny_credential_lease_duration_seconds` observes
	...
	// §16.1 — `lenny_credential_rotation_total` counts
	...
	// §4.9 — `lenny_gateway_credential_fallback_exhausted_total`
	...
	// §16.1 — `lenny_credential_pool_utilization` is the ratio of
	...
	// §4.9 — `lenny_gateway_credential_leases_swept_total` counts
	...
	// §16.1 — `lenny_gateway_llm_proxy_active_connections` is the
	...
	// §16.1 — `lenny_gateway_llm_translation_duration_seconds`
	...
	// §16.1 — `lenny_gateway_llm_translation_errors_total`
	...
	// §5.2 / §16.1 — `lenny_slot_failure_total` counts
	...
	slotFailure, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Concurrent-workspace slot failure count (§5.2).",
	...
	slotRehydration, err := metrics.NewCounter(prometheus.CounterOpts{
		...
		Help: "Post-recovery slot-counter rehydration events (§5.2).",
	}, []string{"pod", "pool"})
	if err != nil {
		return m, err
	}
