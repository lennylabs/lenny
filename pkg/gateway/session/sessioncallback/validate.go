...
// envelope carries in details.reason. spec: §15.1.
...
// reason codes carried on a ValidationError. The §15.1 list is
...
// metadataHosts are the §14 well-known cloud metadata
...
// Resolver is the DNS seam the §14 DNS-pinning step uses. The
...
type Validator struct {
	// AllowedDomains is the §14 callbackUrlAllowedDomains. When
...
// §14 DNS pin resolved, which the delivery transport dials
...
func (v *Validator) Validate(ctx context.Context, raw string) (Result, error) {
	...
	// spec: §14 — HTTPS only; no http, no non-HTTP schemes.
	...
	// spec: §14 — IP literals, localhost, loopback, and
	...
	// spec: §14 — cloud metadata hostnames are rejected
	...
	// spec: §14 — when the deployer allowlist is non-empty, the
	...
	// spec: §14 — resolve the hostname and reject when any
...
// any subdomain of suffix (but not the bare suffix). spec: §14.
...
// IPv4-mapped form of any of those. spec: §14.
