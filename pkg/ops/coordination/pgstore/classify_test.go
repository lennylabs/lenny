...
// TestClassifyErr_OperatorIntervention pins the §25.4 remediation-lock
// tier fall-through contract at the connectivity classifier. The Storage
// Tiers require that the lock service "attempts storage tiers in order,
// falling back on failure," with Tier 2 (Redis) used when Postgres is
// unreachable. The tiered Service triggers that fall-through on
// coordination.ErrStoreUnavailable, so a Postgres outage must surface as
// that sentinel. During a Postgres administrator shutdown or failover
// window the server answers an in-flight request with a FATAL
// operator_intervention SQLSTATE (57P01 admin_shutdown, 57P02
...
// signals, so classifyErr must map them to coordination.ErrStoreUnavailable
// rather than surfacing them as a hard error. A genuine server-side query
...
func TestClassifyErr_OperatorIntervention_spec_25_4(t *testing.T) {
	cases := []struct {
		...
		{"wrapped admin_shutdown", fmt.Errorf("query: %w", &pgconn.PgError{Code: "57P01"}), true, false},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantUnavail {
				if !errors.Is(got, coordination.ErrStoreUnavailable) {
					t.Errorf("classifyErr(%v) = %v, want coordination.ErrStoreUnavailable", tc.err, got)
				}
			}
			if tc.wantPassThrgh {
