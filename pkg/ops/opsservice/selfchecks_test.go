...
// and "...`k8s_api` evaluation" (§25.4 "Event-driven supplements", spec).
//
...
// `credentialPools`, `controllers`, `circuitBreakers` — §25.7 "Runbook Authoring Guidelines", spec: "`components` — maps to the
...
func TestSelfHealthCheckNamesAreSnakeCase_spec_25_4_2494(t *testing.T) {
	...
	// spec: §25.4 name postgres_pool, redis_consumer_lag,
	...
	for _, c := range cases {
		...
		for _, r := range c.want {
			...
			t.Errorf("%s: check name %q is not snake_case (spec §25.4)", c.name, c.want)
			break
		}
	}
	if CheckCertManager != "cert_manager" {
		t.Errorf("CheckCertManager = %q, want %q (§25.4 self-health snake_case convention; the camelCase certManager name belongs to the distinct §25.3 gateway health API component list, §25.7)", CheckCertManager, "cert_manager")
	}
}
...
//
// spec: §25.4 "OIDC Token Lifecycle (GatewayClient)".
func TestGatewayAuthComponentNameMatchesOIDCLifecycleCitation_spec_25_4_1999(t *testing.T) {
	t.Skip("spec self-contradiction: §25.4 \"OIDC Token Lifecycle\" cites the gateway-auth component in kebab-case while the Self-Health Checks naming convention pinned by TestSelfHealthCheckNamesAreSnakeCase_spec_25_4_2494 requires the same check name in snake_case (gateway_auth); which citation governs needs a spec decision, tracked separately in TEST-GAPS.md")
	const specCitedComponentName = "gateway-auth"
	if CheckGatewayAuth != specCitedComponentName {
		t.Errorf("CheckGatewayAuth = %q, want %q per §25.4", CheckGatewayAuth, specCitedComponentName)
	}
}
