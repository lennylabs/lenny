...
│                                                                     │
│  NEW — Ops-facing endpoints on the admin API:                       │
...
│                                                                     │
│  EXISTING — /metrics (Prometheus scrape target)                     │
│                                                                     │
...
These endpoints are added to the existing admin API on the gateway's main port. They require `platform-admin` or `tenant-admin` role (same as the rest of the admin API). No separate listener, port, goroutine pool, or TLS configuration is introduced.
...
**Headless Service for buffer polling.** The Helm chart adds a headless Service (`lenny-gateway-pods`, `clusterIP: None`) alongside the existing ClusterIP Service. `lenny-ops` uses DNS SRV lookup (`lenny-gateway-pods.{namespace}.svc`) to discover all gateway pod IPs. This Service is used exclusively for the event buffer fallback — all other `GatewayClient` calls use the standard ClusterIP Service (`lenny-gateway`).
...
ops:
  ...
  resources:
    ...
    limits:
      ...
      memory: 512Mi
  gateway:
    url: "http://lenny-gateway:8080"     # internal ClusterIP Service
  ...
  rateLimiting:
    ...
    burst: 50
  tls:
    internalEnabled: false               # mTLS between lenny-ops and gateway (zero-trust clusters)
    certSecretName: ""
  leaderElection:
...
The Helm chart adds the resources below. All resources live in the `{Release.Namespace}` namespace (default `lenny-system`); the chart is parameterized for namespace overrides via standard Helm `--namespace` and `Release.Namespace` substitution.
...
`lenny-ops` uses the platform's `StoreRouter` (Section 12.4) to access Postgres and Redis. The routing rules:
...
`lenny-ops` calls the gateway's admin API as a regular authenticated HTTP client. It uses a dedicated service account (`lenny-ops-sa`) with `platform-admin` role. The service account's JWT token includes `caller_type: "service"` for audit trail distinction.
...
type GatewayClient struct {
    baseURL          string            // ClusterIP Service: http://lenny-gateway:8080
...
**Internal (optional).** For zero-trust clusters where internal traffic must also be encrypted, deployers enable `ops.tls.internalEnabled`. When enabled, `lenny-ops` listens on TLS (port 8090) using the cert from `ops.tls.certSecretName`. The gateway client in `lenny-ops` can also be configured for mTLS when calling the gateway's admin API. This is opt-in — most clusters rely on Ingress TLS termination and trust the internal network.
...
|---|---|---|
| `health`, `recommendations` | Gateway admin API | Direct to gateway URL (existing `--server` flag) |
| `events`, `diagnose`, `runbooks`, `upgrade`, `audit`, `drift`, `backup`, `locks`, `escalations`, `logs`, `mcp-management` | `lenny-ops` | New `--ops-server` flag, or auto-discovered via `GET /v1/admin/platform/version` (gateway response includes `opsServiceURL`) |
