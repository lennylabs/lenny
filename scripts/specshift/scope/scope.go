...
// carrier through its comments like any other tracked Go file,
...
// prohibition covers in a Go file is the comment, which GoCommentSpans
// states and the pass applies on top of this answer.
...
// GoCommentSpans returns the byte span of every comment of a Go
// carrier, each as a half-open [lo, hi) pair over the content, in source
// order. Every comment of a tracked Go file is inside the naming law's
// reserved-phrase prohibition: the header block of the file, the
// documentation of a declaration, the prose written between the elements
// of a package-level composite literal, an implementation comment inside
// a function body, and a comment trailing the code it annotates alike.
//
// The population is stated over comments rather than over the subset the
// parser attaches to a declaration, because the law's Go domain is the
// comment text of the file and the annotations a test carries sit inside
// a function body as often as above one. A rule that admitted a subset
// would leave the sites outside it with no pass able to write them while
// the naming lint reported them, which is the writerless site the shared
// domain exists to prevent, and it would offset the pass's occurrence
// numbering from the enumeration the sense register is keyed by.
//
// The one Go position the law leaves outside the pass is the string
// literal, which carries operator-facing text; the pass admits a literal
// only where the pinned-literal register names it.
//
...
// skipped. Reading it whole would rewrite an operator-facing literal the
// law does not govern, and skipping it would leave every comment it
// carries with no writer.
func GoCommentSpans(target, content string) ([][2]int, error) {
	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, target, content, parser.ParseComments|parser.SkipObjectResolution)
	if err != nil {
		return nil, fmt.Errorf("read the comments of %s: %w", target, err)
	}
	var out [][2]int
