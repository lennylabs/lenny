...
//
// The run's confinement decides whether this channel runs and nothing
// more: a run covering no path-keyed register skips it, and a run
// covering one rekeys every path-keyed register the move invalidates,
// because a run that moves a carrier moves every key written for it.
