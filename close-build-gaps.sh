...
S1. **Only OPEN findings that reference an approved proposal.** A finding qualifies only when its body references a proposal under `proposals/` whose status is `Approved`. A proposal is EITHER a directory (`proposals/NNNN_kind_slug/`) OR a single file (`proposals/NNNN_kind_slug.md`); both forms exist in the tree and both are valid. Read the status with `node .claude/tools/proposal-status.mjs <proposal> --field status`, which handles both layouts and is the only supported reader; it prints exactly one of `Draft`, `Reviewed`, `Approved`, `Implemented`, or `Retired`. Only `Approved` qualifies. Do NOT parse a Status bullet by hand: sixteen spellings of it exist and the tool is what reconciles them. Find candidates with:

       grep -n '^### - \[ \] F-.*— OPEN' BUILD-GAPS.md | head -40

   then, for each, read its body and extract the referenced proposal, which is either `proposals/NNNN_kind_slug/` or `proposals/NNNN_kind_slug.md`. Read its status with the tool above. Skip every OPEN finding that references no proposal or whose proposal is not `Approved` — those are out of scope for this loop; leave them untouched.
