...
| `OutstandingInflightAtRotationCeiling` | `lenny_credential_rotation_inflight_ceiling_hit_total` incremented | 300s in-flight gate ceiling hit during a non-proactive rotation. The gate polls the pod-wide per-provider in-flight counter, so either the rotating session's runtime failed to emit `llm_request_completed` or a co-tenant session on the same pod held an outstanding request for the same provider |
...
| `ElicitationContentTamperDetected` | `increase(lenny_elicitation_content_tamper_detected_total{enforcement_mode="enforce"}[5m]) > 0` — any non-zero count on the `enforce` stream indicates an intermediate pod re-emitted an MCP `elicitation/create` wire frame for an existing elicitation whose `{message, schema}` pair diverges from the gateway-recorded original; per the gateway-origin-binding invariant in §9.2, under effective mode `enforce` the forward was dropped with `ELICITATION_CONTENT_TAMPERED` and the modified text never reached the client. Divergences under effective mode `detect-only` are surfaced by the separate `ElicitationContentIntegrityPermissiveTamper` warning alert below; effective mode `off` does not run the detector and produces no counter stream. | Investigate the `tampering_pod` label on the firing alert: suspend the associated session, review the runtime's prompt-injection posture, and if the tampering pod is an agent runtime, consider revoking its delegation lease. Correlate with the paired `elicitation.content_tamper_detected` audit event for `elicitation_id`, `origin_pod`, `divergent_fields`, `enforcement_mode`, `forward_outcome`, and `original_sha256` / `attempted_sha256` context. |
| `ElicitationContentIntegrityPermissiveTamper` | `increase(lenny_elicitation_content_tamper_detected_total{enforcement_mode="detect-only"}[5m]) > 0` — any non-zero count on the `detect-only` stream indicates a `{message, schema}` divergence was observed and forwarded to the client (the `detect-only` mode does not drop the forward). Warning severity because user-visible text was still delivered and no forward was dropped; see §9.2 enforcement modes. | Follow [elicitation-content-integrity-weakened](../runbooks/elicitation-content-integrity-weakened.html): if the divergence is an expected pre-enforcement canary signal, investigate whether the transformation is legitimate before promoting the tenant to `enforce`; if unexpected, treat as a tamper attempt and investigate the `tampering_pod` as for the critical alert. |
| `ElicitationContentIntegrityWeakened` | Standing warning: fires continuously while any tenant has an **effective** elicitation content integrity enforcement mode weaker than `enforce` (either `storedMode` was set to `detect-only` or `off`, or the platform floor `.Values.security.elicitationContentIntegrity.floor` is weaker than `enforce` and the tenant did not strengthen it). Distinct from `ElicitationContentIntegrityPermissiveTamper` — this alert fires on the standing configuration alone, regardless of whether any tamper has been observed. | Follow [elicitation-content-integrity-weakened](../runbooks/elicitation-content-integrity-weakened.html): correlate with the most recent `tenant.elicitation_content_integrity_changed` and `platform.elicitation_content_integrity_floor_changed` audit events to identify affected tenants and recorded `justification` values; raise tenant stored mode via `PUT /v1/admin/tenants/{id}/elicitation-content-integrity` or raise the platform floor via Helm. |
...
| `PoolSecurityDegraded` | `lenny_pool_security_degraded == 1` for any pool | A pool is rendering nonce-only `SO_PEERCRED` pods: its `deploymentModel: sidecar` runtime sets `requireSoPeercred: false` and the pool carries `acknowledgeNonceOnlyAuth: true`, or a nonce-only pod remains after the runtime field was reverted. Follow [nonce-only-mode](../runbooks/nonce-only-mode.html); confirm the gVisor `SO_PEERCRED` divergence is still present and re-run the Phase 3.5 integration tests when the container runtime version changes |
