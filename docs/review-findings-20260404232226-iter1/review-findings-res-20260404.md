...
### RES-003 Postgres failover window (up to 30s) creates inconsistency risk for in-flight session state writes [High] — Fixed
**Section:** 12.3, 10.1
...
**Resolution:** Recommendations (1) and (4) addressed; recommendations (2) and (3) declined with rationale. Section 12.3 now includes a **Write durability categories during failover** table that explicitly classifies each write category (committed state transitions, in-flight transactions, billing buffer, quota counters, coordination leases, audit buffer) as durable, at-risk, or unaffected, with the specific recovery mechanism for each. A **Maximum billing gap** paragraph quantifies the worst-case billing loss: ~30 events per replica (bounded by `billingFlushBatchSize` cap of 50), occurring only under the triple-simultaneous failure of gateway crash + pod loss + Postgres failover. Recommendation (2) — `session_generation` on all state writes — was already implemented as `coordination_generation` in Section 10.1 (fixed under SLC-002); the existing generation counter is used for all coordinator fencing and prevents stale/duplicate writes during failover recovery. Recommendation (3) — local SQLite for the billing write-ahead buffer — was declined: it introduces a new runtime dependency (SQLite) and operational complexity (local disk management, crash-consistent SQLite WAL handling) for a marginal improvement. The existing design already bounds loss to ~30 events per replica in the rarest failure scenario, and pod-reported cumulative totals provide full reconstruction when pods survive (the common case). The cost-benefit does not justify the added dependency.

---

### RES-004 Gateway preStop drain hook does not guarantee zero-downtime for long-lived streams [High] — Fixed
**Section:** 10.1, 17.8
...
**Resolution:** All four recommendations addressed. Section 10.1 preStop hook drain was rewritten with a 4-stage breakdown: (1) readiness probe set to `false` immediately at preStop start, closing the load balancer lag window before any drain logic begins, (2) wait for in-flight checkpoints to complete (capped at 30s) to prevent SIGKILL from corrupting checkpoint state, (3) poll for active stream drain over the remaining grace period, (4) SIGKILL deadline with reconnect expectation. A new **Long-running sessions and rolling updates** paragraph acknowledges that sessions exceeding `terminationGracePeriodSeconds` will always be interrupted by rolling updates, recommends sticky routing at the load balancer layer for deployments with long-lived sessions, and introduces the `lenny_gateway_sigkill_streams_total` metric for monitoring forced disconnections. Section 17.8.2 capacity tier table now includes a **Gateway scale-down time** row documenting the worst-case scale-down duration per tier (2 min for Tier 1, 7 min for Tier 2, 8.3 min for Tier 3), calculated from the one-pod-at-a-time (Tier 1/2) or three-pods-at-a-time (Tier 3) scale-down policies.

---

### RES-005 Warm Pool Controller crash blast radius: 15-second leader election gap freezes all pod creation [High] — Fixed
**Section:** 4.6.1, 17.8
...
**Resolution:** All four recommendations addressed. (1) Corrected the failover window throughout the spec: Section 4.6.1 leader election paragraph now distinguishes clean shutdown (near-zero via `LeaderCallbacks.OnStoppedLeading` voluntary lease release) from crash (worst-case 25s = `leaseDuration + renewDeadline`). All `minWarm` formulas and examples updated to use `failover_seconds = 25`. The example in Section 4.6.1 now reads `minWarm >= 2 * 35 + 4 * 10 = 110` (was `2 * 25 + 4 * 10 = 90`). Section 17.8 formula paragraph updated to specify 25s failover explicitly. Section 17.8 recommended `minWarm` values updated (Tier 1: 20, Tier 2: 175, Tier 3: 1050) to reflect the corrected failover window. The rolling update paragraph (Section 12.2) also updated. Section 4.6.2 leader election paragraph updated to reference the same failover model. (2) Added `ControllerLeaderElectionFailed` critical alert to Section 16.5 alert inventory and a dedicated monitoring paragraph in Section 4.6.1, tracked via `lenny_controller_leader_lease_renewal_age_seconds` gauge. (3) Added a **Simultaneous controller failover** paragraph in Section 4.6.1 analyzing the WPC+PSC simultaneous failure scenario: blast radius is additive (not multiplicatively worse) because PSC only adjusts scaling parameters that the WPC reads from CRDs; stale CRD values remain valid during the 25s gap. Confirms `podClaimQueueTimeout` (30s) covers the scenario. Recommends pod anti-affinity to prevent co-located leader failures. (4) Section 17.8 `minWarm` table updated with values computed using the corrected 25s failover window. Note: K8S-012 (voluntary lease release on clean shutdown) is a separate unfixed finding — the spec now documents the voluntary release behavior as the design intent, and K8S-012 tracks its implementation. SLC-006 (coordinator-loss detection) addresses pod-level coordinator loss via gRPC keepalive, which is orthogonal to controller leader election and does not change this analysis.

---

### RES-006 PoolScalingController failure leaves pools at stale desired state indefinitely [High] — Fixed
**Section:** 4.6.2
...
**Resolution:** All four recommendations addressed. Section 4.6.2 now includes a **Reconciliation drift detection** block specifying: (1) `lenny_pool_config_reconciliation_lag_seconds` gauge (by pool) added to the metrics inventory in Section 16.1, recording time since last successful CRD reconciliation. (2) `PoolConfigDrift` warning alert added to Section 16.5 — fires when any pool's Postgres `pool_config_generation` differs from its CRD `lenny.dev/config-generation` annotation for > 60s. Critically, detection runs in the gateway (not the PSC itself), so the alert fires even when the controller is completely down. (3) `PUT /v1/admin/pools/{name}` and `GET /v1/admin/pools/{name}` responses now include a `syncStatus` field (`"pending"` or `"synced"`) so operators are not misled into believing changes are instantly effective. (4) `GET /v1/admin/pools/{name}/sync-status` endpoint added to the admin API table (Section 15.1), returning `postgresGeneration`, `crdGeneration`, `lastReconciledAt`, `lagSeconds`, and `inSync` for detailed reconciliation diagnostics.

---

### RES-007 Delegation tree budget operations use Redis atomics but have no durability guarantee across Redis restart [High] — Fixed
**Section:** 8.3, 12.4
...
**Resolution:** All four recommendations addressed. (1) Section 11.2 now explicitly specifies that delegation budget counters (`maxTreeSize` active count, `maxTokenBudget` consumed, per-node allocations) are persisted to the `delegation_tree_budget` table in the same Postgres checkpoint transaction as token usage counters, ensuring atomicity between the two. (2) Section 12.4 adds a **Delegation budget counter reconciliation on Redis recovery** paragraph: on Redis restart/reconnection, the gateway re-validates all active delegation trees before accepting new `delegate_task` requests, using a two-source reconstruction (Postgres checkpoint + live pod enumeration from the SessionStore). New delegations are rejected with a retryable error during the reconciliation window (< 5s). (3) Section 11.2 adds a **Crash Recovery for Delegation Budget Counters** paragraph specifying that irrecoverable trees (stale Postgres checkpoint AND lost coordinating gateway replica) are moved to `awaiting_client_action` with reason `BUDGET_STATE_UNRECOVERABLE`, preventing over-budget delegations. (4) `lenny_delegation_budget_reconstruction_total` counter (labeled by `outcome`: `success`, `irrecoverable`) added to the metrics inventory (Section 16.1), and `DelegationBudgetIrrecoverable` warning alert added to Section 16.5.
