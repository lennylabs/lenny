...
### CRD-002 Kubernetes Secrets Holding Pool Credentials Have No Rotation or Encryption Guidance [High] — Fixed
**Section:** 4.9
...
**Resolution:** Items 2 and 3 were already addressed by the OPS-003 fix, which added the "Credential Lifecycle and Secret Management" subsection to Section 4.9 — including a credential rotation workflow (update K8s Secret, informer detects within 30s, new leases use new material, emergency revocation for immediate rotation) and a GitOps pattern section covering ESO, sealed-secrets, Vault, AWS Secrets Manager, and GCP Secret Manager integration. For item 1, Section 4.9 now includes a production requirement callout box mandating `EncryptionConfiguration` with `aescbc`/`aesgcm`/`kms` provider for the `secrets` resource type, with verification instructions for both cloud-managed and self-managed clusters. Section 17.6 preflight table adds an `etcd Secret encryption` warning check that alerts operators in production environments.

---

### CRD-003 Full-Tier Credential Rotation via Lifecycle Channel Has No Timeout or Failure Path [High] — Fixed
**Section:** 4.7, 4.9
...
**Resolution:** Section 4.7 now includes a "Full-tier credential rotation protocol" specification addressing all three recommendation items: (1) A 60-second timeout on `credentials_acknowledged` with automatic fallback to Standard-tier rotation (checkpoint + restart) and a `credential_rotation_timeout` warning event plus `lenny_credential_rotation_timeout_total` counter metric. (2) An in-flight request completion gate — the adapter waits for any active LLM request to complete before sending `credentials_rotated`, and queues new outgoing requests during the rotation window until `credentials_acknowledged` is received. The gate wait duration is recorded in the `lenny_credential_rotation_inflight_wait_seconds` histogram, and a `credential_rotation_inflight_wait_long` warning event is emitted if the wait exceeds 60 seconds, giving operators visibility into long-running in-flight requests that delay rotation. (3) An explicit old credential grace period — the old credential is NOT invalidated at the provider until either `credentials_acknowledged` is received or the 60-second timeout elapses and the fallback path takes over, preventing mid-stream auth failures on in-flight requests. The `lenny_credential_rotation_grace_period_seconds` histogram tracks actual grace period durations.

---

### CRD-004 LLM Proxy Extraction Trigger Has No Defined Threshold or Operability Criteria [High] — Fixed
...
**Resolution:** All three recommendation items addressed across Sections 4.1 and 4.9. (1) Section 4.1 extraction table now includes a refined threshold: `lenny_llm_proxy_active_connections` exceeding 60% of `maxConcurrent` across all replicas for 10+ minutes and growing faster than HPA can respond. A new "LLM Proxy-to-session ratio guidance" paragraph provides concrete ratio-based planning guidance: extraction recommended above 0.5:1, sustainable below 0.3:1, monitor GC pressure in between. (2) Section 4.9 (LLM Reverse Proxy) now documents the circuit breaker open-state behavior in detail: immediate `PROVIDER_UNAVAILABLE` error (no silent hang), adapter receives `PROVIDER_UNAVAILABLE` event on the control channel for relay to the runtime, in-flight streams are preserved, and half-open retry with configurable cooldown (default 30s). (3) A dedicated `LLMProxyCircuitOpen` alert was deemed unnecessary — the existing `GatewaySubsystemCircuitOpen` Warning alert (Section 16.5) already covers all gateway subsystem circuit breakers including LLM Proxy, with per-subsystem granularity via the `{subsystem}` label on `lenny_gateway_{subsystem}_circuit_state`. This is explicitly noted in the new Section 4.9 documentation for cross-reference clarity.

---

### CRD-005 Three Credential Modes Are Underdocumented for Operators — Interaction Effects Unspecified [High] — Fixed
**Section:** 4.9
...
**Recommendation:**
1. Explicitly state that user-supplied credentials (pre-authorized via `POST /v1/credentials`) are stored in the `TokenStore` with the same envelope encryption as OAuth refresh tokens, and are subject to the same KMS rotation procedure.
...
**Resolution:** All three items fixed. (1) The "Three Credential Modes" table now explicitly states that user-supplied credentials (pre-registered via `POST /v1/credentials`) are stored in `TokenStore` with the same envelope encryption via KMS as OAuth refresh tokens, including per-tenant DEK wrapping, KMS key rotation coverage (Section 10.5), tenant-scoped RLS, and T4 Restricted classification. A new "User-scoped credential storage" callout paragraph details the encryption lifecycle. (2) The pre-authorized flow is now fully specified in Section 4.9 with: user credential management endpoints (`POST/GET/DELETE /v1/credentials`), scoping rules (`tenant_id, user_id, provider` tuple derived from JWT), automatic resolution at session creation (Token Service lookup, lease materialization, fallback behavior), `userCredentialsEnabled` flag controlling whether pre-registered credentials are resolved, storage in `TokenStore` with existing envelope encryption, security treatment (logging exclusion for secret material, audit events for registration and deletion), and the `USER_CREDENTIAL_NOT_FOUND` error code. Credential elicitation via MCP was removed as a security measure — the MCP message path is subject to logging, tracing, and transcript persistence, making it unsuitable for credential material. The flow works identically across MCP, REST, OpenAI-compatible, and A2A clients. (3) The Fallback Flow now includes step 3: `maxRotationsPerSession` exhaustion check — when the rotation counter exceeds the limit, the session is terminated with `CREDENTIAL_FALLBACK_EXHAUSTED` (category: POLICY), a `credential.fallback_exhausted` audit event is emitted, and the `lenny_gateway_credential_fallback_exhausted_total` counter is incremented. A new "`requiresRuntimeRestart` precedence resolution" paragraph specifies the three-level precedence: runtime `credentialCapabilities.hotRotation` declaration is authoritative, policy-level `requiresRuntimeRestart` is a documentation default, and the resolved rotation mode is recorded in session metadata and the `credential.leased` audit event.

**Post-resolution update (2026-04-06):** The `credentialPolicy` has been moved from the Runtime definition to the tenant configuration. The Runtime now declares only capability information (`supportedProviders`, `credentialCapabilities`); the tenant config owns the `credentialPolicy` including `providerPools` (per-provider pool mapping), `preferredSource`, fallback settings, and user credential flags. At session creation, the gateway intersects Runtime capabilities with tenant policy. This change does not affect the CRD-005 fixes — the three credential modes, pre-authorized flow, fallback exhaustion behavior, and `requiresRuntimeRestart` precedence resolution remain as specified. Additionally, sessions now support multi-provider credential maps (one lease per provider in the intersection), with per-provider fallback and a shared `maxRotationsPerSession` counter across all providers.
