---
layout: default
title: "kms-key-probe-stale"
parent: "Runbooks"
triggers:
  - alert: KMSKeyProbeStale
    severity: critical
components:
  - platform
symptoms:
  - "the T4 per-tenant KMS key probe reports the tenant's key unusable"
tags:
  - chaos
related: []
---
...
The `KMSKeyProbeStale` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
