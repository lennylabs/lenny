...
components:
  - platform
symptoms:
  - "a platform-admin invoked acknowledgeHoldOverride to bypass a §12.8 legal hold"
tags:
  - chaos
related: []
---
...
The `LegalHoldOverrideUsed` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
