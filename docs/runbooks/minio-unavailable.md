...
components:
  - platform
symptoms:
  - "MinIO is unreachable; ArtifactStore writes fall back to §12.5 minimal state"
tags:
  - chaos
related: []
---
...
The `MinIOUnavailable` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
