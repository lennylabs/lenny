---
layout: default
title: "lease-extension-cool-off"
parent: "Runbooks"
triggers:
  - alert: LeaseExtensionCoolOffPersist
    severity: warning
components:
  - platform
symptoms:
  - "a §8.6 lease-extension cool-off was persisted into the lease record"
tags:
  - chaos
related: []
---
...
The `LeaseExtensionCoolOffPersist` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
