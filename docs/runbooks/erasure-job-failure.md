...
components:
  - platform
symptoms:
  - "a §12.8 GDPR erasure job entered the failed phase"
tags:
  - chaos
related: []
---
...
The `ErasureJobFailed` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
