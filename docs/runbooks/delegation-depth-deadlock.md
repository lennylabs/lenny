---
layout: default
title: "delegation-depth-deadlock"
parent: "Runbooks"
triggers:
  - alert: DelegationDepthDeadlock
    severity: warning
components:
  - platform
symptoms:
  - "a delegation tree reached the §8 max-depth cycle-detection threshold"
tags:
  - chaos
related: []
---
...
The `DelegationDepthDeadlock` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
