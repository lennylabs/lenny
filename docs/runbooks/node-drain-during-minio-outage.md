---
layout: default
title: "node-drain-during-minio-outage"
parent: "Runbooks"
triggers:
  - alert: NodeDrainDuringMinIOOutage
    severity: critical
components:
  - platform
symptoms:
  - "a node drain ran while MinIO was unavailable; checkpoints landed in minimal state"
tags:
  - chaos
related: []
---
...
The `NodeDrainDuringMinIOOutage` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
