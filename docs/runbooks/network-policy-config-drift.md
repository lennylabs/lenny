---
layout: default
title: "network-policy-config-drift"
parent: "Runbooks"
triggers:
  - alert: NetworkPolicyConfigDrift
    severity: warning
components:
  - platform
symptoms:
  - "the agent-namespace NetworkPolicies diverged from their rendered manifests"
tags:
  - chaos
related: []
---
...
The `NetworkPolicyConfigDrift` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
