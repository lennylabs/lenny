---
layout: default
title: "double-claim-verification"
parent: "Runbooks"
triggers:
  - alert: DoubleClaimDetected
    severity: critical
components:
  - platform
symptoms:
...
The `DoubleClaimDetected` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
