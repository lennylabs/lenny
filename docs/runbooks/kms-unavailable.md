...
components:
  - platform
symptoms:
  - "the configured KMS endpoint is unreachable"
tags:
  - chaos
related: []
---
...
The `KMSUnavailable` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
