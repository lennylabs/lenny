...
components:
  - platform
symptoms:
  - "a T3 or T4 tenant breached the §12.8 SLA deadline for an erasure job"
tags:
  - chaos
related: []
---
...
The `T3T4SLABreach` alert fires when the §16.5 condition documented in the alert rule holds for its `for:` window. See [Metrics Reference §Alert rules](../reference/metrics.html#alert-rules) for the exact PromQL.
