...
The adapter writes `/run/lenny/adapter-manifest.json` before spawning your binary. The manifest is one pod-global file, read-only to the agent container, and complete and authoritative when your binary starts. The adapter writes it before each session's runtime start, including each session on a recycling pool, so the manifest is authoritative for the session whose start last wrote it. On a pod holding more than one bound session, a later start replaces the manifest's `sessionId` and `mcpNonce` while an earlier session's runtime is still processing.
