...
| `lenny_adapter_set_tracing_context_dropped_total` | Counter | -- | Increments when the adapter drops a runtime's `set_tracing_context` frame because the per-session identifier the frame resolves to is not the session of the Attach stream that delivered it, or because the adapter's slot registry no longer holds that identifier with a bound session. A frame carrying no per-session identifier on a pod holding more than one slot is rejected before this point and counted by `lenny_adapter_unaddressed_frame_rejected_total` instead. Outside the default scrape set. | Operational monitoring. |
| `lenny_adapter_unaddressed_frame_rejected_total` | Counter | `frame_type` | Increments once per Attach stream that rejects a session-scoped JSONL frame carrying no per-session identifier on a pod holding more than one slot. The rejection is taken in each stream's demultiplexer, so one such frame on a pod holding two live Attach streams increments the counter twice. Outside the default scrape set. | Operational monitoring. |
