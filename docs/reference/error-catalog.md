...
| `DELEGATION_CYCLE_DETECTED` | 400 | Delegation rejected because the target's resolved `(runtime_name, pool_name)` tuple appears in the caller's delegation lineage and the §8.2 three-layer self-recursion gate did not admit the hop. `details.cycleRuntimeName` and `details.cyclePoolName` identify the cycle. `details.blockedBy` (`platform` \| `runtime` \| `policy`) names the first layer (in declared order) whose `allowSelfRecursion` evaluated `false`; `details.effectiveSettings` carries the resolved `{mode, platform, runtime, policy}` tuple so operators can identify which layer to flip without reading three configs. Only emitted when `gateway.cycleDetection.mode: enforce`; under `warn` the hop is admitted with a `delegation.cycle_warning` audit event, and under `permissive` no check runs. | Either choose a different delegation target to break the cycle, or (if self-recursion is intended) flip the layer named in `details.blockedBy` to `true` (Helm `gateway.allowSelfRecursion: yes`, `Runtime.spec.allowSelfRecursion: true`, or `DelegationPolicy.allowSelfRecursion: true`). All three layers must opt in. |
| `MESSAGEPART_TOO_LARGE` | 413 | A `MessagePart` payload exceeds the per-part size limit (50 MB). `details.partIndex`, `details.sizeBytes`, and `details.limitBytes` are included. | Split the output into smaller parts or reduce the payload size. |
...
| `MESSAGEPART_INLINE_REF_CONFLICT` | 400 | A `MessagePart` has both `inline` and `ref` fields set, which are mutually exclusive. | Set exactly one: `inline` for direct embedding or `ref` for external blob reference. |
...
| `CONFIRMATION_REQUIRED` | 422 | A destructive admin operation (such as a schema down-migration) requires an explicit confirmation flag that was not supplied. The request is rejected before any side effect runs. Not retryable: the same request fails identically until the confirmation flag is set. | Re-issue the request with the confirmation flag set (for example `{"confirm": true}`). |
| `SETUP_COMMAND_FAILED` | 422 | A session setup command exited non-zero (or hit its hard timeout), which the runtime adapter reports as a deterministic failure. Surfaced wherever the gateway runs setup commands: `POST /v1/sessions/{id}/finalize`, `POST /v1/sessions/start`, `POST /v1/sessions/{id}/start` (concurrent-workspace pools), and `POST /v1/sessions/{id}/resume`. `details.reason` is `setup_command_failed`; the per-command stdout and stderr are retrievable via `GET /v1/sessions/{id}/setup-output`. Not retryable: the command fails identically until the workspace plan or setup script changes. A non-deterministic setup-window failure (a crashed pod or a transport timeout) instead surfaces as the retryable `SESSION_CREATION_FAILED`, `STARTING_FAILED`, or `RESUME_FAILED`. | Inspect the setup-command output, correct the workspace plan or setup script, and create a new session. |
...
| `SESSION_CREATION_FAILED` | 503 | The atomic session-creation unit failed for a generic reason (warm-pod claim, workspace materialization, or a setup-window failure outside a more specific code). `Retry-After` is set. Retryable: creation is a single atomic unit and a retry rebinds a fresh pod. | Retry after the `Retry-After` window. If persistent, check pool and credential availability. |
| `STARTING_FAILED` | 503 | The two-step `POST /v1/sessions/{id}/start` transition failed for a generic reason. `Retry-After` is set. Retryable for the same reason as `SESSION_CREATION_FAILED`: a retry rebinds a fresh pod. | Retry after the `Retry-After` window. |
| `RESUME_FAILED` | 503 | A `POST /v1/sessions/{id}/resume` restore onto a fresh warm pool failed for a generic reason (pool or credential exhaustion, a token-service outage, or a transient setup-window transport failure). `Retry-After` is set. Retryable: the session row stays in `awaiting_client_action` so the explicit resume retry succeeds once the condition clears. | Retry `POST /v1/sessions/{id}/resume` after the `Retry-After` window. |
| `KMS_SIGNING_UNAVAILABLE` | 503 | The gateway JWT signer could not reach the signing backend (the signer circuit breaker is open), so a session token could not be minted. Existing sessions are unaffected until token expiry. | Retry after the signing backend recovers. |
| `SERVICE_UNAVAILABLE` | 503 | The gateway could not complete the request because a backing store or dependency it consulted returned a transient error rather than a definite answer. Returned by the message-injection gate when a runtime-store or per-tenant capability-override-store read fails, and by the pool-resolution path when the pool lookup is transiently unresolvable; both fail closed rather than degrade open. `Retry-After` is set. | Retry after the `Retry-After` window, once the dependency recovers. |
...
| `DELEGATION_POLICY_WEAKENING` | 422 | Delegation rejected because the child lease weakens an inherited `DelegationPolicy` axis. Covers two fields: (1) `maxDelegationPolicy` — the child cannot expand the effective delegation authority beyond the parent's; `details.parentPolicy` and `details.childPolicy` are included. (2) `allowSelfRecursion` — the child cannot widen `false → true` (a child may only narrow `true → false`); `details.parentValue` and `details.childValue` are included. `details.field` (`maxDelegationPolicy` \| `allowSelfRecursion`) is set in both cases so callers can disambiguate. | Narrow the offending axis in the child lease — keep `maxDelegationPolicy` no broader than the parent's, and never set `allowSelfRecursion: true` when the parent has `false`. |
...
| Code | Description |
|:-----|:------------|
| `UNREGISTERED_PART_TYPE` | A `MessagePart` carries an unprefixed `type` not present in the platform-defined registry. The part is passed through with a custom-type-to-`text` fallback and a warning annotation. Third-party types should use the `x-<vendor>/` namespace prefix. |
