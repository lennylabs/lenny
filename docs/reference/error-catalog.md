...
| `ELICITATION_CONTENT_TAMPERED` | 409 | An intermediate pod re-emitted an MCP `elicitation/create` wire frame for an existing `elicitation_id` with a `{message, schema}` pair diverging from the gateway-recorded original. Per the gateway-origin-binding invariant, the gateway is the authoritative source for elicitation display text; forwarding hops may not rewrite it. `details.elicitationId`, `details.originPod`, and `details.tamperingPod` are included. The forward is dropped before the modified text reaches the client, and `lenny_elicitation_content_tamper_detected_total` fires `ElicitationContentTamperDetected`. | To present transformed text (translation, rephrasing) to a different audience, emit a new `lenny/request_elicitation` establishing a fresh `elicitation_id`; do not rewrite an existing one. Investigate the `tampering_pod` for prompt-injection compromise. |
...
| `EPHEMERAL_CONTAINER_CRED_UID_FORBIDDEN` | 422 | `pods/ephemeralcontainers` subresource request rejected by the `lenny-ephemeral-container-cred-guard` ValidatingAdmissionWebhook because the proposed ephemeral container would reach through the agent pod's credential-file read boundary. `details.reason` carries a sub-code: `runAsUser_equals_adapter_uid`, `runAsUser_equals_agent_uid`, `cred_readers_gid_in_supplementalGroups`, `cred_readers_gid_in_runAsGroup`, `runAsUser_absent`, `runAsGroup_absent`, or `supplementalGroups_absent`. `details.targetPod` identifies the target pod. | Resubmit the ephemeral container with `securityContext.runAsUser`/`runAsGroup`/`supplementalGroups` explicitly set to values outside the adapter UID, agent UID, and the `lenny-cred-readers` GID. |
...
| `CONTENT_POLICY_WEAKENING` | 422 | Delegation rejected because the child lease removes a content policy interceptor that the parent had (i.e., sets `contentPolicy.interceptorRef: null` when the parent had a non-null reference). | Retain the parent's `contentPolicy.interceptorRef` in the child lease. |
