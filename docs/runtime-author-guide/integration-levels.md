...
| **Simple response shorthand** (`{"type":"response","slotId":"...","text":"..."}`) | Yes | Yes | Yes |
| **Minimal output part fields** (only `type` + `inline` required) | Yes | Yes | Yes |
| **Minimal message fields** (only `type`, `id`, `input`, and `slotId` needed; a runtime echoes `slotId` on the frames it emits in response) | Yes | Yes | Yes |
