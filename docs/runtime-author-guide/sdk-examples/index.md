...
3. **Produce a summary** as a `response` on stdout, carrying the `slotId` the inbound `message` was addressed with. The examples concatenate truncated file contents; a production summarizer would pass the contents to an LLM.
...
- Echoing the inbound `slotId` on every session-scoped frame emitted in response
