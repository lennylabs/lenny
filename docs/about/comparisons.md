...
| **Execution modes**        | session (`sessionPolicy`: recycling, concurrent slots) / service | N/A            | N/A                       | N/A                         | N/A (workflow-based)     | N/A                         | N/A (graph-based)               |
...
| **Execution modes**     | `session` (a managed session bound to a claimed pod, parameterized by `sessionPolicy` for pod recycling and concurrent slots) and `service` (each message routed to any ready replica). Pool scaling derives from the policy properties.              | Single sandbox per request. No pod reuse or multiplexing.                                                                      |
...
| **Credentials** | Leased, pool-managed, rotatable; gateway mediates the LLM proxy in the default proxy-delivery mode, with optional direct delivery for runtimes that need to call the provider themselves. | `OPEN_SANDBOX_API_KEY` for management-API auth; no per-session leasing or LLM-proxy model documented. |
...
- **Mixed workload patterns** -- you need both interactive agent sessions (`session` mode with `recycle.enabled: false`) and high-throughput processing (`session` mode with pod recycling or concurrent sessions, or `service` mode) on the same platform, with scaling derived from the `sessionPolicy`.
...
- **Credential management** -- centralized credential pools with leasing and rotation. In the default proxy-delivery mode, the gateway talks to LLM providers on behalf of agent pods, so pods never hold real provider API keys; pools can alternatively use direct delivery, in which a short-lived, lease-scoped credential is materialized onto the pod for runtimes that must call the provider themselves. Each proxy-mode lease token is cryptographically bound to the pod that requested it, so a token lifted from one pod cannot be used by another.
