...
- **Direct mode.** The gateway materializes a short-lived, lease-scoped credential onto the pod at assignment time and the runtime calls the provider itself. The LLM Proxy is not on the request path, and the pod briefly holds a credential file that is removed on session end and re-leased per session on a recycling pod.
