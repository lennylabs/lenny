...
| 3.5   | Admission policies, lenny-ops first deploy                   | Partial        | The `pkg/admission` decision packages and two baseline webhooks (label-immutability and sandboxclaim-guard) are built and deployable. The `ephemeral-container-cred-guard` decision package is built; its webhook HTTP handler and Helm manifest, plus `pool-config-validator`, `crd-conversion`, the default-deny NetworkPolicy, the phase-stamp ConfigMap, `lenny-ops`, `lenny-preflight`, and `lenny-bootstrap`, are not. |
