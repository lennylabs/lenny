...
runtimes, and installer work. The §10.7 experiment surface — including the
admission-time variant-pool isolation-monotonicity check — is complete. The remaining
pure-Go refinements are the experiment-admission tenant-floor advisory check (§10.7,
needs the tenant policy `minIsolationProfile`) and the `PreExportMaterialization`
interceptor (needs the §8.7 file-export model).
