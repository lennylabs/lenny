...
- Delegation-tree recovery: archive-on-terminal is wired across every terminal path;
  the archive-and-replay loop, the `cascadeOnFailure` policy, the
  `children_reattached` event, and the orphan-cleanup job are all in place. The
  remainder is the bottom-up reattach traversal with the `maxLevelRecoverySeconds` /
  `maxTreeRecoverySeconds` deadlines — the recovery sweep that resumes failed
  delegation-tree nodes leaves-first. The `maxOrphanTasksPerTenant` delegation-time
  cap (fall back to `cancel_all` when a tenant is over its orphan budget) is also
  still open.
