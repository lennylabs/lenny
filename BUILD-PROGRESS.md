...
| 11    | Advanced credentials, multi-provider translators, revocation | Partial        | The revocation cache exists. The §4.9 `azure_openai` LLM-proxy translator is built (`pkg/gateway/llmproxy`, alongside `anthropic_direct`). The `aws_bedrock` and `vertex_ai` translators, hot credential rotation, emergency revocation propagation, the `credentialPolicy` fallback chain, and the `CredentialRenewalWorker` are not.                                                                                                                                                                                                        |
...
Phase 11 (§18.26, advanced credentials / multi-provider translators) is in progress.
The §4.9 `azure_openai` LLM-proxy translator is built, modeled on `anthropic_direct`.
The next chunk is the remaining translators: `vertex_ai` and `aws_bedrock` (both serve
the Anthropic Messages format for Claude with a body transformation — `anthropic_version`
swapped for the top-level `model` — and provider-specific auth; `aws_bedrock`
additionally needs AWS SigV4 or Bedrock-API-key handling). Then hot credential
rotation, emergency revocation propagation, the `credentialPolicy` fallback chain, and
the `CredentialRenewalWorker`.
