...
| 3     | Pool scaling, delegation policy, runtime upgrade, mTLS       | Partial        | The PoolScalingController and the `RuntimeUpgrade` state substrate exist. The §8.3 `DelegationPolicy` first-class resource is built: `delegationpolicystore` is the platform-global registry and `/v1/admin/delegation-policies` serves the §15.1 CRUD with the `EXPORT_SCAN_REQUIRES_INTERCEPTOR` admission check. The Kubernetes `DelegationPolicy` CRD and controller sync, the delegation-time tag-matching evaluation, the `RESOURCE_HAS_DEPENDENTS` deletion guard, the `agent_pod_state` mirror write path, CIDR-drift detection, and the SDK-warm circuit-breaker logic are not built. `pkg/mtls` exists; the cert-manager PKI wiring is partial.                                                        |
