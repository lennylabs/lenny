...
| 3.5   | Admission policies, lenny-ops first deploy                   | Partial        | The `pkg/admission` decision packages and three baseline webhooks (label-immutability, sandboxclaim-guard, ephemeral-container-cred-guard) are built and deployable — decision logic, `cmd/lenny-webhook` handler, and Helm manifest. `pool-config-validator`, `crd-conversion`, the default-deny NetworkPolicy, the phase-stamp ConfigMap, `lenny-ops`, `lenny-preflight`, and `lenny-bootstrap` are not. |
