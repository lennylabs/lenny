...
- Delegation-tree recovery: archive-on-terminal is wired across every terminal path;
  the archive-and-replay loop, the `cascadeOnFailure` policy (with the
  `maxOrphanTasksPerTenant` cap), the `children_reattached` event, and the
  orphan-cleanup job are all in place. The remaining §8.10 piece is the bottom-up
  reattach traversal with the `maxLevelRecoverySeconds` / `maxTreeRecoverySeconds`
  deadlines — the recovery sweep that resumes failed delegation-tree nodes
  leaves-first.
