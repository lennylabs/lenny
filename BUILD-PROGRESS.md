...
| 9     | Delegation, delegation-echo                                  | Partial        | `pkg/delegation/cycle`, `pkg/delegation/lease`, `pkg/delegation/tracing`, and the gateway delegation service are built. The §8.5 platform MCP tool surface is complete: `lenny/create_session`, `lenny/send_message` (with `inReplyTo` input resolution), `lenny/get_task_tree`, `lenny/delegate_task`, `lenny/cancel_child`, `lenny/await_children`, `lenny/discover_agents`, `lenny/set_tracing_context`, `lenny/output`, and `lenny/request_input` (`pkg/gateway/inputwait` registry, §11.3 timeout). The delegation service propagates a parent's §8.3 tracingContext onto each child. The §4 `RequestInterceptor` chain framework (`pkg/gateway/interceptor`) is built and the `PreMessageDelivery` phase is wired into `lenny/send_message`. The §8.10 `session_tree_archive` store (`pkg/gateway/treearchive`) is built. Not built: the `delegation-echo` runtime, the `PreDelegation` and `PreExportMaterialization` interceptor wiring, external gRPC interceptors, the `ExtendLease` lease-extension control plane, and the delegation-tree recovery wiring (archive-on-terminal, replay-on-resume, the bottom-up reattach traversal, and the `children_reattached` event).                                                                                                                                                                                                 |
...
(`pkg/gateway/treearchive`). Remaining Phase 9 work: the `delegation-echo` reference
runtime, the `ExtendLease` lease-extension control plane, and the delegation-tree
recovery wiring on top of the `treearchive` store.

Each remaining Phase 9 item carries a dependency to scope first:

- Delegation-tree recovery wiring: the `treearchive` store is built; the wiring
  archives a child's §8.8 TaskResult on every terminal transition (a cross-cutting
  change across the sessionserver terminal paths) and replays the archive when a
  ...
  direct parent, so the wiring must reconcile the tree-root key with the
