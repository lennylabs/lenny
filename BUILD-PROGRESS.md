...
`POST /v1/admin/legal-hold`, and the step-0 erasure preflight that rejects an erasure
when the target user owns a held session. The next tractable pure-Go chunk is the
artifact-retention GC honoring `legal_hold` (the §12.8 rule that a held session's
workspace, transcript, and checkpoints are exempt from TTL rotation), followed by the
§11.4 `full_revoke` session-termination step. The pod-RPC `Terminate` fan-out, Redis
cached-auth invalidation, and credential-lease revocation remain infrastructure-bound.
