...
proactive-renewal loop (`pkg/gateway/credrenewal`), and the `credentialPolicy`
fallback chain (`pkg/gateway/credfallback` — the ordered-pool selector with
per-pool `cooldownOnRateLimit`). The next chunk is the remaining credential-lifecycle
work: hot rotation (the `credentials_rotated` / `credentials_acknowledged` adapter
handshake) and emergency revocation propagation through Redis pub/sub.
