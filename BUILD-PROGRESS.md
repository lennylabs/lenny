...
- Delegation-tree recovery: archive-on-terminal is wired across every terminal path
  (terminate, cancel, start-failure, watchdog forced-failure, and expiry); the
  archive-and-replay loop, the `cascadeOnFailure` policy, and the
  `children_reattached` event are all in place. The remainder is the bottom-up
  reattach traversal with the `maxLevelRecoverySeconds` / `maxTreeRecoverySeconds`
  deadlines, the `await_completion` cascade timer (`cascadeTimeoutSeconds`), and the
  orphan-cleanup Job (`maxOrphanTasksPerTenant`).
