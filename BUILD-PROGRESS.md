...
the `lenny/request_elicitation` tool, the §9.1 `maxElicitationsPerSession` budget, and
the tenant elicitation-content-integrity admin endpoints are built. The next chunk is
the rest of Phase 10: the gateway virtual MCP server (§9.1) and the hop-by-hop
elicitation chain that forwards an elicitation up the task tree applying depth
suppression and content integrity, plus the `lenny_elicitation_dropped_total` counter.
