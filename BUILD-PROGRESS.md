...
runtimes, and installer work. The remaining pure-Go items are smaller refinements: the
experiment-admission isolation-monotonicity validation (§10.7, needs cross-resource
pool resolution) and the `PreExportMaterialization` interceptor (needs the §8.7
file-export model).
