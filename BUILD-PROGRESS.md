...
| 9     | Delegation, delegation-echo                                  | Partial        | `pkg/delegation/cycle`, `pkg/delegation/lease`, `pkg/delegation/tracing`, and the gateway delegation service are built. The §8.5 platform MCP tool surface is complete: `lenny/create_session`, `lenny/send_message` (with `inReplyTo` input resolution), `lenny/get_task_tree`, `lenny/delegate_task`, `lenny/cancel_child`, `lenny/await_children`, `lenny/discover_agents`, `lenny/set_tracing_context`, `lenny/output`, and `lenny/request_input` (`pkg/gateway/inputwait` registry, §11.3 timeout). The delegation service propagates a parent's §8.3 tracingContext onto each child. The §4 `RequestInterceptor` chain framework (`pkg/gateway/interceptor`) is built and the `PreMessageDelivery` phase is wired into `lenny/send_message`. The §8.10 `session_tree_archive` store (`pkg/gateway/treearchive`) is built; child sessions are archived to it on terminal transitions — `lenny/cancel_child` archives the cancelled subtree, and the sessionserver archives a child on terminate and on cancel (`DELETE`). Not built: the `delegation-echo` runtime, the `PreDelegation` and `PreExportMaterialization` interceptor wiring, external gRPC interceptors, the `ExtendLease` lease-extension control plane, and the rest of the delegation-tree recovery wiring (archive on the start-failure and expiry terminal paths, replay-on-resume, the bottom-up reattach traversal, and the `children_reattached` event).                                                                                                                                                                                                 |
...
`ReportUsage` adapter RPC; the §8.10 `session_tree_archive` store
(`pkg/gateway/treearchive`); and archive-on-terminal wiring (`lenny/cancel_child` plus
the sessionserver terminate and cancel paths). Remaining Phase 9 work: the
`delegation-echo` reference runtime, the `ExtendLease` lease-extension control plane,
and the rest of the delegation-tree recovery wiring.

Each remaining Phase 9 item carries a dependency to scope first:

- Delegation-tree recovery wiring: `treearchive` is built and child sessions are
  archived on the terminate and cancel terminal paths. The remaining wiring archives
  the start-failure (`failSession`) and expiry paths, and replays the archive when a
  resumed parent re-issues `lenny/await_children`. Note that `treearchive` keys by
  `(root_session_id, node_session_id)` whereas `await_children` authorizes on the
  direct parent, so the replay wiring must reconcile the tree-root key with the
  direct-child authorization model.
