...
| 3.5   | Admission policies, lenny-ops first deploy                   | Partial        | The `pkg/admission` decision packages and three webhook handlers (label-immutability, sandboxclaim-guard, ephemeral-container-cred-guard) are built and served by `cmd/lenny-webhook`. The `ephemeral-container-cred-guard` Helm `ValidatingWebhookConfiguration` manifest, `pool-config-validator`, `crd-conversion`, the default-deny NetworkPolicy, the phase-stamp ConfigMap, `lenny-ops`, `lenny-preflight`, and `lenny-bootstrap` are not. |
