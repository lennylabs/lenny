...
- Delegation-tree recovery: the archive-and-replay loop is wired for the terminate,
  cancel, and start-failure terminal paths. The remainder is archiving the expiry
  path, the bottom-up reattach traversal with the `maxLevelRecoverySeconds` /
  `maxTreeRecoverySeconds` deadlines, the `children_reattached` event, and the
  `cascadeOnFailure` policy.
