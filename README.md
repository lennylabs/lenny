...
- **Bundled monitoring.** Prometheus alerting rules drop into any standard observability stack, and a Grafana dashboard covers core health signals. The chart renders the latency and availability SLOs as OpenSLO v1 documents under `monitoring.openslo.enabled` for OpenSLO-compatible tooling.
