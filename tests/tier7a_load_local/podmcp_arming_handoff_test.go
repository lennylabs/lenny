...
func TestPodMCPArmingSurvivesSessionHandoff_spec_15_4_3(t *testing.T) {
	...
	departedNonce := manifestNonce(t, manifestDir)
	if departedNonce == "" {
		t.Fatal("the pod manifest carries no MCP nonce for the departing session")
	}
	...
	if nonce == departedNonce {
		t.Fatal("the successor's start did not rewrite the pod manifest nonce")
	}
	...
	// The successor's claim took the arming over rather than leaving the
	// departed session's server bound to the one pod-wide socket, so the
	// retired nonce is refused.
	if mcpInitializeAccepted(t, s.MCPSocket, departedNonce) {
		t.Error("the platform MCP server still authenticates the departed session's manifest nonce")
	}
