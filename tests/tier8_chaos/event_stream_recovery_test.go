...
func TestOpsEventStreamRecoveryFlushReEmitsBufferedEventsAfterRedisRecovers(t *testing.T) {
	...
	// The outage window: lenny-ops emits into the local ring only (Publish
	// does not XADD; the outage means the fan-out XADD failed). The pre event
	// is also in the local ring (it was emitted locally too).
