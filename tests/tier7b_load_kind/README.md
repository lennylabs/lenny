...
- `tier7b_load_kind/` (this directory) — Kind-based smoke runs that drive a short, low-VU workload against the e2e cluster and diff the result against a stored baseline. Runs on every PR.
- `tier12_load_cloud/` — Opt-in cloud-profile runs that exercise the §12.7 production sizing targets (100+ concurrent claims, 50-child fan-out, 500+ streaming sessions). Drives a real cloud cluster via `scripts/cloud/<provider>/run-load.sh`.
...
- `tests/tier12_load_cloud/` — the cloud-scale companion
