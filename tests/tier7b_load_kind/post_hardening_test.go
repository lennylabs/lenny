...
// re-validation). The TESTING.md §13.31 phase gate re-runs the TESTING.md §13.29
...
//
// The TESTING.md §14.5 phase gate runs at the TESTING.md §14.5 release gate against a
...
// it would measure is not the production overhead the TESTING.md §13.31 gate
// asserts.
//
// The companion TESTING.md §13.31 TestFullSystemWithHardeningLoad in
// scaffolds_test.go is the TESTING.md §13.31 composite (every TESTING.md §13.29 scenario
// re-run); this test is the TESTING.md §13.31 sub-scenario for the
// pod_claim_latency hot path, isolated so a regression on the claim
// path can be diagnosed without re-running every other TESTING.md §13.29
// scenario.
...
// diagnosis: the TESTING.md §13.31 post-hardening SLO re-validation is a
// phase-gated composite. The §14 security hardening (cosign image
// verification, the §13.1 pod-security admission webhook,
// NetworkPolicy refinement, gateway mTLS) is enabled at the TESTING.md §14.5
...
// overhead the TESTING.md §13.31 gate measures has no surface on the smoke run.
// The post_hardening_slo scenario is the TESTING.md §13.31 substrate the cloud
...
func TestPostHardeningSLO(t *testing.T) {
	...
	t.Skip("phase-gated: the TESTING.md §13.31 post-hardening SLO re-validation runs at the " +
		...
		"install does not apply that hardening, so the overhead the TESTING.md §13.31 gate " +
