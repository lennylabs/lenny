...
// validates on every gateway-to-pod RPC. The session identifier is the
// request's only address, so the pull names whose counters it reads
// without a second field. The case reads the compiled descriptor, so a
// field addition, rename, or renumber that survives a coordinated
...
// nothing the pod can reject it on), a second address was reintroduced
// beside session_id, or a field was renumbered (breaking gateway/adapter
// binary compatibility on the ReportUsage pull). Re-edit
...
func TestReportUsageRequestWireContract(t *testing.T) {
	...
	if got := fields.Len(); got != len(want) {
		t.Fatalf("ReportUsageRequest has %d fields, want %d (cumulative must be present per §11.2 crash recovery and coordination_generation per the §10.1 fence, with session_id the only address)", got, len(want))
	}
