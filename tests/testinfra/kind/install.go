...
// (cred-shell-echo-pool, elicitation-echo-pool) and the §5.3 sandboxed
// reference pool (gvisor-echo-pool) declare their model on the Runtime
// CRD instead.
func deploymentModel(pool string) string {
	...
	case strings.Contains(pool, "cred-shell-echo") || strings.Contains(pool, "elicitation-echo") ||
		strings.Contains(pool, "gvisor-echo"):
