...
func TestConcurrentMaxPodUptimeRoundTrip_spec_6_2(t *testing.T) {
	...
	// longer round-trips (migration 0167 retired the column), so the re-read
	// row carries an empty ConcurrencyStyle that ValidateConcurrentConfig
	// would reject on re-validation of this still-live concurrent-mode pool;
	// the mutate re-establishes it. spec: §5.2 (execution modes), §12.6.
