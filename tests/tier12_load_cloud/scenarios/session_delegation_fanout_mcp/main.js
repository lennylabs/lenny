...
//
// k6 scenario: session_delegation_fanout_mcp
//
// Cloud-load §12.7 MCP fan-out scenario. Each iteration creates a
// parent session via /v1/sessions/start, fans out LENNY_FANOUT
// children through the §8.2 lenny/delegate_task JSON-RPC tool, then
// terminates every claimed session.
...
export default function () {
  ...
  for (let i = 0; i < FANOUT; i++) {
    ...
    if (
      check(res, {
        'mcp ok': (r) => r.status === 200,
        'mcp no error': (r) => r.body && !r.body.includes('"error"'),
      }) && res.body
    ) {
