...
// or directly as `go run ./tests/tier7_load/scenarios/startup_latency.go`.
...
func main() {
	var (
		...
		output  = flag.String("output", "", "write JSON to this path (default: tests/tier7_load/baselines/startup_latency.json)")
	...
	if outPath == "" {
		...
		outPath = filepath.Join(root, "tests", "tier7_load", "baselines", "startup_latency.json")
	}
