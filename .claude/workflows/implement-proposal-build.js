...
const RULES =
  ...
  "\n\nPER-STEP VERIFICATION — must both TEST the step AND avoid the 180s no-progress watchdog (a subagent is force-killed if a foreground command emits no output for ~180s, then the build aborts). DO test every step; do NOT skip tests. But NEVER run a whole-repo `lenny-test --max-tier unit`/`static` per step (lint + `go vet` + schema/codegen over the entire repo exceeds 180s), and NEVER pipe any verification through `| tail`/`| head` (buffers all output, always trips the watchdog). Instead, for THIS step run SCOPED, STREAMING checks on the changed packages: `go build ./<changed-pkg>/...`, `go vet ./<changed-pkg>/...`, `golangci-lint run ./<changed-pkg>/...`, and `go test ./<changed-pkg>/... -count=1` with NO pipe to tail (let output stream). Those exercise the step's tier 0 and tier 1 in seconds. For each higher tier the step genuinely reaches (tier 2 envtest, tier 4 integration, tier 5 e2e), still RUN it, but in the BACKGROUND (Bash `run_in_background: true`) writing to a log, then poll the log every ~30-60s with short `tail -n 40 <log>` reads until it finishes — the polls keep the watchdog reset while the command runs detached. Scope per step to what changed; the whole-repo full-suite run is reserved for the final Verify phase (which also uses the background-poll pattern). If a pre-existing whole-repo lint/test failure is unrelated to this step's changed packages, note it and do not treat it as this step's failure.";
  ...
  while (attempt < maxStepAttempts && !(stepGreen && stepReviewClean)) {
    ...
    const sv = await agent(
      "Independently verify ONE just-implemented build step of an applied spec proposal. You did not write this code.\n\n" +
        ...
        ". STREAM all output — do NOT pipe through `| tail`/`| head` (buffering emits nothing for the run's duration and trips the 180s no-progress watchdog, aborting the build). Run the scoped tier 0/1 commands above directly (they finish in seconds). For each higher tier (tier 2 envtest, tier 4 integration, tier 5 e2e), run it in the BACKGROUND (Bash `run_in_background: true`) writing to a log, then poll with short `tail -n 40 <log>` reads every ~30-60s until it finishes, so the polls keep the watchdog reset while it runs detached; never run a whole-repo `lenny-test --max-tier unit` in the foreground per step. Set green=true only if every tier that can run here passes (skip only a tier that genuinely needs a cloud-only resource, noting it); a pre-existing whole-repo failure unrelated to this step's changed packages is not this step's failure. List any real failures precisely so they can be fixed. Coverage is checked once over the whole change at the end, not here.",
...
let verify = await agent(
  "Verify the completed implementation of an applied spec proposal builds and its tests pass across every tier the change reached.\n\n" +
    ...
    ". Run each tier with STREAMING output, never piped through `| tail`/`| head` (buffering emits nothing and trips the 180s no-progress watchdog, aborting the run). For any whole-change `lenny-test --changed --max-tier <tier>` run that may exceed ~120s, run it in the BACKGROUND (Bash `run_in_background: true`) writing to a log and poll with short `tail -n 40 <log>` reads every ~30-60s until it finishes; bring infrastructure up as needed. Run `lenny-test coverage --diff " +
    baseRef +
...
while (
  ...
  verify = await agent(
    "Re-run the reached tiers for the implementation in " +
      ...
      ". Run with STREAMING output (never piped through `| tail`); for any `lenny-test --changed --max-tier <tier>` run that may exceed ~120s, run it in the BACKGROUND (Bash `run_in_background: true`) writing to a log and poll with `tail -n 40 <log>` every ~30-60s until it finishes (this avoids the 180s no-progress watchdog), bringing infrastructure up as needed. Apply the same coverage gate: green=true requires every reached tier to pass AND changed-line coverage (via `lenny-test coverage --diff " +
      baseRef +
      "`) at least " +
      coverageFloor +
...
if (green && reviewFixApplied) {
  const recheck = await agent(
    "Re-run the reached tiers for the implementation in " +
      ...
      (tierSet.join(", ") || "from the diff") +
      ". Run with STREAMING output (never piped through `| tail`); for any `lenny-test --changed --max-tier <tier>` run that may exceed ~120s, run it in the BACKGROUND (Bash `run_in_background: true`) writing to a log and poll with `tail -n 40 <log>` every ~30-60s until it finishes (avoids the 180s no-progress watchdog). Apply the coverage gate (changed-line coverage at least " +
      coverageFloor +
