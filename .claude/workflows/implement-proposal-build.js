...
const RULES =
  ...
  "\n\nVERIFICATION WATCHDOG (critical): a subagent is force-killed if a single foreground command emits no output for ~180 seconds, after which the build aborts. A whole-repo `lenny-test --max-tier unit`/`static` run (lint, `go vet`, schema/codegen checks) exceeds that, and piping any long command through `| tail`/`| head` buffers all output until completion, so it always trips the watchdog. AVOID IT: run any verification command that may exceed ~120s in the BACKGROUND (Bash `run_in_background: true`) writing to a log file, then poll the log every ~30-60s with short foreground reads (`tail -n 40 <log>`) until the command finishes — each poll emits output and resets the watchdog while the command runs detached. Never run a long verification in the foreground piped through `| tail`. Prefer scoping runs to the changed packages (`go test ./<changed-pkg>/... -count=1`, `golangci-lint run ./<changed-pkg>/...`) so each finishes quickly; use the background-poll pattern for any whole-repo run.";
...
let plan = await agent(
  "Plan the code implementation of an applied spec proposal.\n\n" +
    ...
    " IMPORTANT — this proposal may be PARTIALLY IMPLEMENTED already: earlier build steps are committed on the current branch (run `git log --oneline -40` and look for commits referencing this proposal, e.g. '0002 S1:', 'S3:', ...). Grep the CURRENT code to determine what is already implemented, and plan ONLY the steps still needed to finish the proposal from the current state. Do NOT include a step whose surface is already implemented and committed — confirm by grep that each planned step's work is genuinely not-yet-present (e.g. if `executionMode` is already the `session;service` enum and `concurrencyStyle`/`taskPolicy` are already gone, do not re-plan those). The already-committed steps stay; you are sequencing only the remainder.",
  ...
  while (attempt < maxStepAttempts && !(stepGreen && stepReviewClean)) {
    ...
    const sv = await agent(
      "Independently verify ONE just-implemented build step of an applied spec proposal. You did not write this code.\n\n" +
        ...
        ". Use `lenny-test --changed --max-tier <tier>` and bring infrastructure up with `lenny-test infra up` when a tier needs it. Set green=true only if every tier that can run here passes (skip only a tier that genuinely needs a cloud-only resource, noting it). List any failures precisely so they can be fixed. Coverage is checked once over the whole change at the end, not here. VERIFICATION WATCHDOG: a command that emits no output for ~180s force-kills you and aborts the build; a whole-repo `lenny-test` run exceeds that. Run any verification that may exceed ~120s in the BACKGROUND (Bash `run_in_background: true`) writing to a log, and poll the log with short `tail -n 40 <log>` reads every ~30-60s until it completes, so output keeps the watchdog reset. Never pipe a long run through `| tail` in the foreground. Prefer scoping to the changed packages (`go test ./<changed-pkg>/... -count=1`) so runs finish fast.",
