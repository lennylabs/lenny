  ...
  while (attempt < maxStepAttempts && !(stepGreen && stepReviewClean)) {
    ...
    const sv = await agent(
      "Independently verify ONE just-implemented build step of an applied spec proposal. You did not write this code.\n\n" +
        ...
        ". STREAM all output — do NOT pipe through `| tail`/`| head` (buffering emits nothing for the run's duration and trips the 180s no-progress watchdog, aborting the build). Run the scoped tier 0/1 commands above directly (they finish in seconds). For each higher tier (tier 2 envtest, tier 4 integration, tier 5 e2e), run it in the BACKGROUND (Bash `run_in_background: true`) writing to a log, then poll with short `tail -n 40 <log>` reads every ~30-60s until it finishes, so the polls keep the watchdog reset while it runs detached; never run a whole-repo `lenny-test --max-tier unit` in the foreground per step. Set green=true only if every tier that can run here passes (skip only a tier that genuinely needs a cloud-only resource, noting it); a pre-existing whole-repo failure unrelated to this step's changed packages is not this step's failure. List any real failures precisely so they can be fixed. Coverage is checked once over the whole change at the end, not here. BRANCH SAFETY: never `git checkout` a branch or commit to compare against a baseline — use `git diff <SHA>..HEAD` or `git show <SHA>:<path>` (you only run tests and report; do not change the checkout or the current branch).",
