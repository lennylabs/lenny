  ...
  while (attempt < maxStepAttempts && !(stepGreen && stepReviewClean)) {
    ...
    const sv = await agent(
      "Independently verify ONE just-implemented build step of an applied spec proposal. You did not write this code.\n\n" +
        ...
        ". MEMORY-SAFE + no-stall: run every command in the FOREGROUND, one at a time, SCOPED to the changed packages — NEVER use `run_in_background` for tests and NEVER run whole-repo `go test -race ./...` or `lenny-test --max-tier unit` (orphaned/concurrent envtest etcd+kube-apiserver and the race detector OOM-crash this 16GB host). Run the scoped tier 0/1 commands above directly (no `| tail`; they finish in seconds, streaming so no watchdog stall). For a tier-2 envtest package this step changes, run it FOREGROUND scoped to that one package (`go test ./<changed-envtest-pkg>/... -count=1 -p 1` with `KUBEBUILDER_ASSETS` set) so only one etcd+apiserver is alive, then reap strays (`pkill -f kube-apiserver 2>/dev/null; pkill -f 'etcd' 2>/dev/null`) before the next. Use `-p 2` (or `-p 1` for envtest/integration) to cap memory. Set green=true only if every tier that can run here passes (skip only a tier that genuinely needs a cloud-only resource, noting it); a pre-existing whole-repo failure unrelated to this step's changed packages is not this step's failure. List any real failures precisely so they can be fixed. Coverage is checked once over the whole change at the end, not here. BRANCH SAFETY: never `git checkout` a branch or commit to compare against a baseline — use `git diff <SHA>..HEAD` or `git show <SHA>:<path>` (you only run tests and report; do not change the checkout or the current branch).",
...
let verify = await agent(
  "Verify the completed implementation of an applied spec proposal builds and its tests pass across every tier the change reached.\n\n" +
    ...
    ". MEMORY-SAFE on this 16GB host: run each tier in the FOREGROUND, one at a time, SCOPED to the changed packages (`lenny-test --changed --max-tier <tier>` selects only changed packages; add go-test parallelism `-p 1`/`-p 2`). NEVER use `run_in_background` for tests and NEVER run a whole-repo `go test -race ./...` — orphaned/concurrent envtest etcd+kube-apiserver and the race detector OOM-crash the machine. Run tier-2 envtest packages one at a time and reap strays (`pkill -f kube-apiserver 2>/dev/null; pkill -f 'etcd' 2>/dev/null`) between them. Stream output (no `| tail`); scoped runs stay under the 180s watchdog. Bring infrastructure up as needed. Run `lenny-test coverage --diff " +
    baseRef +
