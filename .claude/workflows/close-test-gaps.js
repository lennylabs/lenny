...
//                      specific question for a human to answer. A pure
//                      test-infrastructure design choice (how to test, not
//                      what the spec requires) is NOT needs-human — the
//                      runner decides it under the no-regression rule and
//                      routes test (see STEP 5b).
// ---------------------------------------------------------------------------
phase('Close')
...
function implementPrompt(f, priorIssues) {
  return [
    ...
    '',
    'STEP 5a-infra — before choosing needs-human, ask whether the unresolved question is about how to TEST the behavior or about what the SPEC requires. A pure test-infrastructure design choice — whether to add a test-only constructor or exported accessor, which fixture/container/tier to drive it from, how to structure a harness, whether an existing helper can be reused — is YOURS to decide, not a human\'s: pick the option and route="test", under one hard constraint: a new or modified test must not break any existing test, nor change an existing test\'s behavior in a way that invalidates that test\'s assertion. A test-only product addition (an exported constructor or accessor) is allowed when it alters no existing runtime path, and is NOT a needs-human escalation. Only when the unresolved question is genuinely about spec alignment (the spec is silent or contradictory on the behavior itself, or a real product-behavior change is required) do you proceed to STEP 5b. The one exception that still routes needs-human here is a resource genuinely absent locally (paid cloud account, gVisor/Kata nodes, a live external SaaS, an externally published image): do not fabricate it — record the blocker in the finding\'s Dependencies via humanQuestion and leave it unselected.',
    '',
