# change-proposal: design notes

Maintainer reference for `.claude/workflows/change-proposal.js` and `.claude/workflows/change-proposal-decisions.js`: why each stage is built the way it is, with the measurements that shaped it. The operator guide is `SKILL.md`, which is the only file loaded when the skill runs. The workflow source comments are canonical where the two differ.
...
**An answer lands at the lowest rung that holds.** The answer designer and the `resolve` and
`out-of-scope-wrong` appliers carry `LEAN_STAGING`, this phase's form of the fix stage's edit ladder:
already staged, replace in place, point, one sentence, and only then add. A `resolve` applier used to cite
the ground as file:line and name the authority beside the answer, which put a justification into the staged
files for every item the phase resolved. The authority now goes in the applier's review-log line, and the
staged text states the requirement.
...
**site expansion** runs first, one agent per confirmed finding (`opus` at low effort), in parallel. It starts from the sites the finding already names and answers one question about the rest of the repository: if this fix lands, which other text becomes wrong? The test is falsification rather than relatedness, so a site that discusses the same subject and stays true is not reported; consistent restatement is excluded by the review bar. Results land in `potentiallyRelatedSites` on the finding, never merged into `where` or `evidence`, because those two survived two verifiers and these did not. Proposal sites and tree sites are kept apart: the fixer edits the first and may not touch the second, where a falsified site means the proposal is missing an edit site. Inside the proposal it asks a second question as well: which text does the fix make unnecessary? Such a site carries `effect: obsolete`: it stays true, but exists only because of what the fix removes or replaces, such as the history or defence of a replaced mechanism. An obsolete tree site is dropped, because nothing in the loop deletes text outside the proposal.

**One rule, one home.** The writer, the designer, and the fixer all work to the same rule: a rule, predicate, or contract is stated once, in its normative home, which is the staged spec text when there is one, and every other site cites it by heading and number. A fix that changes a rule changes it in its home; a fix that finds a second full statement replaces it with a citation. The same holds for the reasoning behind a decision: it has one home, the decision's summary entry or the deliverable that owns it, and a justification, rejected alternative, or safety argument written out beyond one clause at a second site is a copy the single-source lens reports and the materiality skeptic confirms. The fixer was previously told to propagate a changed predicate to every section that states it, which maintained the copies: on one measured run a cascade stated at five sites absorbed 49 rounds, each fix re-synchronising four copies and drifting the fifth. Staged code and test text is the exception, since code cannot cite prose, and it references the rule by number in a comment.
...
**The edit ladder.** The designer, the fixer, and the follow-up fixer all carry `LEAN_EDIT`, after the Ponytail skill's ladder of laziness (github.com/DietrichGebert/ponytail). Measured on one proposal after sixteen rounds, about a quarter of its staged non-spec file could go without changing what gets built: reasoning restated at three to five sites, history of a design the tree never had, file:line evidence proving a choice safe, and paragraphs written so a reviewer would not raise a point again. Each was a fixer answering a finding by adding text where the reviewer was looking, and the fixer prompt told it to: it said to record the rationale in the proposal and to give every claim file:line evidence. The ladder makes addition the last resort. An agent takes the first rung that closes the finding: no staged change (the answer is a `DECISION` in the log), delete, replace in place, point at an existing statement, one sentence, and only then add. A reader test sends defences to the log shard, because the refuted-findings memory and the review log are the loop's defence against a returning finding, and a never-cut list protects fail-closed predicates, wire contracts, orderings, implementor traps, anti-vacuity notes, gate constraints, and deliberate spec divergences. The design records its `rung`, the fixer reports `netLines`, and the round logs what its fixes added and removed, which the introspection pass reads. A reviewer's `suggested_fix` is asked for the smallest edit too, because it seeds every trivial fix. The follow-up fixer reduces a drifted parallel to a citation before it considers editing it into agreement, and records its corrections in a log shard. After a redesign the applier sweeps for the replaced design's names and deletes the history they carry.
...
## The lens cache: why it is opt-in
...
## The base tier: why it is pinned
...
The workflows are canonical at `.claude/workflows/change-proposal.js` and `.claude/workflows/change-proposal-decisions.js`. The behavioural tests are `.claude/tests/change-proposal.test.mjs` and its siblings; run `node .claude/tests/run.mjs` after editing a workflow. A workflow script cannot touch a file: the sandbox gives it only `agent`, `parallel`, `pipeline`, `phase`, `log`, `workflow`, `budget`, and `args`, so anything that reads or writes a file goes through an agent. Put design reasoning in the source comments or in this file rather than in `SKILL.md`, which is loaded on every run and carries the operator procedure only. When a convergence run surfaces a confirmed error class no lens covers, add it to the lens that owns it, and keep the finding bar's exclusions intact: they are what stops the loop converging on nitpicks.
