...
- For each item it proposes a disposition (`resolve`, `human`, `implementor`, `out-of-scope-stands`, `out-of-scope-wrong`, or an impact row), and one falsifier per item gates it. A refuted disposition is set aside rather than replaced.
...
- An item the review loop later reverts is marked CONTESTED and routed to the human. It is never re-applied.
- It may edit both change files, the summary, the review log, and the problem statement's record. The checklist is out of bounds, and under `lockSpecChanges` so are the staged spec edits: a resolution that needs them is recorded for the operator with the edit it would have made, and you report it.
