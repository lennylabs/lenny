...
6. On `status: "spec-unappliable"`: an edit could not be applied, so the run stopped at it rather than skipping it. For an authored edit that means its anchor could not be located; for a mechanical edit it means its script exited non-zero, its applied diff did not match its dry run, or its command is not in the tree (which is also what a missing prerequisite proposal looks like). Report the `substep`, the unappliable edits with their reasons, and that sub-steps before it are already committed. The proposal must state the missing anchor, title, or value before a re-run; revert `spec/` to the last sub-step commit first.
