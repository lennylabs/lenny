-- §8.6 durable lease-extension grant counters. Migration
...
-- same row-lock transaction that re-checks the denial flag (the §8.6 in-flight atomic check: "the same Postgres transaction that
-- increments the tree budget counters").
--
-- These ext_* columns hold each §8.6 extendable dimension's
...
-- serialize, closing the §8.6 race where a user rejection is
...
-- last-write time is the database clock, consistent with the §8.6 UTC-only / database-clock rule the cool_off_expiry comparison
-- already follows.
--
