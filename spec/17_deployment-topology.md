...
| ----------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway                 | Deployment + Service + Ingress            | HPA, multi-zone, topology spread. **PodDisruptionBudget**: `maxUnavailable: 1` at **every tier** (hard ceiling of one concurrent voluntary eviction regardless of replica count). **Rolling update strategy**: `maxUnavailable: 1, maxSurge: 25%`. Rationale: the gateway preStop hook ([§10.1](10_gateway-internals.md#101-horizontal-scaling) preStop stage 2 and the CheckpointBarrier fan-out) fans out CheckpointBarrier to up to 400 pods per replica. **The PDB and the rolling-update strategy govern independent disruption axes** — the rolling-update strategy bounds Deployment-initiated rollouts (image bumps, spec changes), while the PDB bounds every other voluntary disruption (`kubectl drain`, cluster-autoscaler node consolidation, preemption). With the Kubernetes default `maxUnavailable: 25%` at Tier 3 (~20 replicas), up to 5 replicas drain concurrently during a rollout = up to 2,000 simultaneous MinIO checkpoint uploads; with a replica-proportional PDB (e.g., `minAvailable: ceil(replicas/2)`), up to 10 replicas may evict concurrently during a multi-node drain = up to 4,000 simultaneous uploads. Both cases exceed the [§10.1](10_gateway-internals.md#101-horizontal-scaling) per-replica 400-pod budget and the [§17.8.2](#1782-capacity-tier-reference) MinIO throughput budget. The flat `maxUnavailable: 1` on both the PDB and the rolling-update strategy caps the fan-out to one replica's 400-pod quota at a time across both axes, and — being a constant integer — side-steps two YAML-expressibility and tier-sizing pitfalls: (a) a formula like `ceil(replicas/2)` is not valid PDB syntax (PDB accepts only integers or percentages), and (b) `minAvailable: 2` at Tier 1 (`minReplicas: 2`) would leave zero disruption budget and block every node drain indefinitely. **Scale-down consequence.** Because HPA-driven evictions are voluntary disruptions, the same PDB also governs scale-down throughput: the effective scale-down rate is bounded at one pod per `pod_termination_seconds` (≈ 60–120s including the preStop tiered cap and connection drain), not by the HPA `scaleDown` policy. The HPA policy at every tier is therefore pinned to `1 pod / 60s` so Kubernetes does not attempt evictions the PDB cannot satisfy (see [§17.8.2](#1782-capacity-tier-reference) "Scale-down rate is PDB-bound"); the trade-off — slower trough recovery in exchange for bounded checkpoint fan-out — is an explicit availability-over-elasticity choice, observable via `lenny_pdb_blocked_evictions_total` ([§16.1](16_observability.md#161-metrics)). **Node-drain consequence.** The same PDB also serializes evictions during `kubectl drain` of any node hosting more than one gateway pod (worst-case single-node drain time = `pods_on_node × terminationGracePeriodSeconds`); operators draining Tier 3 nodes must size drain-tool timeouts accordingly and SHOULD apply a hostname-level `podAntiAffinity` rule to cap co-location at one pod per node — see [§17.8.2](#1782-capacity-tier-reference) "Tier 3 node-drain expectations". Single-container pod — the LLM Proxy subsystem and its native Go translator run inside the gateway binary ([§4.9](04_system-components.md#49-credential-leasing-service) Native translator). No sidecar, no separate container, no intra-pod loopback listener. |
...
| Agent Pods              | Pods owned by `Sandbox` CRD               | RuntimeClass per pool; preStop checkpoint hook for active pods; optional PDB per pool on warm (idle) pods holding a warm floor of `minWarm - 1` during voluntary disruption (no PDB below `minWarm` 2) |
...
9. **`lenny-pool-config-validator`** `ValidatingAdmissionWebhook` — applies the semantic budget rules ([§10.1](10_gateway-internals.md#101-horizontal-scaling) agent-pod grace floor and BarrierAck floor) to every `SandboxTemplate.spec`/`SandboxWarmPool.spec` write and additionally applies the `userInfo`-based authorization-denial rule to manual writes (see [Section 4.6.3](04_system-components.md#463-crd-field-ownership-and-write-boundaries)).
...
11. **`lenny-drain-readiness`** `ValidatingAdmissionWebhook` — pre-drain MinIO health check before pod eviction (see [Section 12.5](12_storage-architecture.md#125-artifact-store) and NET-037 in [Section 13.2](13_security-model.md#132-network-isolation)).
...
13. **`lenny-ephemeral-container-cred-guard`** `ValidatingAdmissionWebhook` — scoped to the `pods/ephemeralcontainers` subresource in every agent namespace (matched via `.Values.agentNamespaces`). Rejects any ephemeral-container request on any of **four** conditions: (i) the ephemeral container's `securityContext.runAsUser` equals the target pod's adapter UID or agent UID, (ii) `securityContext.runAsGroup` or `securityContext.supplementalGroups` includes the `lenny-cred-readers` GID, (iii) any of `runAsUser`, `runAsGroup`, or `supplementalGroups` is absent on the ephemeral container's `securityContext` (absent values would inherit pod-level defaults, including the fsGroup that grants `/run/lenny/slots/{sessionId}/credentials.json` read access), or (iv) the ephemeral container's `volumeMounts` reference the pod-level credential tmpfs volume by name, or any entry has a `mountPath` equal to `/run/lenny` or beginning with the prefix `/run/lenny/`. Condition (iv) closes the fsGroup-inheritance side-channel that conditions (i)–(iii) cannot close on their own (see [Section 13.1](13_security-model.md#131-pod-security) for the full four-condition rationale). Rejections emit `EPHEMERAL_CONTAINER_CRED_UID_FORBIDDEN` ([Section 15.1](15_external-api-surface.md#151-rest-api)). Closes the ephemeral-container credential-boundary bypass noted in [Section 13.1](13_security-model.md#131-pod-security).

**High-availability requirement applies to every entry above.** The `replicas: 2` + `podDisruptionBudget.minAvailable: 1` HA requirement stated in the "Admission webhook failure mode" paragraph below applies uniformly to **every** webhook and CRD conversion webhook enumerated in this list — not only to the RuntimeClass-aware admission policy webhook. The Helm chart configures `replicas: 2` and `podDisruptionBudget.minAvailable: 1` on the Deployment backing each webhook Service (`lenny-label-immutability`, `lenny-direct-mode-isolation`, `lenny-sandboxclaim-guard`, `lenny-data-residency-validator`, `lenny-pool-config-validator`, `lenny-t4-node-isolation`, `lenny-drain-readiness`, `lenny-crd-conversion`, `lenny-ephemeral-container-cred-guard`). Every one of these Deployments carries the `lenny.dev/component: admission-webhook` pod label so that the single `podSelector` in the `lenny-system` admission-webhook NetworkPolicy row ([§13.2](13_security-model.md#132-network-isolation)) matches every webhook pod; the canonical-selector requirement in §13.2 (NET-047/NET-050) forbids per-webhook label variants on that canonical key, and the `lenny-preflight` Job fails the install if any of the enumerated Deployments' pods are missing the `lenny.dev/component: admission-webhook` label. **Additive per-webhook labels for egress-narrowing (NET-068).** In addition to the canonical label, the `lenny-drain-readiness` Deployment carries an additive per-webhook pod label `lenny.dev/webhook-name: drain-readiness` — applied only to that Deployment's pods — so that the §13.2 admission-webhook NetworkPolicy row can render a narrower sub-rule that grants Gateway internal-port egress (TCP `{{ .Values.gateway.internalPort }}`, default 8080) exclusively to the drain-readiness pods. Without this additive label, the shared canonical selector would grant that egress to every admission-webhook Deployment, violating least privilege (the `lenny-drain-readiness` and `lenny-sandboxtemplate-deletion-guard` Deployments carry a documented per-webhook egress label and a corresponding §13.2 gateway-egress sub-rule; the remaining webhooks are purely in-process validators with no legitimate need to reach the gateway's internal admin port). The additive label is permitted by Exception 2 of the NET-047/NET-050 canonical-selector invariant ([§13.2](13_security-model.md#132-network-isolation)) and MAY appear only in egress allow-lists paired with the canonical `lenny.dev/component: admission-webhook` key; it MUST NOT appear on the ingress side of any NetworkPolicy rule. Chart authors MUST NOT add per-webhook labels to any webhook other than `lenny-drain-readiness` and `lenny-sandboxtemplate-deletion-guard` (the two that now carry the additive label) unless a future egress-narrowing need is documented in §13.2 alongside a corresponding sub-rule; the `lenny-preflight` audit treats unreferenced additive labels as lint-level warnings. The `lenny-sandboxtemplate-deletion-guard` Deployment (the §10.5 runtime-upgrade safety webhook, deployed with the same `replicas: 2` and `podDisruptionBudget.minAvailable: 1` HA guarantee as the webhook Deployments above) carries the additive per-webhook label `lenny.dev/webhook-name: sandboxtemplate-deletion-guard`, applied only to that Deployment's pods, so that the §13.2 admission-webhook NetworkPolicy row can render a narrower sub-rule granting Gateway internal-port egress (TCP `{{ .Values.gateway.internalPort }}`, default 8080) exclusively to the deletion-guard pods for its `GET /internal/runtime-upgrade/active` runtime-upgrade probe. Without this additive label, the shared canonical selector would grant that egress to every admission-webhook Deployment, violating least privilege. Like `lenny-pod-security` and `lenny-tenant-label-immutability`, the deletion-guard is a baseline fail-closed webhook that is not enumerated in the numbered admission-policies catalogue above and has no dedicated per-webhook §16.5 alert; it is not covered by the `AdmissionWebhookUnavailable` alert, whose `up{job="lenny-admission-policy-webhook"}` expression is scoped to the RuntimeClass-aware policy webhook. The additive label is permitted by Exception 2 of the NET-047/NET-050 canonical-selector invariant ([§13.2](13_security-model.md#132-network-isolation)) and MAY appear only in egress allow-lists paired with the canonical `lenny.dev/component: admission-webhook` key. All fail-closed webhooks (`failurePolicy: Fail`) — which includes every webhook in the list above — must maintain the 99.9% availability SLO; an availability drop below this threshold triggers the per-webhook unavailability alerts enumerated in [Section 16.5](16_observability.md#165-alerting-rules-and-slos) (`SandboxClaimGuardUnavailable`, `DataResidencyWebhookUnavailable`, `PoolConfigValidatorUnavailable`, `LabelImmutabilityWebhookUnavailable`, `DirectModeIsolationWebhookUnavailable`, `T4NodeIsolationWebhookUnavailable`, `DrainReadinessWebhookUnavailable`, `CrdConversionWebhookUnavailable`, `EphemeralContainerCredGuardUnavailable`, and the common `AdmissionWebhookUnavailable`). Each of the nine webhook Deployments and the `lenny-crd-conversion` CRD conversion webhook has a 1:1 alert in this list, ensuring operators receive paging/warning signals on unavailability of any admission plane component.

**Preflight enumeration check (`lenny-preflight`).** The `lenny-preflight` Job ([§17.9](#179-deployment-answer-files) — `Checks performed`) enumerates the deployed `ValidatingWebhookConfiguration` and `CustomResourceDefinition.spec.conversion.webhook` resources in the target cluster and verifies that the expected set is present before allowing the install or upgrade to proceed. The check is **fail-closed**: any missing webhook causes the Job to fail with `"expected ValidatingWebhookConfiguration '<name>' not found; chart-rendered webhook is missing — re-render with the current chart or run 'helm template' and diff against the expected set"`. This prevents a Helm chart author from shipping an incomplete chart that silently omits one of the fail-closed webhooks whose absence would go undetected at runtime.

**Feature-gated chart inventory (single source of truth).** The expected set is **not** a hard-coded list of all nine webhooks; it is derived from the same Helm feature flags that gate the rendering of the webhook templates under `templates/admission-policies/`, so that "expected" and "rendered" are always in lockstep throughout the [§18](18_build-sequence.md) build sequence. The chart defines three feature-flag Helm values (with the defaults shown):

| Helm value                    | Default | Gates (`templates/admission-policies/`)                                 | First-deploy phase |
| ----------------------------- | ------- | ----------------------------------------------------------------------- | ------------------ |
| `features.llmProxy`           | `false` | `lenny-direct-mode-isolation` (item 6)                                  | Phase 5.8          |
| `features.drainReadiness`     | `false` | `lenny-drain-readiness` (item 11)                                       | Phase 8            |
| `features.compliance`         | `false` | `lenny-data-residency-validator` (item 8), `lenny-t4-node-isolation` (item 10) | Phase 13   |

The five remaining webhooks — `lenny-label-immutability` (item 5), `lenny-sandboxclaim-guard` (item 7), `lenny-pool-config-validator` (item 9), the `lenny-crd-conversion` conversion webhook (item 12), and `lenny-ephemeral-container-cred-guard` (item 13) — are unconditionally rendered and always expected; they are the Phase 3.5 baseline that every subsequent phase builds on. The `lenny-preflight` Job reads the same three feature-flag Helm values (injected into the Job's ConfigMap at template time) and composes its expected set by union: baseline five ∪ `{lenny-direct-mode-isolation}` if `features.llmProxy` ∪ `{lenny-drain-readiness}` if `features.drainReadiness` ∪ `{lenny-data-residency-validator, lenny-t4-node-isolation}` if `features.compliance`. A flag that is `false` causes the corresponding webhook template to be **omitted** from the render AND causes the preflight expected set to **exclude** that webhook — so a Phase 3.5 install (all three flags `false`) expects exactly the five baseline entries (four validating webhooks plus the conversion webhook) and passes, while a Phase 13 install (all three flags `true`) expects all nine entries. Flipping a flag from `true` to `false` after a phase has been reached is an invalid downgrade (the [§16.5](16_observability.md#165-alerting-rules-and-slos) per-webhook unavailability alerts and associated runtime enforcement paths depend on the webhook's continued presence), and the downgrade prohibition is **actively enforced at four layers** — persisted phase-stamp ConfigMap, fail-closed chart render-time validation, `lenny-preflight` phase-stamp consistency check (`PREFLIGHT_PHASE_STAMP_MISMATCH`, the primary enforcement for GitOps deployers), and a runtime `AdmissionPlaneFeatureFlagDowngrade` Warning alert — per the "Feature-flag downgrade enforcement" sub-section below. This closes the narrative-without-enforcement gap where a `helm upgrade` with a reverted `values.yaml` would silently vanish a fail-closed admission webhook, producing an invisible admission-plane regression whose runtime `*Unavailable` alerts would also fall silent (because the `PrometheusRule` template is gated on the same flag that removed the webhook Deployment).

**High-availability contract applies to every rendered webhook.** The uniform HA contract stated above (`replicas: 2`, `podDisruptionBudget.minAvailable: 1`, `failurePolicy: Fail`, canonical `lenny.dev/component: admission-webhook` pod label) applies to every webhook that is rendered in the current phase — never more, never less. Per-webhook unavailability alerts fire only for webhooks whose feature flag is `true` (chart templating gates both the `ValidatingWebhookConfiguration` and the corresponding `PrometheusRule` on the same flag), so a pre-Phase-13 install does not produce spurious paging for webhooks that have not yet been deployed.

**Feature-flag downgrade enforcement (phase-stamp ConfigMap + render-time validation + preflight mismatch check + runtime alert).** The narrative prohibition above ("flipping a flag from `true` to `false` … is an invalid downgrade") is actively enforced by four cooperating layers; an operator attempting a downgrade hits the render-time fail-closed gate before the upgrade reaches the cluster, and any silent drift that bypasses the chart (e.g., out-of-band `kubectl delete` of a `ValidatingWebhookConfiguration`) is caught by the runtime alert. The four layers are:

1. **Persisted phase-stamp ConfigMap (`lenny-deployment-phase-stamp`).** The chart renders a namespace-scoped ConfigMap in `{Release.Namespace}` named `lenny-deployment-phase-stamp` with the following schema: one data key per feature flag (`features.llmProxy`, `features.drainReadiness`, `features.compliance`, plus any future flag added to the [Feature-gated chart inventory table above](#172-namespace-layout)); each value is a two-field object `{ "enabled": true, "enabledAt": "<RFC3339 timestamp>" }` serialized as JSON. **Write semantics are strictly append-only**: a chart render writes a new key only when the flag transitions `false → true`, and re-renders preserve the original `enabledAt` timestamp (`lookup` reads the existing value and passes it through); the chart MUST NOT overwrite or remove a key whose `enabled` is already `true`. Ownership is the Helm chart (labeled `helm.sh/release-name` + `helm.sh/release-namespace`), not a controller — no component mutates the ConfigMap at runtime. The ConfigMap is rendered at `helm.sh/hook-weight: "-20"` (earlier than `lenny-preflight` at `-10`) so it exists before the preflight Job runs. RBAC on the ConfigMap is read-only for `lenny-preflight-sa` and `lenny-ops-sa`; no `update` / `patch` / `delete` verbs are granted to any Lenny component (operators with cluster-admin can still `kubectl delete configmap lenny-deployment-phase-stamp` to perform the explicit, audited reset described below).

2. **Fail-closed chart render-time validation (`helm install` / `helm upgrade` only).** On every `helm install` and `helm upgrade`, a pre-render template function reads the existing phase-stamp ConfigMap via Helm's `lookup "v1" "ConfigMap" .Release.Namespace "lenny-deployment-phase-stamp"` primitive and fails the render with the error code `PHASE_STAMP_FEATURE_FLAG_DOWNGRADE` if any feature flag that is recorded in the phase-stamp with `enabled: true` is being rendered as `false` in the incoming values. **Scope limitation:** per Helm's documented semantics, `lookup` returns an empty map under `helm template` (including `helm template --dry-run`) because those commands do not connect to a live cluster; the render-time guard is therefore effective only during `helm install` and `helm upgrade`. **GitOps deployers** (ArgoCD with Helm-based rendering, Flux Helm Controller using `helm template`, or any pipeline that uses `helm template` to produce manifests for Server-Side Apply) MUST rely on the `lenny-preflight` Job's `Phase-stamp consistency` check (§17.9 row `PREFLIGHT_PHASE_STAMP_MISMATCH`) as the fail-closed pre-Sync gate — this check runs against the live cluster and is the effective enforcement layer when the render path is `helm template`. Operators running GitOps SHOULD invoke `lenny-ctl preflight --config <values.yaml>` (the CLI equivalent of the preflight Job, see §17.9 "CLI equivalent") as a pre-Sync hook in ArgoCD/Flux so the downgrade is rejected before the Sync operation begins. The fail-closed message format is: `"PHASE_STAMP_FEATURE_FLAG_DOWNGRADE: feature flag '<flag>' was enabled at <enabledAt> per lenny-deployment-phase-stamp but is being rendered as false; admission-plane feature-flag downgrade is prohibited. To proceed with an explicit, audited downgrade, set --set acceptFeatureFlagDowngrade.<flag>=true, which emits `deployment.feature_flag_downgrade_acknowledged` audit event (catalogued in [§16.7](16_observability.md#167-section-25-audit-events)) and retains the phase-stamp entry — the accompanying webhook will stop rendering but the phase-stamp will still flag the posture as degraded for the runtime alert."`. The `acceptFeatureFlagDowngrade.<flag>=true` override is the sole authorized path and requires an operator to explicitly acknowledge the degraded admission-plane posture; the override does NOT clear the phase-stamp entry (so the runtime alert continues to fire until the operator either re-flips the flag to `true` or issues `kubectl delete configmap lenny-deployment-phase-stamp` and re-installs the chart, matching the `POST /v1/admin/tenants/{id}/compliance-profile/decommission` pattern in [§11.7](11_policy-and-controls.md#117-audit-logging)). The `lookup` returns an empty result on the initial `helm install` (before the ConfigMap exists), in which case the render proceeds and the ConfigMap is rendered for the first time with the install's initial `true`-flagged set. An integration test (`tests/integration/phase_stamp_downgrade_test.go`) parameterises every `(from, to)` flag combination and asserts that (a) `true → false` without the override fails the render **under `helm install` / `helm upgrade`**, (b) `true → false` with the override succeeds and emits the audit event, (c) `false → true` re-renders correctly with a preserved or newly-written `enabledAt`, and (d) `true → true` preserves the original `enabledAt`. A companion test asserts that under `helm template` the `lookup` returns empty and the render-time guard does NOT fail, verifying that the preflight Job's `PREFLIGHT_PHASE_STAMP_MISMATCH` check is the sole enforcement in the GitOps code path.

3. **`lenny-preflight` mismatch check (primary enforcement for GitOps; also catches out-of-band mutation between phase-stamp and Helm render).** The `lenny-preflight` Job gains a new `Phase-stamp consistency` check that compares the feature-flag values in the Job's ConfigMap (injected from `.Values.features.*` at template time) against the current `lenny-deployment-phase-stamp` ConfigMap: for every flag with `enabled: true` recorded in the phase-stamp, the preflight Job's flag value MUST also be `true`, unless the operator has set `acceptFeatureFlagDowngrade.<flag>: true` (same value as used by the chart render-time override above). On mismatch, the Job fails with `PREFLIGHT_PHASE_STAMP_MISMATCH: flag '<flag>' is recorded as enabled in lenny-deployment-phase-stamp but the incoming values render it as disabled without acceptFeatureFlagDowngrade.<flag>=true`. This check is the **sole fail-closed enforcement** in the GitOps deployment path (where `helm template` is the rendering primitive and the layer-2 render-time `lookup` guard is a no-op per its scope limitation above); in the `helm install` / `helm upgrade` code path it additionally catches (a) the narrow race window where the phase-stamp ConfigMap was deleted between render and apply, and (b) out-of-band mutations (e.g., `kubectl delete configmap lenny-deployment-phase-stamp`) that occurred after render but before the Job executed. The Job runs at `helm.sh/hook-weight: "-10"` which places it after the phase-stamp ConfigMap (`-20`) but before the webhook Deployments, so a mismatch aborts the install before any webhook-gated resource is applied.

4. **`AdmissionPlaneFeatureFlagDowngrade` Warning alert ([§16.5](16_observability.md#165-alerting-rules-and-slos)).** Catches runtime drift where the phase-stamp ConfigMap records `enabled: true` for a flag but the corresponding `ValidatingWebhookConfiguration` (from the Feature-gated chart inventory above) is absent from the cluster. **The canonical PromQL expression, its per-`(flag, webhook)`-pair rule decomposition, the labelling mechanism via chart-rendered `kube_configmap_labels` metric labels on the phase-stamp ConfigMap, and the mandatory `kube-state-metrics --metric-labels-allowlist=configmaps=[lenny.dev/flag-*]` operator configuration are all defined at [§16.5](16_observability.md#165-alerting-rules-and-slos) `AdmissionPlaneFeatureFlagDowngrade`, which is the single source of truth for this alert — this layer describes only the enforcement posture the alert serves, not its expression body.** Fires with severity Warning (not Critical) because the flag-flip is typically an operator mistake, not an ongoing attack; the `*Unavailable` per-webhook alerts would NOT fire (their gating `PrometheusRule` also vanishes when the flag flips to `false`), so this alert is the sole signal for the class of drift described in the iter7 KIN-028 finding. Cross-references the `docs/runbooks/admission-plane-feature-flag-downgrade.md` runbook ([§17.7](#177-operational-runbooks)) which enumerates the remediation sequence: (a) determine whether the flag-flip was intentional (audit log — `deployment.feature_flag_downgrade_acknowledged` event), (b) if unintentional, re-flip the flag to `true` and `helm upgrade` (the phase-stamp still records `enabled: true`, so the upgrade is a no-op on the ConfigMap but re-renders the webhook Deployment + `ValidatingWebhookConfiguration` + gated `PrometheusRule`), (c) if intentional and the downgrade is permanent, issue `kubectl delete configmap lenny-deployment-phase-stamp -n {{ .Release.Namespace }}` to reset the posture and re-run `helm upgrade` (the chart will render a fresh phase-stamp reflecting the new current state).

**Interaction with the baseline (always-rendered) webhooks.** The five baseline webhooks (`lenny-label-immutability`, `lenny-sandboxclaim-guard`, `lenny-pool-config-validator`, `lenny-crd-conversion`, `lenny-ephemeral-container-cred-guard`) have no feature-flag gate and therefore have no phase-stamp entry — they are always expected from Phase 3.5 onward, and their absence is caught by the existing `lenny-preflight` admission-webhook inventory check ([§17.9](#179-deployment-answer-files) `Checks performed`) rather than by the phase-stamp mechanism. The SEC-017 addition of `lenny-ephemeral-container-cred-guard` (item 13) is part of this baseline set; its phase-stamp is effectively the Phase 3.5 baseline and is not recorded separately.

An **integration test suite** (`tests/integration/admission_policy_test.go`) verifies that controller-generated pod specs for each RuntimeClass pass the deployed admission policies, preventing policy/spec drift from causing warm pool deadlock. A companion suite (`tests/integration/admission_webhook_inventory_test.go`) is **parameterised by the same three `features.*` flags**: the test table runs four cases — (1) all flags `false` (Phase 3.5 baseline: five entries expected), (2) `features.llmProxy=true` only (Phase 5.8: six entries), (3) `features.llmProxy=true, features.drainReadiness=true` (Phase 8: seven entries), (4) all flags `true` (Phase 13 and beyond: nine entries) — and asserts that the rendered webhook set and the preflight-computed expected set match in each configuration. This prevents regression in either direction: a chart template gated on the wrong flag and a preflight expected-set computation that disagrees with the chart are both caught in CI. A parallel suite (`tests/integration/core_deployment_inventory_test.go`) covers the unconditional core-Deployment inventory — the `lenny-ops` Deployment and its supporting resources from [§17.8.5](#1785-mandatory-lenny-ops-deployment) (headless `lenny-gateway-pods` Service, `lenny-ops-leader` Lease, `lenny-backup-sa` ServiceAccount, the four NetworkPolicies, and the `lenny-ops` PodDisruptionBudget) — and fail-closes on absence so a chart-author omission of a mandatory core deployment cannot ship silently the way iter3/iter4 caught for gated webhooks; this suite requires no feature-flag parameterisation because `lenny-ops` is present in every phase from 3.5 onward.

**Elicitation content integrity platform floor (phase-stamp ConfigMap co-tenant).** The `lenny-deployment-phase-stamp` ConfigMap (whose feature-flag schema is defined above) carries one additional data key — `security.elicitationContentIntegrity.floor` — that records the deployment-scope minimum-enforcement floor for the per-tenant elicitation content integrity enforcement mode ([§9.2](09_mcp-integration.md#92-elicitation-chain)). The value is a scalar string `"enforce" | "detect-only" | "off"` (default `"off"`) rendered from `.Values.security.elicitationContentIntegrity.floor` at `helm install`/`helm upgrade` time. Unlike the feature-flag keys on the same ConfigMap — whose write semantics are strictly append-only and which are phase-advance signals — the floor key is rewritten on every render (the floor may legitimately be raised or lowered by operator intent, and each transition is separately audited on the `platform.elicitation_content_integrity_floor_changed` audit event catalogued in [§16.7](16_observability.md#167-section-25-audit-events)). The gateway reads this key at startup and on ConfigMap change events and applies it as the lower bound of every tenant's effective enforcement mode: `effective_mode = max(floor, tenant_stored_mode)` under ordering `off < detect-only < enforce`. No chart render-time downgrade guard is applied to this key (lowering the floor is a legitimate, operator-audited posture change — not an invalid phase downgrade); the `ElicitationContentIntegrityWeakened` warning alert ([§16.5](16_observability.md#165-alerting-rules-and-slos)) fires continuously whenever the effective mode is weaker than `enforce` for any tenant, which is the runtime steady-state signal that the deployment is operating below the maximum-integrity posture. RBAC on the floor key mirrors the feature-flag keys on the same ConfigMap (read-only for `lenny-preflight-sa` and `lenny-ops-sa`; no `update`/`patch`/`delete` grants to any Lenny component; only `helm upgrade` or direct operator `kubectl` action changes the value). A floor change that raises the effective mode for one or more tenants also emits one `tenant.elicitation_content_integrity_floor_clamp` audit event per affected tenant ([§16.7](16_observability.md#167-section-25-audit-events)) for full audit traceability of the fanout.
...
- **ResourceQuota** (configurable via `.Values.agentNamespaces[].resourceQuota`): caps total pods, aggregate CPU requests, aggregate memory requests, and aggregate ephemeral storage per namespace. Defaults are tier-scaled — see the "ResourceQuota tier defaults" table in [Section 17.8.2](#1782-capacity-tier-reference) for the per-tier values and the sizing formula. The **Tier 1** defaults for the `lenny-agents` namespace are `pods: 200`, `requests.cpu: "400"`, `requests.memory: "800Gi"`, `requests.ephemeral-storage: "2Ti"`; Tier 2 and Tier 3 defaults are larger by the factors published in the tier table (Tier 3 ships `pods: 15000` because a single Tier 3 hot pool already requires `minWarm ≈ 1,260` and up to ~3,400 with the delegation burst term, plus up to 10,000 active session pods — see [§17.8.2](#1782-capacity-tier-reference) "Warm pool sizing" and "Delegation fan-out sizing"). The sizing floor for any namespace is: `pods >= (sum of all pool maxWarm for pools targeting the namespace) + (tier session-concurrency limit × 1.2) + (tier concurrent delegation fan-out cap)`; CPU/memory/ephemeral-storage scale from the same headcount times the per-pod request envelope from [Section 17.8.1](#1781-operational-defaults--quick-reference). Operators selecting a capacity tier **must** set `.Values.capacityPlanning.tier` to the matching value — the chart renders the tier-specific ResourceQuota defaults from that input rather than silently falling through the Tier 1 example. If the quota is lower than the pool's target size, the warm pool controller will be unable to create pods; the `lenny_warmpool_warmup_failure_total{error_type="resource_quota_exceeded"}` counter increments and the `PoolScaleoutBlockedByQuota` alert ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)) fires. ResourceQuota changes in Kubernetes take effect at the next admission decision but do not evict existing pods, so a post-hoc quota raise unblocks further scale-up without disturbing already-running pods.
...
- Postgres: continuous WAL archival + daily base backups to object storage (see [§25.11](25_agent-operability.md#2511-backup-and-restore-api) for the backup pipeline, Helm values, and agent-operable API).
- MinIO ArtifactStore: **continuous asynchronous bucket replication** to an off-cluster destination (see [§25.11](25_agent-operability.md#2511-backup-and-restore-api) "ArtifactStore Backup"). RPO is expressed as replication lag (15m at Tier 2/3, monitored via `lenny_minio_replication_lag_seconds`); RTO matches the Postgres RTO because target-promotion is a Helm-values change. Daily-snapshot-as-backup is **not** sufficient for v1 because it leaves the RPO misaligned with Postgres and exposes compliance-relevant object retrieval (legal hold, erasure receipts) to a full day of data loss.
- Restore testing recommended monthly (configurable by deployer via CronJob schedule) via `lenny-restore-test` CronJob that: (1) creates a temporary Postgres instance from the latest base backup + WAL, (2) verifies schema integrity and row counts against the primary, (3) runs a smoke query (e.g., list recent sessions), (4) exercises the ArtifactStore replication target via sampled HEAD reads ([§25.11](25_agent-operability.md#2511-backup-and-restore-api) "Test restore"), (5) records elapsed restore time and emits `lenny_restore_test_success` / `lenny_restore_test_duration_seconds` metrics, and (6) tears down the test instance. Alert if measured RTO exceeds targets (< 30s Postgres, < 5min MinIO) or if the ArtifactStore sampled-HEAD success rate drops below 99%. MinIO restore is validated in the same job via a test bucket restore and object checksum comparison.
...
lenny image import my-agent:dev     # Loads the image into the embedded k3s containerd store
                                    # See §24.19.1 for full reference (Embedded Mode only)
...
- `playground.devTenantId` — tenant bound to the dev HMAC JWT `tenant_id` claim when `playground.authMode=dev` (see [§27.2](27_web-playground.md#272-placement-and-gating) / [§27.3](27_web-playground.md#273-authentication)). **Helm `values.schema.json` constraints:** `type: string`, `pattern: ^[a-zA-Z0-9_-]{1,128}$`, `maxLength: 128`, `default: "default"`. Optional at the schema level; required when `playground.authMode == dev` — this cross-field conditional cannot be expressed in JSON Schema alone and is therefore enforced by the preflight row below and, as a startup backstop, by `LENNY_PLAYGROUND_DEV_TENANT_REQUIRED` (see [§27.3](27_web-playground.md#273-authentication)). The format pattern matches the canonical `tenant_id` regex from [§10.2](10_gateway-internals.md#102-authentication) (`TENANT_CLAIM_INVALID_FORMAT`), so a typo such as a trailing whitespace or a `.` character is rejected at `helm install`/`helm upgrade --dry-run` time rather than surfacing as a gateway `CrashLoopBackOff` with `LENNY_PLAYGROUND_DEV_TENANT_INVALID` at pod startup.
...
2. **Init Job: `lenny-bootstrap`.** The Helm chart includes a Kubernetes `Job` (with `helm.sh/hook: post-install,post-upgrade` and `helm.sh/hook-weight: "10"`) that runs after the gateway and database migrations are ready. The Job executes `lenny-ctl bootstrap --from-values /etc/lenny/bootstrap-values.yaml` against the admin API. The bootstrap values ConfigMap is rendered from the `bootstrap` Helm section.
   ...
   **Ordering with `playground.devTenantId`.** Because the Job is `post-install`, it runs *after* the gateway Deployment becomes Ready. This is compatible with the `playground.authMode=dev` tenant-id gate by design: per [§10.3](10_gateway-internals.md#103-mtls-pki) (startup-configuration validation table) and [§27.3](27_web-playground.md#273-authentication), the gateway's `playground.devTenantId` tenant-existence check is **Ready-gated at the `/playground/*` route**, not startup-gated. On a fresh `helm install` with `playground.enabled=true` + `playground.authMode=dev`, the gateway pod starts and reports `GET /healthz` Ready even though Postgres holds no tenant rows yet; the bootstrap Job's readiness poll (§17.6) therefore completes and the Job proceeds to seed the default tenant. Until the tenant row commits, `/playground/*` requests return `503 LENNY_PLAYGROUND_DEV_TENANT_NOT_SEEDED` — this is the transient bootstrap-ordering signal, not a misconfiguration. After the Job commits the tenant row the next playground request succeeds without any gateway restart or rollout. This avoids the deadlock that would arise if tenant-existence were a startup gate (gateway refuses to start → post-install hook never runs → tenant is never seeded).
...
6. **Initial admin credential — Kubernetes Secret handling.** The bootstrap Job creates an initial `platform-admin` user (username: `lenny-admin`) with a generated API token. The token is written to a Kubernetes Secret `lenny-system/lenny-admin-token` with the following properties:
   ...
   - **Secret shape:**
     ...
     ```
   - **Rotation procedure:** To rotate the initial admin token: (1) the operator runs `lenny-ctl admin users rotate-token --user lenny-admin`, which calls the dedicated `POST /v1/admin/users/{user}/rotate-token` route on the gateway; (2) the gateway mints the new token in process, records it, patches the `lenny-admin-token` Secret with the new token value, and then durably revokes the prior token — the CLI does not mint the token or patch the Secret; (3) the old token is immediately invalidated (not a grace period). Any systems using the old token must be updated to read from the Secret.
...
| OTLP TLS handshake (`otlp-tls`) (OTLP-068) | When `observability.otlpEndpoint` is set AND `observability.otlpTlsEnabled: true`: open a TCP connection to the configured endpoint, perform a TLS 1.2+ handshake using the cluster trust bundle plus any deployer-supplied CA (`observability.otlpCaBundle`), and verify the server certificate's SAN covers the endpoint hostname. Fail-closed if the handshake cannot complete or the SAN does not match. Additionally fail if `observability.otlpEndpoint` begins with `http://` while TLS is enabled. Skipped in dev mode. When `observability.otlpTlsEnabled: false`, additionally verify that the `observability.acknowledgeOtlpPlaintext: true` guard is present outside dev mode (mirrors the chart-level `required` guard; fails with the same message if unset). | `OTLP TLS handshake to '<endpoint>' failed: <error>. Trace payloads expose tenant/session metadata; TLS is required (Section 13.2, OTLP-068). Re-point `observability.otlpEndpoint` at a TLS-terminating collector, or set `observability.otlpTlsEnabled: false` with `acknowledgeOtlpPlaintext: true` in dev only.` |
| Ops admin-API TLS handshake (`ops-admin-tls`) (NET-070) | When `ops.tls.internalEnabled: true` (default in non-dev profiles): open a TCP connection to the gateway's internal TLS port (`{{ .Values.gateway.internalTLSPort }}`, default 8443) via the `lenny-gateway` ClusterIP Service, perform a TLS 1.2+ handshake using the cluster trust bundle plus any deployer-supplied CA (`ops.tls.caBundleConfigMap`), and verify the server certificate's SAN covers the ClusterIP hostname. Fail-closed if the handshake cannot complete or the SAN does not match. Additionally fail if `ops.gateway.url` begins with `http://` while `internalEnabled: true`. Skipped in dev mode. When `ops.tls.internalEnabled: false`, additionally verify that the `ops.acknowledgePlaintextAdminAPI: true` guard is present outside dev mode (mirrors the chart-level `required` guard; fails with the same message if unset) — mirrors the `otlp-tls` plaintext-acknowledgment pattern. Symmetric with `otlp-tls` above (NET-070 parity with OTLP-068). | `lenny-ops admin-API TLS handshake to 'lenny-gateway:<port>' failed: <error>. The admin-API link carries platform-admin JWTs and admin payloads; TLS is required (Section 25.4 "TLS", NET-070). Re-verify `ops.tls.certSecretName` points at a cert-manager-issued certificate with a SAN covering 'lenny-gateway', or set `ops.tls.internalEnabled: false` with `ops.acknowledgePlaintextAdminAPI: true` in dev only.` |
...
| Legal-hold escrow per-region coverage | When `features.compliance: true`: for each region declared in `storage.regions`, verify that `storage.regions.<region>.legalHoldEscrow.{endpoint, bucket, kmsKeyId, escrowKekId}` is fully populated (no empty scalars) and that the escrow bucket endpoint is reachable and the escrow KMS key is resolvable. When `storage.regions` is empty (single-region deployment), verify `storage.legalHoldEscrowDefault.{endpoint, bucket, kmsKeyId}` is fully populated instead. Fail-closed: any missing field or unreachable endpoint aborts the install/upgrade, matching the `LEGAL_HOLD_ESCROW_REGION_UNRESOLVABLE` runtime failure mode (CMP-054). | `CONFIG_INVALID: storage.regions.<region>.legalHoldEscrow incomplete — field '<field>' is empty or endpoint/KMS unreachable. Required for Phase 3.5 of tenant force-delete lifecycle (Section 12.8). Fail-closed mirror of backup/artifact-replication per-region coverage.` / `CONFIG_INVALID: storage.legalHoldEscrowDefault incomplete in single-region deployment — configure the escrow endpoint, bucket, and KMS key before install.` |
...
| Gateway/lenny-ops private-range parity | Parse the rendered `allow-gateway-egress-llm-upstream` NetworkPolicy (§13.2) and the `lenny-ops-egress` NetworkPolicy (§25.4). Assert that every entry in `egressCIDRs.excludePrivate` appears (as an exact string) in both rules' `except` block on the **same-family** `ipBlock` peer (IPv4 entries on the `0.0.0.0/0` peer, IPv6 entries on the `::/0` peer — per NET-062, each rule emits two parallel `ipBlock` peers because K8s NetworkPolicy validation forbids cross-family `except` entries). The two rules are NOT required to be set-equal (gateway legitimately adds cluster pod/service CIDR and IMDS exclusions that do not apply to `lenny-ops-egress`); the check is limited to `excludePrivate` membership on both sides. Guards against future edits that weaken only one of the two SSRF surfaces documented in NET-057. | `SSRF private-range `except` list drift detected: `<cidr>` from `egressCIDRs.excludePrivate` is missing from `<rule_name>`'s `except` block on the `<family>` ipBlock peer. Both `allow-gateway-egress-llm-upstream` (§13.2) and `lenny-ops-egress` (§25.4) MUST render every entry in `egressCIDRs.excludePrivate` into the same-family peer (NET-057, NET-062). Re-run helm install/upgrade after reconciling the values.` |
| NetworkPolicy ipBlock family parity | Enumerate every `ipBlock` in every Lenny-rendered NetworkPolicy and assert that each entry in `except` shares the address family of the enclosing `cidr` (IPv4 entries only on `0.0.0.0/0`-rooted blocks, IPv6 entries only on `::/0`-rooted blocks). Kubernetes `NetworkPolicySpec.egress[].to[].ipBlock` requires per-block family uniformity; strict CNIs (Cilium, Calico) reject cross-family manifests at admission, while lenient CNIs silently drop the off-family entries and admit the manifest, producing a silent SSRF hole (NET-062). Fail-closed is required because the lenient-CNI outcome is the more dangerous one. | `NetworkPolicy `<name>` contains an `ipBlock` whose `cidr: <cidr>` is IPv4 but `except` entry `<except_cidr>` is IPv6 (or vice versa). Kubernetes NetworkPolicy requires each ipBlock's `except` entries to share the enclosing `cidr`'s address family (NET-062). Split the rule into two parallel `ipBlock` peers, one per family, with family-matching `except` entries on each.` |
| NetworkPolicy DNS `podSelector` parity (NET-067) | Enumerate every Lenny-rendered NetworkPolicy egress rule whose `ports` list contains UDP/53 or TCP/53. For each such rule, assert that every peer in the `to:` list pairs a `namespaceSelector` with a destination `podSelector` matching a concrete DNS-server label (`k8s-app: kube-dns` for `kube-system` CoreDNS; `lenny.dev/component: coredns` for the dedicated `lenny-system` CoreDNS). A namespace-only selector resolves to every pod in the target namespace — `kube-system` hosts dozens of system pods (metrics-server, kube-proxy, CSI drivers, cloud-provider controllers), any of which becomes reachable on UDP/TCP 53 without a pod selector. Fail-closed at install/upgrade. | `NetworkPolicy `<name>` has a DNS egress rule (UDP/TCP 53) whose peer omits a destination `podSelector`. DNS egress without a pod selector is a policy violation (NET-067): pair `namespaceSelector: { kubernetes.io/metadata.name: kube-system }` with `podSelector: { matchLabels: { k8s-app: kube-dns } }` for the cluster CoreDNS, or `namespaceSelector: { kubernetes.io/metadata.name: lenny-system }` with `podSelector: { matchLabels: { lenny.dev/component: coredns } }` for the dedicated Lenny CoreDNS.` |
| Checkpoint object-store NetworkPolicy parity (NET-071) | Assert that a rendered `allow-pod-egress-objectstore` with an in-cluster arm is paired with a matching MinIO ingress clause admitting agent-namespace pods on the TLS port, and that neither is rendered without the other. Assert that the cloud-managed arm renders a non-empty `objectStorage.egressCIDRs` list. Fail-closed at install/upgrade. |
| Checkpoint T4 default-encryption configuration | For a `gcs` or `azure` backend serving any `workspaceTier: T4` tenant, assert that the deployment declares the per-tenant bucket CMEK (GCS) or the container default encryption scope with override prevention (Azure) that the presigned `PUT` cannot sign per-request (§12). Fail-closed at install/upgrade, because the mint issues no encryption header on these backends and cannot fail closed at request time. |
...
| T4 node isolation webhook             | When `features.compliance: true`: verify that the `lenny-t4-node-isolation` `ValidatingWebhookConfiguration` exists and that its `caBundle` field is non-empty. When `features.compliance: false` (Phase 3.5 through Phase 8), this check is skipped because the webhook is not rendered (see [§17.2](#172-namespace-layout) "Feature-gated chart inventory"). When any pool references a T4 Runtime and the webhook is absent or misconfigured, T4 pods may be admitted to shared nodes.                                   | `lenny-t4-node-isolation ValidatingWebhookConfiguration not found or caBundle empty; required for T4 dedicated-node enforcement (Section 6.4)`                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Drain-readiness webhook               | When `features.drainReadiness: true`: verify that the `lenny-drain-readiness` `ValidatingWebhookConfiguration` exists and that its `caBundle` field is non-empty. When `features.drainReadiness: false` (Phase 3.5 through Phase 5.8), this check is skipped because the webhook is not rendered (see [§17.2](#172-namespace-layout) "Feature-gated chart inventory"). When the webhook is absent or misconfigured, node drains will not check MinIO health before pod eviction.                                                        | `lenny-drain-readiness ValidatingWebhookConfiguration not found or caBundle empty; required for pre-drain MinIO health check (Section 12.5)`                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Admission webhook inventory           | Enumerate deployed `ValidatingWebhookConfiguration` resources and `CustomResourceDefinition.spec.conversion.webhook` endpoints in the cluster. Verify the expected set is present; the expected set is composed at preflight render time from the three chart feature flags `features.llmProxy`, `features.drainReadiness`, and `features.compliance` per the [§17.2](#172-namespace-layout) "Feature-gated chart inventory" rule — baseline entries `lenny-label-immutability`, `lenny-sandboxclaim-guard`, `lenny-pool-config-validator`, the `lenny-crd-conversion` conversion webhook for each Lenny CRD, and `lenny-ephemeral-container-cred-guard` are always expected (the five-entry Phase 3.5 baseline per [§17.2 ](#172-namespace-layout)); `lenny-direct-mode-isolation` is added iff `features.llmProxy`, `lenny-drain-readiness` iff `features.drainReadiness`, and `lenny-data-residency-validator` + `lenny-t4-node-isolation` iff `features.compliance`. For each present `ValidatingWebhookConfiguration`, additionally verify `failurePolicy: Fail` and a non-empty `caBundle`. Fail-closed: any missing entry aborts the install. This prevents chart-author omissions (a webhook dropped from `templates/admission-policies/`) from shipping silently and producing fail-open gaps at runtime, while also allowing pre-Phase-13 chart slices to install cleanly because the expected set tracks the rendered set exactly. See [§17.2](#172-namespace-layout). | `expected ValidatingWebhookConfiguration '<name>' not found; chart-rendered webhook is missing — re-render with the current chart or run 'helm template' and diff against the expected set (Section 17.2 admission-policies enumeration and Feature-gated chart inventory)` / `ValidatingWebhookConfiguration '<name>' has failurePolicy='<value>'; expected 'Fail' for fail-closed behavior` |
| Phase-stamp consistency (KIN-028) | Read the `lenny-deployment-phase-stamp` ConfigMap in `{Release.Namespace}` (the append-only phase-stamp per [§17.2](#172-namespace-layout) "Feature-flag downgrade enforcement"); for every flag with `enabled: true` recorded in the phase-stamp, assert that the Job's injected `features.<flag>` value is `true`, unless `acceptFeatureFlagDowngrade.<flag>: true` is also set (same override as the chart render-time gate). This is the **sole fail-closed downgrade gate for GitOps deployments** (ArgoCD, Flux Helm Controller) because those pipelines render via `helm template`, under which Helm's `lookup` function returns an empty map and the chart-level render-time guard is a no-op. In the `helm install` / `helm upgrade` code path the check additionally catches (a) the narrow race where the phase-stamp ConfigMap was deleted between render and apply (so the render-time `lookup` saw no ConfigMap) and (b) any out-of-band mutation between render and preflight execution. Symmetric with the chart-level `PHASE_STAMP_FEATURE_FLAG_DOWNGRADE` error and closes the iter7 KIN-028 enforcement gap. The Job ignores the phase-stamp entirely on the first install (when the ConfigMap does not exist) — this is the only case in which a `false → true → false` trajectory can be undetected, and by definition the first install is not a downgrade. | `PREFLIGHT_PHASE_STAMP_MISMATCH: flag '<flag>' is recorded as enabled in lenny-deployment-phase-stamp (enabledAt=<ts>) but the incoming values render it as disabled without acceptFeatureFlagDowngrade.<flag>=true. Re-enable the flag or pass --set acceptFeatureFlagDowngrade.<flag>=true to acknowledge the degraded admission-plane posture (Section 17.2 Feature-flag downgrade enforcement).` |
...
| `lenny-ops-sa` RBAC                   | Verify that the `lenny-ops-sa` ServiceAccount has the RBAC permissions documented in [§25.4](25_agent-operability.md#254-the-lenny-ops-service). Uses `kubectl auth can-i` against each rule in the canonical RBAC table (Lease coordination, Deployment patches, CRD reads, ConfigMap reads, Secret reads for backup credentials, Job create/watch). Unconditional from Phase 3.5 onward, per the [§17.8.5](#1785-mandatory-lenny-ops-deployment) mandatory-`lenny-ops` contract — there is no `features.ops` flag because chart validation rejects any attempt to disable `lenny-ops`, so this check fires on every preflight run. | `ServiceAccount lenny-ops-sa is missing required permissions: <rules>. Re-render the chart or apply the Role/ClusterRole templates in deploy/helm/lenny/templates/ops/rbac.yaml`                                                                                                                                                                                                                                                                                                                                                                                              |
...
| SPIFFE trust domain uniqueness (NET-064) | Enumerate the `lenny.dev/spiffe-trust-domain` annotation on every existing `lenny-gateway` Deployment in the target cluster and verify the value under installation is absent from that set. Fail-closed: collision aborts the install. Paired with the chart's `required` guard on `global.spiffeTrustDomain` so an unset value fails at templating and a set-but-duplicate value fails at preflight (see [§10.3](10_gateway-internals.md#103-mtls-pki)). | `global.spiffeTrustDomain '<value>' is already in use by Lenny Deployment '<ns>/<name>'. Two deployments sharing a trust domain have overlapping SPIFFE URIs, enabling cross-deployment pod impersonation. Choose a deployment-unique value (e.g. 'lenny-<cluster>-<namespace>') — see Section 10.3 (NET-064).` |
| SA token audience uniqueness (NET-064)   | Enumerate the `lenny.dev/sa-token-audience` annotation on every existing `lenny-gateway` Deployment in the target cluster and verify the value under installation is absent from that set. Fail-closed: collision aborts the install. Paired with the chart's `required` guard on `global.saTokenAudience` so an unset value fails at templating and a set-but-duplicate value fails at preflight (see [§10.3](10_gateway-internals.md#103-mtls-pki)). | `global.saTokenAudience '<value>' is already in use by Lenny Deployment '<ns>/<name>'. A shared audience allows SA tokens minted for one deployment's gateway to be accepted by another's, enabling cross-deployment token replay. Choose a deployment-unique value (e.g. 'lenny-gateway-<cluster>') — see Section 10.3 (NET-064).` |
| `playground.devTenantId` format and presence | When `playground.enabled: true`: (a) verify `playground.devTenantId` (if set) matches `^[a-zA-Z0-9_-]{1,128}$` — mirrors the canonical `tenant_id` regex at [§10.2](10_gateway-internals.md#102-authentication) so the format gate is a single source of truth across OIDC-claim extraction, admin-API tenant creation, and the playground Helm value; (b) when `playground.authMode: dev`, verify the value is non-empty (the JSON Schema cannot express the `authMode == dev ⇒ devTenantId set` cross-field conditional, so preflight is the install-time fail-fast layer); (c) when `playground.authMode: dev` **and** `auth.multiTenant: true`, reject `playground.devTenantId: default` while more than one tenant is seeded in `bootstrap.tenants` (matches the Helm-validate rule in [§27.3](27_web-playground.md#273-authentication)). Runs in all modes (including `global.devMode: true`) because it catches operator typos that otherwise manifest only as gateway `CrashLoopBackOff` with `LENNY_PLAYGROUND_DEV_TENANT_INVALID` / `LENNY_PLAYGROUND_DEV_TENANT_REQUIRED` after `helm install` appears to succeed. Also triggers under `helm install --dry-run` / `lenny-ctl values validate` so GitOps pipelines fail at template time rather than at pod startup. | `playground.devTenantId '<value>' does not match required format '^[a-zA-Z0-9_-]{1,128}$' (Section 27.2, 10.2). Correct the value before re-running helm install.` / `playground.authMode=dev requires playground.devTenantId to be set (Section 27.3). Add playground.devTenantId to values.yaml.` / `playground.authMode=dev with auth.multiTenant=true requires an explicit playground.devTenantId (not 'default') when multiple tenants are seeded; chose one of: <tenant-id-list> (Section 27.3).` |
| `playground.apiKeyMode` (warning) | When `playground.enabled: true` **and** `playground.authMode: apiKey` **and** `global.devMode: false` **and** `playground.acknowledgeApiKeyMode: false`: emit a non-blocking `WARNING`. Rationale: `apiKey` mode ships a bearer-token paste form as its primary UX, which is operationally similar to a credential-phishing web form; operators SHOULD prefer `authMode=oidc` for human-user access ([§27.3](27_web-playground.md#273-authentication), [§27.9](27_web-playground.md#279-security-considerations)). Mirrors the acknowledgement pattern for `monitoring.acknowledgeNoPrometheus` ([§25.4](25_agent-operability.md#254-the-lenny-ops-service)). Non-blocking because `apiKey` is a legitimate production mode for operator-driven / headless workflows post-iter3; the check exists so the operator explicitly acknowledges the human-user phishing-surface tradeoff rather than silently defaulting into it. | `WARNING: playground.authMode=apiKey ships a bearer-token paste form — a standard phishing surface when presented to human users. Prefer playground.authMode=oidc for human-user access (Section 27.3, 27.9). To acknowledge this posture and silence the warning, set playground.acknowledgeApiKeyMode=true in Helm values. Alternatively, set playground.enabled=false or switch to playground.authMode=oidc.` |
...
- **Postgres failover** (`docs/runbooks/postgres-failover.md`)
  - *Trigger:* `PostgresReplicationLagHigh` or `SessionStoreUnavailable` alert fires; gateway logs `pq: connection refused` errors.
...
- **MinIO failure** (`docs/runbooks/minio-failure.md`)
  - *Trigger:* `MinIOUnavailable` alert fires ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)); workspace upload/download failures; `lenny_artifact_upload_error_total{error_type="minio_unreachable"}` counter increments. Paired signals: `WorkspaceSealStuck` fires if seal-and-export stays in retry beyond `maxWorkspaceSealDurationSeconds` and `CheckpointStorageUnavailable` fires if a checkpoint upload exhausts its retry budget during eviction.
...
- **Token Service outage** (`docs/runbooks/token-service-outage.md`)
  - *Trigger:* `TokenServiceUnavailable` alert fires ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)); circuit breaker in open state for > 30s; finalize for credential-requiring pools returns `CREDENTIAL_MATERIALIZATION_ERROR` (the credential lease is materialized at finalize, per §7.1; existing sessions are unaffected until credential lease expiry).
...
- **Legal-hold quota pressure** (`docs/runbooks/legal-hold-quota-pressure.md`)
  - *Trigger:* `LegalHoldCheckpointAccumulationProjectedBreach` alert fires ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)) — `(lenny_storage_quota_bytes_used + sum by (tenant_id) (lenny_legal_hold_checkpoint_projected_growth_bytes)) > 0.9 * lenny_tenant_storage_quota_bytes and on(tenant_id) lenny_tenant_legal_hold_active_count > 0`. The 24-hour projection of legal-hold checkpoint accumulation for the affected tenant would cross 90% of `storageQuotaBytes` if current hold rotation-exemption continues. Fires ahead of reactive thresholds (`StorageQuotaHigh` / `CheckpointStorageHigh` at 80%) so that operators see the legal-hold-driven growth component distinctly before the general-purpose quota alerts fire.
  - *Diagnosis:* (1) Enumerate active holds for the tenant via the audit trail: `GET /v1/admin/audit-events?event_type=legal_hold.set&tenant_id=<id>&since=<oldest_open_hold_window>` and subtract `legal_hold.cleared` entries to identify the currently-in-force set; alternatively query `SELECT session_id, set_at, set_by, note FROM sessions WHERE tenant_id = '<id>' AND legal_hold = true` (and the analogous `artifacts` / legal-hold ledger queries for audit-range and workspace-snapshot holds — see [Section 12.8](12_storage-architecture.md#128-compliance-interfaces) Legal hold). (2) Cross-correlate the alert label `root_session_id` with the identified holds to confirm which held session(s) dominate the projected growth. (3) Inspect `lenny_checkpoint_storage_bytes_total{tenant_id="<id>"}` (per-pool breakdown) and `lenny_legal_hold_checkpoint_projected_growth_bytes{tenant_id="<id>"}` (per-session breakdown) to quantify current vs. projected pressure. (4) Check the tenant's configured `storageQuotaBytes` via `GET /v1/admin/tenants/<id>` — compare current + projected against this ceiling.
  - *Remediation:* This runbook is a decision tree, not a service-restoration procedure — the platform does not auto-resolve and must not force deletion during a hold (see [Section 12.8](12_storage-architecture.md#128-compliance-interfaces) Legal hold and checkpoint rotation, "spoliation avoidance"). The operator chooses one path: **(A) Raise the tenant's quota** — if the held evidence is expected to continue accumulating for the remaining hold duration, enlarge `storageQuotaBytes` via `PUT /v1/admin/tenants/{id}` with an increased value that covers the projected 24-hour growth plus additional headroom for non-checkpoint artifacts (workspace snapshots, uploads, transcripts, eviction contexts — all share the same bucket per [Section 12.5](12_storage-architecture.md#125-artifact-store)). Use the per-pod accumulation rates documented in [§12.5](12_storage-architecture.md#125-artifact-store) Checkpoint-storage sizing guidance to size the increase; record the rationale (specific hold IDs, expected duration) alongside the admin action in the audit trail. **(B) Accept the held-session freeze** — if the hold is expected to clear soon or the increase would exceed the deployment's cost envelope, acknowledge that once `storageQuotaBytes` is reached, new checkpoint writes for the held session (and every other write in the tenant) will reject with `STORAGE_QUOTA_EXCEEDED`. The held session's existing checkpoints remain retained until the hold is lifted — rotation stays suspended — but no *newer* forensic state will be captured. Document this choice and the compliance/discovery implications in the incident record; notify legal/compliance stakeholders that freshness of held evidence will degrade until either the hold clears or the quota is raised. **After either path:** If path A was taken, re-check `POST /v1/admin/drift/snapshot/refresh` per the drift-snapshot-refresh runbook so the bootstrap snapshot reflects the elevated quota. If path B was taken and the alert remains active, suppress the alert for the tenant for a bounded operator-chosen window (not a permanent silence) so subsequent re-assessments still page on-call.

- **Admission-plane feature-flag downgrade** (`docs/runbooks/admission-plane-feature-flag-downgrade.md`)
  - *Trigger:* `AdmissionPlaneFeatureFlagDowngrade` — phase-stamp ConfigMap records `features.<flag>.enabled=true` (with an `enabledAt` RFC3339 timestamp) but the corresponding `ValidatingWebhookConfiguration` is absent from the cluster for > 2 minutes (see [Section 16.5](16_observability.md#165-alerting-rules-and-slos)). Fail-closed condition: the admission plane is no longer enforcing a policy it previously enforced, violating the phase-stamp append-only invariant established in [§17.2](#172-namespace-layout) "Feature-flag downgrade enforcement".
  - *Diagnosis:* (1) Read the committed phase-stamp: `kubectl -n lenny-system get configmap lenny-deployment-phase-stamp -o jsonpath='{.data}'`. (2) List currently-installed webhooks: `kubectl get validatingwebhookconfiguration -l app.kubernetes.io/name=lenny`. (3) Reference the flag-to-webhook mapping from [§17.2](#172-namespace-layout) feature-gated chart inventory — `features.llmProxy` → `lenny-direct-mode-isolation`; `features.drainReadiness` → `lenny-drain-readiness`; `features.compliance` → `lenny-data-residency-validator` AND `lenny-t4-node-isolation` (both must be present when the flag is true). (4) Identify whether a legitimate downgrade acknowledgement was recorded: `GET /v1/admin/audit-events?event_type=deployment.feature_flag_downgrade_acknowledged&since=24h` — if the acknowledgement is present but the webhook is still missing, the chart render committed but the admission object failed to install (retry the upgrade); if no acknowledgement is present, the divergence is unauthorized and requires immediate remediation.
  - *Remediation:* (1) **If the downgrade was unintentional** (e.g., a CI pipeline regressed the values file): restore the webhook via `helm upgrade lenny … --set features.<flag>=true` — chart render-time validation passes because the phase-stamp already records the flag as enabled, and the admission plane returns to the enforcing state. Verify recovery via `kubectl get validatingwebhookconfigurations -l app.kubernetes.io/name=lenny` and confirm the `AdmissionPlaneFeatureFlagDowngrade` alert clears within one evaluation cycle. (2) **If the downgrade is intentional and auditable**: commit an explicit acknowledgement via `helm upgrade lenny … --set features.<flag>=false --set acceptFeatureFlagDowngrade.<flag>=true`, which emits a `deployment.feature_flag_downgrade_acknowledged` audit event and rewrites the phase-stamp entry in place. (3) Do NOT hand-edit the `lenny-deployment-phase-stamp` ConfigMap — the chart is the sole writer; manual edits violate the append-only contract and will be reverted on the next upgrade. Cross-reference: [§17.2](#172-namespace-layout) "Feature-flag downgrade enforcement", [§16.5](16_observability.md#165-alerting-rules-and-slos) `AdmissionPlaneFeatureFlagDowngrade`, `docs/runbooks/admission-plane-feature-flag-downgrade.md` for the full step-by-step recovery procedure.
...
| Partial-checkpoint chunk size (`partialChunkSizeBytes`) | 16 MiB                                                        | [§10.1](10_gateway-internals.md#101-horizontal-scaling)     |
...
| Partial-recovery threshold fraction (`gateway.partialRecoveryThresholdFraction`) | 0.5 (a fraction in `[0.0, 1.0]`; resume consumes a partial manifest only when `workspace_bytes_uploaded >= baseline_full_checkpoint_bytes * partialRecoveryThresholdFraction`; denominator is frozen on the manifest row at intent-row INSERT time — see iter3 CPS-003 / iter4 CPS-009) | [§10.1](10_gateway-internals.md#101-horizontal-scaling)     |
...
| EventBus retranscribe sweep interval (`eventBus.retryInterval`) | 60 s                                                                    | [§12.6](12_storage-architecture.md#126-interface-design)     |
| EventBus retranscribe max attempts before terminal failure (`eventBus.maxRetryAttempts`) | 20                                                  | [§12.6](12_storage-architecture.md#126-interface-design), [§16.5](16_observability.md#165-alerting-rules-and-slos) |
| T4 KMS continuous probe interval (`storage.t4KmsProbeInterval`; min floor 60 s enforced at Helm-validate) | 300 s (5 min) | [§12.5](12_storage-architecture.md#125-artifact-store), [§16.5](16_observability.md#165-alerting-rules-and-slos) |
| T4 KMS continuous probe rate limit (`storage.t4KmsProbeRateLimit`) | 10 probes/s | [§12.5](12_storage-architecture.md#125-artifact-store) |
...
| HPA scale-down pods per period             | 1 / 60s                    | 1 / 60s                    | 1 / 60s                    |
...
| Gateway scale-down time (max→min replicas) | ~2 min (4→2, 1 pod/60s)    | ~7 min (10→3, 1 pod/60s)   | ~25 min (30→5, PDB-bound at 1 pod / ~60–120s; see note below) |
| Minimum healthy gateway replicas (alert)   | 2                          | 3                          | 5                          |

> **Scale-down rate is PDB-bound, not HPA-policy-bound.** The flat `maxUnavailable: 1` PDB on the gateway Deployment ([§17.1](#171-kubernetes-resources)) applies to every voluntary disruption — including HPA-triggered evictions — and caps concurrent voluntary evictions at one across all tiers. The effective scale-down floor is therefore `1 / pod_termination_seconds`, where `pod_termination_seconds` is dominated by the preStop tiered checkpoint cap (up to 90s for 512 MB workspaces) plus connection drain (~30s), i.e., ~60–120s per pod. At Tier 3 this means draining 25 replicas (30 → 5) takes roughly `25 × 60s = 25 min` in the best case and `25 × 120s = 50 min` in the worst case — the 8.3 min figure that a raw `3 pods / 60s` HPA policy would imply is unreachable under the PDB ceiling. This is an intentional trade-off: availability (at most one replica draining at any moment, bounding the CheckpointBarrier fan-out to one replica's 400-pod quota — see [§17.1](#171-kubernetes-resources) and [§10.1](10_gateway-internals.md#101-horizontal-scaling) preStop stage 2) takes precedence over rapid scale-down. The HPA `scaleDown` policy is accordingly set to `1 pod / 60s` at every tier so Kubernetes does not repeatedly attempt evictions the PDB cannot satisfy (which would produce continuous 429 rejection log spam from the eviction API and unnecessary controller churn). Operators observe the PDB-bound scale-down floor via `lenny_pdb_blocked_evictions_total` ([§16.1](16_observability.md#161-metrics)) and the `PDBBlockedEvictions` alert ([§16.5](16_observability.md#165-alerting-rules-and-slos)); a sustained non-zero rate under normal scale-down is expected only when the HPA desired-replica target moves faster than the PDB floor permits and is itself a signal that either `minReplicas` is set too high or the scale-down trough is unusually deep.

> **Tier 3 node-drain expectations.** The same `maxUnavailable: 1` PDB also serializes evictions during `kubectl drain` of any node hosting more than one gateway pod. Because topology spread across zones is probabilistic, at Tier 3 (5–30 replicas) a worker node can realistically host 2–3 gateway pods. Worst-case single-node drain time is therefore:
>
> ```
> worst_case_single_node_drain_time = pods_on_node × terminationGracePeriodSeconds
> ```
>
> At Tier 3 (`terminationGracePeriodSeconds: 300`) this yields up to **10 minutes** for 2 co-located pods and **15 minutes** for 3. Operators running `kubectl drain` for cluster-autoscaler consolidation, spot-interruption rehearsals, or node-image patching MUST size `--timeout` (and any IaC wrapper's drain-timeout equivalent) to exceed this bound; a shorter timeout causes the tooling to fall back to force-delete, which bypasses the preStop checkpoint guarantees the PDB was introduced to preserve.
>
> To cap single-node drain time at one grace period (≤ 5 min at Tier 3) regardless of replica count, deployments SHOULD add a soft `podAntiAffinity` rule at Tier 3 with `topologyKey: kubernetes.io/hostname` so the scheduler prefers spreading gateway pods across distinct nodes. A required (`requiredDuringSchedulingIgnoredDuringExecution`) rule is acceptable when the cluster has at least `maxReplicas` schedulable nodes in the gateway's node pool; otherwise use `preferredDuringSchedulingIgnoredDuringExecution` to avoid blocking scale-up when a zone temporarily runs short of nodes. Tier 1/2 deployments MAY omit this rule: at 4 and 10 maximum replicas with multi-zone topology spread, worst-case co-location is low and the 240s grace period keeps single-node drain under 12 min even at 3 co-located pods. Operators observe drain serialization through `lenny_pdb_blocked_evictions_total` ([§16.1](16_observability.md#161-metrics)) — a sustained spike during a node drain indicates the PDB is serializing evictions on that node and drain time will follow the formula above.
...
>
> **Rationale and validation gate for the Tier 3 1.2 factor.** The 1.2 multiplier at Tier 3 assumes that in-window claim variance during the 35-second `failover + pod-warm warmup` gap is bounded by **≤ 20% above the 30 claims/second steady-state rate** (i.e., the P99 of arrival count over any 35s window does not exceed `1.2 × 30 × 35 = 1,260`). For SDK-warm pools the window is wider (`failover_seconds + pod_warmup_seconds` with `pod_warmup_seconds` ≥ 30s); the 20% bound and the 1,260 baseline apply to the pod-warm case only. This assumption is plausible only when (a) the gateway's queue-release behavior after failover recovery does not create a thundering herd (see [§4.6.1](04_system-components.md#461-warm-pool-controller-pod-lifecycle) "Fallback preconditions" and the Postgres-backed fallback claim path, which smooths post-failover claim arrival) and (b) observed `burst_p99_claims` for the pool remains at or below `0.2 × base_demand_p95` during a 60s rolling window. At 30 claims/s the *absolute* headroom of 210 pods (`1,260 − 1,050`) is larger than Tier 1's 7-pod absolute headroom despite the smaller fractional margin; this is the intended trade-off. **Operators must validate the assumption with the failover-warm-serve SLO below.** If the SLO is not met — or if `burst_p99_claims` exceeds the 20% bound — operators MUST raise `poolScaling.safetyFactor` universally (e.g., to 1.3 or 1.5) rather than making tier-specific or pool-specific overrides. The controller applies one `safety_factor` value per pool; there is no tier-dependent code path that computes it differently from the formula in [Section 4.6.2](04_system-components.md#462-poolscalingcontroller-pool-configuration) — the per-tier table values are only defaults.
>
> **Failover-warm-serve SLO (validation gate for `safety_factor`).** Production deployments SHOULD measure `lenny_warmpool_failover_claims_served_fraction` = (claims served from warm pool during a `failover_seconds + pod_warmup_seconds` post-failover window) / (total claims arriving during that window) — `35s` for pod-warm pools with the 10s baseline; wider for SDK-warm pools per the configured `podWarmupSecondsBaseline`. The target is **≥ 0.99** (at most 1% of claims during a failover window fall through to cold-start). If this SLO is not met over a rolling 30-day window containing at least one observed failover event (or a chaos-drill synthetic failover, see [Section 17.7](#177-operational-runbooks)), the `safety_factor` is under-sized for the deployment's burst profile and MUST be increased. See [Section 16.5](16_observability.md#165-alerting-rules-and-slos) for the metric and burn-rate alert definition.

Formula: `minWarm >= claim_rate * safety_factor * (failover_seconds + pod_warmup_seconds) + burst_p99_claims * pod_warmup_seconds`. The `safety_factor` column in the table above (1.5 for Tier 1/2, 1.2 for Tier 3) scales the steady-state term to provide a buffer above the raw demand estimate (see [Section 4.6.2](04_system-components.md#462-poolscalingcontroller-pool-configuration) for the full formula with `safety_factor`). The first term covers sustained demand during failover; the burst term (see [Section 4.6.2](04_system-components.md#462-poolscalingcontroller-pool-configuration)) reserves headroom for demand spikes that outpace pool refill. **Use `failover_seconds = 25` (worst-case crash scenario: `leaseDuration + renewDeadline = 15s + 10s`).** The "Production `minWarm`" baselines in the table above assume **pod-warm pools** with `pod_warmup_seconds = 10` (container pull cached + runtime startup, no SDK pre-initialization), yielding a `25s failover + 10s warmup = 35s` window. For **SDK-warm pools** (`scalingPolicy.podWarmupSecondsBaseline` ≥ 30s), operators must substitute the observed `pod_warmup_seconds` in both terms — a 30s SDK init extends the window to 55s and raises the Tier 3 baseline from 1,260 to `ceil(30 × 1.2 × 55) = 1,980`, and the Tier 1/2 baselines proportionally. The burst term adds further headroom proportional to warmup latency and observed burst intensity. Note: clean shutdown (rolling update) reduces failover to near-zero via voluntary lease release, but sizing must cover the crash case.
...
minWarm >= adjusted_claim_rate * safety_factor * (failover_seconds + pod_warmup_seconds) / mode_factor
...
minWarm >= ceil(38 * 1.2 * (25 + 10) + (burst_p99_claims + 50) * 10)
        = ceil(38 * 1.2 * 35 + (burst_p99_claims + 50) * 10)
        = ceil(1,596 + (burst_p99_claims + 50) * 10)
```
With `burst_p99_claims = 0` (first deployment, no historical data): `ceil(1,596 + 500) = 2,096`. With observed `burst_p99_claims = 10`: `ceil(1,596 + 600) = 2,196`. For SDK-warm pools with `pod_warmup_seconds = 30`, substitute 30 for 10 in both terms: `ceil(38 * 1.2 * 55 + (burst_p99_claims + 50) * 30)` yields 2,508 + 1,500 = 4,008 with no burst, 4,308 with observed `burst_p99_claims = 10`.

The production baseline of `minWarm: 1,260` (Tier 3 "Production `minWarm`" row in the table above) covers session-creation demand only and assumes a pod-warm pool (`pod_warmup_seconds = 10`). Deployments where a significant fraction of sessions use the `orchestrator` preset or other high-fan-out leases should increase `minWarm` using the formula above — roughly **2,100** pods for a pod-warm pool (with burst term, zero historical burst data), rising to approximately **4,000** for SDK-warm pools at the 30s `pod_warmup_seconds` baseline. If `orchestrator`-preset sessions are rare (< 10% of sessions), the production baseline of 1,260 remains adequate for pod-warm pools. Operators should monitor `lenny_warmpool_idle_pods` and `lenny_pod_claim_queue_wait_seconds` P99 during the first week (see first-week monitoring workflow below) to confirm whether delegation fan-out is material at their actual traffic mix.

> **Note:** Distributing warm pods across multiple hot pools reduces per-pool `minWarm` proportionally. At Tier 3 with 10 hot pools, pod-warm baseline (`pod_warmup_seconds = 10`), and zero historical burst data, the per-pool delegation-adjusted `minWarm` is approximately `ceil(2,096 / 10) = 210`, well within normal operating range. SDK-warm pools at the 30s warmup baseline require proportionally higher per-pool values (~400 with the same fan-out assumptions).

**ResourceQuota tier defaults (`lenny-agents` namespace):**

The ResourceQuota attached to each agent namespace ([Section 17.2](#172-namespace-layout)) MUST accommodate the sum of warm-pool and active-session pods the tier can produce, with headroom for delegation fan-out. The chart derives the defaults below from `.Values.capacityPlanning.tier`; operators overriding the tier MUST ensure the quota row they pin matches the tier's upper bound.

| Parameter                            | Tier 1      | Tier 2       | Tier 3        |
| ------------------------------------ | ----------- | ------------ | ------------- |
| `pods`                               | 200         | 2,500        | 15,000        |
| `requests.cpu`                       | `"400"`     | `"5000"`     | `"30000"`     |
| `requests.memory`                    | `"800Gi"`   | `"10Ti"`     | `"60Ti"`      |
| `requests.ephemeral-storage`         | `"2Ti"`     | `"25Ti"`     | `"150Ti"`     |

**Sizing formula.** For any tier `T`, the `pods` floor is:

```
pods_floor(T) = sum(pool.maxWarm for pool targeting namespace)
              + session_concurrency_limit(T) * 1.2
              + concurrent_delegation_fanout_cap(T)
```

At Tier 3 this evaluates to roughly `(≈2,100 delegation-adjusted warm pods for pod-warm pools, up to ≈4,000 for SDK-warm pools across all pools) + (10,000 active sessions × 1.2) + (500 concurrent delegation children) ≈ 14,600–16,500`; the published default of **15,000** is the chart's rounded starting point and assumes a mixed deployment with the Tier 3 `maxWarm` envelope sized for the delegation-adjusted demand. Deployments that run SDK-warm pools at the upper end of `pod_warmup_seconds` (60–90s) or configure larger aggregate pool envelopes MUST raise the quota via `.Values.agentNamespaces[].resourceQuota.pods`. `requests.cpu`, `requests.memory`, and `requests.ephemeral-storage` scale from the same pod headcount multiplied by the per-pod request envelope from [Section 17.8.1](#1781-operational-defaults--quick-reference); the defaults above assume 2 CPU / 4 GiB memory / 10 GiB ephemeral-storage per pod (mid-range agent sizing). Pools that configure a heavier per-pod request envelope (e.g., ML runtimes) MUST re-compute the aggregate quota from their actual request values rather than relying on the defaults.

**Post-hoc quota raises.** ResourceQuota changes in Kubernetes take effect at the next admission decision but do not evict existing pods ([kubernetes.io/docs/concepts/policy/resource-quotas/](https://kubernetes.io/docs/concepts/policy/resource-quotas/)). A mid-incident quota raise therefore unblocks the next pod creations without disturbing running sessions; operators can respond to a `PoolScaleoutBlockedByQuota` firing by raising the relevant `requests.*` or `pods` entry and allowing scale-up to resume.
...
For first deployments, use the per-tier `minWarm` values in the table above as conservative starting points. These values assume: (a) `failover_seconds` of 25s (worst-case crash scenario), (b) `pod_warmup_seconds` of 10s — the **pod-warm baseline** (container pull cached + runtime startup, no SDK pre-initialization; see the terminology note in [Section 4.6.2](04_system-components.md#462-poolscalingcontroller-pool-configuration) on the `pod_startup_seconds` shorthand), (c) the formula `claim_rate * (failover_seconds + pod_warmup_seconds)` as the primary term, and (d) a single hot pool per runtime. If your deployment has multiple hot pools, divide the per-tier `minWarm` across pools proportionally to expected traffic share. If pod warmup in your environment is significantly longer (e.g., large container images, slow registry, SDK-warm pools), increase `minWarm` proportionally. **SDK-warm pools:** For pools that pre-initialize the SDK (`scalingPolicy.podWarmupSecondsBaseline` typically 30–90s), `pod_warmup_seconds` far exceeds the 10s pod-warm baseline. Operators must substitute the observed SDK-warm pod creation-to-ready time for `pod_warmup_seconds` in both the base-demand term (`failover_seconds + pod_warmup_seconds`) and the burst term (`burst_p99_claims * pod_warmup_seconds`) of the full formula in [Section 4.6.2](04_system-components.md#462-poolscalingcontroller-pool-configuration) rather than relying on the baseline table values.
...
| Minimum MinIO aggregate throughput (burst, max workspace) ⁴   | ~140 MB/s | ~1.2 GB/s | ~12 GB/s |

> ⁴ **Burst-throughput derivation.** The "burst, max workspace" row is the aggregate write throughput MinIO must sustain when every active session checkpoints a 512 MB workspace at the steady-state checkpoint rate — i.e., `max_workspace_upload_bandwidth × 1.4` (applying the 40% headroom factor defended in the Tier 3 budget narrative below). Derivations: Tier 1 (~100 MB/s × 1.4 ≈ 140 MB/s), Tier 2 (~850 MB/s × 1.4 ≈ 1.2 GB/s), Tier 3 (~8.5 GB/s × 1.4 ≈ 12 GB/s). These minimums align with the topology columns: a 4-node MinIO erasure-coded cluster clears 1.2 GB/s on commodity NVMe, and the 8-node Tier 3 topology delivers ~10–12 GB/s (per the narrative below).
...
>
> **Topology-vs-capacity invariant.** Any MinIO topology selection must satisfy:
>
> ```
> replicas × maxSessionsPerReplica × avg_workspace_bytes / periodicCheckpointIntervalSeconds
>     ≤ minio_aggregate_write_throughput × 0.7
> ```
>
> At Tier 3 this evaluates to `30 × 400 × 100 MB / 600s = 2.0 GB/s ≤ 12 GB/s × 0.7 ≈ 8.4 GB/s` (safe for steady-state); the 8.5 GB/s burst ceiling above captures the max-workspace variant of the same formula with `avg_workspace_bytes = 512 MB`. Operators who increase `maxSessionsPerReplica`, raise `autoscaling.maxReplicas`, or relax the 512 MB workspace cap MUST re-verify the invariant against their MinIO delivered throughput and, if necessary, either expand the MinIO cluster (e.g., 16-node NVMe @ ~22 GB/s for 2× headroom over 8.5 GB/s) or lower the offending factor. **When the invariant is violated:** checkpoint writes back-pressure at the S3 client, `lenny_checkpoint_duration_seconds` P95 rises above 2.5s and the `CheckpointDurationHigh` alert fires; during voluntary disruption the gateway preStop drain ([§10.1](10_gateway-internals.md#101-horizontal-scaling)) will exceed its tier-specific drain timeout (Tier 3: 120s) and the kubelet will SIGKILL pods mid-checkpoint, which the controller recovers via the last durable checkpoint per [§4.4](04_system-components.md#44-event--checkpoint-store) — at the cost of replayed in-flight state for any session whose checkpoint was still uploading. Operators who expect sustained operation near the invariant should pre-empt this failure mode by expanding MinIO rather than relying on recovery.
...
| ---- | -------------- | ----------- |
| LLM Proxy extraction check | `lenny_gateway_llm_proxy_active_connections / lenny_gateway_active_sessions` sustained below **0.3:1** at Tier 2 peak for ≥15 min | Extract LLM Proxy to dedicated service before proceeding (see [§4.1](04_system-components.md#41-edge-gateway-replicas) extraction triggers) |
...
3. **Warm pools:** Update `minWarm` to Tier 3 production baseline (1,260 per hot pool, per the "Production `minWarm`" row in [§17.8.2](#1782-capacity-tier-reference)) or delegation-adjusted value from [§17.8.2](#1782-capacity-tier-reference).
