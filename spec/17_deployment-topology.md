...
The four remaining webhooks — `lenny-label-immutability` (item 5), `lenny-sandboxclaim-guard` (item 7), `lenny-pool-config-validator` (item 9), and the `lenny-crd-conversion` conversion webhook (item 12) — are unconditionally rendered and always expected; they are the Phase 3.5 baseline that every subsequent phase builds on. The `lenny-preflight` Job reads the same three feature-flag Helm values (injected into the Job's ConfigMap at template time) and composes its expected set by union: baseline four ∪ `{lenny-direct-mode-isolation}` if `features.llmProxy` ∪ `{lenny-drain-readiness}` if `features.drainReadiness` ∪ `{lenny-data-residency-validator, lenny-t4-node-isolation}` if `features.compliance`. A flag that is `false` causes the corresponding webhook template to be **omitted** from the render AND causes the preflight expected set to **exclude** that webhook — so a Phase 3.5 install (all three flags `false`) expects exactly the five baseline entries (four webhooks plus the conversion webhook) and passes, while a Phase 13 install (all three flags `true`) expects all eight entries. Flipping a flag from `true` to `false` after a phase has been reached is an invalid downgrade: the [§16.5](16_observability.md#165-alerting-rules-and-slos) per-webhook unavailability alerts and associated runtime enforcement paths depend on the webhook's continued presence.
...
- **Legal-hold quota pressure** (`docs/runbooks/legal-hold-quota-pressure.md`)
  - *Trigger:* `LegalHoldCheckpointAccumulationProjectedBreach` alert fires ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)) — `(lenny_storage_quota_bytes_used + sum by (tenant_id) (lenny_legal_hold_checkpoint_projected_growth_bytes)) > 0.9 * storageQuotaBytes` for any tenant with `lenny_tenant_legal_hold_active_count > 0`. The 24-hour projection of legal-hold checkpoint accumulation for the affected tenant would cross 90% of `storageQuotaBytes` if current hold rotation-exemption continues. Fires ahead of reactive thresholds (`StorageQuotaHigh` / `CheckpointStorageHigh` at 80%) so that operators see the legal-hold-driven growth component distinctly before the general-purpose quota alerts fire.
