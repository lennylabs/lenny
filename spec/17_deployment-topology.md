...
13. **`lenny-ephemeral-container-cred-guard`** `ValidatingAdmissionWebhook` — scoped to the `pods/ephemeralcontainers` subresource in every agent namespace (matched via `.Values.agentNamespaces`). Rejects any ephemeral-container request where the ephemeral container's `securityContext.runAsUser` equals the target pod's adapter UID or agent UID, where `securityContext.runAsGroup` or `securityContext.supplementalGroups` includes the `lenny-cred-readers` GID, or where any of `runAsUser`, `runAsGroup`, or `supplementalGroups` is absent on the ephemeral container's `securityContext` (absent values would inherit pod-level defaults, including the fsGroup that grants `/run/lenny/credentials.json` read access). Rejections emit `EPHEMERAL_CONTAINER_CRED_UID_FORBIDDEN` ([Section 15.1](15_external-api-surface.md#151-rest-api)). Closes the ephemeral-container credential-boundary bypass noted in [Section 13.1](13_security-model.md#131-pod-security).
...
**Feature-flag downgrade enforcement (phase-stamp ConfigMap + render-time validation + runtime alert).** The narrative prohibition above ("flipping a flag from `true` to `false` … is an invalid downgrade") is actively enforced by three cooperating layers; an operator attempting a downgrade hits the render-time fail-closed gate before the upgrade reaches the cluster, and any silent drift that bypasses the chart (e.g., out-of-band `kubectl delete` of a `ValidatingWebhookConfiguration`) is caught by the runtime alert. The three layers are:
...
- **Admission-plane feature-flag downgrade** (`docs/runbooks/admission-plane-feature-flag-downgrade.md`)
  ...
  - *Remediation:* (1) **If the downgrade was unintentional** (e.g., a CI pipeline regressed the values file): restore the webhook via `helm upgrade lenny … --set features.<flag>=true` — chart render-time validation passes because the phase-stamp already records the flag as enabled, and the admission plane returns to the enforcing state. (2) **If the downgrade is intentional and auditable**: commit an explicit acknowledgement via `helm upgrade lenny … --set features.<flag>=false --set acceptFeatureFlagDowngrade.<flag>=true`, which emits a `deployment.feature_flag_downgrade_acknowledged` audit event and rewrites the phase-stamp entry in place. (3) Do NOT hand-edit the `lenny-deployment-phase-stamp` ConfigMap — the chart is the sole writer; manual edits violate the append-only contract and will be reverted on the next upgrade. (4) For emergency rollout halts while investigating, pair with `admin.freezeAdmission` from the "Admission webhook storm" runbook entry in this section to stop all admission traffic until remediation completes. Cross-reference: [§17.2](#172-namespace-layout) "Feature-flag downgrade enforcement", [§16.5](16_observability.md#165-alerting-rules-and-slos) `AdmissionPlaneFeatureFlagDowngrade`.
