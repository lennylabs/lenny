...
| ----------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway                 | Deployment + Service + Ingress            | HPA, PDB, multi-zone, topology spread. Pod includes a LiteLLM sidecar container (image `lenny/litellm-hardened:<lenny-version>` — the signed hardened wrapper described in [§4.9](04_system-components.md#49-credential-leasing-service) LiteLLM sidecar hardening; port `4000` bound to loopback only) on gateway pods where any active CredentialPool has `deliveryMode: proxy`. The sidecar is optional on single-tenant direct-mode-only deployments. No separate `Service`, `NodePort`, or `Ingress` is created for the sidecar — the listener is intra-pod loopback (`127.0.0.1:4000`) only. Configured via Helm values under `llmProxy.litellm.{image, port, config, resources}`; the chart's default `image` value is the signed wrapper, and admission policy ([§13.1](13_security-model.md#131-pod-security)) refuses to admit gateway pods whose sidecar image is not the signed wrapper. |
...
| LiteLLM sidecar CPU request / limit (per gateway pod, proxy mode only) | 200m / 500m                | 500m / 1500m               | 1000m / 3000m              |
| LiteLLM sidecar memory request / limit (per gateway pod, proxy mode only) | 256Mi / 512Mi              | 512Mi / 1Gi                | 1Gi / 2Gi                  |
...
> **LiteLLM sidecar footprint.** The sidecar rows above apply only when `credentialPools[*].deliveryMode: proxy` is set and the sidecar is consequently rendered into the gateway pod ([§17.1](#171-kubernetes-resources)). The values are defaults; production deployments MUST re-measure under their own workload because LiteLLM's per-request CPU varies with model/provider mix. HPA targets (CPU and `request_queue_depth`) are evaluated at the **pod** level (summed across both containers) and do not need separate per-container HPA rules — if the sidecar saturates, the whole pod trips the HPA. The Go-rewrite-trigger signals in [§4.9](04_system-components.md#49-credential-leasing-service) LiteLLM sidecar hardening measure the sidecar's **share** of pod resources; operators watch those ratios, not absolute sidecar utilization, to decide whether the sidecar is carrying its weight.
...
> ⁵ **Tier 3 `billingRedisStreamMaxLen` derivation:** At Tier 3 the aggregate billing event rate is ~600/s. The MAXLEN must absorb events during the Postgres failover RTO (< 30s) with a 2x safety factor: `600 × 60 × 2 = 72,000`. At the default 50,000, the stream fills in ~83 seconds -- insufficient for any Postgres outage exceeding that duration. Tier 1/2 billing rates (~6/s and ~60/s respectively) are comfortably within the 50,000 default (fill time: ~2.3 hours and ~14 minutes).
