...
| Adapter ↔ Runtime (intra-pod)    | MCP (local Unix socket servers)                     | Platform tools, per-connector tool servers. See the `CH-MCP-PLATFORM` and `CH-MCP-CONNECTOR` cards in [Section 28.5.3](28_communication-channels.md#2853-intra-pod).                                                            |
...
**Elicitation content integrity (gateway-origin binding).** The gateway is the authoritative source for elicitation display text. At origination time the gateway records the original `{message, schema}` pair (the two required inputs of the `lenny/request_elicitation` tool defined in [Section 8.5](08_recursive-delegation.md#85-delegation-tools)) keyed by `elicitation_id` and the declared `origin_pod`, and the client UI is rendered from this stored original, rather than from whatever an intermediate hop re-emits. Intermediate pods forward elicitations upstream by `elicitation_id` only; they MAY observe the original content (for policy, logging, or deciding whether to suppress or dismiss) but MUST NOT modify the rendered text. In v1 the gateway resolves the elicitation chain internally: it walks the [Section 8](08_recursive-delegation.md) delegation tree from the raising session up to the resolver (an intercepting parent or the human-facing edge) and delivers the gateway-recorded original to that resolver. No intermediate pod re-emits the `{message, schema}` payload, so the gateway-origin binding holds by construction: the client renders only the gateway-recorded original, and no v1 wire path lets an intermediate hop substitute altered text. The gateway's content-integrity check is defined for any forward hop that does re-emit a frame: the gateway canonicalizes the re-emitted `{message, schema}` pair (stable key ordering, UTF-8 normalization) and compares its SHA-256 digest against the digest recorded at origination. v1 provides no per-hop re-emission wire mechanism; the native MCP `elicitation/create` frame of the [Section 15.2.1](15_external-api-surface.md#1521-restmcp-consistency-contract) per-kind wire projection is the gateway's client-facing projection rather than a pod-to-gateway re-emission path. No hop re-emits, so the digest check is the dormant enforcement point that activates when per-hop re-emission is introduced. The behavior on divergence is governed by the tenant's **elicitation content integrity enforcement mode**, clamped from below by a platform-level floor.

**Enforcement modes.** Each tenant carries an `elicitationContentIntegrityEnforcement` configuration ([Section 15.1](15_external-api-surface.md#151-rest-api) tenant admin API) whose `mode` is one of:

- **`enforce`** (tenant default). Any divergence is rejected with `ELICITATION_CONTENT_TAMPERED` (`PERMANENT`/409; see [Section 15.1](15_external-api-surface.md#151-rest-api)); the originating pod sees its pending elicitation fail with a tamper signal, the tampering pod receives this error on its forward call, the gateway emits the `elicitation.content_tamper_detected` audit event ([Section 16.7](16_observability.md#167-section-25-audit-events)) with `enforcement_mode: "enforce"`, and `lenny_elicitation_content_tamper_detected_total{origin_pod, tampering_pod, enforcement_mode="enforce"}` ([Section 16.1](16_observability.md#161-metrics)) is incremented, firing the `ElicitationContentTamperDetected` critical alert ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)).
- **`detect-only`.** The divergent payload is forwarded to the client as received (the gateway-recorded original is NOT substituted back in), but the gateway still emits the `elicitation.content_tamper_detected` audit event with `enforcement_mode: "detect-only"` and increments the same counter with the `enforcement_mode="detect-only"` label, firing the separate `ElicitationContentIntegrityPermissiveTamper` warning alert ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)). Intended as a pre-enforcement canary so operators can validate that no legitimate intermediate transformations trigger the detector before promoting the tenant to `enforce`.
- **`off`.** No canonicalization or SHA-256 comparison is performed; the forwarded frame is delivered without a tamper check, no audit event is emitted, and no counter is incremented. The counter carries no `enforcement_mode="off"` stream — the detector does not run.

**Platform floor.** The platform operator configures a minimum-enforcement floor via the Helm value `.Values.security.elicitationContentIntegrity.floor` (default `off`; see [Section 17.2](17_deployment-topology.md#172-namespace-layout)), rendered into the `lenny-deployment-phase-stamp` ConfigMap at install/upgrade time. Under the ordering `off < detect-only < enforce`, the effective mode for a tenant is `max(platform_floor, tenant_stored_mode)` — the stricter of the two wins. The tenant's stored mode is preserved across platform-floor changes; `GET /v1/admin/tenants/{id}/elicitation-content-integrity` returns both `storedMode` (the tenant's configured value) and `effectiveMode` (the resolved clamped value in force). A floor change that raises the effective mode for one or more tenants emits a `platform.elicitation_content_integrity_floor_changed` audit event for the platform transition and one `tenant.elicitation_content_integrity_floor_clamp` audit event per affected tenant ([Section 16.7](16_observability.md#167-section-25-audit-events)).

**Justification requirement.** A `PUT /v1/admin/tenants/{id}/elicitation-content-integrity` request that sets `mode` to `detect-only` or `off` MUST include a non-empty `justification` string; omitting it returns `400 ELICITATION_INTEGRITY_JUSTIFICATION_REQUIRED`. A request that would leave the tenant's stored mode strictly below the platform floor is rejected with `400 ELICITATION_INTEGRITY_BELOW_PLATFORM_FLOOR` — the stored mode is only accepted if it satisfies the floor or if the platform floor is `off`. The stored `mode`, `justification`, `changedAt`, and `changedBy` (the operator's OIDC `sub`) are recorded on the tenant and reported by the `tenant.elicitation_content_integrity_changed` audit event on every write. Whenever the **effective** mode is weaker than `enforce` — because the tenant configured a weaker mode, because the platform floor is weaker than `enforce` and the tenant did not strengthen it, or both — the standing `ElicitationContentIntegrityWeakened` warning alert fires continuously so operators remain aware of the reduced-integrity posture ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)).

To present transformed text (translation, rephrasing, audience-targeted summarization) to a different viewer, an intermediate pod MUST emit a new `lenny/request_elicitation` establishing a fresh `elicitation_id` and its own `origin_pod`; the gateway renders the new elicitation independently and records its content under the new ID, and the client UI presents each elicitation with its own provenance. This invariant — strictly enforced only when the effective mode is `enforce` — binds the user-visible text to the declared `origin_pod` for every elicitation the user sees, and extends the gateway-mediates-every-hop property already stated in this section to cover content as well as control flow. Under `detect-only` the binding is advisory (divergence is recorded but user-visible text is whatever the forwarding pod emitted); under `off` no binding check is performed.
