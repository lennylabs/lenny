...
**Elicitation content integrity (gateway-origin binding).** The gateway is the authoritative source for elicitation display text. At origination time the gateway records the original `{title, description, schema, inputs}` keyed by `elicitation_id` and the declared `origin_pod`, and the client UI is rendered from this stored original — not from whatever an intermediate hop re-emits. Intermediate pods forward elicitations upstream by `elicitation_id` only; they MAY observe the original content (for policy, logging, or deciding whether to suppress or dismiss) but MUST NOT modify the rendered text. If a forwarding pod emits a `lenny/request_elicitation` re-issuance that references an existing `elicitation_id` with divergent `{title, description, schema, inputs}`, the gateway drops the attempt with `ELICITATION_CONTENT_TAMPERED` (`PERMANENT`/409), emits the `elicitation.content_tamper_detected` audit event, and increments `lenny_elicitation_content_tamper_detected_total{origin_pod, tampering_pod}` ([Section 16.1](16_observability.md#161-metrics)); the `ElicitationContentTamperDetected` alert ([Section 16.5](16_observability.md#165-alerting-rules-and-slos)) pages on non-zero counts because any increment is an active prompt-injection or hostile-runtime signal. To present transformed text (translation, rephrasing, audience-targeted summarization) to a different viewer, an intermediate pod MUST emit a new `lenny/request_elicitation` establishing a fresh `elicitation_id` and its own `origin_pod`; the gateway renders the new elicitation independently and records its content under the new ID, and the client UI presents each elicitation with its own provenance. This invariant binds the user-visible text to the declared `origin_pod` for every elicitation the user sees, and extends the gateway-mediates-every-hop property already stated in this section to cover content as well as control flow.
