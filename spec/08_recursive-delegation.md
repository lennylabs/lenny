...
2. Gateway validates against parent's effective delegation policy and lease (depth, fan-out, budget). **Redis independence of depth and cycle checks:** `maxDelegationDepth` and cycle detection use the delegation lineage stored in the session record (Postgres-backed), not Redis counters. These checks remain fully enforced during Redis outages. `maxTreeSize` and `maxParallelChildren` depend on Redis-backed budget counters and are subject to the fail-closed behavior described in [Section 12.4](12_storage-architecture.md#124-redis-ha-and-failure-modes).
   ...
   **Retry semantics on `AUDIT_CONCURRENCY_TIMEOUT` during child minting.** If the per-tenant audit advisory lock ([§11.7](11_policy-and-controls.md#117-audit-logging) item 3) times out during the child-token exchange (the exchange's audit write cannot acquire the lock within `audit.lock.acquireTimeoutMs`), the exchange fails closed: no child token is issued, no child pod is allocated, and the parent's `lenny/delegate_task` call returns `DELEGATION_AUDIT_CONTENTION` (retriable). The parent agent MUST retry the **entire** `lenny/delegate_task` call rather than retrying only the token exchange step — this guarantees that the full admission pipeline (policy evaluation, cycle detection, interceptors, and the actor-token freshness check) re-runs on the retry, preserving correctness if the parent was rotated during the retry interval. The gateway internally retries the audit-write transaction `audit.lock.maxRetries` times before surfacing `DELEGATION_AUDIT_CONTENTION` to the caller, so in practice contention-driven retries are invisible to the client except under sustained Postgres pressure.
2a. **Cycle detection:** The gateway checks the full runtime lineage recorded in the delegation lease (the chain of resolved `(runtime_name, pool_name)` tuples from root to current node). If the resolved target's `(runtime_name, pool_name)` tuple appears anywhere in the caller's lineage, the gateway applies the cycle-detection decision matrix below before any pod allocation. Cycle detection uses runtime identity — not `session_id` — because the child session does not exist yet at this point (pod allocation happens in step 5). This catches runtime-identity cycles (e.g., A→B→A, where A and B are `(runtime_name, pool_name)` tuples) that `maxDepth` alone cannot prevent. `maxDepth` prevents infinite forwarding depth; cycle detection prevents circular wait deadlocks where the same runtime identity reappears in the delegation lineage. The [§8.8](#88-taskrecord-and-taskresult-schema) subtree deadlock detector covers subtree-internal circular waits (e.g., sibling sessions blocking on each other); lineage cycle detection covers the complementary case where a delegation chain revisits a runtime identity. **Pool-differentiated cycles** (e.g., `A/pool1 → B → A/pool2`) are intentionally **not** detected as cycles because pool differentiation is a legitimate deployment pattern — the same runtime binary may be deployed in different pools with different resource classes, isolation profiles, or credential configurations, and delegating across pools is a valid use case. `maxDepth` is the safety net that bounds any pool-differentiated delegation chain.

   **Cycle-detection decision matrix (three-layer AND gate under `mode: enforce`).** The gateway's behaviour on a detected lineage cycle is governed by two platform-wide Helm settings and two resource-level opt-ins (see [Section 8.3](#83-delegation-policy-and-lease) for the Helm settings documentation):

   1. **Outer gate — `gateway.cycleDetection.mode`** (`enforce` \| `warn` \| `permissive`, default `enforce`).
       - `enforce` (default) — the three-layer AND gate below decides whether to admit or reject. This is the only mode in which the `allowSelfRecursion` opt-ins have effect.
       - `warn` — the delegation is **always admitted** regardless of layer configuration; the gateway emits the `delegation.cycle_warning` audit event and increments `lenny_delegation_would_have_blocked_total{layer, mode="warn"}` for every layer that would have blocked under `enforce`. Intended for diagnostic rollouts; not recommended for production.
       - `permissive` — the delegation is **always admitted** and no event is emitted. Intended for development clusters only; the `CycleDetectionModeUnsafe` warning alert fires continuously while in effect.
   2. **Three-layer AND gate** (evaluated only when `mode: enforce`). A self-recursive hop — one where the resolved target's `(runtime_name, pool_name)` tuple already appears in the caller's lineage — is admitted **if and only if** all three layers opt in. If any layer is `false`, the delegation is rejected with `DELEGATION_CYCLE_DETECTED`:
       - **Layer 1 — Platform:** Helm value `gateway.allowSelfRecursion` (`yes` \| `no`, default `yes`). `no` is a master gate that rejects every self-recursive hop regardless of runtime or policy configuration.
       - **Layer 2 — Runtime:** the effective Runtime's `allowSelfRecursion` field (default `false`; see [Section 5.1](05_runtime-registry-and-pool-model.md#51-runtime)).
       - **Layer 3 — DelegationPolicy:** the effective `DelegationPolicy.allowSelfRecursion` field resolved at delegation time (default `false`; see [Section 8.3](#83-delegation-policy-and-lease)).
   3. **Error `details`.** When the gateway rejects with `DELEGATION_CYCLE_DETECTED`, `details.blockedBy` carries the first layer (in declared order `platform` → `runtime` → `policy`) whose value is `false`, and `details.effectiveSettings` carries the resolved `{mode, platform, runtime, policy}` tuple so operators can distinguish "which layer needs to be flipped" without reading three configs. `details.cycleRuntimeName` and `details.cyclePoolName` continue to identify the offending identity (see [Section 15.1](15_external-api-surface.md#151-rest-api) error catalog).
   4. **Audit events.** An admission under all three layers saying `true` emits `delegation.self_recursion_allowed` recording the `{runtime_name, pool_name, depth, effectiveSettings}` tuple for every hop. A rejection emits the usual admission-rejection audit pair; a `warn`-mode bypass emits `delegation.cycle_warning` carrying the would-have-blocking layer set. A change to `gateway.cycleDetection.mode` via Helm upgrade emits `gateway.cycle_detection_mode_changed` once per transition (see [Section 16.6](16_observability.md#166-operational-events-catalog) and [Section 16.7](16_observability.md#167-section-25-audit-events)).

   **Non-self-recursive cycles remain rejected unconditionally.** The three-layer AND gate applies only to hops whose target identity is one that already appears in the caller's lineage (true self-recursion). Cycles of the form `A → B → A` where the intermediate hop to `B` first introduced a different identity are also detected here — the gate is over lineage repetition of any single identity. There is no "allow cycles between unrelated runtimes" knob; only the hop that would close the cycle is evaluated against the three-layer gate, and only when the repeating identity is the one the delegation target resolves to.

2a-bis. **Effective `maxDepth` resolution (normative, always enforced).** Every effective delegation lease MUST carry a positive integer `maxDepth`. The gateway resolves it at lease issuance time by taking the **first** value present in the following list, in order, and applies it to every hop regardless of `gateway.cycleDetection.mode`:

   1. Explicit `delegationLease.maxDepth` supplied by the client at session creation (or propagated verbatim from the parent lease for children).
   2. The `maxDepth` of the resolved `delegationPresets` entry referenced by the lease (see [Section 8.3](#83-delegation-policy-and-lease) delegation presets).
   3. The `delegationLease.maxDepth` field of the effective Runtime's `defaultPoolConfig` / runtime-level default lease.
   4. The `maxDepth` declared on the effective `DelegationPolicy` (when present — a policy-level ceiling that cannot be widened by downstream overrides).
   5. The Helm fallback `gateway.delegation.defaultMaxDepth` (default `10`).

   This rule guarantees that no delegation chain can grow without bound under any mode: `mode: permissive` disables runtime-identity cycle checks but does not disable `maxDepth`. Child leases carry the resolved value forward; they may narrow (smaller integer) but never widen. The resolved value is recorded in `delegationLease.maxDepth` at lease issuance and is never re-resolved against live Helm values mid-tree.
...
contentPolicy:
  ...
  scanExportedFiles: false # when true, route each exported file through interceptorRef at PreExportMaterialization before the file is materialized in the child's workspace
  maxExportedFileSize: 10485760 # per-file byte ceiling when scanExportedFiles is true (default: 10 MiB); also caps the interceptor payload size
allowSelfRecursion: false # default false. Layer 3 of the §8.2 three-layer AND gate; admission requires platform=yes AND Runtime.allowSelfRecursion=true AND this field=true. Monotonic with §8.3 inheritance: child leases may narrow (true → false) but cannot widen (false → true).
```

**`contentPolicy` enforcement (prompt injection mitigation):** The gateway enforces `contentPolicy` on every `delegate_task` call. `maxInputSize` is a hard byte-size limit on `TaskSpec.input` — delegations exceeding it are rejected with `INPUT_TOO_LARGE` before pod allocation. When `interceptorRef` is set, the gateway invokes the referenced `RequestInterceptor` at the `PreDelegation` phase (see [Section 4.8](04_system-components.md#48-gateway-policy-engine)) with the full `TaskSpec.input` as payload. The interceptor can `ALLOW`, `REJECT`, or `MODIFY` the content. This is the primary hook for deployers to integrate external content classifiers (e.g., prompt injection detectors) into delegation chains. `contentPolicy` is inherited by child leases and can only be made stricter (smaller `maxInputSize`, same or more restrictive `interceptorRef`, `scanExportedFiles` transitioning only `false → true`, same or smaller `maxExportedFileSize`).

**`scanExportedFiles` enforcement (file-channel prompt injection mitigation).** When `scanExportedFiles: true`, the gateway routes **each** file enumerated by the parent's `delegate_task.fileExport` spec through the referenced `interceptorRef` at the `PreExportMaterialization` phase ([Section 4.8](04_system-components.md#48-gateway-policy-engine)) **before** persisting the file into the child's workspace. This closes the delegation-mediated prompt injection vector documented in [Section 8.7](#87-file-export-model): without this control, a compromised parent can smuggle attacker-controlled text into the child's context by writing it to an exported file (e.g., to a runtime-recognized instruction file such as `CLAUDE.md` or `AGENTS.md`). The interceptor can `ALLOW` the file (persist unchanged), `MODIFY` the bytes (e.g., strip instruction-file fragments, redact sensitive data; the gateway re-derives the file's stored `sha256` from the returned bytes), or `REJECT` the file (block the export — the delegation fails with `EXPORT_FILE_SCAN_REJECTED`). Normative rules:

1. **Interceptor dependency.** `scanExportedFiles: true` requires `interceptorRef: <name>`. A policy with `scanExportedFiles: true` and `interceptorRef: null` is rejected at admission (`POST`/`PUT /v1/admin/delegation-policies`) with `EXPORT_SCAN_REQUIRES_INTERCEPTOR`. There is no platform-default scan pipeline; per [Section 22.3](22_explicit-non-decisions.md), content classification is deployer-owned.
2. **Per-file ceiling.** Each file's byte length MUST NOT exceed `contentPolicy.maxExportedFileSize`. Files larger than this limit are rejected at materialization time with `EXPORT_FILE_SCAN_SIZE_EXCEEDED` before any interceptor call is made; the whole export fails. This bounds the `PreExportMaterialization` gRPC payload to a known ceiling (default: 10 MiB) and prevents single-file pathology from stalling the interceptor. `fileExportLimits.maxTotalSize` ([Section 8.7](#87-file-export-model)) continues to cap the aggregate export size independently.
3. **Failure mode.** Interceptor timeout or error on a per-file call follows the named interceptor's existing `failPolicy`:
    - `fail-closed` → that file is rejected → the export fails with `EXPORT_FILE_SCAN_UNAVAILABLE` (`TRANSIENT`, HTTP 503).
    - `fail-open` → that file is admitted and the gateway emits a `delegation.export_scan_failed_open` audit event ([Section 11.7](11_policy-and-controls.md#117-audit-logging)) carrying `interceptor_ref`, `file_path`, and `session_id`.
   The §8.3 rule-5 weakening cooldown on the interceptor applies unchanged — it is keyed on the interceptor's `failPolicy` transition, not on any individual phase.
4. **Scan ordering.** `PreExportMaterialization` fires **after** `PreDelegation` has passed for the enclosing `delegate_task` call and **before** the gateway writes any exported byte to the child's durable workspace storage. Per-file calls are issued sequentially in the order the files appear in the resolved `fileExport` spec; a `REJECT` on the first rejected file short-circuits the remaining files in the same export and the whole delegation is rolled back (no partial materialization).
5. **Metrics and audit.** Each scan outcome increments `lenny_export_file_scans_total{outcome}` ([Section 16.1](16_observability.md#161-metrics)) with `outcome ∈ {admitted, modified, rejected, failed_open, failed_closed}`. `REJECT` outcomes additionally emit a `delegation.export_file_scan_rejected` audit event.

**`scanExportedFiles` restrictiveness.** The field participates in the `contentPolicy` inheritance rule on child leases alongside `interceptorRef` and `maxInputSize`. Using the ordering `false < true` (tightening means "require scanning"):

| Parent | Child | Admission |
|---|---|---|
| `false` | `false` | permitted — same as parent |
| `false` | `true` | permitted — child tightens (strictly more restrictive) |
| `true` | `true` | permitted — same as parent |
| `true` | `false` | **rejected** with `CONTENT_POLICY_WEAKENING` — removing the scan requirement weakens the parent's posture |

`maxExportedFileSize` is a protective ceiling on payload size rather than a content-policy axis; child leases MAY set a **smaller** value than the parent (tighter bound). A child MUST NOT set a larger value than the parent; attempts to do so are rejected with `CONTENT_POLICY_WEAKENING`. When the child has `scanExportedFiles: false` and the parent has `false`, `maxExportedFileSize` is not evaluated (no scanning is performed) but is still validated against the parent's value at admission for consistency of the inherited policy.

**`scanExportedFiles` weakening cooldown.** A `true → false` transition of `scanExportedFiles` on a `DelegationPolicy` via `PUT /v1/admin/delegation-policies/{name}` is a weakening of the delegation-export content posture. To prevent a stolen or coerced admin credential from using a timing-observable flip to open a predictable bulk-bypass window, the gateway applies the same cooldown semantics specified for `failPolicy` weakening (rule 5 below): during `gateway.interceptorWeakeningCooldownSeconds` (default 60s, cluster-scoped, admin-immutable) following such a transition, `delegate_task` calls whose effective `DelegationPolicy` is the affected policy reject with `INTERCEPTOR_WEAKENING_COOLDOWN` (`TRANSIENT`, HTTP 503). The `transition_ts` is server-minted on the admin API and not admin-API-writable (same mechanism as `failPolicy` transitions; client-supplied values are rejected with `INTERCEPTOR_COOLDOWN_IMMUTABLE`). The audit event `interceptor.fail_policy_weakened` is not re-used for this transition — a dedicated `delegation_policy.export_scan_weakened` event ([Section 11.7](11_policy-and-controls.md#117-audit-logging)) records the `policy_name`, `old_scanExportedFiles`, `new_scanExportedFiles`, `transition_ts`, `cooldown_seconds`, and writer identity. Strengthening transitions (`false → true`) emit the companion `delegation_policy.export_scan_strengthened` event and take effect immediately without cooldown.
...
2. **Null-to-non-null:** The child sets an `interceptorRef` when the parent had `null`. This is always permitted (adding a check is strictly more restrictive).
3. **Non-null-to-null:** The child sets `interceptorRef: null` when the parent had a non-null reference. This is **rejected** with `CONTENT_POLICY_WEAKENING` — removing a content check always weakens policy.
4. **Different non-null reference:** The child names a different interceptor than the parent's. The gateway **rejects** this with `CONTENT_POLICY_INTERCEPTOR_SUBSTITUTION`. A child lease cannot substitute the parent's named interceptor with an unrelated one, because the platform cannot verify the substitute is equally or more restrictive. This rejection is unconditional — the platform does not accept out-of-band claims (e.g., registry annotations, deployer assertions) that a different interceptor internally "chains" the parent's; such claims are unverifiable and would defeat the identity-based monotonicity property this rule set upholds. Deployers who need composition MUST implement it inside a single named interceptor and keep the same `interceptorRef` across the delegation boundary (condition 1).
...
- **Tree recovery does not re-evaluate approved delegations.** When the gateway recovers a node after a pod failure ([Section 8.10](#810-delegation-tree-recovery)), it does not re-evaluate the node's lease against the **live** policy state — the recovering node resumes under the lease that was approved at original `delegate_task` time, even if a `DelegationPolicy` or `maxDelegationPolicy` referenced anywhere on the path from root to that node has since been narrowed. This is a direct consequence of the above rule: once a child session is running, its delegation was already approved, and a pod-level failure does not reopen that approval. See §8.10 for the full recovery-path policy contract.
...
- **`snapshotPolicyAtLease: true` option.** When set on the `DelegationPolicy`, the gateway snapshots the set of matching pool IDs for **all** `DelegationPolicy` resources in the effective policy computation — including `delegationPolicyRef`, `maxDelegationPolicy`, and any derived-runtime policy — at lease issuance time (when the root session starts) and records them in the delegation lease. All subsequent `delegate_task` calls in that tree evaluate policy against the snapshotted pool set rather than live labels. This provides stable, predictable delegation behavior for long-running trees at the cost of not picking up pool label changes mid-tree. The snapshot is stored as `snapshotted_pool_ids` in the lease record. Default: `false`. **Scope of snapshot:** `snapshotPolicyAtLease` snapshots the policy document and the set of matching pool IDs only — it does not snapshot the configuration of referenced interceptors (e.g., `contentPolicy.interceptorRef`). The interceptor named by `contentPolicy.interceptorRef` is always invoked with its live configuration at the time of each `delegate_task` call, regardless of whether the policy was snapshotted. This means that updating an interceptor's `failPolicy` (e.g., from `fail-closed` to `fail-open`) or its filtering rules affects all active delegation trees that reference it, even trees with snapshotted policies. Operators should be aware that the `interceptor.fail_policy_weakened` audit event ([Section 4.8](04_system-components.md#48-gateway-policy-engine)) surfaces such changes.
  ...
  **Interceptor configuration lifecycle (explicit rules).** To eliminate ambiguity about how interceptor config changes interact with delegation trees, the following rules are normative:

    1. **No snapshotting, no caching.** The gateway MUST NOT snapshot or cache interceptor configuration (`endpoint`, `failPolicy`, `timeout`, `priority`, or registration state) into the delegation lease. The interceptor registry ([Section 4.8](04_system-components.md#48-gateway-policy-engine)) is the single source of truth, read per invocation.
    2. **Detection mechanism.** Interceptor configuration changes are detected via the admin API (`PUT /v1/admin/interceptors/{name}`) and propagated to all gateway replicas through the standard config-reload bus used for `DelegationPolicy` and pool updates (see [Section 4.8](04_system-components.md#48-gateway-policy-engine)). The propagation SLO is ≤ 5 seconds cluster-wide; gateway replicas that have not yet observed the update continue to invoke the prior configuration until their local registry refreshes.
    3. **Application point — per-invocation, not per-delegation.** A `delegate_task` call that has already passed the `PreDelegation` interceptor phase is bound to the configuration that was live **at the time the interceptor fired for that call**. An in-flight call does not "see" a mid-call config change; subsequent `delegate_task` calls — including those issued later in the same delegation tree — re-read the registry and use whatever configuration is live when their `PreDelegation` phase executes. There is no notion of "interceptor config at delegation-submission time" versus "interceptor config at interceptor-firing time": the two are equivalent because `PreDelegation` fires synchronously within the `delegate_task` request path before pod allocation.
    4. **Retroactivity.** Interceptor config changes are **never retroactive** against already-approved delegations. An already-running child session whose `delegate_task` passed content inspection under the prior `failPolicy` is not re-inspected, re-approved, or cancelled when the config changes. The child's remaining lifetime is governed by its existing lease; subsequent `delegate_task` calls it issues use the new live config when they fire.
    5. **Oscillation and rollback semantics.** Toggling `failPolicy` between `fail-closed` and `fail-open` (in either direction, any number of times) never invalidates existing leases and never triggers retroactive rejection or approval of past delegations. Each `delegate_task` call is evaluated exactly once at the moment its `PreDelegation` phase runs, against the configuration then live; later oscillations of the same interceptor do not revisit that decision. The `interceptor.fail_policy_weakened` and `interceptor.fail_policy_strengthened` audit events (Section 11.7) record each transition so operators can reconstruct which configuration was in force for any given `delegate_task` timestamp.

       **Weakening cooldown.** To prevent an attacker (or a rushed operator) with admin-API access from using a timing-observable `fail-closed → fail-open` flip to create a predictable bulk-approval window — the gateway enforces a mandatory cooldown on every transition that weakens an interceptor's posture (specifically: any `fail-closed → fail-open` change on an interceptor named by the `contentPolicy.interceptorRef` of at least one active `DelegationPolicy`). During `gateway.interceptorWeakeningCooldownSeconds` (default: **60 s**, minimum: 30 s, maximum: 600 s) following such a transition, the gateway rejects every `delegate_task` and `lenny/send_message` call whose effective `DelegationPolicy` references the affected interceptor with `INTERCEPTOR_WEAKENING_COOLDOWN` (`TRANSIENT`, HTTP 503; see [Section 15.1](15_external-api-surface.md#151-rest-api)). The cooldown window is enforced uniformly across all gateway replicas — the transition timestamp is recorded in the shared interceptor registry alongside the new `failPolicy` value, and each replica computes `now - transition_ts < cooldownSeconds` at invocation time rather than relying on local clocks. Back-to-back weakening transitions do not stack: the cooldown window resets on each subsequent `fail-closed → fail-open` transition (but strengthening transitions `fail-open → fail-closed` never set a cooldown and never extend an in-flight cooldown). Rapid oscillation therefore cannot shorten the effective fail-open exposure window below `cooldownSeconds` — the first flip costs the attacker `cooldownSeconds` of rejected requests before any fail-open-exploitable gap opens, and the transition is always audited via `interceptor.fail_policy_weakened`. An `interceptor.weakening_cooldown_active` audit event is emitted once per cooldown window entry (not per rejected request), recording `interceptor_ref`, `transition_ts`, `cooldown_seconds`, `affected_policy_count`, and `affected_policy_names`. Strengthening transitions (`fail-open → fail-closed`) are not rate-limited and take effect immediately — tightening posture is always permitted without delay.

       **`transition_ts` is server-minted and admin-immutable (SEC-013).** The cooldown control is designed against the threat model of a stolen or coerced admin credential performing an unauthorized `fail-closed → fail-open` flip. To prevent that same credential from simultaneously collapsing the cooldown window to zero, the `transition_ts` field and the effective cooldown duration are not admin-API-writable:
           - **Server-minted timestamp.** On every `PUT /v1/admin/interceptors/{name}` request that effects a `fail-closed → fail-open` transition, the gateway mints `transition_ts` server-side from the gateway's monotonic clock-synchronized wall clock at the instant the new `failPolicy` is persisted. Any `transition_ts` value present in the request body is rejected with `INTERCEPTOR_COOLDOWN_IMMUTABLE` (`POLICY`, HTTP 400; see [Section 15.1](15_external-api-surface.md#151-rest-api)) before any state change is persisted — the request is rejected as a whole, not silently stripped, so that a compromised client cannot probe for acceptance. The registry schema separates `transition_ts` into a storage location that the admin-API write path does not expose as a writable field (logically a separate row / field set from the admin-editable interceptor configuration); even a direct admin-API PATCH on the interceptor cannot target it.
           - **Cooldown duration is cluster-scoped, not per-interceptor.** `gateway.interceptorWeakeningCooldownSeconds` is a gateway configuration value (Helm value / cluster config), not a field on the interceptor resource. There is no `cooldownSeconds` field in `POST /v1/admin/interceptors` or `PUT /v1/admin/interceptors/{name}`; the admin-API credential cannot change the cooldown duration. Changing it requires cluster-level operator access (Helm upgrade or equivalent config rollout), which is a distinct credential domain from `interceptors:write`.
           - **Meta-cooldown on configuration-level reductions.** When the cluster-scoped `gateway.interceptorWeakeningCooldownSeconds` value is reduced (e.g., via Helm upgrade from 120 s to 30 s), the shorter value does **not** apply to any pending cooldown — each in-flight cooldown is evaluated against the `cooldownSeconds` value that was in force at its `transition_ts`, which is recorded in the registry alongside the transition. This prevents an operator who obtains cluster-config write access from cutting short an already-active cooldown for a past weakening.
           - **Append-only audit.** Every `fail-closed → fail-open` transition emits an `interceptor.fail_policy_weakened` event (already defined in [Section 11.7](11_policy-and-controls.md#117-audit-logging)) carrying the writer's identity (`actor.user.uid`, `caller_kind`), the old and new `failPolicy` values, the server-minted `transition_ts`, the `cooldown_seconds` in force at transition time, and the `affected_policy_count` / `affected_policy_names`. The audit log is append-only and hash-chained (§11.7), providing the tamper-evident record of who weakened which interceptor and when. Rejected `INTERCEPTOR_COOLDOWN_IMMUTABLE` attempts are also recorded via the standard admin-API denial audit event, so a compromised credential attempting to forge `transition_ts` leaves a trace.
    6. **Deletion guard.** An interceptor referenced by any active (non-terminal) `DelegationPolicy` cannot be deleted; the admin API rejects such requests with `RESOURCE_HAS_DEPENDENTS` (same mechanism as the `DelegationPolicy` deletion guard earlier in §8.3). This removes the "dangling `interceptorRef`" failure mode from consideration.
...
**`maxDelegationPolicy` inheritance semantics (child lease construction).** When a parent calls `lenny/delegate_task`, the child's `maxDelegationPolicy` is resolved as follows:

1. **Child omits `maxDelegationPolicy` (field absent or explicitly `null`):** The child lease MUST inherit the parent's **effective** `maxDelegationPolicy` value verbatim — including the case where the parent's effective cap is itself `null` (no cap). A `null` on the child is never interpreted as "fresh policy context" or "remove the parent's cap"; it is always shorthand for "use the parent's effective cap." This is the fail-safe default and preserves least-privilege across the delegation tree.
2. **Child specifies a non-null `maxDelegationPolicy`:** The named policy MUST be at least as restrictive as the parent's effective `maxDelegationPolicy` (i.e., its rule set, when intersected with the parent's effective cap, MUST equal the child's named policy — no rule may grant access that the parent's cap denies). The gateway validates this at lease issuance time and rejects the delegation with `DELEGATION_POLICY_WEAKENING` (see [Section 15.1](15_external-api-surface.md#151-rest-api)) if the child's named policy would expand the effective authority beyond the parent's cap. A child MAY always choose a strictly stricter cap than the parent's.
3. **Parent's effective `maxDelegationPolicy` is `null`:** Any child value — `null` or any non-null named policy — is accepted (rule 2's "at least as restrictive" check is trivially satisfied when there is no parent cap).

This mirrors the non-null-to-null rule already defined for `contentPolicy.interceptorRef` above: removing or weakening a session-level cap is never permitted across a delegation boundary. The parent's **effective** cap (not its declared cap) is used for inheritance because the parent itself may have inherited a stricter cap from its own ancestors — children must inherit the transitively narrowest cap seen anywhere on the path from root to parent.

**Example:** A runtime has `delegationPolicyRef: "broad-policy"` (allows all platform runtimes). A session lease specifies `maxDelegationPolicy: "read-only-policy"` (allows only read-only agent runtimes). The effective policy allows only runtimes that satisfy both `broad-policy` AND `read-only-policy`. Child leases spawned from this session cannot expand beyond `read-only-policy` even if they reference `broad-policy` directly. A child that omits `maxDelegationPolicy` (or sets it to `null`) inherits `read-only-policy` as its effective cap.
...
**`messagingRateLimit`** — rate limits for `lenny/send_message`. `maxPerMinute` is a per-session outbound fixed-window per-minute burst limit; `maxPerSession` is a per-session lifetime cap. `maxInboundPerMinute` is a per-session **inbound** aggregate limit — the gateway enforces this on the **receiving** session, counting all messages arriving from any sender in the delegation tree. This prevents N compromised siblings from flooding a single target at N × `maxPerMinute`; regardless of the number of senders, the target accepts at most `maxInboundPerMinute` messages per fixed one-minute window. Because each window is fixed rather than sliding, a burst straddling a wall-clock-minute boundary can transiently reach up to twice a per-minute ceiling — twice `maxPerMinute` outbound, and twice `maxInboundPerMinute` in aggregate at a target regardless of sender count — before the window resets. Messages exceeding the inbound limit receive a `RATE_LIMITED` delivery receipt. Exceeding any limit returns `RATE_LIMITED`. Child leases inherit the parent's limits (or stricter). Defaults are deployment-configurable via Helm.

**`treeVisibility`** — controls the scope of the task tree visible to the session via `lenny/get_task_tree` ([Section 8.5](#85-delegation-tools)). Enum with three values, ordered from broadest to narrowest: `full` (the session sees the entire subtree rooted at the tree root, including siblings and their descendants), `parent-and-self` (the session sees only its own node and its direct parent's node), `self-only` (the session sees only its own node). Default: `"full"`. This field is the visibility boundary that pairs with the `messagingScope` resolution in [Section 7.2](07_session-lifecycle.md#72-interactive-session-model): `messagingScope: siblings` requires `treeVisibility: full`, and the gateway rejects `lenny/delegate_task` with `TREE_VISIBILITY_INSUFFICIENT_FOR_MESSAGING_SCOPE` when a child's resolved effective `messagingScope` is `siblings` and the child lease's `treeVisibility` is not `full` (see [Section 8.5](#85-delegation-tools)).

**`treeVisibility` inheritance semantics (child lease construction).** Mirroring the monotonicity rules for `minIsolationProfile`, `maxDelegationPolicy`, and `contentPolicy.interceptorRef`, a child lease's `treeVisibility` must be **at least as narrow** as the parent's effective `treeVisibility`. The ordering `full → parent-and-self → self-only` is strict: a child may narrow visibility at any hop, but may never widen it.

1. **Child omits `treeVisibility` (field absent):** The child lease inherits the parent's effective `treeVisibility` verbatim. The spec-defined default of `"full"` applies only at the root session (where no parent exists); below the root, absence means inheritance.
2. **Child specifies a `treeVisibility` at the same or narrower level than the parent's effective value:** Accepted. For example, a `full` parent may issue a `parent-and-self` or `self-only` child lease; a `parent-and-self` parent may issue a `self-only` child lease (but not a `full` one).
3. **Child specifies a `treeVisibility` broader than the parent's effective value:** The gateway **rejects** the delegation with `TREE_VISIBILITY_WEAKENING` (`POLICY`, HTTP 422; see [Section 15.1](15_external-api-surface.md#151-rest-api)) before pod allocation. The error `details.parentTreeVisibility` and `details.childTreeVisibility` identify both sides of the mismatch. This rejection is analogous to `CONTENT_POLICY_WEAKENING` and `DELEGATION_POLICY_WEAKENING` — widening a visibility boundary across a delegation boundary is never permitted, because a low-trust child could otherwise escape a parent-imposed narrowing by declaring a broader visibility on its own outgoing lease.

**`treeVisibility` snapshot and extension behavior.** `treeVisibility` is carried into the delegation lease record at lease issuance time and is **not** part of `snapshotPolicyAtLease` snapshotting — `snapshotPolicyAtLease` scopes to policy documents and pool sets only (see the snapshot-scope rule above), while `treeVisibility` is a per-lease visibility boundary that is already recorded on the lease itself. The field is also **not extendable** via lease extensions ([Section 8.6](#86-lease-extension)): like `maxDepth`, `minIsolationProfile`, `delegationPolicyRef`, and `perChildRetryBudget`, it is a security boundary rather than a resource budget. A subtree that requires broader visibility must be launched under a new lease issued by an ancestor whose effective `treeVisibility` is already at least as broad as the requested value — an extension cannot widen it.

**`treeVisibility` vs. `messagingScope` — delegation-time compatibility check (normative).** `messagingScope` is **not** a per-delegation lease field; it is resolved from the deployment/tenant/runtime hierarchy at session-creation time ([Section 7.2](07_session-lifecycle.md#72-interactive-session-model), "Effective scope" rule — narrowest of deployment `maxScope`, tenant `scope` if set, and top-most parent runtime `scope` if set). Therefore the `TREE_VISIBILITY_INSUFFICIENT_FOR_MESSAGING_SCOPE` check executes as follows at delegation time:
1. The gateway resolves the **child's** effective `messagingScope` per the §7.2 hierarchy, using the child runtime's configuration and the tenant/deployment settings that would apply to the child at session creation.
2. The gateway resolves the **child's** effective `treeVisibility` per the inheritance rules above (the child lease's declared value if present, otherwise the parent's effective value).
3. If the resolved effective `messagingScope` is `siblings` **and** the resolved effective `treeVisibility` is not `full`, the gateway rejects `lenny/delegate_task` with `TREE_VISIBILITY_INSUFFICIENT_FOR_MESSAGING_SCOPE` ([Section 15.1](15_external-api-surface.md#151-rest-api)) before pod allocation. The error `details` include `effectiveMessagingScope` (the resolved value that triggered the check), `effectiveTreeVisibility` (the resolved value that failed the compatibility requirement), and `requiredTreeVisibility: "full"` so SDK authors can identify which side produced the mismatch. Remediation is either (a) narrow the effective `messagingScope` (via deployment/tenant/runtime configuration) to `direct` for this child's runtime or tenant, or (b) leave the child lease's `treeVisibility` at `full` (do not declare a narrower value on the outgoing lease).

**Post-delegation configuration change — existing-tree behavior.** The `messagingScope` configuration hierarchy may change during a tree's lifetime (e.g., a tenant raises its `messaging.scope` from `direct` to `siblings` after the tree is already running). Per [Section 7.2](07_session-lifecycle.md#72-interactive-session-model), deployment- and tenant-level configuration changes do **not** re-evaluate active child sessions — once a delegation has been approved, the gateway does not retroactively reject it or alter its lease. If a post-delegation hierarchy change would produce an effective `messagingScope: siblings` for an active child whose lease carries `treeVisibility != full`, the gateway **preserves the existing lease's `treeVisibility`** and narrows the child's effective `messagingScope` to the strictest value consistent with that lease (`direct`, since `siblings` is the only scope that requires `treeVisibility: full`). The new wider `messagingScope` applies only to children whose leases are issued **after** the hierarchy change. This matches the "once approved, active sessions are not re-evaluated" principle already stated for delegation lease issuance and avoids a window where a running child observes a broader messaging capability than its visibility boundary permits.

**`allowSelfRecursion` inheritance semantics (child lease construction).** The `allowSelfRecursion` field of the effective `DelegationPolicy` is the third layer of the §8.2 cycle-detection AND gate. Like `minIsolationProfile`, `maxDelegationPolicy`, and `treeVisibility`, it is **monotonic across delegation hops**: a child lease's effective `allowSelfRecursion` may equal or be stricter than (i.e., transition `true → false`) the parent's effective value, but may never widen it (`false → true`).

1. **Child omits `allowSelfRecursion` (field absent):** The child lease inherits the parent's effective `allowSelfRecursion` verbatim. The default of `false` applies only at the root session; below the root, absence means inheritance.
2. **Child specifies a value at the same or stricter level than the parent's effective value:** Accepted. A `true` parent may issue a `true` or `false` child lease; a `false` parent may issue a `false` child lease.
3. **Child specifies `true` while the parent's effective value is `false`:** The gateway **rejects** the delegation with `DELEGATION_POLICY_WEAKENING` (`POLICY`, HTTP 422; see [Section 15.1](15_external-api-surface.md#151-rest-api)) before pod allocation. `details.field: "allowSelfRecursion"`, `details.parentValue: false`, and `details.childValue: true` identify the violation. This is the same error code used for `maxDelegationPolicy` weakening — `allowSelfRecursion` is part of the `DelegationPolicy` resource and inherits the same monotonicity discipline.

**`allowSelfRecursion` snapshot and extension behavior.** `allowSelfRecursion` is carried into the delegation lease record at lease issuance time. It is **not extendable** via lease extensions ([Section 8.6](#86-lease-extension)) — like `maxDepth`, `minIsolationProfile`, `delegationPolicyRef`, `perChildRetryBudget`, and `treeVisibility`, it is a security boundary, not a resource budget. A subtree that requires `allowSelfRecursion: true` must be launched under a new lease issued by an ancestor whose effective `allowSelfRecursion` is already `true`; an extension cannot widen it. `allowSelfRecursion` participates in `snapshotPolicyAtLease` snapshotting alongside the rest of the `DelegationPolicy` document — when `snapshotPolicyAtLease: true`, the value at root-lease issuance time is preserved for the entire tree even if the underlying `DelegationPolicy` is mutated later.

**Helm settings governing cycle detection and delegation depth.** The platform-wide knobs referenced by §8.2 are configured via Helm values (admin-API-immutable; changing them requires a `helm upgrade`):

| Helm value | Type | Default | Meaning |
|---|---|---|---|
| `gateway.cycleDetection.mode` | enum (`enforce` \| `warn` \| `permissive`) | `enforce` | Outer gate over cycle detection. `enforce` runs the §8.2 three-layer AND gate. `warn` admits all self-recursive hops and emits `delegation.cycle_warning`. `permissive` admits all self-recursive hops silently and raises the `CycleDetectionModeUnsafe` warning alert. |
| `gateway.allowSelfRecursion` | enum (`yes` \| `no`) | `yes` | Layer 1 of the three-layer AND gate. `no` is a master gate that forces every self-recursive hop to be rejected regardless of runtime or policy opt-in. |
| `gateway.delegation.defaultMaxDepth` | integer (≥ 1) | `10` | Last-resort fallback for the §8.2 effective `maxDepth` resolution chain. Guarantees every effective lease has a positive integer `maxDepth` even when none of the upstream sources (lease, preset, runtime, policy) sets one. Applied unconditionally — `mode: permissive` does not bypass this fallback. |

A change to any of these values via `helm upgrade` emits a corresponding audit event (`gateway.cycle_detection_mode_changed`, `gateway.allow_self_recursion_changed`, `gateway.default_max_depth_changed`) under the platform tenant. These events follow the same render-time emission discipline as `deployment.feature_flag_downgrade_acknowledged` ([Section 16.7](16_observability.md#167-section-25-audit-events)).
...
**Credential pool identity in multi-hop `inherit` chains (cross-environment compatibility check reference pool).** In a multi-hop delegation tree where a `delegate_task` hop uses `credentialPropagation: inherit`, the child does not receive a fresh credential pool reference — it receives the same **origin pool** that the parent is already using. That origin pool traces back through any number of contiguous `inherit` hops to the session at the top of the `inherit` chain (the session where `independent` was last used, the root session if the chain reaches the root, or no origin pool at all when a `deny` hop terminated the chain, because a `deny` hop grants no credentials and therefore establishes no origin pool for a descendant `inherit` hop to draw from). The provider compatibility check defined above always compares the **origin pool's** providers against the immediate target runtime's `supportedProviders` — never an intermediate parent's runtime, and never a "reconstructed" pool from any intermediate hop. Each hop re-checks the still-inherited origin pool at delegation time, so providers are validated independently at every environment boundary along the `inherit` chain.

**Worked multi-hop example:**

```
Root (Env A)                       ← assigned credential pool P_A (providers: {anthropic, openai})
  credentialPropagation: inherit
  delegates to →
    Child (Env B, cross-env)       ← check: P_A.providers ∩ ChildRuntime_B.supportedProviders
                                   ← inherits P_A (same origin pool)
      credentialPropagation: inherit
      delegates to →
        GrandChild (Env C, cross-env)  ← check: P_A.providers ∩ GrandChildRuntime_C.supportedProviders
                                       ← inherits P_A (still the same origin pool)
```

At the Child → GrandChild hop, the compatibility check compares **P_A's** providers against GrandChildRuntime_C's `supportedProviders` — **not** Env B's providers, **not** any pool that Child might have had if it had used `independent`, and **not** an intersection accumulated across hops. If at any hop in the `inherit` chain a parent instead uses `credentialPropagation: independent`, that hop establishes a new origin pool for its subtree, and subsequent `inherit` hops below it are checked against the new origin pool. This rule is a direct consequence of the per-hop model described earlier: each hop's `credentialPropagation` governs only that single hop, and `inherit` always forwards the parent's live pool reference unchanged.
...
**Extendable fields:** `maxChildrenTotal`, `maxParallelChildren`, `maxTokenBudget`, `maxTreeSize`, `perChildMaxAge`, `fileExportLimits`. Not extendable: `maxDepth`, `minIsolationProfile`, `delegationPolicyRef`, `perChildRetryBudget`, `treeVisibility`, `allowSelfRecursion` (these are security or reliability boundaries, not resource budgets).
...
3. **User approves:**
   ...
   - All requests tied to a single elicitation + cool-off period are processed as part of the approved batch. Each extension response includes the **granted amount** (`grantedTokenBudget`, `grantedChildren`, etc.) so the gateway LLM Proxy knows exactly what was granted. If a request's grant was capped, the response status is `PARTIALLY_GRANTED` with the actual amounts. If the grant is **zero** because the ceiling was already reached, the response status is `CEILING_REACHED` (not success) — the gateway LLM Proxy MUST treat this as a terminal condition and return `BUDGET_EXHAUSTED` to the runtime rather than re-requesting the extension. The gateway LLM Proxy MUST NOT re-request an extension after receiving `CEILING_REACHED`; doing so would create an infinite loop (each request returns zero grant, each LLM call fails, the proxy requests again). Extension response statuses: `GRANTED` (full amount), `PARTIALLY_GRANTED` (capped but non-zero), `CEILING_REACHED` (zero grant — ceiling hit), `REJECTED` (user denied).
...
4. **User rejects:**
   ...
   - **Durability:** The `extension-denied` flag and the rejection cool-off expiry timestamp are persisted to the `delegation_tree_budget` Postgres table (keyed by `root_session_id`) as part of the same periodic checkpoint transaction that records token and delegation budget counters (see [Section 11.1](11_policy-and-controls.md#111-admission-and-fairness)). On coordinator handoff, the new gateway replica reads these fields from Postgres before accepting any new extension requests for the tree. This ensures that a user rejection cannot be bypassed by a gateway restart or rolling update.
     - **Query semantics (handoff-safe):** Every extension request MUST read the `extension-denied` flag and `cool_off_expiry` for the affected subtree from Postgres before granting — the gateway MUST NOT serve these fields from an in-memory cache that could be stale across a coordinator handoff. If the flag is set and `cool_off_expiry > NOW()`, the gateway auto-rejects the request with `EXTENSION_COOL_OFF_ACTIVE` and does not enter the elicitation path. The newly elected replica therefore observes the denial on the very first extension request it processes, without needing to replay in-flight state from the predecessor.
     - **In-flight requests (atomic check):** Extension requests that began before the flag was set but have not yet committed MUST re-check the `extension-denied` flag and `cool_off_expiry` **inside the same Postgres transaction** that increments the tree budget counters. The read of the flag and the write of the counter increment are serialized under the row lock on the `delegation_tree_budget` row (keyed by `root_session_id`, with a row-scoped `subtree_id` key for subtree-specific fields). If the flag is found set within the transaction, the gateway rolls back the budget increment and returns `EXTENSION_COOL_OFF_ACTIVE` instead of `GRANTED`. This closes the race where a `REJECTED` outcome is persisted between an earlier flag read and the grant commit.
     - **Clock reference (UTC only):** `cool_off_expiry` is stored as `TIMESTAMPTZ` (UTC). All comparisons against the expiry MUST use the database's `NOW()` function (or `clock_timestamp()` for non-transactional checks) — the gateway MUST NOT compare `cool_off_expiry` to its local wall-clock (`time.Now()` in Go). This eliminates cool-off bypass or over-extension caused by clock skew between gateway replicas, and makes the cool-off window authoritative on the database regardless of which replica is elected.
...
- **Archive entries in exported files inherit the upload archive validators.** If an exported file is an archive (or if the child's `workspacePlan` references it via `uploadArchive`), the same normative limits defined in [§13.4](13_security-model.md#134-upload-security) / [§7.4](07_session-lifecycle.md#74-upload-safety) (max decompressed size, ratio, entry count, per-entry size, path depth and length, non-regular-entry rejection, symlink rules) apply when the child's gateway-mediated materialization unpacks it. Violations surface as `UPLOAD_ARCHIVE_LIMIT_EXCEEDED` on the child-session materialization path.
...
- **Optional per-file content scanning (`PreExportMaterialization`).** When the parent lease's effective `contentPolicy.scanExportedFiles` is `true` (see [§8.3](#83-delegation-policy-and-lease)), the gateway invokes `contentPolicy.interceptorRef` at the `PreExportMaterialization` phase ([§4.8](04_system-components.md#48-gateway-policy-engine)) once per exported file before persisting the file into the child's workspace. Files exceeding `contentPolicy.maxExportedFileSize` are rejected with `EXPORT_FILE_SCAN_SIZE_EXCEEDED` before any interceptor call fires. The interceptor may `ALLOW` (persist unchanged), `MODIFY` (persist the returned bytes; the gateway re-derives the file's `sha256` from them), or `REJECT` (fail the delegation with `EXPORT_FILE_SCAN_REJECTED`). Interceptor errors follow the referenced interceptor's `failPolicy`. When `scanExportedFiles: false` (the default), no interceptor call is made on the export path and the below "Security note" applies.

**Security note — exported files are untrusted input by default:** When the parent lease's `contentPolicy.scanExportedFiles` is `false` (the default), the gateway validates export structure (path bounds, size, file count, archive validators) but does not inspect file content. A compromised or manipulated parent agent can include files with adversarial content — including files that agent runtimes treat as instruction sources (e.g., `CLAUDE.md`, `.claude/settings.json`, `AGENTS.md`, `.cursorrules`). The `contentPolicy.interceptorRef` on `DelegationPolicy` covers `TaskSpec.input` (`PreDelegation` phase) and, when `scanExportedFiles: true`, each exported file (`PreExportMaterialization` phase) — but does not fire on exported files when `scanExportedFiles` is `false`. Deployers have three options for defending against this vector:

1. **Enable per-file scanning on the delegation policy.** Set `contentPolicy.scanExportedFiles: true` with a backing `contentPolicy.interceptorRef` that inspects file bytes at `PreExportMaterialization`. This is the platform-provided control and is the recommended choice for deployments that already run an external content classifier for `TaskSpec.input` — the same interceptor handles both code paths.
2. **Sandbox instruction files at the runtime layer.** Configure agent runtimes to ignore or sandbox instruction files injected from external sources (e.g., a runtime-level `ignoreInstructionFiles` allowlist). This is a runtime-author responsibility, documented in the runtime-author guide, and provides defense-in-depth when scanning is also enabled.
3. **Inspect at the workspace-plan boundary.** Front the child session creation with an interceptor that inspects the workspace plan's `inlineFile` entries before they are written to the child's workspace. This applies to deployments that do not use the delegation file-export path but materialize workspaces via explicit plans.

Child agent runtimes and deployers SHOULD treat all workspace files received via delegation as untrusted input regardless of which of the above options is active — no platform-side scanner replaces the runtime-layer hardening that is its own defense-in-depth layer.
...
The canonical task state machine is the session state machine ([Section 7.2](07_session-lifecycle.md#72-interactive-session-model)) under the name external protocols give it. A session is the only unit of execution and each session has exactly one execution, so an MCP or A2A Task is the protocol surface of a session; the nodes of the task tree ([Section 8.9](#89-task-tree)) are sessions, and `lenny/delegate_task` creates a child session. Lenny defines these states independently of any external protocol, and external protocol adapters map to and from them at the boundary. The `TaskRecord` and `TaskResult` schemas and the `taskId` field are the external-protocol-facing names Lenny keeps for the durable record of a session's execution: the record is the protocol surface of a session, and external protocol adapters map between it and the canonical session state machine at the boundary. Lenny's purely-internal identifiers for the same concern carry Lenny's own session vocabulary.
...
**Policy enforcement during recovery (no re-evaluation of existing delegations).** Tree recovery resumes nodes whose delegations were already approved at original `delegate_task` time. The gateway MUST NOT re-evaluate any node's `delegationPolicyRef`, `maxDelegationPolicy`, `contentPolicy`, `minIsolationProfile`, or other lease-scoped policy fields against the **live** policy state when bringing a node back up. Recovery uses the delegation lease record persisted at original approval time (via `SessionStore`), which includes the node's effective `maxDelegationPolicy` (after inheritance resolution per §8.3), its `delegationPolicyRef`, and — when `snapshotPolicyAtLease: true` was set on the root — the `snapshotted_pool_ids` set. This mirrors the §8.3 principle that "once a child session is running, its delegation was already approved": policy narrowing applied to an intermediate node's `maxDelegationPolicy` between original delegation and recovery does not retroactively invalidate leases for that node or for any of its descendants. The concrete implications by concern are:

- **Existing child resumption.** A recovering child session attaches to the new pod with its original lease unchanged. Policy changes that would have rejected the original `delegate_task` (e.g., the child's runtime no longer matches the parent's narrowed `maxDelegationPolicy`) do **not** cause the recovering child to fail — the lease is a capability that survives the pod loss.
- **New delegations spawned by a resumed node.** After recovery completes, a resumed node calling `lenny/delegate_task` performs policy evaluation using the **same rules as pre-failure**: if the root lease has `snapshotPolicyAtLease: true`, the snapshotted pool set and lease-captured policy document are used; if `snapshotPolicyAtLease: false` (default), live policy is evaluated at call time. Recovery does not change which branch applies — it simply resumes the lease that the resumed node already holds.
- **Live interceptor configuration still applies.** Consistent with the §8.3 scope-of-snapshot rule, `contentPolicy.interceptorRef` is always invoked with its **live configuration** at the time of each post-recovery `delegate_task` call, even when `snapshotPolicyAtLease: true`. Snapshotting covers the policy document and pool set only, not interceptor internals.
- **Policy deletion during recovery window.** A `DelegationPolicy` referenced by an active (non-terminal) lease cannot be deleted — §8.3's `RESOURCE_HAS_DEPENDENTS` guard blocks deletion while any lease (including recovering nodes) references it. Recovery therefore never encounters a dangling `delegationPolicyRef` or `maxDelegationPolicy` reference.
