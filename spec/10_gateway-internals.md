...
2. **Wait for in-flight checkpoints (tiered cap).** If any sessions coordinated by this replica have checkpoints in progress (triggered by periodic scheduling or pre-drain), the hook waits for those checkpoints to complete before proceeding. This prevents SIGKILL from interrupting a checkpoint upload, which would leave checkpoint state inconsistent. The wait uses a **tiered cap** based on the session's last measured workspace size, rather than a fixed 30-second cap (which is insufficient for sessions with hundreds of MB workspaces):
   ...
   **CRD validation rule — tiered cap + BarrierAck budget:** The `SandboxWarmPool` CRD admission webhook (`lenny-pool-config-validator` — see [Section 4.6.3](04_system-components.md#463-crd-field-ownership-and-write-boundaries)) enforces that the worst-case preStop budget does not exceed `terminationGracePeriodSeconds`. Specifically, the webhook rejects any pool configuration where:
...
**Playground cookie-to-bearer exchange (`playground.authMode=oidc` only).** The bundled web playground ([§27](27_web-playground.md)) accepts browser traffic via an OIDC authorization-code flow instead of a caller-managed bearer. The gateway stores the validated OIDC session behind an opaque, HttpOnly, `/playground/`-scoped cookie and mints short-lived MCP bearer tokens on demand through `POST /v1/playground/token`. The minted bearers are standard gateway session-capability JWTs produced by the same `JWTSigner` described above — they inherit KMS signing, key rotation, claim structure, and revocation semantics — with an added `origin: "playground"` claim. The complete flow (login, exchange, refresh, revocation, failure modes) is specified in [§27.3.1](27_web-playground.md#2731-oidc-cookie-to-mcp-bearer-exchange); no playground-specific codepath exists on the MCP WebSocket or admin APIs — only the cookie-auth endpoints are playground-specific.
...
| `noEnvironmentPolicy` | `deny-all` or `allow-all` | Governs runtime access for users with no environment membership (see [Section 10.6](#106-environment-resource-and-rbac-model) and [Section 11](11_policy-and-controls.md#11-policy-and-controls)). An omitted value MUST be treated as `deny-all` *only when explicitly defaulted by the Helm chart*; the gateway does not infer a default at runtime — the value must reach the gateway as an explicit setting so that a misconfigured chart (with the default stripped) fails closed at startup rather than silently running with undefined semantics. |
