...
| `uploadFile`    | `path`, `uploadRef`                                              | `mode` (octal string, default `0644`; see `mode` field notes below for the `^0[0-7]{3,4}$` pattern and bit-mask constraints)                                                                                                                                                                                                                                                    | Place a previously uploaded single file (via the upload API) at `path` under `/workspace/slots/{sessionId}/current/`.                                                                                                                                                          |
| `uploadArchive` | `pathPrefix`, `uploadRef`, `format` (`tar`, `tar.gz`, or `zip`)  | `stripComponents` (integer, default `0`)                                                                                                                                                                                                                                                                                                                                         | Extract a previously uploaded archive under `pathPrefix` relative to `/workspace/slots/{sessionId}/current/`.                                                                                                                                                                  |
  ...
  **`cursor-cli` runtime:**
  ...
  ```json
  {
    ...
    "properties": {
      ...
      "streamingMode":    { "type": "boolean", "default": true },
      "rulesFile":        { "type": "string", "description": "Relative path under /workspace/slots/{sessionId}/current to a rules file (e.g. .cursor/rules)" }
    },
    "additionalProperties": false
  }
  ```
