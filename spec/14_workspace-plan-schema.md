...
**`gitClone.auth` object.** When set, `auth.mode` must be `"credential-lease"` and `auth.leaseScope` must be one of the [§26.2](26_reference-runtime-catalog.md#262-shared-patterns-for-coding-agent-runtimes) credential-lease scopes — `vcs.github.read` for read-only clones (the common case) or `vcs.github.write` when the session will push back to the remote. The gateway resolves the scope against the tenant's configured Git-hosting credential pool ([§4.9](04_system-components.md#49-credential-leasing-service)) and performs the clone with a short-lived token; the `git` client inside the pod later uses an in-pod credential helper that calls the gateway's token endpoint for any subsequent operations (pull, push). Omit `auth` for public repositories.
