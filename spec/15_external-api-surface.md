...
| `GET`    | `/v1/admin/circuit-breakers`                                    | List all circuit breakers and their current state. Requires `platform-admin`. See [Section 11.6](11_policy-and-controls.md#116-circuit-breakers).                                                                                                                                                                                                                              |
| `GET`    | `/v1/admin/circuit-breakers/{name}`                             | Get state for a single circuit breaker. Requires `platform-admin`. See [Section 11.6](11_policy-and-controls.md#116-circuit-breakers).                                                                                                                                                                                                                                         |
| `POST`   | `/v1/admin/circuit-breakers/{name}/open`                        | Open (activate) a circuit breaker. Body: `{"reason": <string>, "limit_tier": "runtime"\|"pool"\|"connector"\|"operation_type", "scope": <tier-specific matcher>}`. Rejects with `422 INVALID_BREAKER_SCOPE` if `limit_tier` or `scope` is absent, malformed, or inconsistent with the breaker's persisted scope (scope is immutable across open→closed→open for the same `{name}`). Requires `platform-admin`. Emits `circuit_breaker.state_changed` audit event. See [Section 11.6](11_policy-and-controls.md#116-circuit-breakers). |
| `POST`   | `/v1/admin/circuit-breakers/{name}/close`                       | Close (deactivate) a circuit breaker. Body is empty. The persisted `limit_tier` and `scope` are retained. Requires `platform-admin`. Emits `circuit_breaker.state_changed` audit event. See [Section 11.6](11_policy-and-controls.md#116-circuit-breakers).                                                                                                                      |
