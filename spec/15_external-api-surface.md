...
| `PUT`    | `/v1/admin/tenants/{id}`                                        | Update a tenant (requires `If-Match`). A `workspaceTier: T4` update — whether a new promotion or an idempotent re-assert — runs the admin-time KMS availability probe described in [Section 12.5](12_storage-architecture.md#125-artifact-store); on success the response body carries the same `t4KmsLastProbeSuccessAt` timestamp documented under the `GET` endpoint above. The `complianceProfile` field is subject to the one-way ratchet `none < soc2 < fedramp < hipaa` ([§11.7](11_policy-and-controls.md#117-audit-logging) "Compliance profile downgrade ratchet") — a request that lowers the value is rejected with `422 COMPLIANCE_PROFILE_DOWNGRADE_PROHIBITED` ([§15.4](#154-errors-and-degradation)); legitimate wind-down requires `POST /v1/admin/tenants/{id}/compliance-profile/decommission` (below). The `workspaceTier` field is similarly ratcheted (stricter only) by [§12.9](12_storage-architecture.md#129-data-classification). |
...
| `GIT_CLONE_REF_UNRESOLVABLE`  | `PERMANENT` | 422         | Session creation rejected because the gateway could not resolve a `gitClone.ref` to an immutable commit SHA via `git ls-remote` at session creation. `details.url`, `details.ref`, `details.sourceIndex`, and `details.reason` (e.g. `network_error`, `auth_failed`, `ref_not_found`) are included. The caller must supply a ref that exists on the remote, or pre-resolve to a commit SHA. See [Section 14](14_workspace-plan-schema.md) `gitClone.ref` resolution. |
