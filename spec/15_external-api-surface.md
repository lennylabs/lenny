...
| `POST`   | `/v1/admin/circuit-breakers/{name}/close`                       | Close (deactivate) a circuit breaker. Body is empty. The persisted `limit_tier` and `scope` are retained. Requires `platform-admin`. `x-lenny-scope: "tools:circuit_breaker:write"`, `x-lenny-mcp-tool: "lenny_circuit_breaker_close"`, `x-lenny-category: "mutation"`. Emits `circuit_breaker.state_changed` audit event. See [Section 11.6](11_policy-and-controls.md#116-circuit-breakers).                                                                                                                      |
