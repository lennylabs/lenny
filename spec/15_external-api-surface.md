...
| `EPHEMERAL_CONTAINER_CRED_UID_FORBIDDEN` | `PERMANENT` | 422 | `pods/ephemeralcontainers` subresource request rejected by the `lenny-ephemeral-container-cred-guard` ValidatingAdmissionWebhook ([Section 17.2](17_deployment-topology.md#172-namespace-layout) admission-policies inventory item 13) because the proposed ephemeral container would reach through the agent pod's credential-file read boundary ([Section 13.1](13_security-model.md#131-pod-security) `lenny-cred-readers` membership boundary). `details.reason` carries the specific sub-code: `runAsUser_equals_adapter_uid`, `runAsUser_equals_agent_uid`, `cred_readers_gid_in_supplementalGroups`, `cred_readers_gid_in_runAsGroup`, `runAsUser_absent`, `runAsGroup_absent`, or `supplementalGroups_absent`. `details.targetPod` identifies the target pod namespace/name. Not retryable as-is — the caller must submit an ephemeral container whose `securityContext` explicitly sets `runAsUser`/`runAsGroup`/`supplementalGroups` to values outside the adapter UID, agent UID, and `lenny-cred-readers` GID. See [Section 17.2](17_deployment-topology.md#172-namespace-layout) admission-policies inventory and [Section 13.1](13_security-model.md#131-pod-security). |
