...
| `lenny-ctl admin tenants force-delete <id> --acknowledge-hold-override --justification <text>` | Force-delete a tenant despite active legal holds. Without `--acknowledge-hold-override`, the request is rejected with `TENANT_DELETE_BLOCKED_BY_LEGAL_HOLD` when holds exist. With the flag, Phase 3.5 legal-hold segregation re-encrypts held evidence under the platform-managed `legal_hold_escrow_kek` and migrates it to the legal-hold escrow bucket before Phase 4 / 4a tenant KMS destruction. Operator identity and justification are recorded in a `gdpr.legal_hold_overridden_tenant` audit event (retained under `audit.gdprRetentionDays`) and raise the `LegalHoldOverrideUsedTenant` warning alert. See [Section 12.8](12_storage-architecture.md#128-compliance-interfaces) tenant deletion lifecycle Phase 3.5. | `POST /v1/admin/tenants/{id}/force-delete` | `platform-admin` |
