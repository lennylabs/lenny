...
3. **`lenny up` / `lenny down` / `lenny status`** ([§24.19](#2419-local-stack)) manage the Embedded Mode single-binary stack ([§17.4](17_deployment-topology.md#174-local-development-mode-lenny-dev)): `lenny up` renders the production chart under a development profile and runs the gateway and controllers as pods in the embedded k3s on in-process in-memory stores, and the embedded k3s runs as a managed child process on Linux or as a container under Docker Desktop's Linux VM on macOS and Windows. These are local-only commands; they do not call any remote API.
...
| `lenny-ctl admin pools resume-reconciliation --pool <name>` | Clear `PoolScalingAdmissionStuck` state for a pool after the operator has corrected the Postgres configuration causing `lenny-pool-config-validator` rule-set-1 admission denials. Resets the PSC's consecutive-denial counter so the next reconciliation cycle re-attempts an apply ([§4.6.3](04_system-components.md#463-crd-field-ownership-and-write-boundaries)). | `POST /v1/admin/pools/{name}/resume-reconciliation` | `platform-admin` |
...
| `lenny-ctl admin credential-pools revoke-credential --pool <name> --credential <id> --reason <r>` | Emergency revocation of a single credential; affects all active leases (terminated in direct mode, denied in place in proxy mode). See [Section 4.9](04_system-components.md#49-credential-leasing-service) emergency revocation runbook. | `POST /v1/admin/credential-pools/{name}/credentials/{credId}/revoke` | `platform-admin` |
...
| `lenny-ctl admin circuit-breakers open <name> --limit-tier <runtime\|pool\|connector\|operation_type> --scope <key>=<value> --reason <text>` | Manually open a circuit breaker. The `--limit-tier` and `--scope` flags bind the breaker to the declared scope (e.g., `--limit-tier runtime --scope runtime=runtime_python_ml`); `operation_type` scope values are drawn from the closed set `uploads \| delegation_depth \| session_creation \| message_injection` ([Section 11.6](11_policy-and-controls.md#116-circuit-breakers)). | `POST /v1/admin/circuit-breakers/{name}/open` | `platform-admin` |
...
| `lenny-ctl admin tenants force-delete <id> --acknowledge-hold-override --justification <text>` | Force-delete a tenant despite active legal holds. Without `--acknowledge-hold-override`, the request is rejected with `TENANT_DELETE_BLOCKED_BY_LEGAL_HOLD` when holds exist. With the flag, Phase 3.5 legal-hold segregation resolves the tenant's `dataResidencyRegion` (or the deployment single-region default when unset) to a region-scoped `storage.regions.<region>.legalHoldEscrow` entry, re-encrypts held evidence under the region-scoped platform `legal_hold_escrow_kek` (`platform:legal_hold_escrow:<region>`), and migrates it to the region-scoped legal-hold escrow bucket before Phase 4 / 4a tenant KMS destruction. If the resolved region has no complete `legalHoldEscrow` entry or the regional escrow KMS/endpoint is unreachable, Phase 3.5 aborts fail-closed with `LEGAL_HOLD_ESCROW_REGION_UNRESOLVABLE` and raises the `LegalHoldEscrowResidencyViolation` critical alert. Operator identity and justification are recorded in a `gdpr.legal_hold_overridden_tenant` audit event (retained under `audit.gdprRetentionDays`) and raise the `LegalHoldOverrideUsedTenant` warning alert; a `legal_hold.escrow_region_resolved` INFO audit event records the residency decision per force-delete. See [Section 12.8](12_storage-architecture.md#128-compliance-interfaces) tenant deletion lifecycle Phase 3.5. | `POST /v1/admin/tenants/{id}/force-delete` | `platform-admin` |
...
|---------|-------------|----------|
| `lenny runtime init <name> --language {go\|python\|typescript\|binary} --template {chat\|coding\|minimal}` | Generate a new runtime skeleton in `./<name>/`. The emitted skeleton is determined by the **Language × Template matrix** below; two `binary` combinations are rejected with a structured error (see "Unsupported combinations"). | none (local) |
| `lenny runtime validate [<path>]` | Validate a runtime repository against the Runtime Adapter Specification ([§15.4](15_external-api-surface.md#154-runtime-adapter-specification)) and the `runtime.yaml` contract. Reads the optional `integrationLevel` field from `runtime.yaml` ([§5.1](05_runtime-registry-and-pool-model.md#51-runtime); defaults to `basic` if absent) as the **declared** level, then probes the running runtime for its **observed** level (CH-RUNTIMEOPS connect, MCP nonce handshake) and reports declared vs. observed. Runs in the current directory by default. Exits non-zero when `observed < declared`; exits zero with a WARN when `observed > declared`. Serves as the `lenny runtime validate` entry point for the Conformance Test Suite ([§15.4.6](15_external-api-surface.md#1546-conformance-test-suite)). | none (local) |
...
**Language × Template matrix.** Each cell specifies the integration level of the emitted skeleton, whether the runtime-author SDK ([§15.7](15_external-api-surface.md#157-runtime-author-sdks)) is imported, and the template source that drives the `runtime.yaml` and `main.<lang>` pre-wiring. The three SDK languages (`go`, `python`, `typescript`) support all three templates; the `binary` language supports only `minimal`.

| Language | `--template minimal` | `--template coding` | `--template chat` |
|---|---|---|---|
| `go` | **Standard**, SDK-present. Single-handler hello-world echo using the SDK's `Handler` interface from [§15.7](15_external-api-surface.md#157-runtime-author-sdks). `runtime.yaml` declares `integrationLevel: standard`, `capabilities.preConnect: false`, minimal `allowedResourceClasses: [small]`. No workspace pre-wiring. | **Full**, SDK-present. Template copied from the `claude-code` reference runtime ([§26.2](26_reference-runtime-catalog.md#262-shared-patterns-for-coding-agent-runtimes), [§26.3](26_reference-runtime-catalog.md#263-claude-code)) with `isolationProfile: sandboxed`, `/workspace/slots/{sessionId}/current` + `/workspace/output` conventions, `setupCommandPolicy: allowlist`, and the shared coding-agent `WorkspacePlan`. | **Full**, SDK-present. Template copied from the `chat` reference runtime ([§26.7](26_reference-runtime-catalog.md#267-chat)) with empty workspace, `setupCommandPolicy: mode: none`, and LLM-proxy `deliveryMode: proxy` credential wiring. |
| `python` | **Standard**, SDK-present. Equivalent of the Go `minimal` cell using the Python SDK (`lenny-runtime` PyPI package, [§15.7](15_external-api-surface.md#157-runtime-author-sdks)). | **Full**, SDK-present. Python variant of the §26.2 template; same workspace pre-wiring and policy defaults. | **Full**, SDK-present. Python variant of the §26.7 template. |
| `typescript` | **Standard**, SDK-present. Equivalent of the Go `minimal` cell using the TypeScript SDK (`@lennylabs/runtime-sdk` npm package, [§15.7](15_external-api-surface.md#157-runtime-author-sdks)). | **Full**, SDK-present. TypeScript variant of the §26.2 template. | **Full**, SDK-present. TypeScript variant of the §26.7 template. |
| `binary` | **Basic**, **no SDK**. Pure stdin/stdout JSON Lines skeleton (see "Basic-level skeleton" below). | **REJECTED** — `SCAFFOLD_UNSUPPORTED_COMBINATION` (see "Unsupported combinations"). | **REJECTED** — `SCAFFOLD_UNSUPPORTED_COMBINATION` (see "Unsupported combinations"). |

**Unsupported combinations.** `--language binary --template coding` and `--language binary --template chat` are rejected before any files are written, with exit code `5 SCAFFOLD_UNSUPPORTED_COMBINATION` and a stderr message pointing the author to this matrix and to [§15.4.3](15_external-api-surface.md#1543-runtime-integration-levels). The rationale is a hard dependency, not a stylistic choice:

- The `coding` template pre-wires the shared coding-agent workspace plan from [§26.2](26_reference-runtime-catalog.md#262-shared-patterns-for-coding-agent-runtimes), which requires the platform MCP server (for `lenny/output` and related tools) — a **Standard**-level capability per [§15.4.3](15_external-api-surface.md#1543-runtime-integration-levels). The `binary` language emits a Basic-level skeleton with no MCP client, so the coding template's pre-wired tool calls would fail at runtime.
- The `chat` template copies the `chat` reference runtime ([§26.7](26_reference-runtime-catalog.md#267-chat)), which is **Full**-level and relies on the SDK's credential-lease rebind loop for LLM-proxy access via `/run/lenny/slots/{sessionId}/credentials.json` rotations ([§15.7](15_external-api-surface.md#157-runtime-author-sdks)). A raw binary skeleton has neither the credential wrapper nor the CH-RUNTIMEOPS required to consume `credentials_rotated` signals, so the chat template would silently break on the first credential rotation.

Authors who need a Basic-level binary starting point use `--language binary --template minimal` (the only supported `binary` combination). Authors who want coding or chat pre-wiring choose any SDK language, which carries the MCP client and credential-lease helpers required by both templates.

**Exit codes.** `0` on successful skeleton generation; `2 INVALID_RUNTIME_NAME` when `<name>` contains characters outside `[a-z0-9-]` or exceeds the runtime-name length bound; `3 TARGET_DIRECTORY_EXISTS` when `./<name>/` already exists and `--force` is not set; `5 SCAFFOLD_UNSUPPORTED_COMBINATION` when the Language × Template pair is in the `REJECTED` cells above; `6 MISSING_REQUIRED_FLAG` when `--language` or `--template` is omitted. No network call is made, so there is no auth-related exit code.

**Basic-level skeleton emitted by `--language binary --template minimal`.** The `binary`/`minimal` combination deliberately emits a Basic-level-compliant skeleton with **no SDK imports** — the generated `main` file contains only the stdin/stdout JSON Lines loop from [§15.4.4](15_external-api-surface.md#1544-sample-echo-runtime) (read line → parse JSON → switch on `type` → write a response carrying the `sessionId` read from the inbound envelope → flush). The generated loop echoes that per-session identifier on the frames it emits in response, which is the Basic-level echo obligation stated in [§15.4.3](15_external-api-surface.md#1543-runtime-integration-levels) and asserted by the Basic-level category of the Conformance Test Suite ([§15.4.6](15_external-api-surface.md#1546-conformance-test-suite)). No `github.com/lennylabs/runtime-sdk-go`, no `lenny-runtime` Python package, no `@lennylabs/runtime-sdk` dependency appears in the generated Dockerfile or manifest. This preserves the Basic-level "zero Lenny knowledge" promise from [§15.4.3](15_external-api-surface.md#1543-runtime-integration-levels) for authors who prefer to implement the protocol directly. Authors who want SDK conveniences (typed handlers, CH-RUNTIMEOPS helpers, credential-lease refresh loop) instead choose `--language {go|python|typescript}` with any template, which emits the Standard- or Full-level SDK-based skeleton per the matrix above.
...
| Command | Description |
|---------|-------------|
| `lenny up` | Render the production chart under a development profile and start the in-cluster control plane (gateway, controllers, and seeded runtime pool) on the embedded k3s, with no separate Postgres, Redis, KMS, or OIDC process (on macOS and Windows the embedded k3s runs under Docker Desktop; see [§17.4](17_deployment-topology.md#174-local-development-mode-lenny-dev)). Idempotent. Prints the local gateway URL and the non-suppressible "Embedded Mode — NOT for production use" banner. |
...
| `lenny logs [<component>] [--follow]` | Tail merged logs, or filter to one of `gateway`, `controller`, `ops`, the k3s substrate, or `runtime-<name>`. |
| `lenny restart [<component>]` | Roll out a restart of one of the pod-backed components (`gateway`, `controller`, `ops`) as a Kubernetes Deployment rollout-restart through the embedded kubeconfig, without tearing down the rest of the stack. |
...
#### 24.19.1 Image Management

The Embedded Mode stack ships its own in-process containerd instance inside the single-node k3s cluster. Images built with the host Docker daemon (or any OCI-compliant builder) are not automatically visible to k3s; `lenny image import` bridges the host image store into the embedded containerd image store so a freshly built runtime image can be referenced by an `image:` field in `runtime.yaml` without pushing to a remote registry. This is the primary path exercised by the custom-runtime walkthrough ([§17.4](17_deployment-topology.md#174-local-development-mode-lenny-dev)).

| Command | Description | Min Role |
|---------|-------------|----------|
| `lenny image import <reference>` | Load the image `<reference>` (e.g. `my-agent:dev`) from the host Docker daemon into the embedded k3s containerd image store under namespace `k8s.io`. The command reads the image from the host daemon by default; use `--file <tar>` to load from a `docker save` tarball instead. Exits 0 on success. | none (local, Embedded Mode only) |
| `lenny image import <reference> --file <tar>` | Load the image from an OCI / Docker-format tarball (as produced by `docker save` or `skopeo copy --format oci`) into the embedded containerd store. Use in air-gapped or CI environments where the host Docker daemon is unavailable. | none (local, Embedded Mode only) |
| `lenny image import <reference> --namespace <ns>` | Override the target containerd namespace (default `k8s.io`, which is the namespace k3s pulls from). Rarely needed; reserved for debugging. | none (local, Embedded Mode only) |
| `lenny image list` | List images currently present in the embedded containerd image store. | none (local, Embedded Mode only) |
| `lenny image rm <reference>` | Remove an image from the embedded containerd image store. The removal is local to the embedded cluster; host Docker images are untouched. | none (local, Embedded Mode only) |

**Prerequisites.** Embedded Mode must be running (`lenny up` has completed and the embedded k3s is healthy). When importing from the host Docker daemon (no `--file`), the daemon socket must be reachable; on macOS and Windows this is Docker Desktop, on Linux this is `/var/run/docker.sock`.

**Exit codes.** `0` on successful import; `2 INVALID_IMAGE_REFERENCE` when `<reference>` does not parse as a valid OCI reference; `3 EMBEDDED_MODE_REQUIRED` when invoked outside Embedded Mode (same guard as `lenny token print`); `4 K3S_UNAVAILABLE` when the embedded containerd socket cannot be reached (typically because `lenny up` is not running or the cluster is mid-startup — retry after `lenny status` reports `gateway: healthy`).

**Example.** The custom-runtime walkthrough in [§17.4](17_deployment-topology.md#174-local-development-mode-lenny-dev) uses:

```bash
docker build -t my-agent:dev .
lenny image import my-agent:dev     # Loads into the embedded k3s containerd store (namespace k8s.io)
```

**Clustered Mode.** `lenny image import` is rejected in clustered deployments (exits `3 EMBEDDED_MODE_REQUIRED`). In clustered installs runtime authors push the image to a registry reachable by the worker nodes and reference it in `runtime.yaml` as usual — there is no in-cluster image-store equivalent.
