...
| `session.resumed(resumeMode, workspaceLost, workspaceRecoveryFraction?)`   | Session resumed from checkpoint or minimal state. `resumeMode` is one of `full` (full workspace restored from latest checkpoint), `conversation_only` (no workspace; conversation context only — see [§4.4](04_system-components.md#44-event--checkpoint-store) eviction fallback), or `partial_workspace` (workspace partially reconstructed from a partial checkpoint manifest — see [§10.1](10_gateway-internals.md#101-horizontal-scaling) partial-manifest path). `workspaceLost` is boolean. `workspaceRecoveryFraction` (optional, number 0.0–1.0) is present only when `resumeMode: "partial_workspace"` and reports the fraction of the last full checkpoint's workspace bytes that were recovered. |
...
resuming → cancelled                   (client/parent cancels mid-resume — DELETE /v1/sessions/{id}, parent cascade, or operator cancel; see snapshot-close semantics below)
resuming → completed                   (replayed event log shows the agent already emitted session_complete before the crash — see snapshot-close semantics below)
...
4. **Run terminal handling.** Credential lease release (§7.1 step 23), DLQ drain and `message_expired` notifications (§7.2 inbox-drain-on-terminal), `cascadeOnFailure` evaluation for children, and the `session_complete` / `session.cancelled` event emission all proceed exactly as for a terminal transition from `running`.
...
For queued messages that later expire, the gateway emits a `message_expired` event to the sender's event stream: `{ "type": "message_expired", "messageId": "msg_abc123", "reason": "dlq_ttl_expired" }`. The `reason` field draws from a canonical two-value enum defined alongside the `delivery_receipt` schema in [Section 15.4.1](15_external-api-surface.md#1541-adapterbinary-protocol): `dlq_ttl_expired` (pre-terminal DLQ TTL elapsed while the target was in a recovering state) and `target_terminated` (the target transitioned to a terminal state while messages were still buffered, covering both the in-memory/durable inbox drain and the DLQ drain described below).
