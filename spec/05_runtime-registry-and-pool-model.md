...
**Slot (session mode).** A session-mode slot is the unit of per-pod session capacity. It owns a workspace subtree, a credential lease, and a lifecycle, and it is identified by the identifier of the session bound to it. A pod holds between zero and `sessionPolicy.maxConcurrentSessions` slots; a warm pod serving no session holds none. `sessionPolicy.maxConcurrentSessions` is the ceiling on simultaneous slots, and `maxConcurrentSessions: 1` is the smallest value of that ceiling rather than a special case of it. A slot is a pod-side resource rather than a synonym for a session: leaked and failed slots are retained until the pod terminates and still count against pod occupancy. The identity is normative: the gateway mints one identifier at claim time, and that single value is both the session's identifier and its slot's identifier.
...
**Slot-identifier reclaim hold.** The adapter holds a slot's identifier from the deregistration of the slot's registry entry, which opens the hold in the same step under the registry critical section [Section 4.7.1](04_system-components.md#471-role-and-gateway-rpc-contract) states, until the cleanup that reclaims the slot has **completed**, which is when every act that cleanup owes the slot has returned without error. A removal of the slot's workspace directory that the adapter performs before every request still writing under the slot's identifier, such as a `Resume` restoring a checkpoint into that directory, has stopped writing is an act that did not return without error, whatever the removal itself returns, because it has not established that the directory is gone; the table above applies the rows keyed on a failed act to it. The hold outlasts the deregistration because the cleanup's remaining acts are addressed by the slot identifier rather than by the entry, and every attempt at the same session names the same identifier. While the identifier is held the adapter admits no request that [Section 4.7.1](04_system-components.md#471-role-and-gateway-rpc-contract) enumerates as governed by its admission rules and that its rule 1 (the pairing rule) admits, whether that request would create a registry entry under the identifier or resolve one, and refuses it as a transient condition so the caller retries, which is what refuses a [Section 7.4](07_session-lifecycle.md#74-upload-safety) mid-session upload still in flight when the cleanup opens the hold. `Shutdown` is the one request outside the hold, governed by the rule stated for it rather than by this one. A refusal is the only record the adapter makes of the hold: no report and no counter names it, and a bind refused this way is accounted by the gateway as an ordinary transient slot failure. The cleanup's close of the session on the pod's shared runtime process is bounded by the graceful window the reclaiming `Shutdown` carries when it carries one, by that request's own deadline when it carries none, and, for the [Section 10.1](10_gateway-internals.md#101-horizontal-scaling) hold-timeout termination, which runs under no request, by a graceful window of ten seconds. A release that runs its cleanup inside the RPC that requested it ends the hold before that RPC answers when that cleanup completes. The table above states the hold's outcome for each cleanup, and [Section 4.7.1](04_system-components.md#471-role-and-gateway-rpc-contract) states the bind attempt token the adapter stamps on a slot's registry entry and the reclaim that names it.

**Recycle lifecycle (`recycle.enabled: true`).** When the pod's occupancy reaches zero, the gateway patches the pod's `SandboxClaim` to `recycling` and the whole-pod boundary runs: the adapter removes every per-slot credential file `/run/lenny/slots/{sessionId}/credentials.json` (the credential purge precedes any deployer code; see scrub step 0 below), deployer-defined `cleanupCommands` execute in the pod's workspace base `/workspace`, after every ended session's per-slot tree and credential lease have been removed and with the pod-wide directories the deployer's own `setupCommands` created still present, the Lenny whole-pod scrub runs, and the adapter reports the binary outcome via `ReportPodScrub`. On a successful scrub on a `standard` or `in-place` pool the pod is held for its tenant through the claim's `reserved` state and serves the next session; on a `standard` or `in-place` preConnect pool the SDK re-warm runs after the scrub reports success and before the claim enters `reserved`, so every pod that reaches `reserved` or `idle` is SDK-warm ([Section 6.2](06_warm-pod-model.md#62-pod-state-machine)). A `vm-restart` pool instead retires the pod at the occupancy-zero boundary and the gateway provisions a fresh replacement (step 7 below), so a `vm-restart` preConnect pool projects `claimed → draining` after its scrub report rather than running the SDK re-warm leg or entering `reserved`. A session that ends in failure or a crash always retires its pod regardless of recycle settings. `setupCommands` run once per pod at start; per-session setup belongs in the runtime's initialization.

**Recycling and integration levels.** Pod recycling requires no runtime cooperation: the per-slot cleanup and the whole-pod scrub are adapter-executed and gateway-coordinated, with no CH-RUNTIMEOPS exchange between sessions. Recycling works at every integration level (Basic, Standard, and Full), and `recycle.maxSessionsPerPod` applies as configured at every level.

**Lenny scrub procedure.** The whole-pod scrub runs at the recycle boundary, when occupancy reaches zero on a recycling pod and the pod's claim is `recycling` ([Section 6.2](06_warm-pod-model.md#62-pod-state-machine)). The scrub has two phases: a **pre-cleanup credential purge** (step 0) executed by the adapter before `cleanupCommands`, and a **post-cleanup scrub sequence** (steps 1-6) executed after `cleanupCommands` finish. The adapter reports the scrub's binary outcome to the gateway via `ReportPodScrub` ([Section 4.7](04_system-components.md#47-runtime-adapter)); a missing report is bounded by a gateway-side timeout (`cleanupTimeoutSeconds` plus a grace period), after which the pod is retired. The gateway triggers the whole-pod scrub by sending the adapter the `Shutdown` RPC ([Section 4.7](04_system-components.md#47-runtime-adapter)) for the ending session carrying the **recycle disposition**, the pod identity (`podId`), and the pool's scrub parameters (`cleanupCommands` and `cleanupTimeoutSeconds`). The gateway sends this request after it has patched the claim to `recycling` and armed the missing-report timeout, keyed on `podId`. The adapter closes the ending session's runtime, keeps the pod process alive across the recycle boundary, runs the scrub asynchronously, and reports the outcome for `podId` via `ReportPodScrub` on its GatewayControl link. On a `standard` or `in-place` pool the pod then keeps the process alive and reuses it for the next session; a `vm-restart` pod keeps the process alive only long enough to run and report the scrub, then retires at the occupancy-zero recycle boundary. The gateway does not block the response on the scrub. The gateway captures the scrub parameters when the session binds, at the same instant it captures the pool's recycle setting, and delivers them on this request; the adapter holds no recycle state of its own.

**Step 0 (pre-cleanup, before `cleanupCommands`):** Remove every per-slot credential file `/run/lenny/slots/{sessionId}/credentials.json` remaining under `/run/lenny/slots/`. A credential file is a platform-managed security artifact rather than deployer session state, and every one MUST be purged before any deployer-defined code runs. This prevents `cleanupCommands` (which may be deployer-defined and potentially untrusted or buggy) from reading a previous session's credential file. If `cleanupCommands` require credential metadata for custom audit logging, the adapter exposes sanitized metadata (provider name, lease ID) in the cleanup environment variables `LENNY_PREV_CREDENTIAL_PROVIDER` and `LENNY_PREV_LEASE_ID` rather than leaving the full credential file accessible.
...
2. Remove every per-slot workspace tree under `/workspace/slots/` (`rm -rf /workspace/slots/*`).
...
6. Verify scrub by stat-checking `/workspace/slots/`, `/tmp`, `/dev/shm`, and `/run/lenny/slots/` — if any path is non-empty after scrub (or if any per-slot credential file `/run/lenny/slots/{sessionId}/credentials.json` still exists despite step 0), the scrub is marked failed.
...
**Kata/microvm scrub variant.** When `recycle.scrubProfile: vm-restart` is set (permitted on any `isolationProfile: microvm` pool), the standard Lenny scrub (steps 1-6 above) is insufficient because the guest VM itself persists across sessions: the guest kernel's DNS resolver cache, TCP connection tracking state, kernel buffer/page cache, and in-memory filesystem metadata survive the scrub. So the scrub procedure includes an additional step after step 6:
...
- **Session count limit:** The pod's served-session count reaches `recycle.maxSessionsPerPod`. On a single-session pool the pod transitions to `draining` after the current session ends and the whole-pod scrub finishes. On a concurrent pool (`maxConcurrentSessions > 1`) whose `scrubProfile` is not `vm-restart` the pod transitions to `draining` on the session release that drives the served-session count to `maxSessionsPerPod`, decoupled from the whole-pod scrub because a persistently `leaked` slot can hold total occupancy above zero indefinitely; the pod drains its remaining slots gracefully and a replacement is provisioned from the warm pool. `maxSessionsPerPod` is required with no default when `recycle.enabled: true`; the deployer must make an explicit choice based on the workload's sensitivity and the residual state vectors enumerated above. On a `vm-restart` pool `maxSessionsPerPod` remains a required, validated field but does not trigger the retire: every `vm-restart` pod retires at the occupancy-zero recycle boundary with the non-counting `vm_restart_reprovision` reason regardless of `maxSessionsPerPod`, because the `vm-restart` reprovision disposition takes precedence over the session-count trigger. This holds at the `maxSessionsPerPod: 1` boundary, where a sequential `vm-restart` pod's served-session count read back at the recycle boundary equals `maxSessionsPerPod` (1) when the whole-pod scrub reports: the session-count trigger would fire there, but the preceding `vm-restart` reprovision retires first, so the retire reason and the `lenny_gateway_pod_retirement_total` increment do not diverge from a `maxSessionsPerPod >= 2` pool. The field still bounds pool sizing: on a concurrent `vm-restart` pool (`maxConcurrentSessions > 1`) it bounds the number of sessions the pod serves in its single occupancy cycle before the retire.
- **Uptime limit:** The pod's wall-clock uptime since first boot exceeds `recycle.maxPodUptimeSeconds`. The WarmPoolController derives the pod's uptime from its `CreationTimestamp` and level-triggers the transition to `draining` once the limit is exceeded, regardless of session activity; the gateway's `expiredByUptime` check freezes new placement onto an over-uptime pod, and the pod's current sessions drain before termination. `maxPodUptimeSeconds` is optional; if omitted, only `maxSessionsPerPod` and `maxScrubFailures` govern retirement.
...
If `acknowledgeBestEffortScrub` is absent or `false` on a pool with `recycle.enabled: true`, the pool controller rejects the pool definition at validation time with a descriptive error referencing this section. Similarly, `maxSessionsPerPod` is required with no default when recycling is enabled; the pool controller rejects recycling pool definitions that omit it, forcing the deployer to make an explicit reuse-limit choice appropriate to their workload. On a `vm-restart` pool `maxSessionsPerPod` remains required and validated but inert, because every `vm-restart` pod retires at each occupancy-zero recycle boundary regardless of the configured limit (see the Pod retirement policy above). This required-but-inert state is a deliberate, documented outcome: pool and admission validation continue to require the field on every recycling pool without a `vm-restart` carve-out.
...
**Concurrent sessions (`maxConcurrentSessions > 1`).** Setting `sessionPolicy.maxConcurrentSessions` above 1 multiplexes simultaneous sessions onto a single pod, each in its own slot. Each slot gets its own workspace under `/workspace/slots/{sessionId}/` (see [Section 6.4](06_warm-pod-model.md#64-pod-filesystem-layout) for the full per-slot filesystem layout). The gateway tracks per-slot lifecycle. Message delivery uses `sessionId` multiplexing over stdin: the gateway mints a slot's identifier at claim time, and the adapter creates the per-slot directory tree on first reference to that identifier and sets the slot's `cwd` to `/workspace/slots/{sessionId}/current/`; the runtime implements a dispatch loop keyed on `sessionId`; all binary protocol messages (inbound and outbound) in the session-scoped frame set carry `sessionId` on every pod, whatever the pool's `sessionPolicy.maxConcurrentSessions`, and a session-scoped request whose session identifier is empty is rejected at the adapter boundary with `InvalidArgument` before any root is resolved. Cross-slot isolation is process-level and filesystem-level, which is weaker than the one-session-per-pod configuration.
...
A service-mode slot is a different thing from a session-mode slot. It is unnamed per-pod request capacity: a counted in-flight request with no identifier, no session binding, no workspace subtree, no credential lease, and no tracked lifecycle.
...
- **`session`**: `mode_factor = expected_sessions_per_pod_lifetime`. In the default configuration (`maxConcurrentSessions: 1`, `recycle.enabled: false`) each pod serves exactly one session and `mode_factor = 1.0`. On a recycling pool a pod serves multiple sessions before replacement: if a pod typically handles 10 sessions before retirement, the pool needs ~1/10th the pods to serve the same request volume. Measured via the `lenny_pod_session_reuse_count` histogram (p50). **Formula assumption:** the `mode_factor` estimate converges toward the configured `recycle.maxSessionsPerPod` for predictable workloads where pods are not retired early by `maxPodUptimeSeconds` or `maxScrubFailures`. For variable workloads where early retirement is common, use the observed `lenny_pod_session_reuse_count` p50 rather than `recycle.maxSessionsPerPod` as the estimate. A sequential `vm-restart` pool (`maxConcurrentSessions: 1`) serves exactly one session per pod at the occupancy-zero recycle boundary and then retires, so its steady-state `mode_factor` is `1.0` (equivalently, the observed `lenny_pod_session_reuse_count` p50 is 1) rather than converging toward `recycle.maxSessionsPerPod`. A concurrent `vm-restart` pool (`maxConcurrentSessions > 1`) serves the sessions that overlap in its single occupancy cycle before the retire, so its `mode_factor` is above `1.0` and bounded above by `recycle.maxSessionsPerPod`; the PoolScalingController uses the observed `lenny_pod_session_reuse_count` p50 for it, consistent with the variable-workload guidance above.
