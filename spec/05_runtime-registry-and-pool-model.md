...
- **Session count limit:** The pod's served-session count reaches `recycle.maxSessionsPerPod`. After the current session ends and the whole-pod scrub finishes, the pod transitions to `draining` instead of returning to `idle`. A replacement pod is provisioned from the warm pool. `maxSessionsPerPod` is required with no default when `recycle.enabled: true`; the deployer must make an explicit choice based on the workload's sensitivity and the residual state vectors enumerated above. On a `vm-restart` pool `maxSessionsPerPod` remains a required, validated field but does not trigger the retire: every `vm-restart` pod retires at the occupancy-zero recycle boundary with the non-counting `vm_restart_reprovision` reason regardless of `maxSessionsPerPod`, because the `vm-restart` reprovision disposition takes precedence over the session-count trigger. This holds at the `maxSessionsPerPod: 1` boundary, where a sequential `vm-restart` pod's served-session count read back at the recycle boundary equals `maxSessionsPerPod` (1) when the whole-pod scrub reports: the session-count trigger would fire there, but the preceding `vm-restart` reprovision retires first, so the retire reason and the `lenny_pod_retirement_total` increment do not diverge from a `maxSessionsPerPod >= 2` pool. The field still bounds pool sizing: on a concurrent `vm-restart` pool (`maxConcurrentSessions > 1`) it bounds the number of sessions the pod serves in its single occupancy cycle before the retire.
