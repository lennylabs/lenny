...
Pre-attached failure transitions:
  ...
  starting_session ──→ failed            (retries exhausted — see "Pre-attached failure retry policy" below; surfaces to client as session-level `starting → failed` per §7.2)
  starting_session ──→ resume_pending    (post-attached visibility: client observed `starting` state and pod crashed; surfaces to client as session-level `starting → resume_pending` per §7.2 when retries remain)
...
Resuming failure transitions (authoritative enumeration per §7.2 snapshot-close cross-reference):
  ...
  resuming ──→ cancelled             (client terminate / parent cancel / admin cancel arrives during snapshot replay — abort snapshot, skip live seal, release replacement pod, emit status_change(cancelled), run terminal cleanup; see [§7.2](07_session-lifecycle.md#72-interactive-session-model) "Mid-resume terminal transitions — snapshot-close semantics")
  resuming ──→ completed             (terminal session outcome detected during snapshot replay — replayed event log shows agent emitted session_complete before the crash that caused resume_pending; abort snapshot, skip live seal, release replacement pod, emit status_change(completed), persist terminal state; see [§7.2](07_session-lifecycle.md#72-interactive-session-model) "Mid-resume terminal transitions — snapshot-close semantics")

Pre-attach terminal collapse (resume_pending, no pod claimed):
  resume_pending ──→ cancelled       (client POST /terminate, DELETE, parent cancel cascade, or admin cancel arrives before a replacement pod is claimed — no pod attached, no snapshot-close sequence, no CoordinatorFence round-trip; see [§7.2](07_session-lifecycle.md#72-interactive-session-model) "Pre-attach terminal collapse" and [§15.1](15_external-api-surface.md#151-rest-api) POST /terminate preconditions)
  resume_pending ──→ completed       (terminal session outcome materializes before a replacement pod is claimed — e.g., rehydrated budget/lease exhaustion on re-enqueue, TTL wall-clock check, or parent cascade resolving to graceful completion; no pod attached, no snapshot-close, no CoordinatorFence; see [§7.2](07_session-lifecycle.md#72-interactive-session-model) "Pre-attach terminal collapse")
...
Task-mode state transitions (from attached):
  ...
  task_cleanup ──→ idle              (non-preConnect: scrub succeeds, maxTasksPerPod not reached, uptime limit not reached)
  task_cleanup ──→ idle [scrub_warning] (non-preConnect: scrub fails, onCleanupFailure: warn, maxScrubFailures not reached)
  ...
  task_cleanup ──→ draining          (preConnect: true, scrub succeeds, maxTasksPerPod not reached, maxPodUptimeSeconds not reached, host node is unschedulable — SDK re-warm on a cordoned node would produce a soon-to-be-evicted idle pod; see "Host-node schedulability precondition" below)
  task_cleanup ──→ draining [scrub_warning] (preConnect: true, scrub fails with onCleanupFailure: warn, maxScrubFailures not reached, maxTasksPerPod not reached, maxPodUptimeSeconds not reached, host node is unschedulable — same rationale as above, scrub_warning annotation persists on the pod through drain; see "Host-node schedulability precondition" below)
  task_cleanup ──→ sdk_connecting    (preConnect: true, scrub succeeds, maxTasksPerPod not reached, maxPodUptimeSeconds not reached, host node is schedulable — re-warm SDK before returning to idle; see "preConnect re-warm on scrub_warning" note below and "Host-node schedulability precondition" note for the schedulable definition)
  task_cleanup ──→ sdk_connecting [scrub_warning] (preConnect: true, scrub fails with onCleanupFailure: warn, maxScrubFailures not reached, maxTasksPerPod not reached, maxPodUptimeSeconds not reached, host node is schedulable — pod re-warms SDK before returning to idle and the scrub_warning annotation persists on the pod)
...
**Host-node schedulability precondition.** The "host node is schedulable" precondition on both the `task_cleanup → sdk_connecting` and `task_cleanup → sdk_connecting [scrub_warning]` transitions is defined as the pod's host node (`spec.nodeName`) having `.spec.unschedulable == false` in the Kubernetes Node object (equivalently: no `node.kubernetes.io/unschedulable` taint applied by the scheduler when `kubectl cordon` or an autoscaler drain marks the node). The evaluation is performed by the WarmPoolController — not the gateway — and its result is surfaced to the gateway as the pod label `lenny.dev/host-schedulable` (values: `"true"` when the host node is schedulable, `"false"` otherwise). The WarmPoolController maintains this label on every reconcile from its Node informer cache and re-labels all pods on an affected Node within a single reconcile cycle when the Node's cordon/uncordon state changes; see [Section 4.6.1](04_system-components.md#461-warm-pool-controller-pod-lifecycle) "Host-node schedulability labeling (`lenny.dev/host-schedulable`)" for the controller-side contract (including the per-reconcile label maintenance and the cordon/uncordon re-label latency) and the rationale for keeping cluster-scoped Node reads out of the gateway's RBAC surface. The gateway reads this label via its existing `get` access on the `Pods` resource in agent namespaces at the moment of the `task_cleanup` transition — it does not re-check during SDK re-warm, holds no Node verbs on its ServiceAccount, and needs no new RBAC grants ([Section 4.6.3](04_system-components.md#463-crd-field-ownership-and-write-boundaries) "Gateway ServiceAccount RBAC grants"). If the label reads `"false"` (or is absent, which is treated as unschedulable for fail-safe behavior), the pod transitions to `draining` instead (even though `maxTasksPerPod`, `maxPodUptimeSeconds`, and `maxScrubFailures` have not been reached), because SDK re-warm on a cordoned node would produce an idle pod whose next eviction is imminent. The rule applies identically to the scrub-success and scrub-warning preConnect edges: a pod carrying a `scrub_warning` annotation on a cordoned node must not re-enter the warm pool, because returning it to `idle` (even SDK-warm) would hand a soon-to-be-evicted pod to the next claim. This rule applies to all preConnect pools; non-preConnect pools do not re-warm after scrub and are therefore not affected by node schedulability at `task_cleanup` time (their `idle → draining` transition on eviction is handled by the standard pod-disruption path).

**preConnect re-warm on scrub_warning.** Section 6.1 establishes the invariant that all pods in a `preConnect: true` pool are SDK-warm when idle. To preserve this invariant for task-mode pools that use `onCleanupFailure: warn`, a pod whose scrub fails (but has not exhausted `maxScrubFailures`, `maxTasksPerPod`, or `maxPodUptimeSeconds`) routes through `sdk_connecting` before returning to `idle`, exactly as a successful-scrub pod does. The `scrub_warning` annotation and the cumulative `lenny_task_pod_scrub_failure_count` metric ([Section 5.2](05_runtime-registry-and-pool-model.md#52-pool-configuration-and-execution-modes)) persist through the re-warm — the SDK re-initialization does not clear the warning, since the residual-state risk is orthogonal to SDK readiness. If SDK re-warm itself fails during `sdk_connecting`, the pod transitions to `failed` via the standard `sdk_connecting → failed` path (the `scrub_warning` annotation is retained in the pod's audit trail). In non-preConnect pools, the direct `task_cleanup → idle [scrub_warning]` transition applies. This ensures no mixed-state inventory: every idle pod in a preConnect pool is SDK-warm regardless of whether the prior task's scrub succeeded cleanly or produced a warning.
...
**`resuming` failure transitions:** A pod crash, gRPC error, or workspace-restoration hang while the gateway is restoring a session onto a new pod must not leave the session permanently stuck in `resuming`. This subsection is the **authoritative enumeration** of every edge out of `resuming` (per iter3 SES-013); the §7.2 session-state listing cross-references back here. The gateway applies the following transitions:
...
The `maxIdleTimeSeconds` timer fires the `expired` transition independently of `maxSessionAge`. The first timer to fire wins. `maxIdleTimeSeconds` is a pool-level field in `RuntimeDefinition` `limits:` block (default: 600s, per [Section 17.8](17_deployment-topology.md#178-capacity-planning-and-defaults) operational defaults table). **Origin-scoped override:** sessions minted with the `origin: "playground"` JWT claim are subject to a tighter hard cap — the gateway enforces `min(runtime.limits.maxIdleTimeSeconds, playground.maxIdleTimeSeconds)` (default 300s) for these sessions to bound reclamation time after a best-effort browser-close cancel. The claim is stamped by the `/playground/*` ingress path under all three `playground.authMode` values (`oidc`, `apiKey`, `dev`) — see [§27.3](27_web-playground.md#273-authentication) for the per-mode mint points — so the override binds uniformly. See [§27.6](27_web-playground.md#276-session-lifecycle-and-cleanup).
