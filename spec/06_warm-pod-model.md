...
Task-mode state transitions (from attached):
  ...
  task_cleanup ──→ sdk_connecting [scrub_warning] (preConnect: true, scrub fails with onCleanupFailure: warn, maxScrubFailures not reached, maxTasksPerPod not reached, maxPodUptimeSeconds not reached — pod re-warms SDK before returning to idle and the scrub_warning annotation persists on the pod)
...
**Host-node schedulability precondition.** The "host node is schedulable" precondition on the `task_cleanup → sdk_connecting` transition is defined as the pod's host node (`spec.nodeName`) having `.spec.unschedulable == false` in the Kubernetes Node object (equivalently: no `node.kubernetes.io/unschedulable` taint applied by the scheduler when `kubectl cordon` or an autoscaler drain marks the node). The gateway evaluates this condition by reading the Node object via its informer cache at the moment of the `task_cleanup` transition — it does not re-check during SDK re-warm. If the host node is unschedulable, the pod transitions to `draining` instead (even though `maxTasksPerPod`, `maxPodUptimeSeconds`, and `maxScrubFailures` have not been reached), because SDK re-warm on a cordoned node would produce an idle pod whose next eviction is imminent. This rule applies to all preConnect pools; non-preConnect pools do not re-warm after scrub and are therefore not affected by node schedulability at `task_cleanup` time (their `idle → draining` transition on eviction is handled by the standard pod-disruption path).
