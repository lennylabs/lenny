...
Task-mode state transitions (from attached):
  ...
  task_cleanup ──→ sdk_connecting    (preConnect: true, scrub_outcome ∈ {succeeded, warn_within_budget}, maxTasksPerPod not reached, uptime limit not reached, node not draining — re-warm SDK before returning to idle; see "preConnect re-warm on scrub_warning" note below)
