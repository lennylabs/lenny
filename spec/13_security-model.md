...
**Allow object-store egress for checkpoint transfer — supplemental policy (NET-071):**
...
- The tenant prefix, the HTTP method, and the key are bound into the signature on every backend, so a pod that alters any of them produces a signature mismatch and the object store rejects the request before a byte is written or read. The `PUT`-path T4 encryption binding is per-backend: on the SigV4 backends (MinIO and AWS S3) the tenant's SSE-KMS headers and `Content-Length` are folded into the signature (`X-Amz-SignedHeaders`), and the gateway returns those signed header values in `Grant.headers` for the adapter to replay verbatim; the signed set names the headers, not their values, so a `PUT` that omits a value fails `SignatureDoesNotMatch`. GCS V4 signed URLs and Azure SAS cannot carry a per-request encryption key at the pinned SDKs, so on those backends the T4 encryption posture is enforced by a bucket-default CMEK (GCS, per-tenant bucket) or a container-level default encryption scope with override prevention (Azure), independent of the signature. §12 owns the mint-time invariants and the per-provider T4 mechanism; §13.2 owns the threat model.
- The pod holds no `LIST`, `DELETE`, or multipart capability, so it can enumerate no key it was not handed and delete nothing. Every delete is gateway-issued (§10.1). The `GET` capabilities minted on resume name only the chunk keys of the pod's own session, which the same pod authored, so they grant no read the pod did not already have; the gateway resolves the manifest row, checks contiguity, and signs, so the pod chooses no key.
- The residual exposure is that a compromised pod can write up to `checkpointGrantWindow × partialChunkSizeBytes` of attacker-chosen bytes into its own checkpoint prefix within the grant TTL. That bound holds because the gateway clamps every declared chunk length to the `chunk_size_bytes` it chose itself and sent in `Start` (§10.1, §11.2): a `ChunkReady` whose declared length is outside `(0, chunk_size_bytes]` aborts the attempt before any capability is signed, so the size a capability can carry is a gateway constant rather than a pod self-report. Those bytes are charged to the tenant's quota at confirmation and are discarded on the next attempt's supersede-on-write. The exposure is bounded by the tenant prefix, by the `checkpoint_id` UUID, and by the TTL. A compromised pod cannot list the bucket, cannot reach another tenant's prefix, cannot read a key the gateway did not sign for it, and cannot delete; the bytes it can overwrite are bytes it authored.

The policy renders as a supplemental agent-egress NetworkPolicy (`allow-pod-egress-objectstore`) with an in-cluster arm (a `namespaceSelector` on the release namespace paired with `podSelector: { lenny.dev/component: minio }` on `{{ .Values.minio.tlsPort }}`) and a cloud-managed arm (an `ipBlock` peer on TCP 443 from an operator-enumerated CIDR list). Object-store TLS is required and the deployer-supplied CA bundle is projected into agent pods, in the same form as the OTLP TLS requirement note above.

```yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: allow-pod-egress-objectstore
  namespace: lenny-agents # repeated per agent namespace via Helm range
  # Only rendered for pools that write checkpoints
spec:
  podSelector:
    matchLabels:
      lenny.dev/managed: "true"
  egress:
    - to: # In-cluster arm — self-managed MinIO
        - namespaceSelector:
            matchLabels:
              kubernetes.io/metadata.name: "{{ .Release.Namespace }}" # default: lenny-system
          podSelector:
            matchLabels:
              lenny.dev/component: minio
      ports:
        - port: "{{ .Values.minio.tlsPort }}" # default 9443, TLS
          protocol: TCP
    - to: # Cloud-managed arm — operator-enumerated object-store CIDRs (NET-002)
        {{- range .Values.objectStorage.egressCIDRs }}
        - ipBlock:
            cidr: {{ . | quote }}
        {{- end }}
      ports:
...
> **Object-store TLS requirement (NET-071):** Checkpoint chunk payloads carry tenant workspace state, so the object-store hop is in-scope for the same confidentiality posture as the other cross-namespace platform links (Gateway→Token Service, Gateway→Redis, Gateway→MinIO). The agent-to-object-store egress therefore MUST run over TLS. The object store's server certificate MUST be signed by a CA that the agent pods trust (the deployer's cluster-wide trust bundle, or a deployer-supplied CA bundle projected into agent pods when `objectStorage.caBundle` is set); the certificate's SAN MUST cover the configured object-store endpoint hostname.
...
| **MinIO** (`lenny.dev/component: minio`) — self-managed profile only; absent on cloud-managed deployments where the provider's native object storage (S3, GCS, Azure Blob) is used instead | `kube-system` CoreDNS (UDP/TCP 53). | Gateway pods (TCP 9443, TLS — `{{ .Values.minio.tlsPort }}`); `lenny-ops` pods (TCP 9443, TLS — `{{ .Values.minio.tlsPort }}` — backup/restore object operations; NET-051); `lenny-backup` Job pods (TCP 9443, TLS — `{{ .Values.minio.tlsPort }}`). The `lenny-ops` and `lenny-backup` ingress clauses use `podSelector: { matchLabels: { app: lenny-ops } }` / `{ app: lenny-backup }` paired with `namespaceSelector: { matchLabels: { kubernetes.io/metadata.name: "{{ .Release.Namespace }}" } }` per the selector exception at line 201. Agent-namespace pods (TCP `{{ .Values.minio.tlsPort }}`, TLS — checkpoint chunk `PUT`/`GET`, paired with the `allow-pod-egress-objectstore` egress policy in agent namespaces; NET-071), using `namespaceSelector: { matchLabels: { lenny.dev/agent-namespace: "true" } }` paired with `podSelector: { matchLabels: { lenny.dev/managed: "true" } }`. Without this ingress clause the `lenny-system` default-deny policy drops the agent-namespace egress on the MinIO side. This clause is self-managed-profile-only: on cloud-managed object storage there is no ingress-side row, and the cloud-managed checkpoint path rests entirely on the `allow-pod-egress-objectstore` egress `ipBlock` arm. |
| **Admission Webhooks** (`lenny.dev/component: admission-webhook`) — the admission-webhook Deployments enumerated in [§17.2](17_deployment-topology.md#172-namespace-layout) (`lenny-label-immutability`, `lenny-direct-mode-isolation`, `lenny-sandboxclaim-guard`, `lenny-data-residency-validator`, `lenny-pool-config-validator`, `lenny-t4-node-isolation`, `lenny-drain-readiness`, `lenny-crd-conversion`, `lenny-ephemeral-container-cred-guard`). Every admission-webhook Deployment carries the `lenny.dev/component: admission-webhook` label so that the single `podSelector` in this row matches every webhook pod (see NET-047/NET-050 above for the selector-consistency audit that enforces this). The `lenny-drain-readiness` Deployment additionally carries the per-webhook additive label `lenny.dev/webhook-name: drain-readiness`, which narrows the gateway-egress sub-rule below to that single webhook (NET-068; see the additive-label exception clarified in the line 203 invariant and the §17.2 canonical-label note). The `lenny-sandboxtemplate-deletion-guard` Deployment (the §10.5 runtime-upgrade safety webhook acknowledged as a labeled admission-webhook Deployment in [§17.2](17_deployment-topology.md#172-namespace-layout)) likewise carries the additive per-webhook label `lenny.dev/webhook-name: sandboxtemplate-deletion-guard`, which narrows sub-rule (c) below to that single webhook (NET-068). | The following sub-rules are rendered as separate `egress` peers within this NetworkPolicy: **(a) base egress for the admission-webhook Deployments** (selector `lenny.dev/component: admission-webhook`) — `kube-system` CoreDNS (UDP/TCP 53), required for kube-apiserver watch connections and generic name resolution. **(b) drain-readiness-only egress** (selector `lenny.dev/component: admission-webhook` AND `lenny.dev/webhook-name: drain-readiness`) — Gateway internal HTTP port (TCP `{{ .Values.gateway.internalPort }}` — default 8080) for `GET /internal/drain-readiness` callbacks (NET-037). Without sub-rule (b), the `lenny-system` default-deny policy blocks the drain-readiness callback, causing all pod evictions to be permanently rejected by the fail-closed webhook; without sub-rule (b)'s additive `webhook-name` selector, the in-process validators would inherit unnecessary reachability to the gateway's internal admin port in violation of least privilege (NET-068). **(c) sandboxtemplate-deletion-guard-only egress** (selector `lenny.dev/component: admission-webhook` AND `lenny.dev/webhook-name: sandboxtemplate-deletion-guard`) — Gateway internal HTTP port (TCP `{{ .Values.gateway.internalPort }}` — default 8080) for `GET /internal/runtime-upgrade/active` probes (see [§10.5](10_gateway-internals.md#105-upgrade-and-rollback-strategy)). Without sub-rule (c), the `lenny-system` default-deny blocks the deletion-guard's runtime-upgrade probe, so the fail-closed webhook times out and denies a `SandboxTemplate` `DELETE` that has a referencing `SandboxWarmPool`; without sub-rule (c)'s additive `webhook-name` selector, the other in-process webhooks would inherit unnecessary reachability to the gateway's internal admin port in violation of least privilege (NET-068). | kube-apiserver (TCP 443, CIDR `{{ .Values.webhookIngressCIDR }}` — default `0.0.0.0/0`). The kube-apiserver must reach these pods to invoke ValidatingAdmissionWebhook callbacks. Without this ingress rule, the `lenny-system` default-deny policy blocks all webhook callbacks, causing fail-closed webhooks to reject all pod admissions silently. Ingress selectors for this row use only the canonical `lenny.dev/component: admission-webhook` label — the additive `lenny.dev/webhook-name` label is permitted in egress allow-lists only, never in ingress (NET-068). See the `webhookIngressCIDR` note below for cloud-specific tightening guidance. |
...
> **`internet` profile hardening (NET-002):** The `internet` egress NetworkPolicy explicitly **excludes** cluster-internal CIDRs (`egressCIDRs.excludeClusterPodCIDR` and `egressCIDRs.excludeClusterServiceCIDR` in the Helm values) via `except` clauses on the `0.0.0.0/0` CIDR rule. This prevents lateral movement between agent pods even when internet egress is permitted. The `allow-pod-egress-objectstore` supplemental policy (NET-071) is not subject to this explicit-`except` requirement: its cloud-managed arm renders only from the operator-enumerated `objectStorage.egressCIDRs` list and contains no `0.0.0.0/0` rule. When that list is empty and the object-store provider is not `minio`, the policy renders nothing and the `lenny-preflight` Job fails the install.
>
...
Lenny issues bearer tokens through a single canonical endpoint: `POST /v1/oauth/token` ([§15.1](15_external-api-surface.md#151-rest-api)), compliant with [RFC 6749 §5](https://www.rfc-editor.org/rfc/rfc6749#section-5) (token endpoint) and [RFC 8693](https://www.rfc-editor.org/rfc/rfc8693) (token exchange). The token-minting flows — the general admin and service-principal self-rotation grant (the caller presents its current token as `subject_token`, requesting a new token of the same type), credential-lease token issuance, delegation child-token minting, operability scope narrowing — go through this endpoint, either by direct caller invocation (the general self-rotation grant) or by internal Token Service calls (credential leasing, delegation minting). The bootstrap `lenny-admin` Secret rotation is the one exception: it is a gateway-direct in-process mint that does not go through `/v1/oauth/token` (the gateway signs the new admin token in process, patches the `lenny-admin-token` Secret, then durably revokes the prior token; see the gateway-mediated admin-credential rotation ordering below).
...
**Token rotation and revocation — no grace period.** The general admin and service-principal token rotation grant (`grant_type=urn:ietf:params:oauth:grant-type:token-exchange` with the current token as `subject_token`, requesting a new token of the same type) is **atomic and immediate**: inside the write-before-issue transaction that creates the new token, the Token Service also writes `revoked_at = now()` on the previous token's `issued_tokens` row. Only after the transaction COMMITs does the new token leave the gateway. The revocation is then propagated cluster-wide via the `token.revoked` CloudEvents event on the Redis EventBus ([§12.6](12_storage-architecture.md#126-interface-design)), causing peer gateway replicas to load the revocation into their in-memory revocation cache within the propagation latency budget (target: p99 < 50ms, `TokenRevocationPropagationLag`, §16.5).

**Gateway-mediated bootstrap admin-credential rotation ordering.** The bootstrap `lenny-admin` Secret rotation ([§17.6](17_deployment-topology.md#176-packaging-and-installation)) is not a single-transaction token exchange; it is a gateway-direct in-process mint that must persist the new token to the `lenny-admin-token` Secret **before** revoking the prior token, because revoking first and then failing to patch the Secret would strand the Secret on a dead token and lock the operator out. The gateway therefore performs a distinct ordering: (1) mint the new admin token in process and record it in `issued_tokens` together with its `token.exchanged` audit event carrying `exchange_type: admin_rotation` ([§16.7](16_observability.md#167-section-25-audit-events)); (2) patch the `lenny-admin-token` Secret with the new token; (3) only after the patch succeeds, durably revoke the prior token by writing `revoked_at = now()` to Postgres and emit its `token.revoked` audit event carrying `revocation_reason: rotation_replaced`. The revoke is durable: the cross-replica revocation cache push is gated on the durable Postgres write succeeding, so a durable-write failure never leaves a revocation live only in memory that a later cache rehydration would silently un-revoke.

The persist-before-revoke ordering opens a crash window: a hard failure after the Secret patch commits but before the durable revoke commits would leave the prior admin token live with no in-request reclaimer. Three rules close this window without weakening the no-grace-period guarantee:

- **Named predecessor and leader-gated reclaimer.** The rotation records the predecessor token's identifier in the `lenny-admin-token` Secret alongside the current token's identifier, so an orphaned predecessor is durably named before the revoke step. A continuously-running, leader-gated periodic sweep durably revokes the single predecessor the Secret names, with `revocation_reason: rotation_replaced`, whenever that predecessor is still unrevoked. The sweep is idempotent (a no-op once the in-request revoke has committed) and targets only the single named predecessor rather than every non-current token for the subject, so it never revokes a token the general `/v1/oauth/token` self-rotation grant minted for the same platform-admin subject (that grant does not patch the Secret, so its token is never named as the predecessor) and never revokes an in-flight successor recorded but not yet installed in the Secret. The sweep cadence is operator-tunable with a documented default. A reclaimer bound to the bootstrap-provisioning path would not cover this window, because provisioning runs only on an operator bootstrap call and early-returns when the Secret already exists (the post-crash state), so the always-running leader-gated sweep is the surface that covers crash recovery.
- **Revoke-before-overwrite.** The predecessor slot in the Secret holds one identifier and is overwritten on the next rotation, so a rotation durably revokes the predecessor the live Secret currently names before overwriting that slot, and aborts before patching the Secret when that revoke fails, so the slot keeps naming the still-unrevoked predecessor for the next attempt or the sweep. The invariant is that the predecessor slot is never overwritten while it names an unrevoked token unless that token is durably revoked in a committed write that precedes the overwrite, so every predecessor across any number of consecutive crash-then-retry rotations is durably revoked or durably named before its name is lost.
- **Per-subject serialization.** The Secret read-modify-write is not atomic and spans a Kubernetes Secret patch and several independent Postgres transactions, so the whole rotation sequence runs under a per-subject session-scoped Postgres advisory lock held on a single pinned connection across the Secret patch and the separate store transactions, and released on every success, error, and cancellation path. This serializes concurrent rotations for the `lenny-admin` subject so two interleaved rotations cannot drop a successor identifier that is named nowhere in the Secret through a lost-update on the non-atomic read-modify-write, mirroring the advisory-locked concurrent-rotation discipline the actor-token freshness rule below applies to the general exchange path.

With these rules, a second rotation that runs before the sweep fires — whether a sequential crash-then-retry or a concurrent rotation — cannot strand a predecessor, so the no-grace-period guarantee above and the authoritative-durability invariant below both continue to hold for this path with the residual bounded to the sweep interval rather than to the crash window or indefinitely; neither needs normative weakening, because the reclaimer closes the window on its next pass rather than introducing a grace period.
...
**Checkpoint object-store capabilities (presigned):**

```
Gateway resolves the checkpoint key → validates the tenant prefix → mints a presigned capability
                                    → Gateway signs the capability from its own object-store credential
                                    → Pod receives: one presigned URL per chunk (single key, single method, exact Content-Length on PUT, short expiry)
                                    → Pod never receives: object-store credential, LIST/DELETE/multipart capability, any key outside its own checkpoint prefix
```

The gateway mints one presigned capability per chunk from its own object-store credential; no object-store credential is ever projected into an agent pod. The pod `PUT`s checkpoint chunks and, on resume, `GET`s them against these capabilities (NET-071, §13.2).
...
Delegation chains introduce a prompt injection attack surface: a compromised or manipulated parent agent can craft adversarial `TaskSpec.input` payloads — or adversarial **exported workspace files** — targeting child agents. Lenny provides layered mitigations:
...
4. **Exported-file content scanning (opt-in)** — `contentPolicy.scanExportedFiles` on `DelegationPolicy` ([Section 8.3](08_recursive-delegation.md#83-delegation-policy-and-lease)) routes each file enumerated by `delegate_task.fileExport` through the same `contentPolicy.interceptorRef` at the `PreExportMaterialization` phase ([Section 4.8](04_system-components.md#48-gateway-policy-engine)) before the file is persisted into the child's workspace. This covers the delegation-mediated file-injection vector (e.g., a compromised parent writing adversarial text into a runtime-recognized instruction file such as `CLAUDE.md` before exporting it). `maxExportedFileSize` bounds the per-file payload handed to the interceptor (default: 10 MiB). Default: `false` (platform classifier-free per [Section 22.3](22_explicit-non-decisions.md)); deployers opt in per `DelegationPolicy`.
5. **Messaging rate limits** — `messagingRateLimit` on the delegation lease ([Section 8.3](08_recursive-delegation.md#83-delegation-policy-and-lease)) caps `lenny/send_message` volume per session (`maxPerMinute` outbound, `maxPerSession` lifetime). The `maxInboundPerMinute` aggregate limit caps total inbound messages to any single session regardless of the number of senders, preventing N compromised siblings from flooding a target at N × the per-sender rate.
6. **Messaging scope** — `messagingScope` ([Section 7.2](07_session-lifecycle.md#72-interactive-session-model)) restricts which sessions can message each other. Default `direct` limits to parent/children only.
7. **Budget and depth limits** — delegation leases enforce `maxDepth`, `maxTreeSize`, and `maxTokenBudget`, bounding the blast radius of any compromised delegation chain.

**Residual risk without content scanning:** Without `contentPolicy.interceptorRef`, the gateway validates delegation structure (depth, budget, policy tags) but does not inspect content semantics on either the `TaskSpec.input` path or the exported-file path. See [Section 22.3](22_explicit-non-decisions.md) for the explicit non-decision on built-in guardrail logic.

**Residual risk — file export content when `scanExportedFiles: false`:** When `contentPolicy.scanExportedFiles` is `false` (the default), workspace files exported from a parent to a child ([Section 8.7](08_recursive-delegation.md#87-file-export-model)) are not subject to platform content scanning; the gateway provides structural validation only (path bounds, size limits, symlink protection, archive validators). A compromised parent can include adversarial content in any exported file, including files that agent runtimes treat as instruction sources (e.g., `CLAUDE.md`, `AGENTS.md`). Deployers who leave `scanExportedFiles: false` are responsible for treating all workspace files received via delegation as untrusted input and for applying runtime-layer hardening (e.g., a runtime `ignoreInstructionFiles` allowlist). Setting `scanExportedFiles: true` on the `DelegationPolicy` — with a backing `interceptorRef` — promotes this to a platform-enforced control; see [Section 8.7](08_recursive-delegation.md#87-file-export-model) for the full deployer-side mitigation menu.
