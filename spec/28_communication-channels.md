...
- **Exclusivity.** The specification states no exclusivity constraint on this channel and names no
  ...
  the one channel, keyed by `slotId`. A runtime serving a pool that sets
  `sessionPolicy.maxConcurrentSessions` above 1 implements a dispatch loop keyed on `slotId`, because
  each concurrent slot's messages carry a distinct `slotId` assigned by the adapter, which is what
  carries multiple independent concurrent session streams through the one channel.
...
`slotId`, and on a single-session pod no message carries `slotId` (§28.5.3). `CH-MCP-PLATFORM` and
