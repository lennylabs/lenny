...
| `playground.devTenantId` | `default` | Tenant bound to the dev HMAC JWT `tenant_id` claim when `authMode=dev`. Must match `^[a-zA-Z0-9_-]{1,128}$` and name a registered tenant at startup. Helm-validate rejects the chart if `authMode=dev`, `auth.multiTenant=true`, and this value is left at `default` while multiple tenants are seeded. See [§27.3](#273-authentication). |
...
- **`apiKey`:** after the `/playground/*` handler validates the user-supplied bearer token via the standard gateway auth chain ([§10.2](10_gateway-internals.md#102-authentication) — same validation as any non-playground API caller, including `tenant_id` claim extraction and `TENANT_CLAIM_MISSING` / `TENANT_NOT_FOUND` rejections), it invokes the standard session-JWT mint with the `origin: "playground"` claim attached. The attachment is driven by the request's ingress route (`/playground/*`), not by the bearer material — the same token presented on a non-playground route does not produce the claim.
...
- Chat stream: MCP WebSocket at `/mcp/v1/ws` with a session-capability JWT as the bearer. The JWT is obtained from the cookie-to-bearer exchange ([§27.3.1](#2731-oidc-cookie-to-mcp-bearer-exchange)) in `oidc` mode, or minted by the `/playground/*` handler from the user-supplied bearer token in `apiKey` mode and the dev signer in `dev` mode — the WebSocket codepath itself is identical across modes.
