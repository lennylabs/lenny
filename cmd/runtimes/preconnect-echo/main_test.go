...
// container, including the F-6.3.5 SDK-warm preconnect-echo arm. The
// embedded main must define them; the default flag.ExitOnError parser
// exits 2 on an unknown flag, which would crash-loop every §4.7 embedded
// pool. This test pins that the binary parses both injected flags without
// the flag-parse exit. F-9.1.1.
func TestPreconnectMainAcceptsPlatformMCPFlags_spec_9_1(t *testing.T) {
	bin := buildPreconnectEcho(t)
...
// buildPreconnectEcho compiles the preconnect-echo binary into a temp dir
// and returns its path. Building the real binary exercises the actual
// flag registration in main rather than a re-declared copy.
func buildPreconnectEcho(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "preconnect-echo")
	if _, err := os.Stat("main.go"); err != nil {
		t.Fatalf("expected to run from the package directory: %v", err)
	}
	cmd := exec.Command("go", "build", "-o", bin, ".")
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("build preconnect-echo: %v\n%s", err, out)
	}
	return bin
}
