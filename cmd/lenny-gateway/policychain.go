...
func (w *gatewayWiring) buildPolicyChain(
	...
	// §4.8: arm cumulative fail-open escalation on every chain.
	...
	// §4.8: AuthEvaluator (priority 100) is the sole built-in at
	...
	// §4.8: DelegationPolicyEvaluator (priority 250) fires at
	...
	// §4.8: RetryPolicyEvaluator (priority 600) fires at PostRoute
	...
	// active. quotaBudgetTracker is non-nil only in the §12.4
	...
	// §12.4 source (2) / §11.2 in-memory fail-open token accumulator.
	...
	// §12.4 cached_replica_count, shared by the fail-open ceiling
	// (buildFailOpenController) and the §12.4 in-memory budget
	...
	// spec: §12.4 — resolve the quota enforcement mode. The
	...
	if quotaSyncSeconds != *quotaSyncIntervalSeconds && *quotaSyncIntervalSeconds > 0 {
		// spec: §11.2 — clamp the operator-supplied cadence up to
		// the 10s floor so a misconfiguration cannot drive a busy-loop.
		log.Printf("lenny-gateway: §11.2 quotaSyncIntervalSeconds=%d below the %ds floor; clamping to the minimum",
			*quotaSyncIntervalSeconds, quota.MinSyncIntervalSeconds)
	}

	switch {
	case quotaMode == quota.EnforcementModeInMemoryReconciled:
		// spec: §12.4 — the per-replica in-memory budget slice is
		...
		// spec: §11.7 — route policy-rejection audit rows through
