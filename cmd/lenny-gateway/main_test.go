...
func TestNewLLMProxyServerDisabledWhenAddrEmpty(t *testing.T) {
	if srv := newLLMProxyServer("", buildLLMTranslatorRegistry(llmTranslatorConfig{anthropicVersion: "2023-06-01"}), credleasestore.New(), credcache.New(), denylist.New(), interceptor.NewChain()); srv != nil {
...
func TestNewLLMProxyServerBindsConfiguredAddress(t *testing.T) {
	srv := newLLMProxyServer(":8443", buildLLMTranslatorRegistry(llmTranslatorConfig{anthropicVersion: "2023-06-01"}), credleasestore.New(), credcache.New(), denylist.New(), interceptor.NewChain())
	if srv == nil {
