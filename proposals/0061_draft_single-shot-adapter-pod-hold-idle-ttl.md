...
- **Open Responses** clients send only the new turn plus a `previous_response_id`, and proposal 0059 reconstructs the prior conversation by replaying the stored transcript onto the freshly-claimed pod. The rehydration exists *because* the previous turn's pod was released. Holding that pod would make both the re-claim and the replay unnecessary on a continuation.
...
- Builds on proposal 0057 (single-shot compute model, `spec/15:585-606`) and proposal 0059 (Open Responses continuation and transcript rehydration).
