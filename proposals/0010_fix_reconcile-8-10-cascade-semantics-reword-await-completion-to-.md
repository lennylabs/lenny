...
- **Status:** Applied to spec (2026-06-19). Converged after 4 adversarial review rounds (1 finding fixed); signed off by the human approver. Open decision (DEFECT 2 prose register) resolved in favor of the staged §3.3 wording, which keeps the explicit per-user fairness rationale (per-user concurrency quota versus per-tenant orphan cap) as the default-posture exposure.
- **Date:** 2026-06-19.
