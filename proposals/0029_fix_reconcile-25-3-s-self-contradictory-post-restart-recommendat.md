# Proposal: Reconcile §25.3's self-contradictory post-restart recommendations contract: surface the empty-window / data-starved state through the canonical §25.2 degradation envelope (level: degraded) rather than per-item entries, and align the gateway recommendations service and its tests to emit that signal (closes F-25.3.16)

- **Status:** Applied to spec (2026-07-04). Signed off by the user for implementation; all three open decisions resolved with the recommended/staged option, none altering the staged edits: (1) Option A — surface the post-restart signal through the response-level §25.2 degradation envelope (`level: degraded`), keeping `recommendations[]` empty and the committed test, rather than Option B per-item entries; (2) signal via `level: degraded` + a warning, rather than a literal `confidence: 0.0` that would force dropping `omitempty` across every ops-API degradation envelope; (3) close F-25.3.16 for the wholesale-empty (post-restart) case, leaving per-rule partial starvation as a separate follow-on. Genuinely converged on the hardened workflow (4 rounds, 2 consecutive clean rounds, every round complete).
- **Date:** 2026-07-03.
- **Scope:** Reconciles §25.3's internally contradictory account of what `GET /v1/admin/recommendations` returns when the in-memory sliding windows are empty (the post-restart case) to a single coherent behavior, and aligns the gateway recommendations service and its tests to emit it. The finding is F-25.3.16 (`BUILD-GAPS.md:42417`, Medium, OPEN, DEFERRED on the spec contradiction). §25.3 line 586 prescribes a per-item degraded entry (`"confidence": 0.0` and `"dataAvailable": false`) while §25.3 line 605 prescribes no entry, for the identical post-restart trigger; three further passages (`spec/25_agent-operability.md:1432`, `:1470`, `:4989`) assume a surfaced post-restart signal. The resolution keeps `recommendations[]` to triggered, actionable entries only and surfaces the empty-window state through the response-level §25.2 degradation envelope (`level: degraded`), which the response already carries and already stamps. It rewrites the two contradictory §25.3 passages, widens the §25.2 envelope trigger to admit an endpoint whose data quality depends on sufficient in-process history, reconciles the three cross-references, flips one branch in `pkg/gateway/operability/recommendations/service.go`, and updates two tests plus adds one. It adds no new endpoint, RPC, wire field, migration, or store method, and it touches only `spec/25_agent-operability.md`, `pkg/gateway/operability/recommendations/service.go`, `pkg/gateway/operability/recommendations/service_test.go`, and `BUILD-GAPS.md`.
...
These three are mutually consistent with each other and with line 586; line 605 conflicts with them. They presuppose a surfaced per-response signal.
...
- **Detect the empty-window state by aggregating the per-rule `Evaluation.DataAvailable` the evaluators already return, over the whole rule catalog independent of the `?category=` filter.** The `WindowStore` exposes no store-level emptiness method, but the service already reads `e.DataAvailable` (`service.go:298`); reading it before the `if !e.Triggered { continue }` drop, and before the `?category=` filter narrows the emitted array, lets the service track whether any rule in the catalog had data, reusing an existing signal with no new interface method. Aggregating over the full catalog rather than the filtered subset keeps a `?category=` request for a starved rule from reporting the store as wholesale-empty while another rule holds data.
- **This is a reconciliation rather than a new capability.** The envelope field, the `DegradationDegraded` constant, and the stamping site all ship. The change flips one branch, rewrites the contradictory spec prose, widens the §25.2 envelope-scope wording, reconciles three cross-references, and updates two tests plus adds one. It adds no new endpoint, RPC, field, migration, or store method.
...
The §25.2 canonical degradation envelope is the surface every option lives inside. The `RecommendationsResponse` already carries it (`service.go:140-153`), and `GetRecommendations` already constructs and stamps it on every response (`service.go:307-310`). The envelope's `Level` field already admits `degraded` (`conventions.go:104-108`). The evaluators already report per-rule data presence: each metric-present branch sets `DataAvailable: true` and each metric-absent branch returns a zero-value `Evaluation{}` with `DataAvailable` false (`service.go:368-453`), and the loop already reads `e.DataAvailable` when it emits a triggered entry (`service.go:298`).

The reconciliation stays inside this envelope. It resolves the §25.3 contradiction to the response-level reading and widens the §25.2 envelope-scope wording so the gateway's warm-up degradation fits the stated trigger (C1), reconciles the three cross-references that describe the deleted `confidence: 0.0` reading (C2), flips the gateway service's unconditional healthy stamp to a conditional degraded stamp computed from the aggregated per-rule `DataAvailable` across the whole catalog (C3), and updates the two committed tests plus adds contrast tests (C4). No new store method, wire field, or endpoint is introduced.

## 4. Proposed changes

### C1. Reconcile the §25.3 self-contradiction (line 586 versus line 605) and widen the §25.2 envelope trigger to admit insufficient in-process history

**Target:** `spec/25_agent-operability.md`, §25.3 Sliding window aggregation paragraph (line 586) and the §25.3 Degradation subsection (line 605); and §25.2 Canonical Degradation Envelope, the trigger sentence (line 205) and the "Omitted when healthy" clause (line 233). Reference (aligned after C3, no change): `pkg/gateway/operability/recommendations/service.go`.

Lines 586 and 605 prescribe opposite outcomes (a per-item degraded entry versus no entry) for the identical post-restart empty-window state, and `dataAvailable` exists only as a per-item field, so line 586 cannot be satisfied without an emitted entry that line 605 forbids. Both are rewritten to the single coherent behavior: no per-category entries, and a response-level `degradation` envelope reporting `level: degraded`. The §25.2 trigger and omitted-when-healthy wording are widened so the gateway's in-process warm-up degradation fits the envelope's stated scope, since the empty ring buffers are an internal history-accumulation state rather than an external-dependency outage.
...
**Anchor and change (§25.2 envelope trigger, line 205):** Replace
...
**Anchor and change (§25.2 "Omitted when healthy" clause, line 233):** Replace
...
**Anchor and change (aggregate data presence over the whole catalog before applying the `?category=` filter, lines 273-287):** Replace
...
Moving the `?category=` filter below the `evaluated`/`anyData` update, rather than leaving it at the top of the loop body, is the correction: the filter narrows only the emitted `recommendations[]`, while the wholesale-empty determination now scans the whole catalog. The disabled-rule and missing-evaluator skips stay ahead of the aggregation because a disabled or unwired rule genuinely does not evaluate; only the category narrowing moves.
...
**Rationale:** The evaluators already return `DataAvailable` on every branch (`service.go:368-453`), and the loop already reads it at `service.go:298`. Reading it before the `if !e.Triggered { continue }` drop, and aggregating across the whole catalog before the `?category=` filter narrows the emitted array, yields the wholesale-empty signal with no new interface method or wire field. Aggregating over the full catalog rather than the filtered subset is the correction the review requires: the category filter formerly preceded the aggregation (`service.go:273-275`), so a `?category=gateway_scaling` request on a warm store that holds data only in another category (for example `lenny_credential_pool_utilization`) would have seen an empty filtered subset and falsely stamped `degraded`. Because the catalog wires one evaluator per category and most gateway source metrics originate off the gateway registry (`cmd/lenny-gateway/workers.go:1089-1093`), that false `degraded` stamp would be permanent on a never-restarted gateway. Moving the filter below the aggregation removes the false positive. The stamp stays `healthy` when any rule had data (a below-threshold steady state) and flips to `degraded` only when every evaluated rule reported no data (the data-starved case, for example shortly after a gateway restart). The warning names the empty windows without asserting a restart as the sole cause, since some source metrics are absent from the gateway registry regardless of restart. `recommendations[]` and the per-item `DataAvailable` field are unchanged; the degraded envelope is the response-level signal.

### C4. Align the tests to the degraded-envelope behavior and pin the new signal

**Target:** `pkg/gateway/operability/recommendations/service_test.go`, `TestGetRecommendationsEmptyWhenNoData` (lines 14-24), `TestGetRecommendationsStampsCompiledInDefaults_spec_25_13_4848` (lines 155-173), a new `TestGetRecommendationsHealthyEnvelopeWhenBelowThreshold`, and a new `TestGetRecommendationsHealthyWhenCategoryFilterStarvedRuleHasData`; and `pkg/gateway/externalapi/admin/recommendations_test.go`, `TestRecommendationsEndpointCategoryFilter` (lines 50-69), hardened to assert the healthy envelope on the filtered-starved path.

After C3, an empty `WindowStore` yields `Level: degraded`. `TestGetRecommendationsStampsCompiledInDefaults` uses an empty store and asserts `Level == DegradationHealthy` (169-172), so it breaks and must be updated; its purpose (F-25.13.5, `thresholdSource` stays `compiled-in-defaults`) is preserved by giving it a below-threshold sample so a rule reports `DataAvailable`. `TestGetRecommendationsEmptyWhenNoData` still asserts `len == 0` (preserved) and is extended to pin the new degraded signal. Two healthy-with-data cases are added so the distinguishing behavior is covered on both sides: one with data below every threshold, and one where a `?category=` filter narrows the request to a starved rule while another rule holds data, which pins that the wholesale-empty determination scans the whole catalog rather than the filtered subset. The admin-level `TestRecommendationsEndpointCategoryFilter` is hardened with the same envelope assertion at the endpoint boundary.
...
**Anchor and change (new category-filter contrast test, appended after `TestGetRecommendationsHealthyEnvelopeWhenBelowThreshold`):** Add
...
**Anchor and change (harden the admin-level category filter test, `pkg/gateway/externalapi/admin/recommendations_test.go`, `TestRecommendationsEndpointCategoryFilter` lines 62-68):** Replace
...
This test lives in `package admin_test`, which already imports `recommendations`; the `conventions` import is added to its import block.

**Rationale:** The two committed tests encode the pre-reconciliation behavior; C3 changes the empty-store envelope level, so both are updated. The `conventions` import is already present in `service_test.go` (`service_test.go:11`) and is added to `recommendations_test.go` in the admin package. The service-level contrast tests pin the both-sides distinction the finding requires: an empty array with a healthy envelope (data present, below threshold, or a category filter narrowing to a starved rule while another rule holds data) is now observably different from an empty array with a degraded envelope (no data anywhere). The admin-level `TestRecommendationsEndpointCategoryFilter` previously asserted only `len == 0`, so it would have passed silently under a filtered-subset aggregation that stamped `degraded`; the added envelope assertion closes that hole at the endpoint boundary. This is tier-1 pure in-process logic, the tier F-25.3.16 reaches; the category filter narrows only the emitted array and adds no wire field, so no tier-3 contract is newly reached.

### C5. Close F-25.3.16 on application

**Target:** `BUILD-GAPS.md`, F-25.3.16 (line 42417).

On application (after the spec edits land and the code and tests are green), mark F-25.3.16 CLOSED with a note that §25.3's self-contradictory post-restart recommendations contract is reconciled to the response-level §25.2 degradation envelope: line 586 and line 605 both describe an empty `recommendations` array with a `degradation` envelope at `level: degraded`, the §25.2 envelope trigger admits insufficient in-process history, the three §25.4/§25.15 cross-references are aligned, and the gateway service stamps the degraded envelope when no rule in the whole catalog is data-bearing. Record that the finding is closed for the wholesale-empty (post-restart) case and that per-rule partial-starvation surfacing is out of scope (see Open decisions). Reference this proposal id (0029).
...
- **tier-1 (empty-window degraded envelope, spec-named-failure path):** `TestGetRecommendationsEmptyWhenNoData` (updated). Against an empty `WindowStore`, assert `len(resp.Recommendations) == 0` (preserved), `resp.Degradation.Level == DegradationDegraded`, and a non-empty `Warnings` entry, so the post-restart data-starved state is observably distinct from a healthy response. `// spec: 25.3 (post-restart degradation), 25.2 (canonical envelope).`
- **tier-1 (healthy envelope with data below threshold, boundary and contrast path):** `TestGetRecommendationsHealthyEnvelopeWhenBelowThreshold` (new). Record a below-threshold metric (`lenny_credential_pool_utilization = 0.40`), assert `len == 0` and `Level == DegradationHealthy`, so an empty array with data present stays healthy and the degraded signal is reserved for the wholesale-empty case. `// spec: 25.3 (data-present distinction), 25.2 (canonical envelope).`
- **tier-1 (healthy envelope under a category filter narrowing to a starved rule, filter-aggregation path):** `TestGetRecommendationsHealthyWhenCategoryFilterStarvedRuleHasData` (new) and the hardened admin-level `TestRecommendationsEndpointCategoryFilter`. Record `lenny_credential_pool_utilization = 0.40`, request `?category=gateway_scaling` (whose metric is absent), and assert `len == 0` and `Level == DegradationHealthy`, so the wholesale-empty determination scans the whole catalog and a filtered request for a starved rule does not falsely stamp `degraded` while another rule holds data. `// spec: 25.3 (data-present distinction), 25.2 (canonical envelope).`
- **tier-1 (compiled-in-defaults threshold source preserved, regression path):** `TestGetRecommendationsStampsCompiledInDefaults_spec_25_13_4848` (updated). Record a below-threshold sample so a rule reports `DataAvailable`, then assert `ThresholdSource == compiled-in-defaults` (preserved) and `Level == healthy` (now justified by present data), so the F-25.13.5 threshold-source contract still holds after C3. `// spec: 25.13 line 4848 (thresholdSource), 25.2 (canonical envelope).`
- **tier-11 doc and spec consistency (reconciled §25.2/§25.3/§25.4/§25.15 text, cross-reference resolution):** Assert that no §25 surface says the recommendations response returns `confidence: 0.0` for the post-restart case; that the §25.3 Sliding window aggregation paragraph (line 586) and the §25.3 Degradation subsection (line 605) both describe an empty `recommendations` array with a `degradation` envelope at `level: degraded`; that the §25.2 envelope trigger (line 205) and the omitted-when-healthy clause (line 233) admit insufficient in-process history; that the §25.4 Prometheus-requirement bullet (line 1432), the §25.4 operational-consequences row (line 1470), and the §25.15 failure-mode row (line 4989) describe the `degradation.level: "degraded"` envelope; that no §25 surface describes the `degradation.level: "degraded"` state as persisting "for hours after any restart" or "until the ring buffers refill", and that the §25.2, §25.3, §25.4, and §25.15 passages agree the envelope is `degraded` only while every ring buffer is empty and returns to `healthy` as soon as any rule records a sample, so a future multi-hour-duration regression is caught; and that every cross-reference (§25.3 to §25.2, §25.2 to §25.3) resolves to a real anchor. `// spec: 25.2 (canonical envelope), 25.3 (post-restart degradation), 25.4 (Prometheus requirement), 25.15 (failure-mode analysis). // diagnosis: a failure means the reconciled §25.2/§25.3/§25.4/§25.15 passages disagree about the post-restart signal or how long the degraded state lasts, so an agent reading the spec cannot tell whether an empty recommendations array is starved or healthy or how quickly the envelope recovers.`

## Findings closed on application

- **F-25.3.16** (`BUILD-GAPS.md:42417`, "`/v1/admin/recommendations` does not surface the `degradation` / `dataAvailable` post-restart contract", Medium). Closed: §25.3's line-586-versus-605 contradiction is reconciled to the response-level §25.2 degradation envelope (C1), the §25.2 envelope trigger is widened to admit insufficient in-process history (C1), the three §25.4/§25.15 cross-references are aligned (C2), and the gateway service stamps `level: degraded` with a warning when no rule in the whole catalog is data-bearing (C3), pinned by the tests (C4). Closed for the wholesale-empty (post-restart) case; per-rule partial-starvation surfacing is out of scope (Non-goals, Open decisions).
...
- **C3 degraded-envelope aggregation was computed over the category-filtered subset, so a `?category=` request on a warm gateway falsely stamped `degraded` plus a false "gateway recently restarted" warning.** Verified against the tree: the category-filter `continue` (`service.go:273-275`) precedes the point where the staged C3 inserted the `evaluated`/`anyData` counters (`service.go:284`), the catalog wires one evaluator per category (`service.go:366-455`, `pkg/recommendations/rules/rules.go`), and the admin test `TestRecommendationsEndpointCategoryFilter` (`pkg/gateway/externalapi/admin/recommendations_test.go:50-69`) would have passed silently under the bug because it asserted only `len == 0`. Fixed by restructuring C3 so the `?category=` filter is moved below the `evaluated`/`anyData` aggregation: data presence is now aggregated over the whole rule catalog (the disabled-rule and missing-evaluator skips stay ahead of it), and the filter narrows only the emitted `recommendations[]`. Propagated the whole-catalog predicate to the §2 decisions (response-wide/filter-independent framing, the detect-empty-window decision), the §3 summary, the C3 rationale, the C5 close note, the Findings-closed entry, and the Files-touched entry. Softened the code warning from "gateway recently restarted; recommendations resume as the ring buffers refill" to name the empty windows without asserting a restart as the sole cause, because some source metrics originate off the gateway registry and are absent regardless of restart (`cmd/lenny-gateway/workers.go:1089-1093`, `sampler.go:48-68`). Added a tier-1 service test (`TestGetRecommendationsHealthyWhenCategoryFilterStarvedRuleHasData`) and hardened the admin-level `TestRecommendationsEndpointCategoryFilter` to assert the healthy envelope on the filtered-starved path; updated the C4 target, intro, rationale, the tier-1 testing list, and Files-touched to add `pkg/gateway/externalapi/admin/recommendations_test.go`.
- **The Canonical Degradation Envelope was mislabeled §25.4 throughout; it is defined in §25.2.** Verified against the spec: the `## 25.2 Architecture Overview` heading is at `spec/25_agent-operability.md:128`, the `#### Canonical Degradation Envelope` subsection and its trigger sentence at `:203`/`:205`, the "Omitted when healthy" clause at `:233`, and `## 25.4` does not begin until `:769`, so lines 205 and 233 fall inside §25.2. The spec's own convention labels the envelope "(Section 25.2)" (`:1658`, `:1940`, `:1956`, `:2902`). Relabeled every reference to the envelope definition from §25.4 to §25.2 (or `Section 25.2`): the title, the scope sentence, the §2 decisions, the §3 summary, the C1 heading/Target/rationale, the C3 and C4 code and test comments, the C5 note, the Findings-closed entry, Open decision 1, the tier-1 and tier-11 testing items, and the Files-touched entry; and changed the injected cross-reference in the staged §25.3 line-605 text from `[§25.4](#canonical-degradation-envelope)` to `[Section 25.2](#canonical-degradation-envelope)` to match the spec's existing labels. Left the genuine §25.4 references unchanged (the Prometheus Requirement bullet at `:1432` and the Operational consequences table at `:1470`) and the genuine §25.15 reference (the Failure Mode Analysis row at `:4989`).

### Pass 2 (2026-07-03, automated)

- **C2's reworded cross-references, and the matching §25.3:586 and §25.2 staged clauses, described the `degradation.level: "degraded"` envelope as persisting "for hours after any restart" or "until the ring buffers refill", a duration the C3 implementation never produces.** C3 stamps degraded under `if evaluated > 0 && !anyData`, where `anyData` becomes true on the first sample of any source metric. Verified against the tree that a single gauge sample makes an evaluator return `DataAvailable: true` (`pkg/gateway/operability/recommendations/service.go:387-392`, `metricreader.go:152-159`), that the production sampler records an immediate first sample on startup and again every 30 seconds (`sampler.go:20`, `:138-139`; `cmd/lenny-gateway/workers.go:1094`), and that the sampled set includes gateway-registry gauges such as `lenny_credential_pool_utilization` (`sampler.go:62`, `gatewaymetrics/gatewaymetrics_credential.go:134`). For any active gateway the envelope therefore returns to `healthy` within roughly one sample interval of a restart, and it stays `degraded` persistently only when no source metric ever appears in the gateway registry, so neither "for hours" nor "until the ring buffers refill" is a correct duration for the degraded level. Reworded the three C2 passages (§25.4:1432, the §25.4:1470 operational-consequences table cell, and the §25.15:4989 failure-mode row) so the degraded envelope is scoped to the wholesale-empty window and returns to `healthy` as soon as any rule records a sample. Propagated the same predicate to the C1 staged text that shared the drift: the §25.3:586 Sliding window aggregation paragraph (replaced "Until they refill ... reports `level: degraded`" with the empty-buffer scoping), the §25.2 envelope-trigger parenthetical (replaced "empty ... until they refill" with "start empty after a gateway restart" and removed the em-dash before the cross-reference), the §25.2 omitted-when-healthy clause (replaced "reports `degraded` while that history is still accumulating" with the no-history-then-first-sample predicate), and the §25.3:605 warning phrase ("still filling" became "no rule has data yet") so the warning description matches the code warning at C3. Detached the "partial samples between restarts / reduced confidence" observation from the `degradation.level` duration by stating it as a per-item recommendation-reliability caveat. Extended the tier-11 consistency assertion so it requires that no §25 surface describes the degraded state as persisting "for hours" or "until the ring buffers refill" and that §25.2/§25.3/§25.4/§25.15 agree the envelope is `degraded` only while every ring buffer is empty, catching a future multi-hour-duration regression the prior assertion missed. Did not switch C3 to a window-fullness or sample-count predicate; keeping recommendations flagged while the windows are only partially filled would require that different predicate, which is a wider change this proposal does not make.

## Open decisions for review

All three decisions were resolved by sign-off on 2026-07-04 with the recommended/staged option, none altering the staged edits: **(1)** Option A (response-level §25.2 degradation envelope, `level: degraded`, `recommendations[]` stays empty); **(2)** `level: degraded` + a warning rather than a literal `confidence: 0.0`; **(3)** scope to the wholesale-empty post-restart case, per-rule partial starvation left as a separate follow-on. The original framings follow for reference.

1. **Option A (response-level envelope) versus Option B (per-item entries).** Recommended and staged: surface the post-restart signal through the response-level §25.2 degradation envelope at `level: degraded` (Option A). The finding text leaned toward Option B ("the rule emits a recommendation tagged `confidence: 0` / `dataAvailable: false` so an agent can see the rule exists but is starved"). This proposal chooses A because §25.2 designates the envelope as the response-level degradation signal, the post-restart condition is response-wide, and A preserves the committed no-entry test and the actionable-only meaning of `recommendations[]`. Option B's per-item `confidence: 0.0` is serializable (`Recommendation.Confidence` is not `omitempty`), so the choice is on design merits rather than a serialization constraint. If the reviewer requires per-rule granularity, the change set moves to Option B: rewrite line 605 to the emit-entry reading, overturn `TestGetRecommendationsEmptyWhenNoData`, and emit a starved entry per data-absent rule.
...
- `spec/25_agent-operability.md`: C1 (§25.3 Sliding window aggregation paragraph line 586; §25.3 Degradation subsection line 605; §25.2 envelope trigger line 205; §25.2 "Omitted when healthy" clause line 233), C2 (§25.4 Prometheus-requirement capacity-recommendations bullet line 1432; §25.4 operational-consequences Capacity recommendations row line 1470; §25.15 Prometheus-permanently-absent row line 4989).
- `pkg/gateway/operability/recommendations/service.go`: C3 (`GetRecommendations` aggregates per-rule `DataAvailable` over the whole catalog before the `?category=` filter narrows the emitted array, and stamps `level: degraded` with a warning when no rule in the catalog is data-bearing).
- `pkg/gateway/operability/recommendations/service_test.go`: C4 (`TestGetRecommendationsEmptyWhenNoData` extended to pin the degraded envelope; `TestGetRecommendationsStampsCompiledInDefaults_spec_25_13_4848` given a below-threshold sample; `TestGetRecommendationsHealthyEnvelopeWhenBelowThreshold` and `TestGetRecommendationsHealthyWhenCategoryFilterStarvedRuleHasData` added).
- `pkg/gateway/externalapi/admin/recommendations_test.go`: C4 (`TestRecommendationsEndpointCategoryFilter` hardened to assert the healthy envelope on the filtered-starved path; `conventions` import added).
