...
- **Status:** Implemented (2026-07-20). Spec applied and code landed on `proposal-b/c-17-ssa-conflict-retry`; tiers 0/1/2/11 green (independently verified), 98.4% changed-line coverage, design-conformance review clean. Ready for `--no-ff` integration. Verified (2026-07-19) after 4 adversarial review rounds (3 findings fixed). All three open decisions ratified on sign-off: (1) §4.6.3 stuck-episode increment semantics are authoritative; (2) continuation is via controller-runtime requeue; (3) the alert stays at `crd`/`controller` labels with per-resource attribution on the `crd_ssa_conflict_stuck` log.
- **Date:** 2026-07-19.
...
- **RATIFIED on sign-off (2026-07-20).** All three decisions below are ratified as proposed: (1) §4.6.3's stuck-episode increment semantics are authoritative and §16.1 is amended to match; (2) continuation past five consecutive 409s is via controller-runtime requeue, keeping the give-up-at-five loop; (3) the counter stays at `crd`/`controller` labels and the alert description drops the "any single resource" claim, with per-resource attribution on the `crd_ssa_conflict_stuck` log. Implement SPEC-1/2/3, CODE-1/2, DOC-1/2, TEST-1 as staged.
