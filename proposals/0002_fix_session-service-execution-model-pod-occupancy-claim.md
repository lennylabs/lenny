...
- **Status:** Draft for review. Initial recommendation authored from the 2026-06-11 execution-mode design discussion; the first-round open decisions were resolved the same day (Section 12). Seventeen adversarial convergence passes applied 2026-06-11 through 2026-06-12 (Section 11), followed by the promotion of Section 6 to exact anchored edit blocks (Section 11, Revision). Not yet converged.
- **Date:** 2026-06-11.
...
The execution-mode surface has accumulated four classes of defect.
...
Re-key the `lenny-sandboxclaim-guard` paragraph: the `PATCH`/`PUT` precondition widens from the single accept value `claimed` to the coarse per-pod live-claim phases (`claimed`, `sdk_connecting`, and `reserved`) so a `reserved → bound` rebind patch is admitted, the status-enumeration cross-reference names the pod-occupancy binding state, and the `CREATE` rule is stated as per-pod uniqueness with no concurrency exemption. Three edits inside the paragraph beginning `**`SandboxClaim` admission webhook (double-claim prevention):**`.

Replace:

> For `PATCH`/`PUT`, the webhook queries the API server for the referenced `Sandbox` (via `.spec.sandboxRef`) and rejects the request if the `Sandbox.status.phase` is not `claimed` (i.e., the target pod no longer holds an active claim to this `SandboxClaim` — e.g., it has been released back to `idle`, or has progressed into a terminal state such as `terminated`/`failed`). The rejection returns `403 Forbidden` with message `"SandboxClaim stale: referenced Sandbox <name> is in phase <phase>, not claimed; concurrent write rejected"`.

with:

```
For `PATCH`/`PUT`, the webhook queries the API server for the referenced `Sandbox` (via `.spec.sandboxRef`) and admits the write only when `Sandbox.status.phase` is one of the live occupancy phases `claimed`, `sdk_connecting`, or `reserved` ([Section 6.2](06_warm-pod-model.md#62-pod-state-machine)); this accept set admits the `reserved → bound` rebind patch that dispatches a same-tenant session onto a held pod. The webhook rejects the write when the pod has returned to `idle` or has progressed into a terminal state such as `terminated` or `failed`. The rejection returns `403 Forbidden` with message `"SandboxClaim stale: referenced Sandbox <name> is in phase <phase>, not a live occupancy phase; concurrent write rejected"`.
```
...
### 6.75 `spec/17_deployment-topology.md` §17.8.1 (operational-defaults max-idle row)
...
### 6.76 `spec/17_deployment-topology.md` §17.8.2 (delegation child-pool sizing restated in `sessionPolicy` terms)
...
```
### 6.77 `spec/18_build-sequence.md` phase overview and phase table (Phase 12c rename)
...
### 6.78 `spec/18_build-sequence.md` §18.10 (sandboxclaim guard PATCH/PUT accept set)

The Phase 3.5 `sandboxclaim_guard` deliverable pins the guard's PATCH/PUT precondition to the single accept value `claimed`, which contradicts the re-keyed §4.6.1 guard rule (Section 3.2): the per-pod claim's `reserved → bound` rebind patch and the recycle re-warm patches land while the pod projects `reserved` or `sdk_connecting`. In the deliverable bullet that names the ADR-007 enforcement, replace the clause:
...
```
PATCH/PUT rejection when the referenced `Sandbox.status.phase` is not one of the live occupancy-claim phases `claimed`, `sdk_connecting`, or `reserved`
```
...
### 6.79 `spec/18_build-sequence.md` §18.30 (Phase 12c deliverables and exit criteria)
...
### 6.80 `spec/23_competitive-landscape.md` §23 comparison table and §23.1 (execution modes)
...
### 6.81 `spec/24_lenny-ctl-command-reference.md` (mode-reference sweep; no edits)
...
### 6.82 `spec/26_reference-runtime-catalog.md` §26.2 (shared-assets mode re-key)
...
### 6.83 `spec/27_web-playground.md` §27.2 and §27.6 (playground idle override re-key)
...
### 6.84 `TESTING.md` §13.7 (sandboxclaim guard description)

The §13.7 `pkg/admission/sandboxclaim_guard` description pins the PATCH/PUT precondition to the single accept value `claimed` and asserts verbatim spec-message matching, so it must change in lockstep with the rewritten §4.6.1 guard paragraph and the §18.10 deliverable. Replace the bullet:
...
```
- `pkg/admission/sandboxclaim_guard` is the pure-decision logic for the `lenny-sandboxclaim-guard` webhook from §4.6.1 (ADR-007). CREATE rejects when a non-terminal `SandboxClaim` already exists for the same `Sandbox`; PATCH/PUT rejects when the referenced `Sandbox.status.phase` is not one of the live occupancy-claim phases `claimed`, `sdk_connecting`, or `reserved`, so a `reserved → bound` rebind patch is admitted and a write against an `idle` or terminal pod is rejected. Rejection messages match the spec verbatim.
```

### 6.85 `TESTING.md` §13.27 (Phase 12c title and test inventory)
...
| `docs/operator-guide/multi-tenancy.md` and `docs/operator-guide/namespace-and-isolation.md` | Tenant pinning and cross-tenant reuse as derivations; residual-state table. |
...
- **Orphan-GC live-binding-state set listed `sdk_connecting`, which the claim binding-state enum never holds:** the predicate is keyed to the claim binding state (Section 3.3), whose enumeration is `{bound, recycling, reserved, released, failed}` (Sections 3.2 and 5). `sdk_connecting` is a `Sandbox.status.phase` value that the `recycling` binding state projects to on preConnect pools (Section 3.3), not a binding state, so a binding-state-keyed predicate can never match it, and the `recycling` binding state already covers the coordinator-crash-during-scrub case the Pass-3 fix targets. The orphan-GC live-binding-state set in Sections 3.2, 3.3, the Section 6 spec/04:479 directive, the Section 9 test row, and the Pass-3 resolution narrows to the non-terminal binding states other than `reserved`, namely `bound` and `recycling`. `sdk_connecting` is retained only in the guard PATCH/PUT accept-set (Sections 5 and 6) and the coarse Sandbox phase enum (Section 3.3), where it is a `Sandbox.status.phase` value.
...
- **TESTING.md §13.7 Phase 3.5 guard PATCH/PUT description left stale by the guard re-key:** the proposal re-keys the `lenny-sandboxclaim-guard` PATCH/PUT accept-set at spec/04:384 and spec/18:201, but its only TESTING.md directive was scoped to §13.27. TESTING.md:1555 documents the guard's PATCH/PUT rule verbatim ("PATCH/PUT rejects when the referenced `Sandbox.status.phase` is not `claimed`") and asserts "Rejection messages match the spec verbatim," which would no longer hold after application. The TESTING.md directive (line 171) now covers §13.7 as well as §13.27, rewriting the §13.7 guard description to the re-keyed accept-set (reject when the phase is not one of `claimed`, `sdk_connecting`, or `reserved`), and Section 13 names §13.7 alongside §13.27.
...
- **§5.2 slot-assignment atomicity and rehydration paragraphs not re-keyed off `maxConcurrent` / concurrent-workspace:** the spec/05 §5.2 directive enumerated the surfaces it restages but omitted the §5.2 titled slot-counter paragraphs "Slot cleanup" (spec/05:515), "Concurrent-workspace slot assignment atomicity" (spec/05:519), "Post-recovery rehydration atomicity" (spec/05:521), and "Whole-pod replacement trigger" (spec/05:531). After application these would still name the removed `concurrent-workspace` mode and cap the intra-pod Redis slot counter at `maxConcurrent` (the field the proposal makes service-mode-only), so the session-mode concurrency gate the proposal relies on would be capped by the wrong bound, a granularity mismatch in a load-bearing concurrency gate, and the spec/12:189 key-table row the proposal already re-keys (line 165) links to these exact paragraphs. The spec/05 §5.2 directive (line 158) now restages spec/05:515, :519, :521, and :531, dropping the removed mode name and renaming the slot-counter cap and the `ceil(maxConcurrent / 2)` threshold from `maxConcurrent` to `sessionPolicy.maxConcurrentSessions` for the session-mode concurrency path. `pkg/gateway/slotcounter` (with its `Counter.Reserve(ctx, podID, maxConcurrent int32)` cap at slotcounter.go:93,244) and the cap value its `pkg/gateway/podclaim`/`pkg/gateway/podsession` callers pass were added to the Section 13 code list, so the gate reads the session-mode bound.
