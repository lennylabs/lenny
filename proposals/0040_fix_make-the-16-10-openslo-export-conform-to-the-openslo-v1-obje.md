...
- **Status:** Superseded by proposal 0041. During sign-off review the reviewer directed the notification-target representation to Option B (emit a shared top-level `AlertNotificationTarget` document referenced by `targetRef`, rather than a bare conventional `targetRef`) and required the notification target to be deployer-configurable, which in turn requires a §16.10 spec edit (to define the configuration surface and to name the new emitted document kind). That direction changes the core of this proposal's Decisions §2, so it is re-drafted as 0041 rather than patched. This file is retained for history; the object-model analysis in §1 and the split-policy / `alertPolicyRef` mechanics in §4 carry forward unchanged into 0041.
- **Date:** 2026-07-16.
