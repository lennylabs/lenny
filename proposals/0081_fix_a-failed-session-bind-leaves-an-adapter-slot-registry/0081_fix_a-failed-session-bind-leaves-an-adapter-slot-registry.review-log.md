...
CHANGELOG (compaction pass after non-spec rounds 2 to 8 of run 0081-opt6, the `[operator.prune-r8]` prune, and non-spec
round 1 of run 0081-opt7, 2026-09-23). Read the whole ledger, from `[non-spec.2.fix-G1.1]` through
`[non-spec.1.review-test-coverage.1]`, and lifted its durable residue into the block `#### Lifted 2026-09-23 (non-spec
2-8, prune-r8, opt7 non-spec 1)` at the end of `### Settled` and into new `### Traps` entries after the TLS WATCHOUT. The
ledger reuses round ids across runs: its `[non-spec.1.*]` entries are run 0081-opt7 and are the NEWEST, while `[non-spec.1.*]`
ids cited in older Settled lines are run 0081-opt4. Lifted: CODE-14's ownership of the §10.1.4 close-context split, the
S16 fold of CODE-1 and CODE-15, the pointer homes of the literal sweeps, the S-2 window, the send-before-require rule and
the pod-exclusion grounds, the Testing relabels, the per-slot leak record, the untokened-counter reachability home, the
client.go step split, the `ResumeParams` MISTAKE, the prune-r8 decisions, and about sixty FACT and WATCHOUT lines.
Rewritten under `CORRECTS` or a newer disagreeing entry: **CODE-6 still owns the per-member ten-second close context**
(now CODE-14), the `CleanupTimeoutSeconds` population entry (newer reading wins, with an `UNVERIFIED`), the two-sweep
site-count entry, the resume-refusal envelope entry, the §7.3 classifier entry, the golangci-lint entry and the
tree-unchanged entry. Closed and moved out of `### Open`: the leak double-book (confirmed and fixed in CODE-5), the
`PrepareWorkspace` write race (refuted), `s.reclaiming` growth, the adapter-contract mirror, the different-slot scrub
overlap, the CODE-10 wrapped-carrier question, the S4-to-S5 reliance, the R1b row, `**What the change costs.**`, checklist
S19, and the `/resume` setup-command addendum to decision 29 (its premise is false). Closed and dropped from `### Deferred`:
the DOCS-2 `ABORTED` entry (answered by DOCS-2's bind-attempt paragraph) and the ledger's R1b-row Deferred. Deleted as
superseded: the WATCHOUTs on unlabelled CODE-14 cases in the CODE-6 Testing heading and on CODE-7 and CODE-4 both setting
`unconditional_teardown`. Added to `### Open` and `### Deferred`: the reservation release on the caller's context, the
`Binder.Resume` compensation helper, the §7.2 step-1 cancellation gap, the rolled-back-start successor outcome, two
CODE-14 step-ownership leftovers, the 0078 impact-row misattribution and the S23 reason. No `## Retired` section was
created: the ledger is archived whole at the round boundary. Target: NOT reached. The section is about 830 lines (up from
675), because every lifted line is a claim nothing else in the live record carries. The hard compaction before run
...
- **CODE-14 owns the §10.1.4 per-member ten-second close context and the `guarded` conjunct in `terminateHeldSession`** (`CORRECTS` by `[non-spec.3.fix-design-G1.1]`, `[non-spec.3.review-applicability.1]`), in its paragraph **`terminateHeldSession`'s guard and close context.** CODE-6 (S14) names no guard symbol: it closes on the pass `ctx`, releases `terminateHeldSession` on `closeErr == nil && treeErr == nil` and `releaseSessionSlot` on `treeErr == nil`, each with one clause that CODE-14 adds `guarded`. CODE-1 still owns `answerShutdown` and the scrub start.
...
- **Checklist coverage map (`[operator.f13-repair]`):** S1=SPEC-5, S2=SPEC-1, S3=SPEC-2, S4=SPEC-3, S5=SPEC-4, S6=SPEC-6, S7=DOCS-1/2/3 plus the SPEC-3 tier-11 sweep, S8=CODE-3, S9=SCHEMA-1, S10=CODE-9, S11=CODE-7, S12=CODE-8, S13=CODE-4, S14=CODE-6, S15=CODE-14, S16=CODE-1 and CODE-15, S18=CODE-2, S19=CODE-13, S20=CODE-5, S22=CONF-1, S23=DOCS-4, S24-S26=CODE-10..12. S21 is deleted and its id retired; S17 is folded into S16 and its id retired; S16 lands the Testing subsection "Tier-1 adoption-ordering tests, every case walked".
...
- **DECISION (`[non-spec.1.fix-G5.1]`): S16 carries tier 9 and the registered-but-unbound class of the tier-9 matching-reclaim arm,** which needs CODE-15's widening of the tree-removal gate from `bound` to removed, beside the superseded and lease-release arms.
...
- **DECISION (`[non-spec.2.fix-G5.1]`): the two mandatory-field literal sweeps (`ShutdownRequest{` and the `adapterv1.` bind-field grep) have ONE home each, the files-touched "Every other file that grep ... names" bullets.** Checklist S14/S16 point there; Testing "Scope accounting" keeps only the silent-failure reason, the tiers per sweep and the `adapterclient.Client` signature note. The `adapterv1.` qualifier and the `files_updated_test.go` mid-session exception live in the bind-field bullet, whose grep now also carries `\|adapterclient\.ResumeParams{`. This replaces the older "three byte-identical sites" entry.
...
- **Every "reads, verbatim" anchor in SPEC-1 to SPEC-6 resolves exactly once in the tree (spec round 4), and no commit touched `spec/`, `pkg/`, `cmd/`, `tests/`, `docs/` or `schemas/` since cd18be606** (opt7 `[non-spec.1.review-citations.1]`; `[non-spec.1.review-applicability.1]` re-resolved all 26 "reads, verbatim" blocks). A re-sweep is owed only after a commit to those paths. The §29.4 step-13 tail has three hits and is disambiguated by the step.
...
- **DECISION (`[non-spec.2.fix-G3.1]`): S9, S20 and S22 are pointers.** S9 is "SCHEMA-1 in full, except its `ABSENT` claim-register row, which lands with S22"; S20 "CODE-5 in full."; S22 "CONF-1 in full, with the `ABSENT` row SCHEMA-1 stages for it."
...
- **The decision-53 wording "CODE-13, CODE-14 and CODE-15 carry their later steps" was corrected in `[operator.prune-r8]`** to CODE-15 with CODE-1 at S16, CODE-14 at S15 and CODE-13 at S19.

Testing labels:

- **DECISION (opt7 `[non-spec.1.fix-G5.1]`): Testing attributes each case to a DELIVERABLE, never to a checklist step.** Checklist steps are pointers, so Testing labels are the only step-to-test map.
- **DECISION: the adapter tier-1 heading is "### Adapter tests for CODE-6 and CODE-14, tier 1",** with one preamble sentence giving CODE-14 the guard-subject cases and the hold case's "refused at its guard acquisition" assertion (`CORRECTS` `[non-spec.2.fix-design-G1.1]`'s rejection of the rename).
...
- **DECISION: the tier-7a `TestSlotIdentifierReclaimHoldRefusesABindUntilTheCleanupReturns_spec_5_2` lands with CODE-1** ("which routes `Shutdown` through `reclaimSlotLocked`"). The file's other cases also drive a removing `Shutdown`, so the file is first created at S16.
...
- **DECISION (`[non-spec.4.fix-G1.1]`): the files-touched `pkg/adapter/holdstate.go` bullet is a pointer at CODE-14 and CODE-6.** The `terminateHeldSession` release predicate and `runtime_close_failed` live only in CODE-6; the separately deferred guard release only in CODE-14.
...
- **DECISION: CODE-1's removing-arm comment cites CODE-14's "Disposition of an expired acquisition at a removing site" in one sentence** and restates none of it.
...
- **WATCHOUT: S16 carries the tier-1 `exited_cleanly` injected-failure cases in pointer form** (Testing, "Adapter tests
...
- **WATCHOUT: checklist steps are pointers, so a stale Testing label is a step-assignment defect.** When a deliverable is
  re-cut, update the Testing group label and heading with the checklist. The gateway group label is the only owner list
  for its tier-1 files; do not add one back to files-touched. Name a case by what it checks, not by a helper name a later
  fix can rename.
...
- **Would any gate over staged Go doc comments newly fail because of text the staging ADDS?** UNVERIFIED: spec link anchors and N3 phrases were checked (opt7 `[non-spec.1.review-edit-sites.1]`, `[non-spec.1.review-mechanism.1]`); Go doc-comment gates were not.
...
- **Does CODE-13's `Binder.Resume` branch call `compensateFailedSlotBind` (detached ctx) or a bespoke `ShutdownReclaim`?** UNVERIFIED (opt7 `[non-spec.1.review-test-coverage.1]`): the prose says "the compensation" and takes the helper's three arguments off `ResumeRequest`, which matches the helper.
...
- **Should the SPEC-5 §15.1 preamble's "A refusal arriving at any other bind-sequence request reaches the client under the envelope that stage already selects." be cut?** OPEN [`[spec-recheck.1.fix-G2.1]`, `[spec-recheck.2.review-single-source.1]`, opt7 `[non-spec.1.review-single-source.1]`]: it and the spec Edge-cases envelope bullet both restate the §4.7.1 after-rule-9 envelope rule; only the DOCS-3 copy was cut. A spec-lane prune cuts the two commentary copies.
...
- DEFERRED [non-spec-changes.md, CODE-6 and the §10.1.4 parked-member Edge-cases bullet]: the disposition's claim that an
  ...
  wide, since a free guard still acquires. The remedy is a code-lane design choice. UNVERIFIED: `[non-spec.6.review-mechanism.1]`
  (`CORRECTS`) argues the falsity claim is overstated, because an expired acquisition at a pass-2 member still means that
  member's guard was held when the deadline passed, so "premature removal" holds for it too. Left for the code-lane owner
  to settle.
...
### [non-spec.2.fix-G1.1]
DECISION: S15 runs tiers 0 and 1 only; every CODE-14 tier-7a case, including the §10.1.4 hold-termination rerun of the admitted-before-hold case, lands at S16 with CODE-1 — BECAUSE Testing homes `slot_reclaim_hold_race_test.go` and its spec-map and `slotAddressCaseFiles` registration with CODE-1, and splitting the file across S15/S16 would fail the tier-0 slot-address gate at S15 — ALTERNATIVES: create the file and its registration at S15 for the hold-termination rerun (rejected: splits one file across two steps and moves the registration out of its Testing home).
CORRECTS [standing-context DECISION "tier-7a ... lands with CODE-1"] and [non-spec.1 FACT at review-log ~1258]: not every case in the file drives a removing `Shutdown`; the third arm's second run drives a §10.1.4 hold termination. The file still lands whole at S16 because Testing places it with CODE-1, not because every case needs CODE-1's handler.
FACT: the Open entry "Which deliverable lands the tier-7a case ... §10.1.4 second run?" is closed by this fix and was deleted from the review log's `### Open`. — EVIDENCE: implementation-checklist.md S15/S16 lines.

### [non-spec.2.fix-design-G1.1]
DECISION: every CODE-14 tier-7a case lands at S16; S15 runs tiers 0 and 1 only — BECAUSE all CODE-14 tier-7a cases (the TestSlotIdentifierReclaimHold... case, the lock-order case, and "a reclaim against a section admitted before the hold opened" with its §10.1.4 rerun) live in slot_reclaim_hold_race_test.go, which Testing lands with CODE-1 together with its spec-map and slotAddressCaseFiles registration — ALTERNATIVES: landing only the §10.1.4 hold-termination rerun at S15, rejected because it splits one file across two steps and moves the tier-0 registration to S15, which adds text and a sequencing rule.
FACT: closes the review-log ### Open entry "Which deliverable lands the tier-7a case 'a reclaim against a section admitted before the hold opened'"; answer: S16, with the rest of the file. EVIDENCE: non-spec-changes.md Testing **Tier 7a, `TestSlotIdentifierReclaimHoldRefusesABindUntilTheCleanupReturns_spec_5_2`** ("It lands with CODE-1").
WATCHOUT: the tier-7a "concurrent-resolve race" case directly above it covers the bind_attempt stamp, not CODE-14. Do not count it as an S15 tier-7a case.

### [non-spec.2.review-applicability.1]

FACT: the r1 diff touched only non-spec-changes.md and summary.md; the checklist and spec-changes are unchanged since the r1 snapshot. EVIDENCE: `diff -rq` against scratchpad/cp-snap/0081-opt7/non-spec-r1-prefix.
FACT: every label the r1 pointer reductions cite resolves: CODE-5 **The refusals stay retryable on the concurrent-bind path** (non-spec :1157), CODE-6 **The five resolve sites keep the shared helper.** (:1541), CODE-14 **Disposition of an expired acquisition at a removing site** (:1671), CODE-4 **The budget:** (:798), the accepted failure modes **A compensating `Shutdown` can spend its budget waiting on a guarded `Resume`** (:3776) and spec-changes **A retry that meets the reclaim hold spends an attempt on it** (spec-changes :104), the §5.2 table's clean-exit column (spec-changes :594), and the §4.7.1 envelope paragraph after rule 9 (spec-changes, after :1017).
FACT: the deleted chokepoint paragraph lost no content, because CODE-6's staged `ensureSlotStateLocked` doc comment and code carry the three callers, the arm order and the hold test ahead of the map lookup. EVIDENCE: non-spec :1395-1440.
FACT: the per-slot leak re-key's full implementer set is start.go:2848, slothealth.go:121, scrubreport_server.go:192/:468, scrubreporter_seams.go:184, the fakes in scrubreport_server_test.go:286 and scrubreporter_seams_test.go, AND `perReleaseNoopLedger` at tests/tier4_integration/recycle_scrub_path_test.go:928. The last is covered only by CODE-5's "every other DrainLedger implementer" clause and the files-touched Tests list, and S20 carries tier 4. `SlotBindError.SlotID` is the session id (slotfailure.go:61), so the dedupe key agrees with RecordSessionScrub's sessionID.
WATCHOUT: S20 edits the `RecordLeak` doc comment "as CODE-3 leaves it" but its Depends-on does not name S8; the linear order saves it. EVIDENCE: implementation-checklist S20; non-spec :1112.
WATCHOUT: the r1 fix pinned slot_reclaim_hold_race_test.go and its spec-map and slotAddressCaseFiles registration to CODE-1 (S16), but S15 still lists tier 7a, and the only CODE-14 tier-7a work that does not drive a removing Shutdown (the §10.1.4 hold-termination rerun) lives "in the same file". I filed this as a finding. EVIDENCE: non-spec :3521-3527, :3553-3560; checklist S15.
CORRECTS [Open "Which deliverable lands the tier-7a case 'a reclaim against a section admitted before the hold opened'"]: the r1 text now puts the file at S16, so S15's 7a listing is the part that disagrees.

### [non-spec.2.review-mechanism.1]
FACT: The per-slot leak record in CODE-5 ("The persistent leak record is keyed by slot.") checks out against the tree. There is one tracker instance (cmd/lenny-gateway/main.go:728-730 passes slotHealth into NewDrainLedger). The only DrainLedger implementers are drainLedger (pkg/gateway/session/recycle/scrubreporter_seams.go:184), fakeLedger (pkg/gateway/mcpfabric/delegationtree/leasecontrol/scrubreport_server_test.go:286) and perReleaseNoopLedger (tests/tier4_integration/recycle_scrub_path_test.go:928), and the files-touched Tests list already names all three files. Tracker.leaked is read only at slothealth.go:124,153,167,181. Nothing outside slotretry_test.go:355 calls Counts. Gateway SlotID equals SessionID (pkg/gateway/podlifecycle/podsession/slotbinder.go:149), so the report route's session id and accountSlotFailure's sbe.SlotID land on the same key.
FACT: concurrentSlotBinder.BindSlot really does return slot "slot" to every goroutine, and its release always errors (pkg/gateway/sessionserver/slotretry_load_test.go:27-36), so under per-slot keying TestSlotRetryConcurrentLeaksReachThreshold_spec_6_2 would never drain unless the proposal's fixture edit lands. The slotretry_test.go leak cases already pass distinct ids ("slot-1", "slot-2", and "pod-d" and "pod-e" at threshold 1).
OPEN: The per-slot key collapses a same-session retry that leaks on the same pod into one leak, while SlotClaimer.ReleaseSlot(leaked=true) withholds one Redis counter unit per release (pkg/gateway/podlifecycle/podclaim/slotclaimer.go:818-835). ClaimSlot prefers a pod that already hosts the tenant (slotclaimer.go:367-372), so the retry is likely to land on that same pod. Not filed, for two reasons. The shipped slotstate.Registry.MarkLeaked is already keyed by slotID (pkg/sandbox/slotstate/registry.go:99-114), so the gauge has the same granularity. And in spec terms a slot is its identifier, so the Redis double-withhold is a pre-existing property. A human decides whether the unhealthy count should follow slot identity or withheld occupancy.
FACT: All the pointer targets that round-1 fixes introduced resolve: CODE-5 **The refusals stay retryable on the concurrent-bind path**, CODE-6 **The five resolve sites keep the shared helper.**, CODE-4 **The budget:**, the accepted failure mode **A compensating `Shutdown` can spend its budget waiting on a guarded `Resume`**, the spec Edge-cases **A retry that meets the reclaim hold spends an attempt on it** (which carries the wait-and-retry rejection), the §4.7.1 after-rule-9 envelope paragraph (spec-changes.md, directly after rule 9), and the SPEC-6 untokened row. CODE-1's exited_cleanly expression `closeErr == nil && (live || (guarded && treeErr == nil))` agrees with every §5.2 table row that involves a Shutdown.
FACT: Closes part of the Open **Would any gate over staged Go doc comments newly fail because of text the staging ADDS?** I checked tests/tier0_static/slot_assignment_attribution_test.go (the patterns "assigns a slot", "on slot assignment" and "adapter-assign") and slot_absence_claim_comment_test.go (the pattern "has/names no slot") against non-spec-changes.md. The only hit is the DOCS-2 docs row "names no slot" (line 2665), which is docs rather than a Go comment. Neither the N3 reserved phrases nor spec line citations appear in staged Go blocks. adapter_barrier_doc_comment_scope_test.go reads only coordination.go CheckpointBarrier, which the staging does not touch. I did not check the remaining Go-comment gates (checkpoint_*_comment, identifier_resolution).

### [non-spec.2.review-performance.1]
FACT: slothealth.Tracker is one in-memory instance per gateway replica, shared by sessionserver (applySlotRetryPolicy) and the scrub-report drainLedger through cmd/lenny-gateway wiring; across replicas nothing is shared, so CODE-5's per-slot leak set dedupes the report route and the unclean-response route only when both land on one replica, and is otherwise no worse than the shipped per-pod int — EVIDENCE: cmd/lenny-gateway/sessiondeps.go:261, cmd/lenny-gateway/main.go:730, cmd/lenny-gateway/controlserver.go:156
FACT: SlotBindError.SlotID is the session id, and slotstate.Registry.MarkLeaked is already keyed by slot id, so CODE-5's per-slot Tracker.leaked set matches the registry's granularity; the Redis slot counter stays a plain count — EVIDENCE: pkg/gateway/podlifecycle/podsession/slotfailure.go:61, pkg/sandbox/slotstate/registry.go:99-115, pkg/gateway/podlifecycle/podclaim/slotclaimer.go:818-840
FACT: Tracker.Forget has one production caller (the applySlotRetryPolicy drain); a pod drained through drainLedger.RecordLeak is never forgotten, so the per-pod leak map already grows by one entry per leaked pod over a replica's life. CODE-5's set makes each entry a few session-id strings rather than an int, which is not a new bottleneck — EVIDENCE: pkg/gateway/sessionserver/start.go:2865, pkg/gateway/session/recycle/scrubreporter_seams.go:184-196
FACT: a same-session retry that lands on the pod still holding its first attempt's unacknowledged entry is refused SUPERSEDED and its compensation answers superseded, which reads clean, so it decrements the Redis counter and takes the windowed RecordFailure; the per-slot leak set therefore does not hide a second withheld decrement in the ordinary case — EVIDENCE: proposal non-spec-changes.md:3122, :3157
FACT: tests/tier4_integration/recycle_scrub_path_test.go:928 is a third DrainLedger implementer (perReleaseNoopLedger); CODE-5's "every other DrainLedger implementer" covers it, and the file is already on the Tests list (non-spec-changes.md:3991) — EVIDENCE: tests/tier4_integration/recycle_scrub_path_test.go:928

### [non-spec.2.review-reliability.1]
DECISION: no reliability findings in non-spec round 2 of run 0081-opt7 — BECAUSE the only new mechanism in the r1-to-r2 diff is CODE-5's per-slot leak record, and it holds under redelivery and retry: a re-sent ReportSessionScrub now records its slot once where the shipped `t.leaked[pod]++` counted it twice, drainLedger.RecordLeak still re-runs Unhealthy and the idempotent StampDrainRequest on a repeat, so a report the gateway failed with Internal still recovers on the adapter's resend, and the gateway's slotID is the session id (podclaim/slotclaimer.go:760), which ReportSessionScrub already rejects when empty (leasecontrol/scrubreport_server.go:84-87) — ALTERNATIVES: filing "the one shared tracker" as false across gateway replicas was rejected, because each replica's tracker sees at most one of the two routes' records for a slot, so no tracker over-counts and the replica-dilution question is the existing pre-existing Open.
FACT: a same-session retry that lands on the pod where its earlier attempt leaked cannot leak a second Redis slot-counter unit that the per-slot set would then under-count. The surviving A1-stamped entry makes the identity gate refuse A2 (superseded, so Leaked is false), and a deregistered entry whose cleanup failed stays held, so A2's compensation answers ABSENT (clean). This backs `[non-spec.1.fix-design-G1.1]`'s accepted-consequence DECISION. EVIDENCE: non-spec-changes.md CODE-13 superseded paragraph (~:895-905); :937-940
USEFUL [non-spec.1.fix-design-G1.1]: its DrainLedger implementer list and the scrub_report_wiring_test distinct-ids FACT made the edit-site sweep for the RecordLeak signature change a two-grep check.

### [non-spec.2.review-single-source.1]
FACT: the round-1 fix diff (non-spec-changes.md and summary.md only; spec-changes.md and the checklist unchanged) is net reduction. Every new pointer resolves to a live home: CODE-5 **The refusals stay retryable on the concurrent-bind path** (:1157), CODE-6 **The five resolve sites keep the shared helper.** (:1541), accepted failure mode **A compensating `Shutdown` can spend its budget waiting on a guarded `Resume`.** (:3776), spec-changes accepted failure mode **A retry that meets the reclaim hold spends an attempt on it.** (spec-changes.md:104), the §5.2 table's clean-exit column (spec-changes.md:594-600), and the §4.7.1 envelope paragraph after rule 9 (spec-changes.md:1018). EVIDENCE: as cited
FACT: CODE-5 **The persistent leak record is keyed by slot.** (non-spec-changes.md:1101) is the single statement of the per-slot `RecordLeak` keying; the Testing bullet **The leak record is per slot.** asserts it, and the 0079/0080 impact rows mention it in a clause. The Deliverable index row for CODE-5 (summary.md:1045) and files-touched (:3939-3941) carry the three new files. EVIDENCE: as cited
FACT: the chokepoint-placement reason (a comparison in the workspace handlers leaves the claim path ungated) now survives only in the summary decision bullet (summary.md:54-59); no non-spec copy remains after the Design paragraph cut. EVIDENCE: grep "ungated"

### [non-spec.2.review-test-coverage.1]
FACT: the r2 diff touches only non-spec-changes.md and summary.md. The new CODE-5 leak re-key (`RecordLeak(pod, slotID)`) is pinned: tier-1 "The leak record is per slot" case, the `fakeLedger` forwarding assertion in scrubreport_server_test.go, and `drainLedger`'s forwarding is pinned indirectly because `recordNLeaks` (pkg/gateway/session/recycle/scrubreporter_seams_test.go:489-495) must pass distinct slot ids and the existing threshold-drain envtest cases then fail against a constant forward. EVIDENCE: pkg/gateway/session/recycle/scrubreporter_seams.go:184-185; non-spec-changes.md "The leak record is per slot."
FACT: every non-production `DrainLedger` implementer in the tree is `fakeLedger` (leasecontrol/scrubreport_server_test.go:286) and `perReleaseNoopLedger` (tests/tier4_integration/recycle_scrub_path_test.go:928); both files are in the Tests list. cmd/lenny-gateway/scrub_report_wiring_test.go names RecordLeak only in a comment (:289).
WATCHOUT: the r2 sentence "It lands with CODE-1" on `slot_reclaim_hold_race_test.go` moves the §10.1.4 hold-termination second run (the only CODE-14 tier-7a case that drives no `Shutdown`) to S16, while checklist S15 still carries tier 7a and excepts only removing-`Shutdown` cases. Closes the `### Open` entry "Which deliverable lands the tier-7a case 'a reclaim against a section admitted before the hold opened'" as a filed finding. EVIDENCE: implementation-checklist.md S15 line; non-spec-changes.md "Tier 7a, `TestSlotIdentifierReclaimHoldRefusesABindUntilTheCleanupReturns_spec_5_2`".

### [non-spec.3.review-applicability.1]
FACT: The r2-to-r3 diff (review logs excluded) touches only checklist S15/S16: S15 drops tier 7a and defers all of CODE-14's tier-7a cases to S16. Both CODE-14 tier-7a cases (the lock-order assertion and the reclaim against a section admitted before the hold opened) live in slot_reclaim_hold_race_test.go, which lands with CODE-1 at S16, so the new S15 tier list (0, 1) and the S16 wording agree with Testing. EVIDENCE: implementation-checklist.md:41-44; non-spec-changes.md:3521-3575
WATCHOUT: the start-versus-reclaim race in slot_bind_attempt_race_test.go has a Resume arm that exercises CODE-14's Resume guard, but its StartSession arm needs CODE-2's confirmation (S18) and no step names the file's landing step. The r1 fix treats it as outside "CODE-14's tier-7a cases"; do not read S16's "its tier-7a cases" as pulling that file into S16. EVIDENCE: non-spec-changes.md:3467-3502

### [non-spec.3.review-test-coverage.1]
FACT: The r2-to-r3 diff touches only checklist S15/S16: S15 drops tier 7a and every CODE-14 tier-7a case moves to S16. All of CODE-14's tier-7a arms live in tests/tier7a_load_local/slot_reclaim_hold_race_test.go, which lands with CODE-1 at S16. No other proposal site names S15's tiers, so nothing else went stale. EVIDENCE: implementation-checklist.md:41-44; non-spec-changes.md:3521-3523, 3540-3587

### [non-spec.4.fix-G1.1]
- DECISION: `releaseSessionSlotUnderGuard` takes `(sessionID string, guarded bool)`; the guard-acquiring `releaseSessionSlot` passes the boolean `lockSlotGuard` returned and `Resume` passes true — BECAUSE the hold release is deferred inside the body and needs the `guarded` conjunct CODE-6/CODE-14 assign to its `completed` predicate — ALTERNATIVES: body returns treeErr and callers release the hold (moves the release site, duplicates the predicate); wrapper inlines the body (duplicates what the split shares).
- WATCHOUT: the `completed = guarded && treeErr == nil` predicate is not restated in CODE-14's `Resume` paragraph; its homes are CODE-6's hold bullet and CODE-14's **Disposition of an expired acquisition at a removing site**. Do not add a copy there.

### [non-spec.4.fix-G2.1]
DECISION: each new multi-case test file lands whole with the last deliverable its cases need: slot_bind_attempt_race_test.go (three cases plus spec-map and slotAddressCaseFiles rows) with CODE-2 (S18), and slot_credential_reclaim_fence_test.go (three arms plus rows) with CODE-13 (S19). Testing carries one label sentence per file. BECAUSE the StartSession and reverse-ordering arms need CODE-2's confirmation (the shipped noteRuntimeStarted in pkg/adapter/runtimegeneration.go records runtimeLive unconditionally), the Resume arm needs CODE-14 and CODE-15 (S15, S16), concurrent-resolve needs CODE-6 (S14), and the tier-9 lease-release arm needs CODE-13's releaseAttemptCredentials. S18 depends on S14 and S16, and S19 depends on S16, so every case passes at its file's step. ALTERNATIVES: per-arm labels spread across S14, S16 and S19, rejected because they split a file and its tier-0 rows across steps and re-add checklist clauses; landing the race file at S14 with skips, rejected because it adds skip-register entries.
CORRECTS [non-spec.1.fix-G5.1]: the Settled clause "S16 carries tier 9 and the registered-but-unbound class of the tier-9 matching-reclaim arm" no longer holds. The whole tier-9 fence file lands with CODE-13 (S19). S16 keeps tier 9 in its Tiers line because CODE-1's ShutdownRequest sweep touches existing tier-9 files.
WATCHOUT: S14 and S16 keep tiers 7a and 9 even though no new file in those tiers lands there, because the bind-field and ShutdownRequest literal sweeps edit existing tier-7a and tier-9 files. Do not drop those tiers when a new file moves off a step.

### [non-spec.4.fix-G3.1]
DECISION: deleted CODE-13's paragraph "`SUPERSEDED` and `ABSENT` are reclaims acknowledged clean that correctly performed nothing ... on the common retry case." whole, including its closing reason sentence — BECAUSE outcome meanings are §4.7.1 rule 15, the untokened arm and its survival residue are rule 13 and SPEC-5, the acknowledged-clean inclusion is staged §7.1, and the reason the outcome stays out of the leaked disposition is homed in SPEC-2's commentary, which CODE-13 **The leaked disposition, at the caller.** already cites — ALTERNATIVES: the reviewer's pointer sentence plus the kept reason sentence (a second pointer and a second reason statement).
DECISION: the compensating Shutdown's two-bound rule (RPC deadline = budget, graceful window = budget/2) and its SIGTERM-pivot reason live only in CODE-13's `compensateFailedSlotBind` doc comment beside `budget/2`. Deleted the restatement from the summary Decisions bullet "The compensation is best-effort and its own failure is counted." and the "pins half of it as the adapter's graceful window" clause from CODE-4's `slotCleanupBudget` doc comment, which returns the budget and halves nothing — ALTERNATIVES: a summary pointer "Its budget and graceful window are CODE-13's" (unneeded; the Deliverable index points to CODE-13); homing the half-budget rule on `slotCleanupBudget` (wrong function).
DECISION: DOCS-2 **Bind attempt token.** paragraph now ends its second sentence at "name the entry it is entitled to destroy." The deleted colon clause said a no-longer-owning attempt gets `superseded`, which is false when the entry was removed with no successor (rule 11, `absent`); the page's `Shutdown` row already carries both outcomes. Paragraph remains one physical line with no spec section number.
FACT: the Testing case **Per-stage compensation table.** cites CODE-4's **The budget:** only for the floor and asserts the half-budget relation itself; it stays true after the CODE-4 comment edit. EVIDENCE: non-spec-changes.md **Per-stage compensation table.** bullet; `budget/2` at CODE-13's `ShutdownReclaim` call.

### [non-spec.4.fix-G4.1]
DECISION: the queue-head hold is split by subject per `[f6.apply.compensation-queue-hold]` — the summary defects entry **A queued waiter's wait bound does not cover the time it spends behind the FIFO head.** holds only the shipped wait-bound defect (head-only admission, bound tested after the ticket, 30s default, uncapped zero setup timeout) and points at the Edge case for the compensation increment; the non-spec Edge case **A compensating `Shutdown` holds the pool's queue head for its own budget.** holds the increment, its deadline, the WARM_POOL_EXHAUSTED exit and the off-path rejection, and points back at the summary entry for the shipped facts — BECAUSE each site restated the other's half — ALTERNATIVES: re-synchronising both copies (drift), merging into one entry (the decision homes two subjects).
DECISION: the summary cleanup-residue defects entry cites CODE-15's paragraph 'The report stays keyed on `closeErr`' for the ground against re-keying rather than restating its cost argument; CODE-15 is the home.
WATCHOUT: the Edge case still carries `spec/05_runtime-registry-and-pool-model.md:545` and `:440` line citations; proposals/ is outside the citation-gate domain, so they are evidence, but a lens may flag them as drift. Not touched by G4. — EVIDENCE: non-spec-changes.md Edge case **A compensating `Shutdown` holds the pool's queue head for its own budget.**

### [non-spec.4.fix-G5.1]
DECISION: CODE-2 rollback comment, CODE-5 started-session arm comment and CODE-6 ensureSlotStateLocked doc comment each cut to citation + act + at most one trap/reason clause — BECAUSE the reasoning homes are spec-changes §4.7.1 rule 8, the **The stamp-once rule.** and **The registry critical section.** paragraphs, CODE-2's "The rollback deregisters nothing" prose, and CODE-5's FailedPrecondition/classifier prose — ALTERNATIVES: citing "CODE-2's prose"/"CODE-5 gives why" in the Go comment was rejected because a proposal id means nothing once the code lands in pkg/.
WATCHOUT: the rollback comment keeps "Deregister nothing here" deliberately; it is the only warning at the site where an implementor would add releaseSessionSlot, which deletes a successor attempt's staged workspace. Do not cut it as restatement. — EVIDENCE: non-spec-changes.md CODE-2 rollback block; spec-changes.md rule 8.
OPEN: two separate-finding sites left unedited by design: CODE-5's Aborted arm comment names CODE-6/CODE-2 ids in code bound for pkg/gateway; ensureSlotStateLocked doc comment's second paragraph ("no handler carries a second copy") restates CODE-6's lead-in.

### [non-spec.4.fix-design-G1.1]
DECISION: close the releaseSessionSlotUnderGuard gap by giving the body a `guarded bool` parameter (wrapper passes `lockSlotGuard`'s boolean, `Resume` passes `true`), a one-clause replace in CODE-14's `Resume` guard paragraph — BECAUSE the hold's only release is deferred inside the body, which otherwise cannot see the wrapper's expired acquisition — ALTERNATIVES: body returns treeErr and the wrapper releases the hold (moves the release site the CODE-6/CODE-? hold bullets name, larger cascade); wrapper inlines the body (duplicates the body the split exists to share).
FACT: no staged Go block or other staged prose spells the under-guard signature; the only signature sites are non-spec-changes.md CODE-14 `Resume` guard paragraph (the two lines naming both forms). The `completed` statements at CODE-6's hold bullet ("CODE-14 adds the `guarded` conjunct") and CODE-14's disposition paragraph stay true unchanged. EVIDENCE: grep of `releaseSessionSlot(` and `UnderGuard` across the proposal.
WATCHOUT: the fixer must not restate `completed = guarded && treeErr == nil` in the `Resume` paragraph; that predicate's home is the review-log-recorded per-site statement (CODE-6 hold bullet plus CODE-14 disposition), and the paragraph only names who supplies the boolean.

### [non-spec.4.fix-design-G2.1]
DECISION: each new multi-case test file lands whole with the LAST deliverable its cases need: tests/tier7a_load_local/slot_bind_attempt_race_test.go with CODE-2 (S18), tests/tier9_security/slot_credential_reclaim_fence_test.go with CODE-13 (S19). One label sentence per file in Testing; S16's tier-9 clause is deleted. BECAUSE the StartSession arm and the reverse ordering need CODE-2's confirmation (pkg/adapter/runtimegeneration.go noteRuntimeStarted records unconditionally today), the superseded arm needs CODE-1, the registered-but-unbound class needs CODE-15, and the lease-release arm needs CODE-13's releaseAttemptCredentials; S18 depends on S14 and S16, S19 on S16, so every case passes at its file's step and the file plus its spec-map and slotAddressCaseFiles rows land in one commit. ALTERNATIVES: per-arm labels (concurrent-resolve with CODE-6, superseded with CODE-1, lease-release with CODE-13) rejected: it splits each file across steps, forces the tier-0 registrations to land at the earliest step, and needs a checklist clause per split, which is the restating layer the prune removed.
CORRECTS [non-spec.1.fix-G5.1] (Settled: "S16 carries tier 9 and the registered-but-unbound class of the tier-9 matching-reclaim arm"): after G2 the whole credential-fence file lands with CODE-13 at S19. S16 keeps tier 9 only because CODE-1's ShutdownRequest sweep edits existing tier-9 files.
FACT: S14 and S16 keep tier 9, and S14 keeps tier 7a, independently of the new files: the bind-field and ShutdownRequest literal sweeps hit existing tests/tier9_security files (adapter_shared_mcp_surface_test.go, session_teardown_surface_test.go, adapter_hold_termination_surface_test.go, etc.) and tier7a files (shutdown_drain_gate_race_test.go, podmcp_arming_handoff_test.go). EVIDENCE: grep of ShutdownRequest{ and adapterv1.*Request literals over tests/tier9_security and tests/tier7a_load_local.
USEFUL [non-spec.3.review-applicability.1]: its WATCHOUT that no step names slot_bind_attempt_race_test.go's landing step and that S16's "its tier-7a cases" must not be read as pulling it in was the finding's core.

### [non-spec.4.fix-design-G3.1]
DECISION: CODE-13's paragraph "`SUPERSEDED` and `ABSENT` are reclaims acknowledged clean ..." is deleted whole, including its last sentence — BECAUSE the outcome meanings are §4.7.1 rule 15, the untokened arm and its residue are rule 13, "acknowledged clean for superseded/absent" is staged §7.1, and the instruction "the outcome does not enter it" already sits in **The leaked disposition, at the caller.** with SPEC-2's commentary as the reason home — ALTERNATIVES: reviewer's replace-plus-keep-last-sentence (rejected: the kept sentence is a second reason for a branch the code does not have).
DECISION: the compensation budget and graceful window have one home, CODE-13's `compensateFailedSlotBind` doc comment (where `budget/2` is written). CODE-4's `slotCleanupBudget` comment drops "and pins half of it as the adapter's graceful window"; the summary bullet drops "Its budget is ... SIGTERM pivot." — BECAUSE [non-spec.11.fix-G1.1] cites the formula and never instantiates it, and slotCleanupBudget does not halve anything.
DECISION: DOCS-2 **Bind attempt token.** paragraph drops the colon clause "a reclaim naming an attempt that no longer owns the slot answers `superseded` ... started" — BECAUSE the clause is wrong for the removed-entry case (rule 11, `absent`) and the same page's `Shutdown` row already gives both outcomes; "name the entry it is entitled to destroy" keeps the reader fact — ALTERNATIVES: reviewer's reword "removes nothing unless that attempt created the entry" (accurate, but replace is a higher rung than delete).
WATCHOUT: `compensateFailedSlotBind` comment's §11.4 revoke fan-out precedent sentence is a defence, not an instruction; not filed this round. EVIDENCE: non-spec-changes.md CODE-13 doc comment, "The shipped §11.4 revoke fan-out holds the same relation".

### [non-spec.4.fix-design-G4.1]
DECISION: queue-head pair split by subject per `[f6.apply.compensation-queue-hold]` — the summary defects row owns the shipped wait-bound defect (head-only admission, bound tested after the ticket, 30s default, runWithQueue closure, uncapped zero setup timeout); the non-spec Edge case owns the accepted compensation increment, its deadline, the WARM_POOL_EXHAUSTED exit, and the off-path rejection. Each cites the other by bold label — BECAUSE the decision already assigned the two homes — ALTERNATIVES: keeping both full copies in sync (rejected: drift).
DECISION: closeErr keying argument homed in CODE-15's paragraph "The report stays keyed on `closeErr`"; the summary row cites it in one sentence.
WATCHOUT: non-spec-changes.md Testing case near "turns red equally if the report is keyed on both errors" is a discriminator note and must stay; it is not a restatement of CODE-15's argument. EVIDENCE: non-spec-changes.md:3102

### [non-spec.4.fix-design-G5.1]
DECISION: G5 is three trivial REPLACE-in-place cuts of staged Go comments to a spec citation plus the act; the prose paragraphs stay the reason homes (CODE-2 "The rollback deregisters nothing", CODE-5 the FailedPrecondition paragraph and the §7.3 classifier paragraph, spec §4.7.1 critical-section and stamp-once paragraphs) — BECAUSE rule 8 (spec-changes.md "The start-confirmation rule") already states no record, no removal, no cleanup report and why; the comment only needs to point at it — ALTERNATIVES: the reviewer's "CODE-2's prose gives"/"CODE-5 gives why" wording, rejected because proposal deliverable ids mean nothing once the comment lands in pkg/; cite the spec rule instead.
WATCHOUT: the CODE-2 rollback comment must keep a one-clause trap telling the implementor not to release/deregister here (a releaseSessionSlot call deletes a successor attempt's entry); cutting the comment to a bare citation loses the only warning at the site where an implementor would add the call. EVIDENCE: non-spec-changes.md:561-575.
OPEN: separate finding for a later round: ensureSlotStateLocked's second doc paragraph ("the predicate here is the whole of the admission rules ... no handler carries a second copy") restates the CODE-6 prose lead-in above the code block (non-spec-changes.md:1390-1393); and the CODE-5 Aborted arm comment names CODE-6/CODE-2, proposal ids that will be meaningless in pkg/gateway.

### [non-spec.4.review-applicability.1]
FACT: the r3-to-r4 diff (review logs excluded) is only checklist S15/S16 again (S15 tiers 0, 1; all CODE-14 tier-7a cases at S16); it agrees with Testing's "It lands with CODE-1" for slot_reclaim_hold_race_test.go. EVIDENCE: implementation-checklist.md S15/S16; non-spec-changes.md:3521-3523
FACT: every Server/Binder helper the staged Go blocks call either exists in the tree (slotStateLocked slot.go:130, resolveSlotPaths slot.go:93, noteRuntimeStartedLocked runtimegeneration.go:36, startPodScrub podscrub.go:40, reportSessionScrub sessionscrubreporter.go:61, drainViaLifecycle session.go:299, emitFinalUsage session.go:338, cancelPodMCPIfRuntimeIdle slotsession.go:238) or is declared by a staged deliverable (runtimeHoldsLocked CODE-15, removeSlotTreeVia CODE-6, reclaimSlotLocked CODE-6, lockSlotGuard CODE-14, ShutdownReclaim CODE-7, noteCompensationOutcome CODE-9). CredentialAssigner.Release is staged by CODE-13 and both implementers exist (credassign.go:380, client.go:299). Closes the caller's interface-method check with no finding.
WATCHOUT: the tier-9 credential-fence file's three arms need three different steps (superseded arm: CODE-1 at S16, since shipped Shutdown deregisters unconditionally at session.go:238; registered-but-unbound class: CODE-15 at S16; lease-release arm: CODE-13 at S19, since shipped materializeSlot releases no lease), but only the registered-but-unbound class is placed. Filed. EVIDENCE: implementation-checklist.md S14/S16/S19; non-spec-changes.md:3610-3629
WATCHOUT: the tier-4 datastore-crossing case's RecordLeak/gauge assertion cannot pass at S19 if it goes through applySlotRetryPolicy, because the accounting discriminator `sbe.Leaked || relErr != nil` is CODE-5's (S20) and ReleaseSlot(leaked=true) returns nil (slotclaimer.go:831-837). Not filed: the recycle_scrub_path fixture does not wire sessionserver accounting, so which route the case uses is a test-coverage question. EVIDENCE: non-spec-changes.md:1117-1119, :3454-3465
USEFUL [non-spec.3.review-applicability.1]: its WATCHOUT on slot_bind_attempt_race_test.go pointed at the same unplaced-case class; the Settled line "The tier-7a start-versus-reclaim race lands after `live` exists" places that file's race at S18, and landing the whole file at S18 turns nothing red, so it was not filed.
USEFUL [standing Settled "golangci-lint is NON-FATAL in tier 0 today"]: kept the S10 unused-helper question from being re-filed.

### [non-spec.4.review-citations.1]
FACT: the r3-to-r4 diff (review logs excluded) touches only checklist S15/S16 (S15 tiers 0 and 1; every CODE-14 tier-7a case at S16). No citation changed. EVIDENCE: diff against scratchpad/cp-snap/0081-opt7/non-spec-r2-prefix.
FACT: every file:line citation in non-spec-changes, spec-changes, summary and checklist (about 300 unique, resolved against pkg/, cmd/, tests/, schemas/) points at the claimed symbol or statement; no false citation found. Bare basenames are ambiguous against scripts/specshift/testdata copies and cmd/runtimes/echo-concurrent/slot.go; resolve to pkg/adapter or schemas/ first. EVIDENCE: e.g. pkg/adapter/holdstate.go:249 (`_ = s.Runtime.Close`), :254 (`_ = removeSlotTree`), :201 (10s WithTimeout); binder.go:865/867/954/998; pkg/observability/metrics/catalog.go:146,:271.
FACT: every verbatim-quoted spec target in spec-changes.md exists in spec/ as quoted, except the §4.6.1 closing sentence, which spec/04_system-components.md:416 spells with a `[Section 6.2](...)` link where the proposal writes `§6.2`; the bullet is replaced whole, so the implementor locates it by its opening. EVIDENCE: spec/04_system-components.md:409,:415,:416.
FACT: every spec anchor in spec-changes and non-spec links resolves to a heading; every § number cited is a spec heading except the §1.x references in the summary's impact rows, which name other proposals' sections. The §4.7.1 rule numbers and bold names (1-15) are used consistently across files.
FACT: the interface-method check for staged Go: `compensateFailedSlotBind` calls `ShutdownReclaim` on `*adapterclient.Client` (concrete, binder.go:523 `Adapter *adapterclient.Client`), staged by CODE-7; CODE-8's `CredentialAssigner` addition is satisfied by credassign.Service.Release (credassign.go:380) and credassign.Client.Release (credassign/client.go:299). `ResumeRequest` carries Pool, SessionID, CleanupTimeoutSeconds and MaxConcurrentSessions (binder.go:602-680).

### [non-spec.4.review-client-surface.1]

FACT: `cmd/lenny-compliance` derives its runtime-response `error.code` catalog from the proto `ErrorCode` enum by regex over the embedded proto (`cmd/lenny-compliance/schemaassert.go:26-60`), with a drift guard against the compiled descriptor (`schemaassert_test.go:37-60`). SCHEMA-1's codes 28 and 29 therefore join the set of codes a runtime may emit in a JSONL response and still pass the §15.4.6 battery. This is below the bar: the enum already carries adapter-only codes such as `PROTOCOL_VERSION_INCOMPATIBLE`, the adapter lifts no runtime code into a gRPC `adapterv1.Error` detail (the only builder is `pkg/adapter/staging.go:209`), and CODE-7's `translateSlotBindRefusal` cannot be reached by a runtime-emitted code. Neither the proposed comment text nor the regex is affected, because the new comments contain no brace and no `=`. — EVIDENCE: cmd/lenny-compliance/schemaassert.go:54-56
FACT: No client-facing parallel of the adapter proto exists outside `pkg/`. No SDK under `sdks/runtime` or `sdks/client` imports `adapterv1` or lists an `ERROR_CODE_*`. `pkg/gateway/externalapi/openapi/openapi.json` names no `SETUP_COMMAND_FAILED` text, and its `setup-output` path declares only 200 and 404. `docs/api/internal.md` has no `Shutdown` or bind-sequence RPC section. As a result, SCHEMA-1, SPEC-5, DOCS-2 and DOCS-3 leave no other client representation stale. — EVIDENCE: pkg/gateway/externalapi/openapi/openapi.json:936-943; docs/api/internal.md:71-290
WATCHOUT: the OpenAPI document is at `pkg/gateway/externalapi/openapi/openapi.json`. The caller prompt's `pkg/gateway/openapi/openapi.json` does not exist. — EVIDENCE: find output
FACT: `tests/tier3_contract/adapter_session_address/session_address_wire_test.go` pins only the reserved `slot_id` numbers and the presence of `session_id`, not closed field sets. The one closed set over messages SCHEMA-1 opens is still `shutdown_recycle_wire_test.go`, which SCHEMA-1 already moves. — EVIDENCE: tests/tier3_contract/adapter_session_address/session_address_wire_test.go:70-99
USEFUL [the Settled envelope lines, `writeSetupCommandError` branches on the gRPC code ALONE and `Binder.Resume` issues no `RunSetup`]: these settled the /resume and concurrent-path envelope questions without re-tracing start.go.

### [non-spec.4.review-docs-alignment.1]

DECISION: filed one finding, DOCS-2's bind-attempt paragraph saying "a reclaim naming an attempt that no longer owns the slot answers `superseded`", which is false where the adapter holds no entry (rule 11 answers `absent`) and contradicts the staged `Shutdown` row two paragraphs above it on the same page — BECAUSE the r2-era rejection of a related angle (archive DECISION at ~45937, alternative (c)) was only on loop scope (spec lane could not take a docs fix), and this is the non-spec lane — ALTERNATIVES: re-filing the rolled-back-start successor outcome (Open, FILED by opt6 `[non-spec.8.review-docs-alignment.1]`), rejected: four earlier lenses declined it as the same species as the twice-refuted accepted residues, and the summary's `SocketRuntimeProcess.Close` defects entries record it; the queue-head hold, rejected: decisions pass settled it `out-of-scope-stands` with a summary defects entry.
FACT: the r3->r4 diff touches only checklist S15/S16 (CODE-14 tier-7a cases move wholly to S16); no docs text changed. EVIDENCE: implementation-checklist.md, S15 and S16 lines
FACT: no docs page besides state-machines.md:138 states a claim-deletion projection clause other than glossary.md:258 (recurring non-finding); no docs page mirrors the §7.2 mid-resume close sequence, the §4.7.9 step 5, or the §6.2 pre-attached client-visibility clause. EVIDENCE: grep -rn "half-claimed\|claim deleted\|pre-attached" docs/
FACT: the only docs carriers of `SETUP_COMMAND_FAILED` and `STARTING_FAILED` are in docs/reference/error-catalog.md; the "a retry rebinds a fresh pod" wording at error-catalog.md:155-156 mirrors spec/15:1132 and is pre-existing. EVIDENCE: docs/reference/error-catalog.md:129,155-157
FACT: the DOCS-2 tier-11 substrings all occur in the staged `Shutdown` row; the gate asserts neither "superseded" nor the bind-attempt paragraph's body, only its bold lead. EVIDENCE: non-spec-changes.md Testing "For DOCS-2"
WATCHOUT: "counts every session served" (docs/reference/configuration.md:91, execution-modes.md:23, operator-guide/configuration.md:307) mirrors the unedited spec/05:404 comment; a running session ended by the §10.1 hold-timeout termination files no report under SPEC-3, so the count omits it. Any fix is a spec edit first; not filed. EVIDENCE: spec/05_runtime-registry-and-pool-model.md:404
USEFUL [Standing context: docs/api/internal.md recurring non-finding]: saved a filing on its gRPC status table lacking ABORTED.

### [non-spec.4.review-edit-sites.1]
FACT: the r3-to-r4 diff (review logs excluded) touches only checklist S15/S16 (S15 drops tier 7a; S16 takes all CODE-14 tier-7a cases). No spec/docs/schemas/charts surface names a step, so nothing outside the checklist went stale. EVIDENCE: implementation-checklist.md:41-44
FACT: edit-site sweep for the identifiers the staging adds found no unstaged spec/docs/schemas/charts carrier: `bind_attempt`, `unconditional_teardown`, `SlotReclaimOutcome`, `mid_session` on PrepareWorkspace, ERROR_CODE 28/29, the two counters and the leaked-slots gauge. charts/ names none of them; docs/api/internal.md documents a divergent StopSession API with no ShutdownRequest, so it is no carrier. EVIDENCE: grep over docs/ spec/ schemas/ charts/
FACT: every tier-11/tier-3 gate that reads a row or sentence the spec lane replaces still holds: recycle_scrub_trigger_consistency_test.go reads only the unchanged remainder of the §4.7 Shutdown row (recycle disposition, podId, ReportPodScrub, "does not block the response on the scrub"); session_scrub_report_addressing gate reads the unchanged addressing sentence; spec_28_register_writers podStateGatewayWrittenSentence is in the SPEC-3 sweep. EVIDENCE: tests/tier11_docs/recycle_scrub_trigger_consistency_test.go:65-99; tests/tier11_docs/session_scrub_report_addressing_doc_reconciliation_test.go:43-76; tests/tier11_docs/spec_28_register_writers_test.go:101-103
FACT: CODE-12's perl matcher catches the two Go claim-deletion carriers the tree holds (binder.go:1193-1194 "the projection returns the pod to `idle`" joined across the comment break, and podclaim/claimer.go:316 "projects `idle`"). EVIDENCE: pkg/gateway/podlifecycle/podsession/binder.go:1193; pkg/gateway/podlifecycle/podclaim/claimer.go:316
WATCHOUT: the proto `Error` comment says "The Code field is the spec-mandated error code from spec §15.1's catalog" (schemas/lenny-adapter.proto:529), which is already false for ten shipped codes; SCHEMA-1's two codes add to a pre-existing gap the summary records (§15.1 adapter-codes defect row). Not filed. EVIDENCE: schemas/lenny-adapter.proto:524-530
OPEN: SPEC-6's `cause` label (values refusal/failure) classifies why the compensated attempt failed, and §16.1.1's "Distinguishing error.type from reason" says a label whose values describe the cause of a failure or rejection is `error_type`. Not filed: the rule is framed as error_type-versus-reason, the name was the operator's (`[operator.post-r16]`), and the tree already ships a `cause` label (gatewaymetrics_sessionlifecycle.go:253). A human decides whether the name stands. EVIDENCE: spec/16_observability.md "Distinguishing `error.type` from `reason`"
DECISION: filed the landing-step gap of tests/tier7a_load_local/slot_bind_attempt_race_test.go — BECAUSE its concurrent-resolve case needs only CODE-6 (S14) while its start-versus-reclaim and reverse-ordering cases need CODE-2's Aborted confirmation (S18), CODE-1's token Shutdown (S16) and CODE-14's Resume guard (S15), and no text names the step that creates the file, though files-touched lands its spec-map and slotAddressCaseFiles rows "in the step that creates it" — ALTERNATIVES: leaving it as the r3 WATCHOUT (rejected: the sibling hold-file omission was accepted and fixed as a finding).

### [non-spec.4.review-fresh.1]
DECISION: no findings in non-spec round 4 (fresh holistic lens) — BECAUSE the r3-to-r4 diff is checklist S15/S16 only (S15 drops tier 7a, S16 takes every CODE-14 tier-7a case), which agrees with Testing's placement of slot_reclaim_hold_race_test.go with CODE-1; spot-checks of CODE-1, CODE-2, CODE-4, CODE-5, CODE-6, CODE-7, CODE-8, CODE-9, CODE-13, CODE-14, CODE-15, SCHEMA-1, DOCS-2, DOCS-3 and the tier-11 block against the tree found no false citation or contradiction — ALTERNATIVES: re-filing the five in-stage `cl.Close()` calls in materializeSlot (rejected: refuted as edit-list polish in round 12, review log Traps); filing `IncSlotCompensationSuperseded(outcome, ...)` taking an outcome the series does not label (rejected: an unused parameter, and CODE-9's table defers labels to SPEC-6's row).
FACT: closes Open **Would any gate over staged Go doc comments newly fail because of text the staging ADDS?** The remaining Go-comment gates do not reach staged text: checkpoint_dropped_slot_column_comment_test.go and checkpoint_scoping_key_comment_test.go read a fixed file list under pkg/gateway/checkpoint and three tier-4/7a checkpoint files (checkpoint_scoping_key_comment_test.go:33,:40-44); identifier_resolution_test.go reads §28.3's retired spellings (@lenny-lifecycle, LifecycleChannel, controlchannel, lifecycle-events, lifecycle-socket, lifecycleChannel, lifecyclechannel; spec/28:149-160), none of which, and no N3 phrase, appears in spec-changes.md or non-spec-changes.md. — EVIDENCE: grep over both files returned nothing.
FACT: closes Open **Does CODE-13's `Binder.Resume` branch call `compensateFailedSlotBind` (detached ctx) or a bespoke `ShutdownReclaim`?** The prose passes exactly the helper's arguments (req.SessionID, req.CleanupTimeoutSeconds, req.MaxConcurrentSessions off ResumeRequest, which declares CleanupTimeoutSeconds int and MaxConcurrentSessions int32 at binder.go:645,:667) and the helper detaches, so it is the helper. — EVIDENCE: non-spec-changes.md CODE-13 **`Binder.Resume` compensates.**; binder.go:602-667.
FACT: interface-method check for the staged gateway calls: CredentialAssigner (binder.go:319-332) declares AssignProto and ReleaseSession only, and CODE-13 widens it with Release(leaseID string), which credassign.Service.Release (credassign.go:380) and credassign.Client.Release (client.go:299) already implement; s.podBinder is *podsession.Binder (sessionserver.go:188), so CODE-5's resume caller passing it as slotBinder compiles. — EVIDENCE: as cited.
WATCHOUT: slot_bind_attempt_race_test.go spans two steps by content: the concurrent-resolve race needs only CODE-6 (S14 carries 7a), and the start-versus-reclaim and reverse-ordering arms need CODE-2's confirmation (S18 carries 7a). The file enters slotAddressCaseFiles through files-touched's general slot*_test.go rule in the step that creates it (slotSubjectFileRE, spec_map_slot_address_registration_test.go:1173). Do not read this as a missing landing step. — EVIDENCE: non-spec-changes.md Testing tier-7a paragraphs; checklist S14, S18.
USEFUL [non-spec.3.review-applicability.1]: its S15/S16 FACT reduced verification of this round's only diff to one read.

### [non-spec.4.review-kubernetes.1]

- FACT: the proposal's only apiserver-facing surfaces are pre-existing: the gateway's `SandboxClaim` DELETE (`drain` -> `podclaim.DeleteClaim`) and the `lenny.dev/drain-request` pod annotation (`DrainSandbox` -> `podclaim.StampDrainRequest`); it adds no CRD field, status write, finalizer or webhook, and the compensation is a direct adapter `Shutdown` RPC with no controller on the path — EVIDENCE: pkg/gateway/podlifecycle/podsession/binder.go:1200-1202, slotbinder.go:601-603
- FACT: SPEC-4's new projection input "the phase the pod currently projects" matches the tree: `occupancy.Current` is the Sandbox's live status.phase and `ProjectOccupancyPhase` keys the no-claim arms on it; the WarmPoolController is the sole `Sandbox.status` writer (§4.6.3 gateway RBAC paragraph), so the read-back is single-writer — EVIDENCE: pkg/controller/warmpool/occupancy.go:32-33, :57-80; spec/04_system-components.md:632
- FACT: the stale `drain` doc comment ("on a recycling pool under its limits the projection returns the pod to `idle`") is inside CODE-12's grep predicate (joined-block match on `returns the pod to \`idle`), so it is staged — EVIDENCE: pkg/gateway/podlifecycle/podsession/binder.go:1193-1194
- FACT: `CredentialAssigner.Release` is staged by CODE-13 (interface gains it); both production implementations exist and the test fakes lacking it are swept by the files-touched `ReleaseSession(` grep bullet — EVIDENCE: credassign.go:380, client.go:299, non-spec-changes.md files-touched "Every other file that `grep -rn \"ReleaseSession(\"`"

### [non-spec.4.review-mechanism.1]
FACT: the r3-to-r4 diff (review logs excluded) touches only checklist S15/S16 (S15 drops tier 7a; every CODE-14 tier-7a case lands at S16). It agrees with Testing: all CODE-14 tier-7a arms live in slot_reclaim_hold_race_test.go, which lands with CODE-1. EVIDENCE: implementation-checklist.md S15/S16; non-spec-changes.md "Tier 7a, `TestSlotIdentifierReclaimHold...`" ("It lands with CODE-1").
DECISION: filed one design defect. CODE-14 names the under-guard body as `releaseSessionSlotUnderGuard(sessionID)`, but the body owns the reclaim-hold release (the deferred release from reclaimSlotLocked), and the guard-acquiring wrapper's expired acquisition must retain the hold through the `guarded` conjunct. The body cannot read a boolean it is not given. BECAUSE the tier-1 expired-acquisition case asserts the retained hold on the guard-acquiring releaseSessionSlot row. ALTERNATIVES: have the body return the release so the wrapper decides. Rejected because it adds a second release site, and the one-release-site property is what the panic case pins.
FACT (closes the directive's MarkLeaked question): slotstate.Registry.MarkLeaked is keyed by slotID and sets the state to Leaked idempotently (pkg/sandbox/slotstate/registry.go:99-114). The report route (drainLedger.RecordLeak) never calls MarkLeaked, and CODE-5 makes Tracker.RecordLeak a per-pod set of slot ids. A leaked report followed by an unclean response for the same slot therefore books one leak on each ledger. EVIDENCE: pkg/gateway/sessionserver/start.go:2844; pkg/gateway/podlifecycle/podsession/slotbinder.go:149 (SlotID == SessionID).
FACT (closes part of Open "Would any gate over staged Go doc comments newly fail"): every §-number in spec-changes.md and non-spec-changes.md resolves to a numbered heading in spec/. tests/tier0_static/citation_document_test.go therefore cannot fail on a bare §-number the staging adds. The checked gates are now the N3 phrases, the slot-attribution and slot-absence gates, and citation-document. Two remain unchecked: checkpoint_*_comment and identifier_resolution. EVIDENCE: a grep of both files against `^#+ N` headings in spec/*.md returned no misses.
FACT (Open "Does rule 8's untokened arm compare equal across a replacement?"): no gateway path produces an untokened entry for a bind. Binder.Prepare always sends FinalizeWorkspace carrying the token, even when stageWorkspace sends no PrepareWorkspace, so an entry Binder.Launch's StartSession or ConfigureWorkspace resolves is already stamped. EVIDENCE: pkg/gateway/podlifecycle/podsession/binder.go:914-925.
FACT (Open "CODE-13's Binder.Resume branch calls compensateFailedSlotBind?"): the prose passes req.SessionID, the minted token, req.CleanupTimeoutSeconds and req.MaxConcurrentSessions, plus sb.Name and slotID. These match compensateFailedSlotBind's eight parameters, and ResumeRequest carries both fields (start.go resumeOnPod literal). EVIDENCE: pkg/gateway/sessionserver/start.go:4006-4041.
FACT: every interface method the staged gateway code calls exists. CredentialAssigner gains Release(leaseID string), which credassign.Service (credassign.go:380) and credassign.Client (client.go:299) already implement. User leases share the lease store, so Service.Release removes them too (binder.go:342-344). user_revocation.go:129's bind.Adapter is a concrete *adapterclient.Client (binder.go:523), so no interface needs widening for unconditional_teardown.

### [non-spec.4.review-performance.1]
FACT: the r2-to-r4 diff (review logs excluded) touches only checklist S15/S16 tier lists and case placement; no mechanism, write path or budget changed, so the round-2 performance findings (none) stand. EVIDENCE: implementation-checklist.md:41-44
FACT: the staging adds no apiserver write per failed bind; the only drain signal is the shipped `lenny.dev/drain-request` stamp through drainLedger/StampDrainRequest, and the compensation is one synchronous adapter RPC per failed bind bounded by `slotCleanupBudget` = max(cleanupTimeoutSeconds/maxConcurrentSessions, 5)s on a detached ctx. EVIDENCE: non-spec-changes.md:798-808, :866-873, :3423
USEFUL [non-spec.2.review-performance.1]: its replica-local slothealth.Tracker and per-slot leak-set FACTs settled the Redis/leak-accounting half of this lens without re-tracing.

### [non-spec.4.review-reliability.1]
DECISION: no reliability findings in non-spec round 4 of run 0081-opt7 — BECAUSE the r3-to-r4 diff touches only checklist S15/S16 (tier 7a moved to S16), and a re-walk of the recovery mechanisms (detached, budget-bounded compensation; attempt-scoped lease release; guard acquisition bounded by caller ctx; resume classifier arms; leaked disposition feeding the drain) found no loss, double-apply, unbounded wait or unreclaimed resource that is not already an accepted failure mode or a recorded defect — ALTERNATIVES: filing the StartSession-rollback-vs-compensation guard race below (rejected: microsecond window between the Shutdown's guard acquisition and its second s.mu, bounded consequence, same class as the existing CODE-14 premature-removal DEFERRED).
FACT: emitFinalUsage is a no-op in production today: Server.Usage (pkg/adapter/server.go:231) has no production UsageMeter implementation (grep for `) Usage(ctx context.Context, sessionID string)` finds none), so running Shutdown's started-arm work on an expired request ctx after an expired guard acquisition loses no final usage report. EVIDENCE: pkg/adapter/session.go:338-347
FACT: Runtime.Close on an expired ctx still terminates the child: resolveShutdownGrace falls back to the configured or default grace when the deadline has passed, and both Close implementations kill after the grace. EVIDENCE: pkg/adapter/mcpruntime.go:312-324; pkg/adapter/socketruntime.go:435-467
FACT: the only production Binder.Resume caller normalises MaxConcurrentSessions to >= 1, so slotCleanupBudget's division on the resume compensation cannot see zero from production. EVIDENCE: pkg/gateway/sessionserver/start.go:4029, :3353-3358
FACT: the concurrent bind path releases no credential lease on failure today (materializeSlot has no releaseCredentials call; only ReleaseSlot at slotbinder.go:549 does), so CODE-13's attempt-scoped release closes a lease leak rather than narrowing an existing release. EVIDENCE: pkg/gateway/podlifecycle/podsession/slotbinder.go:265-356
WATCHOUT: a StartSession rollback on its expired ctx (releaseSessionSlot) whose non-blocking guard attempt fails because a compensating Shutdown holds the guard, and which takes s.mu before that Shutdown's second decision, removes the entry unguarded and retains its hold for the pod's life although both removals succeed; the Shutdown then answers ABSENT (clean), so nothing counts the pod. Bounded by windowed-failure drain on the session's refused retries. Same class as the Deferred CODE-14 premature-removal entry. EVIDENCE: non-spec-changes.md CODE-14 **The acquisition step.** and **Disposition of an expired acquisition at a removing site**; CODE-1 handler body

### [non-spec.4.review-security.1]
DECISION: empty findings for the security lens in non-spec round 4 — BECAUSE the r3-to-r4 diff touches only checklist S15/S16 tier lists, and every security control I re-walked holds: the §11.4 full-revoke fan-out (cmd/lenny-gateway/user_revocation.go:129) is in CODE-4's non-compensating table and every Shutdown in the tree builds through Client.shutdown (pkg/gateway/runtime/adapterclient/client.go:813-819), so no teardown caller is refused by the two-field precondition at S16; the attempt-scoped lease release calls CredentialAssigner.Release(leaseID), which both production implementers already carry (credassign.go:380, client.go:299) and which covers user-source leases because they share the lease store (binder.go:335-343) — ALTERNATIVES: re-filing the SPEC-3 biconditional as a maxSessionsPerPod relaxation for running slots released by the §10.1 hold-timeout termination or the SDK demotion (rejected: the tree files ReportSessionScrub only from Shutdown, pkg/adapter/session.go:279, so the staged rule matches shipped counting; refuted many times in the archive, e.g. review-log-archive.md:60445, :51205).
FACT: the only in-tree ShutdownRequest literal outside generated code is client.go:814, and the only four-argument Shutdown invocations are binder.go:2043, slotbinder.go:542 and user_revocation.go:129; CODE-4's table names all of them plus the two ShutdownRecycle sites. — EVIDENCE: grep over pkg/ and cmd/ excluding _test.go and pb.go
FACT: every method the staged adapter Go blocks call on Server either exists in the tree (noteRuntimeStarted/Locked, noteRuntimeClosed, reportSessionScrub, startPodScrub, drainViaLifecycle, emitFinalUsage, resolveSlotPaths, slotStateLocked, cancelPodMCPIfRuntimeIdle) or is declared by a staged deliverable (runtimeHoldsLocked CODE-15, lockSlotGuard CODE-14, removeSlotTreeVia and reclaimSlotLocked CODE-6); s.Runtime is RuntimeProcess, whose Close(ctx, sessionID) error is at pkg/adapter/session.go:62. — EVIDENCE: pkg/adapter/runtimegeneration.go:26,:36,:58; pkg/adapter/server.go:210
WATCHOUT: the tier-9 file slot_credential_reclaim_fence_test.go has three arms owned by three steps (S14 lists tier 9, S16 names only the registered-but-unbound matching class, S19 the lease release); the superseded arm needs CODE-1's rule-13 compare, which lands at S16, and the checklist names no step for it. Below the security bar (test-step bookkeeping); a test-coverage lens decides. — EVIDENCE: implementation-checklist.md S14/S16/S19; non-spec-changes.md Testing **Tier 9, the credential fence.**
USEFUL [Traps "Security and capacity families are exhausted."]: every in-pod-attacker dress I built collapsed into the Deferred TLS entry, as it says.

### [non-spec.4.review-single-source.1]
FACT: the r3-to-r4 diff (review logs excluded) touches only checklist S15/S16 tier wording; no new restatement was introduced by it. EVIDENCE: diff against scratchpad/cp-snap/0081-opt7/non-spec-r2-prefix.
FACT: the queue-head hold is written out in full at TWO sites, the summary defects entry "A queued waiter's wait bound does not cover the time it spends behind the FIFO head." (summary.md:942-961) and the non-spec Edge case "A compensating `Shutdown` holds the pool's queue head for its own budget." (non-spec-changes.md:3794-3818), including the off-path-compensation rejection ("a worker, a retry policy, its own residue ... a home for the attempt token") verbatim at both. Filed. `[f6.apply.compensation-queue-hold]` created two homes by design (accepted increment vs shipped defect), but each copy restates the other's half.
FACT: the ground for keeping the cleanup-outcome report keyed on closeErr (ordinary session end, persistent never-pruned leak count, threshold 1 at maxConcurrentSessions 1 and 2, one os.RemoveAll failure retires the pod) is written out in CODE-15 (non-spec :363-380) and again in the summary defects entry "A per-slot cleanup whose runtime close succeeds ..." (summary :878-904). Filed.
WATCHOUT: the prose-plus-staged-comment pattern that r1 fixed for CODE-15 survives in CODE-2 (rollback leaves the registry alone: comment :535-547 and prose :561-575), in CODE-5's resume classifier second arm (prose :1168-1176, comment :1202-1208), and in ensureSlotStateLocked's doc comment restating the §4.7.1 stamp-once rule (:1396-1403). Filed each.
FACT: CODE-13's paragraph at non-spec :898-905 restates rule 15's outcome meanings and rule 13's untokened arm. Filed.
FACT: podsession.ResumeRequest already carries CleanupTimeoutSeconds, MaxConcurrentSessions, Pool and SessionID (pkg/gateway/podlifecycle/podsession/binder.go:602-678), so CODE-13's Binder.Resume branch can pass them to compensateFailedSlotBind as written. Supports closing the Open "Does CODE-13's Binder.Resume branch call compensateFailedSlotBind" toward "the helper". EVIDENCE: binder.go:604-606, :649, :661-667.
OPEN: the Open "CODE-6's paragraph 'The reclaim hold has one predicate and two test points.' names acquireSlotGuardForResolve (CODE-14, S15)" is still true at non-spec :1558-1572; filed per the caller's Open-entry instruction.

### [non-spec.4.review-test-coverage.1]
FACT: the r3-to-r4 diff (review logs excluded) is only checklist S15/S16: S15 now runs tiers 0 and 1 and every CODE-14 tier-7a case moves to S16. No other site names S15's tiers. EVIDENCE: implementation-checklist.md:41-44
WATCHOUT: under Settled "Testing attributes each case to a DELIVERABLE" (review-log Settled, opt7 `[non-spec.1.fix-G5.1]`), an unlabelled Testing case has no landing step. In `### Integration and race tests`, only the reclaim-hold file ("It lands with CODE-1") carries a label. `slot_bind_attempt_race_test.go` mixes the concurrent-resolve race (passes at CODE-6/S14) with the start-versus-reclaim race and the reverse ordering (need CODE-2's confirmation, S18; the shipped `noteRuntimeStarted` records unconditionally, pkg/adapter/runtimegeneration.go:26-49). The tier-9 lease-release arm needs CODE-13 (S19), because shipped `materializeSlot` releases no lease on failure (pkg/gateway/podlifecycle/podsession/slotbinder.go:300-324). I filed this as one finding. EVIDENCE: non-spec-changes.md:3467-3519, :3610-3629
CORRECTS [Settled "S16 carries tier 9 and the registered-but-unbound class ... beside the superseded and lease-release arms"]: the lease-release arm cannot pass at S16. S16 does not depend on S19, and the attempt-scoped release is CODE-13's.
FACT: closes Open "Does CODE-13's `Binder.Resume` branch call `compensateFailedSlotBind` ... or a bespoke `ShutdownReclaim`?": either reading is pinned. CODE-13's prose takes exactly the helper's arguments off `ResumeRequest` (non-spec-changes.md:1004-1010). The cancelled-context case's `Binder.Resume` row asserts that a `Shutdown` naming the minted token is still recorded while the caller ctx is done (:3253-3259), so a branch that reclaims on the request ctx fails the test. The coverage question is answered.
OPEN: Open "CODE-6's paragraph 'The reclaim hold has one predicate and two test points.' names `acquireSlotGuardForResolve`" is still true at non-spec-changes.md:1558-1559. It is a step-ownership issue for the mechanism or edit-site lens, not a test gap: Testing attributes the guard-acquisition assertion to CODE-14 (:2781-2782).
