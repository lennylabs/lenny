...
- **The open decisions left with the human are 30, 32, 33, 34, 36 and 50.** 20, 29, 41, 45, 47, 48, 49, 51, 52, 53 and 54 are answered by operator or f13 blocks (54 by `[operator.prune2-r9]`, and 29 by `[operator.prune2-r9]` as reversed by `[operator.d29]`); 28, 31, 35, 40 and 46 by the staging. Decision 35's stale contested flag was cleared in phase state rather than the entry restored (`[operator.f13-repair]`).
...
- **WATCHOUT (opt9 `[spec.1.review-reliability.1]`): since decision 29, a create-time-reserved `/start` retry that meets rule
  ...
  retirement. Settled decision plus accepted residue; do not re-file without a staged-spec remedy (see `### Open`).
...
- **Does the rolled-back-start successor outcome need spec or docs text?** OPEN, FILED (`[non-spec.8.review-docs-alignment.1]`): the non-spec Edge-cases bullet "A rolled-back start can close a successor's runtime session" has a client-visible outcome that no landing spec or docs text states; rule 8 says only "takes the session back off the shared runtime process". Every docs-alignment lens since has declined it; no fix entry is recorded.
...
- **Is a stale coordinator's compensating `Shutdown` after a handoff owed a §7.1 trace?** OPEN (opt8 `[spec.3.review-performance.1]`): spec/10 §10.1 has pods validate `coordination_generation` on every gateway-to-pod RPC, which would reject it and book the slot `leaked`; the tree validates only `CoordinatorFence` and `CheckpointBarrier`. Split-brain-only, below the bar.
...
- **Should the spec residue bullet on the abandoned attempt's late `StartSession` name the create-time-reserved `/start` retry loop?** OPEN (opt9 `[spec.1.review-mechanism.1]`): under reversed decision 29 every client `/start` retry answers the retryable 503 `STARTING_FAILED` and cannot succeed until the pod retires. Below the bar; a human may want the bullet to say so.
