...
### [f13.human-decisions]
DECISION: open decision 51 (whether §5.2 names `SLOT_FAILED` and 422) resolved as leave-to-separate-proposal. Removed entry 51 from the summary's `## Open decisions for human to make`, added one sentence to that section's preamble recording that it left for the unstaged-defects list, and added the row "§5.2. The exhaustion error names no code value or status." at the end of `## Defects in the shipped tree that this proposal does not stage`. No staged change file edited: the staging already edits no §5.2 exhaustion text. Authority: spec/05_runtime-registry-and-pool-model.md:562 (bullet names fields only); pkg/gateway/sessionserver/start.go:312-320 (`writeSlotFailed`, 422 `SLOT_FAILED`); `grep -rn SLOT_FAILED spec docs schemas` returns nothing; spec-changes.md:121-122 and :1060 hold whatever code §5.2 later names.
FACT: the entry's claim that naming the code would settle entry 29 was dropped; entry 29 turns on the setup-command stage and session-mode credential assignment (binder.go:947-951), off the §5.2 slot path.
DECISION: open decision 52 (whether SPEC-6's §16.1 `lenny_adapter_leaked_slots` row describes the gauge as emitted) resolved as keep-as-staged. No staged change file edited: the row at spec-changes.md:1228 already restates §6.2's per-pod, until-termination definition. Removed entry 52 from the summary's `## Open decisions for human to make`, added one sentence to that section's preamble recording the answer, and appended one sentence to the unstaged-defects row "§6.2 attributes `lenny_adapter_leaked_slots` to the adapter, and the gateway emits it." naming the per-replica emission. Authority: spec/06_warm-pod-model.md:160 (persistent per-pod count is the gauge); .claude/rules/spec-driven-development.md (spec is right, code is the defect); pkg/sandbox/slotstate/registry.go:20-23 (in-process map); pkg/gateway/sessionserver/start.go:2844-2846 and :2870-2871 (gauge set only in `applySlotRetryPolicy`).
DECISION: open decision 53 (CODE-1, CODE-4 and CODE-6 each named by several checklist steps) resolved as re-cut, resting on .claude/workflows/change-proposal.js:757 ("Every staged deliverable appears in exactly one step"), :758 (bundling only for one file and one reader) and :3396. In non-spec-changes.md: CODE-1 narrowed to the precondition and cascade (heading, preamble sentence, Targets), and a new `### CODE-15` heading with its own Targets (the `if bound` gate, `Shutdown`'s doc comment, the `SessionScrubReporter` comment, `deregisterSlotLocked`'s doc comment and `runtimeHoldsLocked`) now stands before **The split gates.**; CODE-4 narrowed to mint, carry and `unconditional_teardown` (heading, Targets), with a new **`Binder.Prepare` and `Binder.Resume` mint and carry.** block and a new `### CODE-13` heading with Targets before **The compensation.**, and the two later blocks retitled **`Binder.Prepare` sends no compensation.** and **`Binder.Resume` compensates.**; CODE-6's preamble now names CODE-14 for the guard, its `bindattempt.go` bullet drops the guard helpers and `Server.slotGuards`, **The reclaim hold has one predicate and two test points.** moved above a new `### CODE-14` heading with Targets that precedes the per-slot guard block. Prose references re-pointed to CODE-13, CODE-14 or CODE-15 where the cited text moved, in non-spec-changes.md (including the files-touched entries' disposition citations), spec-changes.md (three sites) and summary.md (ten sites). Entry 53 removed from the summary's open decisions and one preamble sentence added.
FACT: `pkg/adapter/resume.go:50` is `Resume`'s `claimSessionSlot` call, ahead of every path-resolving step; the moved sentence's citation holds.
WATCHOUT: CODE-14's heading names the files its guard edits actually touch (bindattempt.go, server.go, staging.go, resume.go, slotsession.go, session.go, sdkwarm.go, holdstate.go) instead of the design's slot.go, session.go and holdstate.go, because the hand-out helpers live in bindattempt.go, `slotGuards` in server.go and the guarded sections in staging.go and resume.go.
DEFERRED [/home/ec2-user/lenny/proposals/0081_fix_a-failed-session-bind-leaves-an-adapter-slot-registry/0081_fix_a-failed-session-bind-leaves-an-adapter-slot-registry.implementation-checklist.md]: S15 names CODE-6, S17 names CODE-1 and S19 names CODE-4; they should name CODE-14, CODE-15 and CODE-13. S15's "CODE-6's derivation table" and "CODE-6 states once" and S16's "CODE-6's **Disposition of an expired acquisition at a removing site**" should cite CODE-14, and S17's "CODE-1's \"The split gates\" block" should cite CODE-15. S21 should be deleted, its id retired, and S16 should gain one sentence landing the Testing subsection "Tier-1 adoption-ordering tests, every case walked"; no step depends on S21.
DEFERRED [0081_fix_a-failed-session-bind-leaves-an-adapter-slot-registry.summary.md, ## Deliverable index]: the index still lists CODE-1, CODE-4 and CODE-6 with their pre-split files and lacks CODE-13, CODE-14 and CODE-15; the reconciliation pass owns that section.

### [f13.other-proposals]
DECISION: impact row for draft proposal 0082 added to the summary's `## Impacts on other proposals`, before the R1b programme-step row. It records 0082 as Draft (drafted 2026-09-23, 0082 status.md), states that no 0082 deliverable loses its subject, that CODE-4's attempt-scoped lease release precedes 0082 CODE-3's `revokeFinalizeLease` so each lease is released once in either order, and that the proposals edit different §15.1 tables and different functions; the action cell asks for a textual rebase and, if 0082 lands, a new trigger or a guard statement for CODE-8's `Binder.Prepare` arm. Authority: 0082 summary.md:105, 0082 non-spec-changes.md:90-155, non-spec-changes.md:2051, pkg/gateway/credentials/credassign/credassign.go:380 and :400.

### [spec-recheck.1.fix-G1.1]
DECISION: Single homes for three rationales: summary Decisions bullet on `unconditional_teardown` (two-field teardown), spec-changes Design "The adapter holds the slot identifier while its cleanup runs" (reclaim hold), Design "The cleanup-outcome report follows the `running` boundary" (report placement and double count) — BECAUSE rule (g) one reason one home; the summary bullet already carries fail-open, refuse-naming-nothing, bare-scalar/no-wire-presence and recycle precedent — ALTERNATIVES: making the Design paragraph the home for the teardown (rejected: summary bullet is fuller).
WATCHOUT: Deleting SPEC-3 commentary paragraphs orphaned two antecedents: "the paragraph names" (ten-second window) and "The bullet's own reporting sentence". Both now name their target ("the reclaim-hold paragraph", "the `**Slot cleanup:**` bullet"). A later deletion near SPEC-3's commentary must recheck these. — EVIDENCE: spec-changes.md SPEC-3 commentary after the scrub-model retirement-clause paragraph.
FACT: The deleted reclaim-hold commentary claimed a per-slot "process-group kill" resolves from the slot identifier; the tree has no per-slot process-group kill (kills live in Runtime.Close). That false claim is gone with the deletion. — EVIDENCE: orchestrator TREE FACTS; pkg/adapter.
OPEN: non-spec-changes.md Design text (around "refuses a `Shutdown` naming nothing" and "neither has explicit wire presence") restates the summary's `unconditional_teardown` decision; left untouched per design (separate-finding for the non-spec lane).

### [spec-recheck.1.fix-G2.1]
DECISION: the SPEC-5 §15.1 block's preamble is the one home of why the SETUP_COMMAND_FAILED row is widened (second deterministic FailedPrecondition producer at the setup-command request). The closing-commentary paragraph after the exclusion-sentence replacement was deleted whole; the untouched-list §15.1 bullet ends with "The section itself is edited under SPEC-5's §15.1 block."; the Edge-cases bullet "A bind-sequence refusal reaches the client under the envelope its stage already selects." ends with "SPEC-5's §15.1 block and DOCS-3 carry the row's replacements." — BECAUSE the reason was written out at four sites — ALTERNATIVES: keeping one pointer sentence after the exclusion fence (describes the verbatim replacement just above, no implementor acts on it); making the Edge-cases bullet the rationale home (that bullet owns the stage-to-envelope mapping, a different rule).
WATCHOUT: do not re-add commentary after SPEC-5's exclusion-sentence code fence; the fence ends the §15.1 block and the preamble carries the reason. The preamble's sentence "A refusal arriving at any other bind-sequence request reaches the client under the envelope that stage already selects." still restates the Edge-cases rule and was left as a separate finding — EVIDENCE: spec-changes.md SPEC-5 §15.1 preamble.
CORRECTS [archive:56966 DECISION on the SPEC-5 §15.1 rationale paragraph]: that paragraph no longer exists; its producer argument lives only in the SPEC-5 §15.1 preamble, and its envelope pointer lives only in the Edge-cases bullet.

### [spec-recheck.1.fix-G3.1]
DECISION: deleted the SPEC-2 §7.1 commentary paragraph "The paragraph states the connection preference and no correctness rule about it ..." with no pointer in its place — BECAUSE the summary's Watch-out bullet "The §7.1 connection sentence carries no correctness load" is the one home of the reason, and the staged §7.1 text keeps its one-clause reason ("the fence does not depend on the connection") — ALTERNATIVES: a one-sentence pointer to the summary (higher rung, no implementor acts on it).
CORRECTS [review-log-archive WATCHOUT on "The paragraph states the connection preference"]: that paragraph no longer exists in spec-changes.md; the reason lives only in the summary Watch-out bullet.

### [spec-recheck.1.fix-design-G1.1]
DECISION: homes for G1 rationale are the summary `unconditional_teardown` Decisions bullet (two-field teardown), the spec-changes ## Design paragraph "The adapter holds the slot identifier while its cleanup runs" (reclaim hold), and the Design paragraph "The cleanup-outcome report follows the `running` boundary" (report placement and double count). Every SPEC-1/SPEC-3 commentary copy is deleted or cut to a clause/pointer — BECAUSE state each reason once — ALTERNATIVES: making the Design section the two-field home rejected, the summary bullet is the fuller statement and the reason-home rule names summary Decisions first.
WATCHOUT: the SPEC-5 lead-in "for the reason the SPEC-1 commentary above gives" depends on the §4.1 commentary; re-point it to the summary `unconditional_teardown` decision in the same edit — EVIDENCE: spec-changes.md:1020
WATCHOUT: SPEC-3's deleted commentary said "the process-group kill ... resolve[s] from the slot identifier"; no per-slot process-group kill exists (kills live inside Runtime.Close), so do not carry that phrase into any surviving site — EVIDENCE: spec-changes.md:657
OPEN: non-spec-changes.md:76-84 and :2647-2652 restate the two-field argument already given in summary.md:125-133 (separate single-source finding for the non-spec lane).

### [spec-recheck.1.fix-design-G2.1]
DECISION: SPEC-5 §15.1 preamble (the paragraph under the SPEC-5 §15.1 heading) is the one home of why the SETUP_COMMAND_FAILED row is widened; the closing commentary after the exclusion-sentence replacement is DELETED whole (the replacement text is verbatim, so no implementor acts on it), the untouched-list §15.1 bullet's trailing clause becomes "The section itself is edited under SPEC-5's §15.1 block.", and the edge-case bullet's last two sentences become one pointer — BECAUSE rung 2 closes the commentary site and rung 3 the other two — ALTERNATIVES: reviewer's one-line survivor for the commentary (rung 3) rejected, deletion is lower and loses nothing an implementor uses.
WATCHOUT: the SPEC-5 §15.1 preamble's sentence "A refusal arriving at any other bind-sequence request reaches the client under the envelope that stage already selects." restates the Edge-cases bullet "A bind-sequence refusal reaches the client under the envelope its stage already selects." — a separate single-source finding for a later round, not closed here — EVIDENCE: spec-changes.md SPEC-5 §15.1 preamble vs Edge cases bullet.
CORRECTS [review-log:692]: once the closing commentary is deleted, SPEC-5's §15.1 block no longer points at the Edge-cases bullet; the untouched-list bullet's "stated once ... paragraph after rule 9" pointer is unchanged by this fix.

### [spec-recheck.1.fix-design-G3.1]
DECISION: delete the SPEC-2 commentary paragraph "The paragraph states the connection preference and no correctness rule about it. ..." (spec-changes.md, after the staged §7.1 block, before the "### SPEC-2 · ... § 7.2" heading) with no replacement pointer — BECAUSE the summary's Watch-out bullet "The §7.1 connection sentence carries no correctness load" is the reason's one home and the staged §7.1 text carries its own clause ("the fence does not depend on the connection") — ALTERNATIVES: the reviewer's one-sentence pointer; rejected because no implementor acts differently for it (rung 2 closes).
CORRECTS [review-log-archive WATCHOUT on the SPEC-2 connection-preference commentary, archive :53403]: that entry kept the paragraph as commentary "parallel to the Design section's choice record"; it was a second full statement of the summary Watch-out bullet's reason and is now deleted. A later lens should not restore it.

### [spec-recheck.1.review-applicability.1]
DECISION: returned no findings for the applicability lens in spec-recheck round 1 — BECAUSE the only delta to spec-changes.md since convergence is `[f13.human-decisions]`'s three re-points (CODE-4 to CODE-13 twice, CODE-1 to CODE-15 once), and each now names the deliverable that holds the cited text: CODE-13 holds the leaked disposition and `BindReservedSlot`'s release fold (non-spec-changes.md CODE-13 Targets), and CODE-15 holds the `SessionScrubReporter` comment. Every verbatim anchor in SPEC-1 to SPEC-6 matched exactly once in the current tree. The spec-lane checklist order S1 to S6 resolves every forward reference except the ones S1 and S3 already record against S4 — ALTERNATIVES: filing the `## Spec files touched` spec/16 bullet that omits the gauge row (rejected as bookkeeping that blocks no application, because the SPEC-6 block states all three rows explicitly).
FACT: the three spec-changes.md references left on CODE-1 and CODE-6 stay correct after the CODE-13/14/15 split. `answerShutdown` and the scrub start sit inside CODE-1 (non-spec-changes.md, before the `### CODE-15` heading). CODE-6 mints the per-member ten-second close context in `terminateHeldSession`, in its reclaim-hold bullet ("`closeCtx` is the member's own ten-second close context this deliverable mints"). CODE-14 owns only the guard and the expired-acquisition disposition — EVIDENCE: non-spec-changes.md `### CODE-6` reclaim-hold bullet, `### CODE-14` Targets.
FACT: two gates read spec rows that SPEC-1 and SPEC-3 replace, and both stay green under the staged text. `recycle_scrub_trigger_consistency_test.go` reads the §4.7 `Shutdown` row for the strings "recycle disposition", "ReportPodScrub", the three scrub parameters, "does not block the response on the scrub" and the §5.2 link, and all of them sit in the unchanged remainder of the row. `session_scrub_report_addressing_doc_reconciliation_test.go` pins the unchanged addressing sentence. `adapter_metric_catalog_test.go` checks the direction registered-to-catalog only, so S6's §16.1 rows landing before S10 break nothing — EVIDENCE: tests/tier11_docs/recycle_scrub_trigger_consistency_test.go:65-100, tests/tier11_docs/adapter_metric_catalog_test.go:84-110.
WATCHOUT: the SPEC-1 headings say "§ 4.7" for the `Shutdown` and `DemoteSDK` rows, but both RPC tables sit under `#### 4.7.1` in spec/04. The anchors are unique, so application does not fail. Do not refile this — EVIDENCE: spec/04_system-components.md:659-693.
UNVERIFIED: between S4 and S5, the §5.2 retirement clause ("A pod serving one session whose claim the failed bind deletes retires under the §6.2 occupancy projection") is false against the unedited §6.2 and §4.6.1 for a recycling pod. S4's checklist line does not record that forward reliance the way S1 and S3 record theirs. That is a checklist remedy, so it is out of this loop's scope.

### [spec-recheck.1.review-citations.1]
FACT: the whole spec-changes.md delta since the opt3 convergence is three re-pointed deliverable ids (`git diff HEAD` on the file): :392 CODE-4 -> CODE-13, :610 CODE-1 -> CODE-15, :997 CODE-4 -> CODE-13. All three hold: CODE-13 owns the disposition that reads the RPC error and clean-exit flag and the `BindReservedSlot` `sbe.Leaked` row (non-spec-changes.md:985-1000, :1202, :1114-1119); CODE-15's Targets carry the `Server.SessionScrubReporter` field comment (non-spec-changes.md:370). — EVIDENCE: non-spec-changes.md:985, :1202, :363-372
FACT: the remaining CODE-1/CODE-6 references in spec-changes.md (:254 rule 10 and CODE-1's pairing refusal; :667, :681 CODE-6's per-member ten-second close context) still point at the deliverable that owns the text after the f13 split (CODE-6's hold-timeout bullet is at non-spec-changes.md:1490-1498, inside CODE-6, not CODE-14). — EVIDENCE: non-spec-changes.md:1430, :1490-1498, :1786
FACT: no commit has touched spec/ since 911d93b13 (2026-09-08) or pkg/, schemas/, docs/, cmd/ since c5d35bb05 (2026-09-07), so `[spec.4.review-citations.1]`'s anchor sweep still stands; the five file:line citations in the staging (holdstate.go:201, spec/11:264, session.go:219-224, socketruntime.go:470-473, mcpruntime.go:86) were re-read and hold. — EVIDENCE: git log -- spec/ pkg/
USEFUL [spec.4.review-citations.1]: its anchor list let this recheck skip a full anchor re-sweep once git log showed spec/ unchanged.
DECISION: did not re-file the two spec-changes.md Deferreds (the untokened-entry bullet calling the §10.1 hold-timeout termination "a request" at :155-156; the `## Spec files touched` spec/16 bullet and docs-mirror sentence saying "each counter" while SPEC-6 also stages the `lenny_adapter_leaked_slots` gauge row, :1305-1306, :1310-1311) — BECAUSE both sit in commentary that lands in no spec file, SPEC-6 itself is explicit about the gauge, and the archive records the genus-noun one judged below the bar repeatedly — ALTERNATIVES: filing them as contradiction/bookkeeping (rejected: they change no applied statement). A fixer touching either sentence should still correct it.

### [spec-recheck.1.review-client-surface.1]
DECISION: returned no findings for the client-surface lens in spec-recheck round 1 — BECAUSE the spec-changes.md delta since convergence is three commentary re-points (CODE-4 -> CODE-13 at the §7.1 acknowledged-clean commentary and the §6.2 slot_assigned prose, CODE-1 -> CODE-15 in the SPEC-3 carrier table), each verified against the new headings (CODE-13 owns "The leaked disposition, at the caller" and the BindReservedSlot release fold; CODE-15's Targets list the `SessionScrubReporter` comment), and no client surface moved — ALTERNATIVES: filing the §15.1 cause sentence's omission of RunSetup's workspace-root FailedPrecondition producer (rejected: reachable only with an empty --workspace-base, default "/workspace" at cmd/lenny-adapter/main.go:116, and the shipped row already omitted it).
FACT: no client SDK (sdks/client/*, sdks/runtime/*) and no OpenAPI text (pkg/gateway/openapi) names SETUP_COMMAND_FAILED, setup_command_failed, or any adapter proto message, so SPEC-5's §15.1 row widening has only docs/reference/error-catalog.md (DOCS-3) as a parallel representation — EVIDENCE: grep over sdks, pkg/gateway/openapi returns nothing
FACT: the §15.1 endpoint-table rows at spec/15_external-api-surface.md:625, :646, :647, :650, :720 each say "a deterministic non-zero setup command surfaces as ... SETUP_COMMAND_FAILED"; they name one cause without exclusivity, so the widened row does not falsify them — EVIDENCE: spec/15_external-api-surface.md:646-650
FACT: the graceful-shutdown-signal co-tenant condition SPEC-1's §4.7 Shutdown row states is shipped, not new — EVIDENCE: pkg/adapter/session.go:252-259 (`if !boundRemains`)
WATCHOUT: docs/api/internal.md describes a stylized RuntimeAdapter service (StopSession, UploadFiles) that predates the proto; its DemoteSDK section is incomplete but not falsified by SPEC-1, and any fix is docs-lane — EVIDENCE: docs/api/internal.md:265-287
USEFUL [spec.2.review-client-surface.1]: the setupFail-before-slotFailed ordering at pkg/gateway/sessionserver/start.go:139/:186 settled the concurrent-path envelope question without re-derivation.

### [spec-recheck.1.review-docs-alignment.1]
FACT: the only spec-changes.md delta since the opt3 convergence is three commentary re-pointings by `[f13.human-decisions]` (CODE-4 -> CODE-13 at :392 and :997, CODE-1 -> CODE-15 in the SPEC-3 carrier table at :610). Each target heading carries the cited text (non-spec-changes.md:363 CODE-15 names the `SessionScrubReporter` field comment; :985 CODE-13 owns the disposition and the `BindReservedSlot` fold). No docs-facing identifier moved. EVIDENCE: `git diff HEAD -- proposals/0081*/*.spec-changes.md`
DECISION: filed one finding, the `## Spec files touched` spec/16 bullet and the closing docs-mirror paragraph omitting the `lenny_adapter_leaked_slots` gauge row. BECAUSE it is the `### Deferred` entry from spec round 1, still unfixed at spec-changes.md:1309-1310 and :1314-1316, and its fix lands in spec-changes.md — ALTERNATIVES: re-wording "counter" to "series", rejected in favour of the reduction the Deferred entry and the directive prefer.
FACT: the SPEC-3 carrier table's docs rows are complete against a fresh grep of docs/ for ReportSessionScrub, sessions_served, "per-slot cleanup" and "cleanup outcome". adapter-contract.md:84 ("then reports through these RPCs"), runtime-author-guide/lifecycle.md:69, multi-tenancy.md:67-72 and execution-modes.md:63-65 state only that the cleanup runs, so they are not carriers. EVIDENCE: grep -rn -i "ReportSessionScrub\|sessions_served\|per-slot cleanup\|cleanup-outcome" docs
WATCHOUT: `docs/operator-guide/troubleshooting.md:41` "Setup command failures | reason: setup_command_failed" is the warm-pool WARMUP failure reason (`lenny_warmpool_warmup_failure_total` error_type) rather than the §15.1 REST `SETUP_COMMAND_FAILED` code. SPEC-5's new started-session cause does not reach it, so it does not need a DOCS-3 companion. EVIDENCE: docs/operator-guide/troubleshooting.md:36-41; docs/reference/metrics.md:135
USEFUL [archive: rolled-back start closing a successor's runtime, declined at 62853, 64592, 65542, 67069]: four earlier lenses declined it as a code-only accepted mode that the summary's defects list records. Not re-filed.

### [spec.recheck.1.review-edit-sites.1]
FACT: the only spec-changes.md delta since the spec loop converged is three commentary re-pointings from `[f13.human-decisions]` (CODE-4 -> CODE-13 twice, CODE-1 -> CODE-15 once in the SPEC-3 carrier table); no fenced staged text changed. Each new target carries the cited content: CODE-13 holds the clean-exit disposition and the `BindReservedSlot` release fold, CODE-15 holds the `SessionScrubReporter` field comment. EVIDENCE: git diff of spec-changes.md; non-spec-changes.md CODE-13 heading and its `BindReservedSlot` row, CODE-15 Targets bullet on `pkg/adapter/server.go`
FACT: the remaining CODE-1 and CODE-6 pointers in spec-changes.md commentary stay correct after the split: the scrub-clause pointer (SPEC-1 §4.1 commentary) matches CODE-1's `answerShutdown` scrub block, and the ten-second per-member close context stays in CODE-6, not CODE-14. EVIDENCE: grep "ten-second" in non-spec-changes.md lands inside CODE-6's range
FACT: no fenced staged spec block names a CODE, DOCS, SCHEMA or CONF deliverable. EVIDENCE: awk over fenced lines of spec-changes.md
WATCHOUT: the `## Spec files touched` spec/16 bullet and the closing docs-mirror sentence still say "each counter" and leave out the `lenny_adapter_leaked_slots` gauge row, which is the Deferred from spec round 1. I did not re-file it: it is proposal bookkeeping, the gauge row lands through SPEC-6 and CODE-9 anyway, and four later rounds did not re-file it. A fixer tidying this up should reduce the bullet to a pointer at SPEC-6.

### [spec-recheck.1.review-fresh.1]
FACT: the whole spec-changes.md delta since the opt3 convergence is three commentary re-points by `[f13.human-decisions]` (CODE-4 -> CODE-13 twice, CODE-1 -> CODE-15 once, at the §7.1 acknowledged-clean commentary, the SPEC-3 carrier table's `Server.SessionScrubReporter` row, and the SPEC-4 `slot_assigned` paragraph). Each resolves: CODE-13 holds **The leaked disposition, at the caller.** and the `BindReservedSlot` row of the `ReleaseSlotReservation` table; CODE-15's Targets name the `SessionScrubReporter` field comment. No staged spec block changed. EVIDENCE: `git diff` of spec-changes.md; non-spec-changes.md CODE-13 and CODE-15 headings
FACT: re-checked in this round and still resolving exactly as the staging quotes: §4.1 third sentence (spec/04:157), §4.7 `DemoteSDK`/`Shutdown`/`ReportSessionScrub` rows (spec/04:674, :686, :692), the `*Adapter → Gateway RPCs:*` table and the `#### 4.7.2` heading (spec/04:688, :695), §6.1's preConnect-only-at-1 sentence (spec/06:69), §6.2 fence entries (spec/06:95, :148, :150-155) and the `reserved` hold paragraph that follows the fence (spec/06:158), §6.2 Client visibility clause (spec/06:290), §7.2 preamble and steps 2-3 (spec/07:210-214), §7.3 item 4 (spec/07:414), §12.6 write and read clauses and DDL (spec/12:481, :494), §15.4 SDK-warm demotion paragraph and 15.4.1 heading (spec/15:1469, :1471), and `lenny_adapter_leaked_slots{pod_id,pool}` in the tree (gatewaymetrics_credential.go:223)
DECISION: returned no findings — BECAUSE the only delta is correct commentary re-pointing and the normative blocks re-read clean — ALTERNATIVES: filing the two spec-changes.md Deferreds (the "a request" wording for the hold-timeout termination in the Edge-cases bullet, and the `## Spec files touched` spec/16 bullet omitting the gauge row) was rejected: both are unapplied commentary or bookkeeping that leaves the applied spec correct, the bar this lens holds
WATCHOUT: §7.2 step 4 bumps `coordination_generation` after the step-3 reclaim, so the reclaim carries the pre-bump generation and is not fenced by it; a reviewer who reads step 4 first may think the step-3 `Shutdown` is generation-stale. EVIDENCE: spec/07_session-lifecycle.md:214-215

### [spec-recheck.1.review-kubernetes.1]
DECISION: empty findings for the Kubernetes-idiom lens in spec-recheck round 1. BECAUSE the delta since the spec lane converged is the f13 re-cut's three re-pointed CODE references in spec-changes.md (§7.1 commentary "CODE-13's disposition", the SPEC-3 carrier row "Mirrored by CODE-15", the SPEC-4 `slot_assigned` note "once CODE-13 folds `BindReservedSlot`"), which touch no CRD, status, finalizer, webhook or controller surface, and each resolves: CODE-13 holds the disposition and the `BindReservedSlot` row, CODE-15 targets the `Server.SessionScrubReporter` comment. ALTERNATIVES: re-filing SPEC-4's read-back of the pod's own projected phase as a level-triggering anti-pattern, rejected because the tree does exactly that under one field manager and the only gap it opens is the recorded CREATE+bound+DELETE window Open.
FACT: the gateway writes the first `bound` claim status at claim acquisition, before any pod-side bind RPC, so a failed bind's claim DELETE lands on a pod projecting `claimed` except inside the recorded one-reconcile coalescing window. EVIDENCE: pkg/gateway/podlifecycle/podclaim/bindingstate.go:57-78; pkg/controller/warmpool/occupancy.go:104-143
FACT: §4.6.3 names the WarmPoolController sole writer of `Sandbox.status` and gives the gateway no `sandboxes/status` grant, so SPEC-4's "sole writer of that field" parenthetical is true. EVIDENCE: spec/04_system-components.md:632; spec/06_warm-pod-model.md:80
USEFUL [spec.4.review-kubernetes.1]: its occupancy.go transcription check and the pointer to the coalescing-window Open saved a full re-derivation.

### [spec-recheck.1.review-mechanism.1]
FACT: the spec-changes.md delta since the opt3 spec convergence is three citation re-points made by `[f13.human-decisions]` (the decision-53 re-cut): the §7.1 commentary's "CODE-4's disposition" became CODE-13 (the compensation that reads the RPC error and `exited_cleanly` alone), the SPEC-3 carrier table's `Server.SessionScrubReporter` row became CODE-15, and the SPEC-4 `slot_assigned` paragraph's "CODE-4 folds `BindReservedSlot`'s own release error" became CODE-13. All three resolve to the right new heading. EVIDENCE: `git diff` of spec-changes.md; non-spec-changes.md `### CODE-13` Targets (the `BindReservedSlot` bullet), `### CODE-15` Targets (the `SessionScrubReporter` bullet), and the CODE-13 disposition text ("an adapter answering `superseded`, `absent` ... together with `exited_cleanly` false is leaked here").
FACT: the SPEC-6 `cause` label predicate ("`refusal` when the compensated attempt failed on a SUPERSEDED or ALREADY_STARTED refusal") matches CODE-13's `compensationCause`, which returns `refusal` on `errors.Is` against CODE-7's two sentinels and `failure` otherwise. The reclaim-hold refusal and the rule-8 refusal both map to `failure`. That is consistent: after a hold refusal the compensation answers `absent`, which never reaches the superseded counter. EVIDENCE: non-spec-changes.md CODE-13 "`compensationCause(err error) string`".
FACT: the SPEC-4 fence block under "replace the trigger list of the `claimed ──→ draining` entry:" is the REPLACEMENT text. The current spec/06:95 reads "claim deleted on a pod with recycle.enabled: false". The block has no "reads, verbatim" original because the block itself is the new text. EVIDENCE: spec/06_warm-pod-model.md:95; git log -L on that staged block (commit 06b7c6e7c).
WATCHOUT: the two spec-changes.md Deferreds (the "a request" genus noun for the §10.1 hold-timeout termination in the untokened-entry edge-case bullet, and the `## Spec files touched` spec/16 bullet's "one row for each counter" leaving out the gauge) are both in commentary or bookkeeping that lands in no spec file. Several reviewers have already judged them below the bar (archive, round-27/28 entries). A mechanism reviewer should not re-file them. A fixer who touches either sentence for another reason should correct it in the same edit.
OPEN [non-spec-changes.md]: the §10.1.4 per-member ten-second close-context split (`onHoldTimeout` minting the guard-acquisition deadline only) is stated only in the files-touched `holdstate.go` entry. No deliverable body states it. spec-changes.md SPEC-3 commentary and the Testing claim-map note both attribute it to CODE-6, and the guard it depends on is now CODE-14. The non-spec loop should confirm which deliverable's Targets own it.

### [spec.recheck.1.review-performance.1]
FACT: the only spec-changes.md delta since spec round 4 converged (working tree against cd18be606) is three commentary re-pointers from the f13 re-cut: CODE-4 to CODE-13 at the acknowledged-clean predicate paragraph and the `slot_assigned` paragraph, and CODE-1 to CODE-15 in the carrier table's `Server.SessionScrubReporter` row. All three targets exist (non-spec-changes.md `### CODE-13` :985, `### CODE-15` :363). No staged spec text changed, so no write, watch, lock or label changed. EVIDENCE: git diff of spec-changes.md
FACT: SPEC-6's metric rows keep bounded labels (`pool`, `k8s_pod_name`, and `cause` in {refusal, failure}; `pod_id`, `pool` on the gauge), matching the shipped slot-metric cardinality. EVIDENCE: spec-changes.md:1226, :1228
USEFUL [spec.4.review-performance.1]: its no-new-store-write confirmation still holds; the delta is commentary only.

### [spec-recheck.1.review-reliability.1]
FACT: the spec-changes.md delta since the opt3 convergence is three commentary re-pointers from the f13 re-cut (CODE-4 -> CODE-13 twice, CODE-1 -> CODE-15 once); each lands on the heading that now owns the cited text, and none touches applied spec text. EVIDENCE: `git diff HEAD -- proposals/0081*/*.spec-changes.md` (hunks at :392, :610, :997); non-spec-changes.md:363-370 (CODE-15 Targets name the `SessionScrubReporter` comment), :985-999 (CODE-13 owns the compensation and the `BindReservedSlot` release-error fold).
FACT: the ten-second hold-timeout window citation still holds. EVIDENCE: pkg/adapter/holdstate.go:201 `context.WithTimeout(context.Background(), 10*time.Second)`.
DECISION: empty findings for the reliability lens. BECAUSE the delta is pointer-only and the recovery surface (§7.1 reclaim, §7.2 step 3, §5.2 disposition table and hold, gateway-crash-lost compensation recorded as an accepted failure with two named recoveries) is unchanged since `[spec.4.review-reliability.1]`. ALTERNATIVES: re-filing the lost-compensation case as an unreclaimed resource (rejected: recorded in Edge cases as accepted, with the durable re-drive and the reaper routed to their own findings).
USEFUL [spec.4.review-reliability.1]: its unchanged-surface inventory let this round limit fresh reading to the diff.

### [spec-recheck.1.review-security.1]
FACT: the uncommitted delta to spec-changes.md since commit cd18be606 is four pointer relabels (CODE-4 to CODE-13 twice, CODE-1 to CODE-15 in the SPEC-3 carrier table, CODE-4 to CODE-13 in the SPEC-4 slot_assigned paragraph); no staged spec block changed, so no security control moved. EVIDENCE: git diff HEAD on spec-changes.md, hunks at its §7.1 commentary, the SPEC-3 carrier table row for Server.SessionScrubReporter, and the SPEC-4 "No edge is added out of slot_assigned" paragraph
FACT: the §5.2 disposition row "runtime close succeeds and any other act fails -> released, Set, Not entered" matches the shipped Shutdown, which discards removeSlotTree's error and keys the report on closeErr alone. EVIDENCE: pkg/adapter/session.go:273 "_ = removeSlotTree(st)", :282 reportSessionScrub(ctx, sessionID, closeErr)
USEFUL [Settled: open decision 38 and the credentials.json residue from spec.1.review-security.1]: together they close the only security-lens candidate in the full staging, which is that the withdrawn "if cleanup fails, the slot is leaked" universal removes degraded-state surfacing for a failed tree removal. Do not re-file it.
USEFUL [Traps: "Security and capacity families are exhausted"]: every in-pod-attacker variant I tried (a forged unconditional Shutdown, a suppressed leaked report, an undercounted sessions_served) collapses into the adapter gRPC reachability Open or into the pre-existing adapter-sourced report, as that entry says.

### [spec-recheck.1.review-single-source.1]
FACT: the spec-changes.md delta since the spec lane converged is three pointer re-targets from `[f13.human-decisions]` (CODE-4 to CODE-13 at the §7.1 commentary and the §6.2 `slot_assigned` paragraph, CODE-1 to CODE-15 in the SPEC-3 carrier table). All three match the non-spec headings: CODE-13 owns the compensation disposition and the `BindReservedSlot` release fold, and CODE-15 names the `SessionScrubReporter` field comment. EVIDENCE: `git diff HEAD -- *.spec-changes.md`; non-spec-changes.md:363, :370, :985, :1202
DECISION: filed Step-5 reason copies rather than rule copies, BECAUSE the rule inventory is unchanged since round 11 and none of its rules has a second stating site, while the same decision reasoning is written out in the summary's `**Decisions.**`/`**Watch out for.**`, in spec-changes `## Design`, and again in deliverable commentary. ALTERNATIVES: I did not file the SPEC-6 gauge gloss (settled as §16.1 house style) or the §15.1 "stated once" pointer disagreement at :1185 vs :1248 (refuted as polish in spec round 4).
WATCHOUT: the Settled entry "`## Design` ... Deleting it was rejected: ... the two-field rationale lives nowhere else" is false today. The summary's `unconditional_teardown` Decisions bullet (summary.md:125-133) carries the same rationale in full. EVIDENCE: summary.md:125-133; spec-changes.md:61-64, :243-255, :291-294
CORRECTS [Settled, "`## Design (as the spec must state it)` is a choice record"]: the two-field rationale is written out at four sites. When a fixer reduces them, the summary Decisions bullet is the natural home.

### [spec-recheck.2.review-single-source.1]
FACT: the round-1 fix delta to spec-changes.md is deletion-only (Design two-field reason, SPEC-1 §4.1 and `Shutdown`-row commentary, SPEC-2 connection-preference paragraph, SPEC-3 double-count clause, reclaim-hold and scrub-model placement paragraphs, SPEC-5 §15.1 exclusion re-key paragraph, untouched-section §15.1 widening sentence). Every new pointer resolves: "the summary's `unconditional_teardown` decision" is summary.md:125-133, which carries the bare-scalar and `recycle`-precedent reason; "SPEC-5's §15.1 block" is the heading at spec-changes.md:1084. EVIDENCE: diff against scratchpad/cp-snap/0081-opt4/spec-recheck-r1-prefix
WATCHOUT: the setup-command-only mapping of the started-session refusal is still written in the Edge-cases bullet (spec-changes.md:203-214) and in the SPEC-5 §15.1 lead-in (:1086-1092), beside the staged row text itself. Judged a close variant of the round-1 "§15.1 widening rationale at four sites" fix and not refiled; a later prune may cut the lead-in to a pointer at the bullet. EVIDENCE: spec-changes.md:1086-1092

### [operator.f13-repair]
CORRECTS [f13.human-decisions DEFERRED implementation-checklist.md]: S15 names CODE-14, S17 names CODE-15 and S19 names CODE-13, with their CODE-6, CODE-1 and CODE-4 citations re-pointed; S16 cites CODE-14's expired-acquisition disposition and now lands the Testing subsection **Tier-1 adoption-ordering tests, every case walked**; S21 is deleted and its id retired, since no step depended on it.
CORRECTS [f13.human-decisions DEFERRED summary.md ## Deliverable index]: CODE-1, CODE-4 and CODE-6 are narrowed to what the decision-53 re-cut left them, and CODE-13, CODE-14 and CODE-15 are added beside the deliverable each was cut from.
CORRECTS [f13.other-proposals]: the 0082 impact row credits the attempt-scoped lease release to CODE-13, which owns it after the re-cut; the shipped-tree defect row on the credential-file residue names CODE-1, CODE-15 and CODE-6.
DECISION: open decision 35 stays closed. Its phase record carried a contested flag from an early firing, but the staging answers it (CODE-1's exit-helper comment starts the whole-pod scrub after the per-slot cleanup's acts have returned), and the summary preamble records that. The stale flag is cleared in the phase state rather than the entry restored. — ALTERNATIVES: restoring entry 35 for the human (rejected: the question has a staged answer).
FACT: the open-decisions preamble now points at `### Settled`, the standing-context changelog and the archive, since `### Retired` no longer exists; entry 50 now says neither the loop nor the phase derived a recommendation.
