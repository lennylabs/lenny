...
- **Does CODE-9's heading owe `pkg/gateway/podlifecycle/podsession/slotfailure.go`?** UNVERIFIED [`[non-spec.10.review-edit-sites.1]`]: CODE-9's body adds `slotFailureWorkspaceFinalize` without naming its file, and the files-touched list attributes `slotfailure.go` to CODE-9, although the shipped stage constants are declared in `binder.go:288-298`; judged bookkeeping. staticcheck's `unused` marks a const BLOCK used when one member is, so the constant is safe only inside the used group at `binder.go:288-298`.
...
### [operator.prune-r8]
DECISION: summary.md's "Where that lands:" lead-in and its mechanism bullets are deleted — BECAUSE each bullet restated a deliverable whose staged body and Deliverable index entry carry it, and the §15.1 no-row fact stays homed in SPEC-5's paragraph after rule 9 and checklist S1 — ALTERNATIVES: a one-sentence pointer at the Deliverable index, rejected as a restatement of the section's existence.
DECISION: the summary Decisions bullet on the identity refusal now names "the `codes.Aborted` arm CODE-5 adds to `isTransientPodClaimError`" rather than "the accounting helper's new `Aborted` arm" — BECAUSE the deleted mechanism bullet was that phrase's only introduction in the summary, and the arm belongs to the classifier rather than to the accounting helper.
DECISION: summary Watch-out bullets on the pre-`PrepareWorkspace` `stageWorkspace` failure, on `UnhealthyThreshold(2)`, on `TestValidTransitions_spec_6_2` and on `SlotID == SessionID` are deleted — BECAUSE each hazard is homed elsewhere: CODE-13 and the Testing case **The pre-`PrepareWorkspace` workspace failure, separately.** (the deleted bullet also contradicted both, since the compensation is still sent there); the Testing cases **Accounting, at `maxConcurrentSessions: 4` (threshold 2).** and **One `maxConcurrentSessions: 2` case**, which name the discriminating concurrency; CODE-3's closing paragraph, `### Edge-list test for CODE-3, tier 1` and checklist S8; and the non-spec Design **The reclaim hold.** paragraph with CODE-14's `slotGuards` lifetime paragraph and CODE-5's opening.
DECISION: the summary Watch-out bullet on today's `Shutdown` bricking the pod becomes an ordering pointer, "CODE-15 lands before CODE-13 (S16 before S19)", citing CODE-15's bullet on why the runtime teardown must not run for an unstarted session — BECAUSE the per-implementation `Close` mechanics live in that CODE-15 bullet and in the summary defects entry (`[non-spec.1.fix-G6.1]`), while the ordering is what the Watch-out owns. The Non-goals bullet on landing the compensation without the teardown gate now names the defects entry **A pod whose runtime process has been closed cannot serve another session.** as where the listener close is recorded, because the cut bullet was the only earlier text that defined "the listener-close hazard" (`[prune-r8.fix.2]`).
DECISION: the summary Decisions file-collision bullet glosses the reclaim hold at its first use as rule 2's refusal of a bind onto a slot identifier from the deregistration of its entry until the reclaim's cleanup completes, or for the pod's life where it does not — BECAUSE the deleted "Where that lands:" bullets held the summary's only definition before `## Goals` and its only link to rule 2 (`[prune-r8.fix.1]`).
DECISION: the summary `## Goals` bullet on residue removal glosses "the start-confirmation rule" as rule 8 at its now-only summary use — BECAUSE the deleted "Where that lands:" bullet and the reduced CODE-2 index entry were the summary's only links from that name to §4.7.1 rule 8.
DECISION: the summary Watch-out phase-gate bullet keeps its trap sentence and cuts its per-site `allowStarted` values to a pointer at CODE-6's **Production call sites that must thread the parameter.** table — BECAUSE refusing `ConfigureWorkspace`'s idempotent repeat is a trap an implementor would plausibly fall into, and the per-site values have their home in that table.
DECISION: the open-decisions preamble is reduced to one sentence that keeps the stamped-identifier numbering, the reason entries 32, 33, 34, 36 and 50 carry no recommendation, a pointer at the review log's `### Settled` list, changelog and archive for every answered or moved entry, and decision 53 corrected to say that CODE-15 lands with CODE-1 at S16 (CODE-14 at S15, CODE-13 at S19) — BECAUSE the per-entry answer history is held by `### Settled` and by the staged text each answer changed, and the old decision-53 wording ("CODE-13, CODE-14 and CODE-15 carry their later steps") was imprecise after the S17 fold (`[non-spec.7.review-single-source.1]`).
DECISION: the summary Deliverable index entries for SPEC-1 to SPEC-6, SCHEMA-1, CODE-1 to CODE-6, CODE-8 to CODE-15 and CONF-1 are reduced to id, files and heading phrase; each CODE, SCHEMA and CONF phrase is the text after the deliverable heading's last separator, and each SPEC phrase names the sections its staged-edit headings name; CODE-7 and DOCS-1 to DOCS-4 are unchanged; CODE-10 keeps its pointer at the shared-invariants block; CODE-11 keeps its three files (`[index-reconcile.4]`); each entry's file list is its heading's, so CODE-1 lists `pkg/adapter/session.go` alone — BECAUSE the removed prose restated each deliverable's staged body.
DECISION: spec-changes.md's Edge-cases retry bullet, CODE-5's opening and `applySlotRetryPolicy` caller bullet, and CODE-8's urgency paragraph, state current disposition only; "this revision withdrew", "the earlier revision carried ... is withdrawn", "comes out with it", "replaces it and is stronger" and "nothing in this revision needs the append the pointer existed for" are removed, CODE-5 keeps its three grounds and now says that SPEC-2 leaves §5.2's slot-retry `**Max retries:**` bullet as it stands (checklist S3), and CODE-8 rests its ordering on its hard-blocker paragraph — BECAUSE the revision history lives in this log.
DECISION: the non-spec `## Files touched on application (non-spec)` bullets for production, schema, claim-register and docs files are reduced to path, deliverable ids and regen commands; `pkg/adapter/bindattempt.go` lists CODE-6 and CODE-14 and `pkg/adapter/metrics.go` lists CODE-9 alone, because CODE-1's targets say it defines nothing in either; the claim-register bullet keeps which deliverable lands which row (checklist S9 and S22) — BECAUSE every removed symbol list is staged in the named deliverable.
DECISION: the `tests/spec-map.json` and `tests/tier0_static/spec_map_slot_address_registration_test.go` bullets, the comment-carrier bullet and the three sweep bullets are left verbatim — BECAUSE they state gate constraints, the 0079 impact row cites the comment-carrier bullet's file-collision rule, and the sweep greps, the `adapterv1.` qualifier trap and the `files_updated_test.go` exemption have their single home there. In the Tests bullet only the `ReleaseSlotForTest` parenthetical is removed, since CODE-14 states the caller enumeration and the already-cancelled `Resume` row; the `manifest_fields_test.go` widening, the token-service S13 ordering and the tier-11 metric test's registration parentheticals stay.
DECISION: review-log `### Open` loses five entries the ledger closed: the `--client-ca-file` rendering question (`[non-spec.6.review-security.1]`), the `WIRED` rows at S9 and the tier-7a race step (`[non-spec.6.review-fresh.1]`, `[non-spec.6.fix-G1.1]`, `[non-spec.7.review-applicability.1]`), CODE-4's mint-site sentence (`[non-spec.2.review-client-surface.1]`, `[non-spec.2.review-fresh.1]`), and the fresh-connection proposition (`[non-spec.6.review-single-source.1]`). The rendering answer moves to `### Deferred` as a pre-existing security finding, and the `### Traps` client-certificate WATCHOUT now names the flag the binary defines, `--tls-client-ca-file` (`cmd/lenny-adapter/main.go:114`), and points at that Deferred entry, as does the `### Traps` entry **Security and capacity families are exhausted.**
DECISION: the `### Open` entry on CODE-9's heading and `slotfailure.go` now cites as its evidence that CODE-9's body adds `slotFailureWorkspaceFinalize` without naming its file, that the files-touched list attributes `slotfailure.go` to CODE-9, and that the shipped stage constants sit in `binder.go:288-298` — BECAUSE the reduced files-touched list no longer names `slotFailureWorkspaceFinalize`.
DECISION: the Standing context's step references to CODE-15 now name S16, to match the corrected decision-53 sentence and checklist S16: the coverage map reads "S16=CODE-1 and CODE-15" and records S17 as folded into S16 with its id retired, and the tier-9 registered-but-unbound DECISION, the `PrepareWorkspace` `sessionID == ""` entry, the `exited_cleanly` WATCHOUT, the `removeSlotTreeFn` seam's cycle (S14↔S16) and the `runtimeHoldsLocked` non-finding name S16; the fourth-list non-finding on `Depends on:` omitting S17 is deleted because no S17 remains — BECAUSE the r7 fold left these Standing-context sites naming a step the checklist no longer has (`[prune-r8.review-conflict.1]`).
