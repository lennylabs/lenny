...
opaque bind attempt token, and it carries that token on every bind-sequence request that can
create or resolve a registry entry and on the compensating `Shutdown`. The adapter stamps the
...
the review log records where each went. Entries 12, 13, 14, 15, 21, 23, 24, 25 and 37 were answered
by the staging as it already stood, so no change file was edited for them. Entries 7, 26, 27 and 38
...
already runs on this pivot. Entry 20 and entries 32, 33 and 36 carry the question and its ground
...
- **CODE-1** (`pkg/adapter/session.go`, `pkg/adapter/runtimegeneration.go`, `pkg/adapter/server.go`, `pkg/adapter/slot.go`, `pkg/adapter/slotsession.go`): `Shutdown` applies rules 10 through 15, taking the reclaim hold and deferring a release it takes only when the cleanup completed, releasing the slot for any entry the call removed, running the runtime teardown only for a session whose start the adapter has admitted, and filing a cleanup-outcome report only for a slot that reached `running`. Its `exited_cleanly` answer reports the slot-release failure for a reclaim of a slot that did not. The handler gains one exit after the field precondition, and the whole-pod recycle scrub the shipped handler runs as a trailing statement moves into that exit so it runs on every outcome, with the shipped trailing copy deleted in the same edit.
...
- **CODE-6** (`pkg/adapter/bindattempt.go`, `pkg/adapter/slot.go`, `pkg/adapter/slotsession.go`, `pkg/adapter/server.go`, `pkg/adapter/staging.go`, `pkg/adapter/slotcreds.go`, `pkg/adapter/credentials.go`, `pkg/adapter/resume.go`, `pkg/adapter/sdkwarm.go`, `pkg/adapter/holdstate.go`): `slotState.bindAttempt`, the resolve descriptor its callers pass, the `validateBindFields` check each handler carrying the fields runs at its entry under rule 1, the predicate that applies rules 2 through 7 at the one resolve chokepoint under `s.mu`, the per-slot guard over every section that writes or destroys a slot's tree outside `s.mu`, derived section by section rather than enumerated, and rule 2's reclaim hold with the `reclaimSlotLocked` helper every deregister-then-destroy site routes through. In `holdstate.go` the §10.1.4 pass's single ten-second context becomes its guard-acquisition deadline alone and each member mints its own ten-second close context, so one member's guard wait cannot spend a later member's final usage report or runtime close. The hold is tested at the guard acquisition, so a guarded admission RPC is refused rather than left to block for the whole cleanup, and inside the resolve, which is where the unguarded RPCs meet it.
...
- **CODE-10** (the comment carriers the CODE-10 grep returns): the comment carriers of the withdrawn cleanup-outcome reporting universal lose that trigger clause, the comments that state the served-count trigger are re-keyed onto the cleanup-outcome report except where the sentence's subject is that report, where the trigger is deleted instead, and the landed migration's column comment is edited in place. The carrier set is closed by the grep at application time and enumerated nowhere.
- **CODE-11** (the `SETUP_COMMAND_FAILED` cause restatements in `pkg/gateway`): the Go comments that restate the §15.1 `SETUP_COMMAND_FAILED` row's cause and retryability are cut back to a citation of that row, which SPEC-5 rewrites. The REST classification table's group comment, `writeSetupCommandError`'s and `isTransientPodClaimError`'s doc comments and the resume-demotion case's `// diagnosis:` comment each identify the code with the setup-command exit alone today. No classification value, branch or assertion moves.
...
- **DOCS-2** (`docs/reference/adapter-contract.md`): the `Shutdown` row states the two teardowns with their two preconditions, the withheld cleanup-outcome report, the drain gate's own condition and the two-field precondition; the `DemoteSDK` row states that the demotion drops the adapter's slot registry entry and runs that slot's cleanup inside the call; the `ReportSessionScrub` row states which cleanups are reported by pointing at the `Shutdown` row and reports none for any other release; and one paragraph points the reader at the §4.7.1 rules and §15.4's conformance criterion for the bind attempt token.
