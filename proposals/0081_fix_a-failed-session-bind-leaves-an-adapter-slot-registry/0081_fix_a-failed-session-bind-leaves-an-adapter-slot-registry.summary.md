...
- Neither refusal reaches the client as a failure of the client's own request (operator decision
  ...
  `SETUP_COMMAND_FAILED` row gains only a pointer sentence at it, because the row's exclusion
  sentence keys the retryable fallback on the gRPC code and rule 6's refusal arrives on
  `FailedPrecondition`; §6.2's client-visibility clause and the published error catalog, both
...
- **SPEC-5** (`spec/04_system-components.md`, `spec/15_external-api-surface.md`): the §4.7.1 bind attempt token block, one pointer sentence on the §15.1 `SETUP_COMMAND_FAILED` row, and the two §15.4 blocks after the SDK-warm demotion contract.
