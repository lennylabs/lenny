...
45. **Does `lenny_adapter_leaked_slots` now need a §16.1 catalog row?** The gateway publishes
    the series, from `pkg/gateway/metrics/gatewaymetrics`, although §6.2 attributes it to the
    adapter's `/healthz` metadata. The series has no §16.1 row and no `docs/reference/metrics.md` row today, which is
...
- **CODE-10** (the comment carriers the CODE-10 grep returns): the Go, SQL and test comment carriers of the withdrawn cleanup-outcome reporting universal take the reduction CODE-10's arm rule states, and the landed migration's column comment is edited in place. The carrier set is closed by the grep at application time and enumerated nowhere.
...
- **CODE-12** (the comment carriers the CODE-12 grep returns): the Go, SQL and test comments that state what a claim DELETE projects, keyed on the pool's recycle setting or on its retirement limits, or asserting without qualification that it returns the pod to `idle`, lose that outcome and leave it to the §4.6.1 bullets SPEC-4 re-keys. The carrier set is closed by the grep at application time and enumerated nowhere.
