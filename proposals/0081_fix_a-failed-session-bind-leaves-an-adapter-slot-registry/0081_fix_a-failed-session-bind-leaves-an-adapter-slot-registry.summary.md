...
- The rules are stated in §4.7.1 and nowhere else, each rule carrying the gRPC status, the
  `ErrorCode` and the `Shutdown` outcome it answers on, except that rule 2 takes its status from
  §15.4's slot-identifier reclaim-hold block and rules 11 through 14 take theirs from rule 15.
  ...
  is the single home of the setup-window mapping, and §6.2's client-visibility clause is reduced
  to a citation of that row.
...
- **SPEC-5** (`spec/04_system-components.md`, `spec/15_external-api-surface.md`, `spec/06_warm-pod-model.md`): §4.7.1 gains the bind attempt token block after the RPC tables, which states the token, the requests that carry it, the stamp-once rule, the registry critical-section paragraph that is the one statement of what the adapter performs under the registry lock as a unit, and admission and teardown as two ordered cascades of numbered rules, each rule stating the gRPC status, the `ErrorCode` and the `Shutdown` outcome it answers on, with rule 2 taking its status from §15.4's reclaim-hold block and rules 11 through 14 taking theirs from rule 15; §15.4 points at that rule set, states what conformance against the rules means, and carries the slot-identifier reclaim-hold block forward; §15.1's `SETUP_COMMAND_FAILED` row takes the cause, retryability, setup-output and exclusion replacements, which make it the single home of the setup-window mapping, and §6.2's pre-attached client-visibility clause is reduced to a citation of that row. §15.1's error catalog takes no new row for either code.
