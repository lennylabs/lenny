...
- The gateway mints the token at `materializeSlot`, `Binder.Prepare` and `Binder.Resume`,
  ...
  failure and at a failed `Resume`, suppresses it when the failing stage was itself refused on
  either typed refusal, derives the `leaked` disposition from the RPC error and the clean-exit flag, and
...
- A `stageWorkspace` failure raised before `PrepareWorkspace` is sent, through a source-rewrite
  error or a blob-store failure, sends no RPC and leaves no adapter entry (CODE-4). A test
...
`### Settled` and `### Retired` lists carry the remainder, each with its reason. Entry 28 was
...
- **A slot's §4.9 lease-expiry timers are cancelled before the credential file they protect is
  ...
  because both are shipped adapter behaviour, and CODE-1 and CODE-6 keep the deregistration and the
...
- **CODE-1** (`pkg/adapter/session.go`, `pkg/adapter/runtimegeneration.go`, `pkg/adapter/server.go`, `pkg/adapter/slotsession.go`): `Shutdown` applies rules 10 through 15, taking the reclaim hold and deferring a release it takes only when the cleanup completed, releasing the slot for any entry the call removed, running the runtime teardown only for a session whose start the adapter has admitted, and filing a cleanup-outcome report only for a slot that reached `running`. Its `exited_cleanly` answer reports the slot-release failure for a reclaim of a slot that did not. The handler gains one exit after the field precondition, and the whole-pod recycle scrub the shipped handler runs as a trailing statement moves into that exit so it runs on every outcome, with the shipped trailing copy deleted in the same edit.
...
- **CODE-4** (`pkg/gateway/podlifecycle/podsession/slotbinder.go`, `binder.go`, `slotfailure.go`, `bindattempt.go`, `pkg/gateway/sessionserver/upload_to_session.go`): the gateway mints an opaque token per bind attempt at `materializeSlot`, `Binder.Prepare` and `Binder.Resume`, carries it on every request that takes one, sets `unconditional_teardown` at every non-compensating `Shutdown` caller, sends the compensating `Shutdown` at every post-connection bind failure and at a failed `Resume`, suppresses it on either typed refusal, computes the leaked disposition from the RPC error and the clean-exit flag, which the teardown rules make sufficient, and releases the lease identifiers its own attempt minted.
