...
- **Scope:** Twelve pre-existing inconsistencies were flagged while authoring `spec/29` and set aside
  ...
  surfaced five further items. One of them was a claim-register row whose premise did not survive the tree:
  the field the row names exists, so the row's `UNWIRED` status was already correct and there was no status
  to correct. Proposal 0073 retires that row together with the field rather than restatusing it, and §1.11
  records the dropped section. This proposal corrects what can be corrected and states what cannot.
