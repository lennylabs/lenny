...
- **Status:** Draft for review. Revised after 3 adversarial review rounds (2026-06-09).
- **Date:** 2026-06-09.
...
```markdown
- `released` — the gateway has released the pod back to the pool; the referenced `Sandbox.status.phase` has transitioned to `idle` (for a recyclable pod), to `draining`/`terminated` (for a pod being replaced), or, in task mode, to `sdk_connecting` (the `rewarm_sdk` disposition releases the claim while the adapter re-establishes SDK-warm state; [Section 6.2](06_warm-pod-model.md#62-pod-state-machine) "Task-mode disposition driver") or `failed` (the `onCleanupFailure: fail` disposition and the late-report rule both release a claim whose referenced pod is in `failed`). The `SandboxClaim` resource is deleted shortly after entering `released`; it is a terminal phase.
```

The pre-edit bullet restricts `released` to a referenced pod in `idle` or `draining`/`terminated`. The Section 9.3.3 driver paragraph releases the claim on every disposition and cites this enumeration, so without the extension the staged §6.2 text would contradict the §4.6.3 definition on the `rewarm_sdk`, `onCleanupFailure: fail`, and late-report paths. The `bound`/`active`/`released`/`failed` phase-value enumeration itself is unchanged, so the CRD OpenAPI validation sentence below the bullet is unaffected.
...
- **envtest (tier 2):** the gateway SSA Apply of `status.taskState` under the `lenny-gateway` field manager preserves `phase`, `activeSlots`, and `tenantId`; each non-Apply phase yield (the session-mode `draining`, the task-mode `idle` and `sdk_connecting`, and the concurrent-mode last-slot `slot_active → idle`) strips the gateway claim on `status.phase` only, and the WarmPoolController writes `draining → terminated` and `sdk_connecting → idle`; the converted `ReleaseSlot` last-slot yield leaves no gateway claim on `status.phase` for a pooled `idle` concurrent-mode pod, and the WarmPoolController's non-forcing drain Apply lands on it; the watchdog's `task_cleanup → failed` non-Apply update succeeds while the gateway holds the carve-out claim.
...
| File | Change |
|------|--------|
| `spec/04_system-components.md` | §4.7 `ReportTaskCleanup` row; §4.7 `task_complete` and `task_ready` row notes; §4.6.3 ownership-table carve-out rows, gateway status window paragraph, "never force-conflict" scoping, gateway RBAC grant sentence, WarmPoolController RBAC `update` clause, and `SandboxClaim.status.phase` `released` bullet task-mode extension; §4.6.1 orphan-GC phase gate |
