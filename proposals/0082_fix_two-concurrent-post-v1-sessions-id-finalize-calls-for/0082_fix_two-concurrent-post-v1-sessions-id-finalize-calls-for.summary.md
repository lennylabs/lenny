...
- `pkg/gateway/sessionserver/start.go`, `finalize.go`, and `sessionserver.go`: the ready write and every finalize failure write admit only `finalizing` (CODE-1, CODE-3), and handleFinalize revokes the session's lease on the exits CODE-3's **handleFinalize exit dispositions** table assigns to `revokeFinalizeLease`.
...
- handleFinalize issues no reclaim of its own after a lost exit write. CODE-3's handleFinalize rules state the response on each branch, and CODE-3's **handleFinalize exit dispositions** table states who releases the lease and who reclaims the pod on every exit.
- The lease a partial credential assignment leaves on a committed prepare failure is fixed in this proposal rather than left to 0081 (see the partial-assignment rows of CODE-3's **handleFinalize exit dispositions** table).
...
- `podclaim.DeleteClaim` deletes `claim-<podName>` with no owner check. Before adding or moving a reclaim in handleFinalize, read the note "The handler never reclaims after a lost write" under CODE-3's **handleFinalize exit dispositions** table, which states what the no-reclaim rule covers.
...
- A finalize test on a fixture without a pod binder and an in-process adapter never runs `Binder.Prepare`, so it mints no lease and never reaches `prep != nil`. An assertion there about the handler's revoke or reclaim passes vacuously. TEST-1's `finalize_race_test.go` fixture names the fixture that runs Prepare at tier 1.
...
- Every §4.9 lease a finalize call's prepare phase records in the lease store has a named releaser on every handleFinalize exit, as CODE-3's **handleFinalize exit dispositions** table records.
...
- Calling reclaimFinalizedPod when the ready write or a failure write loses to a terminal writer. Rejected for the reason CODE-3's **handleFinalize exit dispositions** table gives.
- Leaving the lease that a partial credential assignment minted on a committed prepare failure to proposal 0081's CODE-4, and recording the leak as a shipped-tree defect. Rejected because 0081 is a draft with no landing order relative to 0082, the revoke is a session-keyed call in a branch this proposal already rewrites, and it composes with 0081's attempt-scoped release.
- Releasing the partially assigned lease inside `Binder.failPhase`, by setting `leaseAssigned` before `assignCredentials` or by releasing unconditionally. Rejected because it duplicates 0081 CODE-4's attempt-scoped release and edits `binder.go`, which 0082 does not otherwise touch.
- Keeping the `prep != nil` gate on the revoke of a lost exit write. Rejected for the reason the note "Every lost exit write whose locked state is terminal revokes, whatever `prep` is" under CODE-3's **handleFinalize exit dispositions** table gives.
...
- **Overtaken prepare failure.** When a terminal writer overtakes a finalize call and the call's prepare phase then fails, the failing step reclaims the pod by name before handleFinalize reaches its failure write. The reclaim is `Binder.failPhase`'s drain, `Binder.Prepare`'s `ReclaimClaimed` after a reconnect failure, or `prepareAtFinalize`'s `reclaimFinalizedPod` on its pool-resolution, credential-pool-resolution, and credential-delivery-isolation exits (`pkg/gateway/podlifecycle/podsession/binder.go`, `pkg/gateway/sessionserver/finalize.go`). The terminal writer's reclaim may already have returned the pod to its pool, and `podclaim.DeleteClaim` deletes `claim-<podName>` with no owner check (`pkg/gateway/podlifecycle/podclaim/claimer.go`), so the delete can remove a successor session's claim on the recycled pod. The exit guards cannot prevent it, because these reclaims run inside `prepareAtFinalize` before any exit write. The hazard predates this proposal and is a consequence of the terminate defect above. The fix is a claim Delete that carries the claim's UID as a precondition, and it is recorded as a follow-up finding that can land with 0081's reclaim work.
- **Failure-write store error.** When `failFinalizing` returns a store error, the row can stay `finalizing` after the pod has been reclaimed. On the prepare-failure branch the binder or `prepareAtFinalize` reclaimed the pod before the failure write. On the plan-parse branch and in Gap-2 sub-branch (i), the handler reclaims it after the failure write. The §6.2 watchdog later moves the row to `failed`, and its terminal reclaim deletes `claim-<podName>` by name a second time, with the same successor-claim hazard. In sub-branch (i) the ready write can also have committed despite its error, for example across a Postgres failover, and the reclaim then removes the pod and the lease of a session that is `ready`. Both behaviors predate this proposal and belong in a separate proposal.
- **Shared-pod reclaim.** A concurrent-workspace session's `PodAssignment` names the pod it holds a slot on, and every slot holder on that pod shares one per-pod claim (`podclaim.SlotClaimer`, `pkg/gateway/podlifecycle/podclaim/slotclaimer.go`; the create-time `ClaimSlot` in `pkg/gateway/sessionserver/start.go`). handleFinalize's plan-parse reclaim and `prepareAtFinalize`'s pool-resolution reclaim run before the pool's concurrency is known, and the terminal writer's `terminalReclaimPreRunning` does not check it either (`pkg/gateway/sessionserver/usage.go`). Each reaches `Binder.ReclaimClaimed`, which deletes that shared claim by name and never calls `Binder.ReleaseSlotReservation`, so it can release the pod out from under every other slot holder and leaves the slot count unreconciled. In Gap-2 sub-branch (i) a concurrent-workspace row has a nil `prep`, so when the failure write commits nothing releases its slot. The behavior predates this proposal. The fix routes a concurrent-workspace row's reclaim through the slot release, and it is recorded as a follow-up finding.
- **Token-Service-only lease.** In Token Service mode `credassign.Client.assign` can leave a lease that the Token Service minted and the gateway never recorded: the proto decode or `recordAssignedLease`'s lease-store `Put` fails after the server committed, the server returns more than one lease, or the call times out after the server committed (`pkg/gateway/credentials/credassign/client.go`). That lease never appears in `LeasesBySession`, so neither `revokeFinalizeLease` nor any terminal reclaim can release it, and it holds its credential-pool slot in the Token Service. The pod never receives it and the LLM proxy cannot resolve it. The behavior predates this proposal, applies to every assignment path, and is recorded as a follow-up finding.
