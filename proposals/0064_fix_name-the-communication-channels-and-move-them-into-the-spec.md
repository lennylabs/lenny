...
This proposal lands one new specification file, appends to the `spec/28_communication-channels.md`
proposal 0067 created with the naming law and the three registers, and lands the tooling that migrates
...
§28.4 headings are not among them, because proposal 0067 created those sections and SPEC-1 writes their
key or their exceptions entry, so this sub-step neither writes nor retires coverage for them.
