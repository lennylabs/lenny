...
SPEC-1 carries their `tests/spec-map.json` key or their `pending-implementation` entry in
`tests/spec-map-exceptions.yaml`, so this sub-step neither writes nor retires coverage for them.
