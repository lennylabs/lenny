...
here while the §28.3 register its bijection reads already exists from proposal 0067, which runs before
SPEC-1, so the bijection first holds at this sub-step's exit. The artifact-register supersession check is a tier-11 gate, and it lands here
...
§28.4 headings are not among them, because proposal 0067 authored them together with their sections and
SPEC-1 wrote their `tests/spec-map.json` key or their `tests/spec-map-exceptions.yaml` entry, so this
sub-step neither writes nor retires coverage for them.
