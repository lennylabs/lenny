...
substituting the map's successor. An entry also records that an occurrence cites no specification section
at all, which is what a regulation clause a compliance page quotes and a pointer at material this
migration never moved are, and the pass leaves such an occurrence exactly as it is written. The pass reads
a section-sign token whose section the specification no longer declares as an occurrence the register
answers for, so the seed population is every such token in the pass's write domain rather than the retired
§15.4 occurrences alone. SPEC-4 retires the register with
...
- **The carve-out class covered markdown links alone, so bare `§15.4.1` citations of carved-out material,
  ...
  than substituting the map's successor. An entry also records that an occurrence cites no specification
  section at all, which the pass leaves as it is written, so the seed covers every section-sign token in
  the pass's write domain whose section the specification no longer declares. TEST-1 gains the fail-closed
  case, a case pinning the recorded escape, and a case pinning that a
