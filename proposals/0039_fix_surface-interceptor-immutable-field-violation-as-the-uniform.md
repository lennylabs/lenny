...
- **Status:** **Implemented (2026-07-16).** Approved (2026-07-16) with Open decision D1 resolved as **Option A** — accept the connector-path deferral (the connector gRPC transport surfaces the distinct §15.1 code and the violated field names in `Reason`; a structured `details.violated_fields` on that transport stays out of scope, guarded by the tier-1 `rejectionFor` code-preservation test). Verified (2026-07-16), converged after 2 adversarial review rounds (0 findings fixed). The status was reconciled on 2026-09-08 against the git history, which carries every deliverable: CODE-ROOT in `c11899d8b`, CODE-ROUTE in `0adc9d1d9`, CODE-DELEG in `67bd1f00e`, and CODE-TOOLRESULT in `3912b03bf`, with the tier-1 cases in `pkg/gateway/policy/interceptor/immutability_test.go` and `route_internal_test.go`.
- **Date:** 2026-07-16.
